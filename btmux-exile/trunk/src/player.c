...
static void decrypt_logindata(player, atrbuf, info)
dbref player;
char *atrbuf;
LDATA *info;
{
    ...
    if (cnt != 17)
	send_channel("Wizards", tprintf("LoginData attrib does not have proper entries on %s. This is either a brand new first login player or a bad LoginData Attribute. Contact hardcoder if not a new player. Attrib = '%s'", Name(player), atrbuf));
...
char *ldate, *lhost, *lusername;
{
    ...
    decrypt_logindata(player, atrbuf, &login_info);
    if (isgood) {
...
char *who;
{
	...
	decrypt_logindata(target, atrbuf, &login_info);
