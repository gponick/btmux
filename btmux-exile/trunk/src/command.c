...
{(char *)"dark",	1,      CA_PUBLIC,      CSET_DARK},
{(char *)"visible",	1,	CA_PUBLIC,	CSET_VISIBLE},
{ NULL,                 0,      0,              0}}; 
...
{(char *)"@startsqlslave",	NULL,		CA_WIZARD,
	0,		CS_NO_ARGS,			boot_sqlslave},
{(char *)"@startfileslave",	NULL,		CA_WIZARD,
...
#define	Protect(x) (cmdp->perms & x)
    ...
    if ((cmdp->callseq & CS_INTERP) || !(interactive ||
	    (cmdp->callseq & CS_NOINTERP))) interp = EV_EVAL | EV_STRIP;
...
char *command, *args[];
{
    ...
    /* Check for Wizbit allcommand logging */
    if (WizRoy(player)) {
	STARTLOG(LOG_ALLWIZBIT, "CMD", "WIZ") {
	    log_name_and_loc(player);
	    lcbuf = alloc_lbuf("process_commands.LOG.wizcmd");
	...
	*slashp++ = '\0';
    ...
    cmdp =
	(CMDENT *) hashfind(lcbuf, &mudstate.command_htab);
    if (cmdp != NULL) {
