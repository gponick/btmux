...
{(char *)"allwizbit",		2,	0,	LOG_ALLWIZBIT},
...
#ifndef STANDALONE
void do_log(player, cause, key, whichlog, logtext)
    dbref player, cause;
    int key;
    char *whichlog, *logtext;
{
    ...
    if (whichlog[0] == '\0') {
	notify(player, "Need a filename.");
        return;
    }
    if (logtext[0] == '\0') {
	notify(player, "Cannot append nothing to a log.");
        return;
    }
    ...
    if (lfhandle == NULL || whichlog == NULL) {
     ...
     return;
#else
fileslave_dolog(player, whichlog, logtext);
#endif
}
#endif
