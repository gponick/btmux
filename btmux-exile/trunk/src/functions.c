...
struct {
    ...
    {
    60 *60 * 24 * 30 * 12, "year", "y"}, { 60 *60 * 24 * 30, "month", "m"}, {
    60 *60 * 24, "day", "d"}, { 60 *60, "hour", "h"}, {
    60, "minute", "m"}, {
    1, "second", "s"}
};
...
{
#if 0
    char *whichlog, *logtext;
    FILE *lfhandle;   /* Logfine handle */
    char *logbuff;    /* A buffer.. */
    char *temp_ptr;   /* A temporary pointer.. */
    int fileflag;     /* Keep an eye on fopen */

    whichlog = fargs[0];
    logtext = fargs[1];
    if (whichlog[0] == NULL) {
        safe_str("#-1 INVALID FILENAME ARG", buff, bufc); 
        return;
    } 
    if (logtext[0] == NULL) {
        safe_str("#-1 INVALID LOGTEXT ARG", buff, bufc); 
        return;
    }
    logbuff=alloc_lbuf("do_log_buffer");
    logbuff[0]='\0';  /* Effectively zero the buffer */ 
    /* Set the filename. Restricting it to a subdir reduces the
       possibility of a security hole. */
    temp_ptr = strrchr(whichlog, '/');
    if (temp_ptr != NULL)
        whichlog = ++temp_ptr;
    sprintf(logbuff, "logs/%s", whichlog);
    lfhandle=fopen(logbuff, "r");
    if (lfhandle==NULL) {
          safe_str("#-1 NOT A VALID LOGFILE", buff, bufc);
          free_lbuf(logbuff);
          return;
        }
     /* Okay, at this point, the file exists. */
     fclose(lfhandle);
     lfhandle=fopen(logbuff, "a");     /* Reopen the file to append */
     fprintf(lfhandle, "%s\n", logtext);
     fclose(lfhandle);                 /* Send and close... */
     free_lbuf(logbuff);
     return;
#else
fileslave_dolog(GOD, fargs[0], fargs[1]);
#endif
}
#endif
...
FUNCTION(fun_nearby)
{
    ...
    if (!(nearby_or_control(player, obj1) ||
	    nearby_or_control(player, obj2))) safe_str("0", buff, bufc);
...
/*    if (player == Owner(player))
	send_channel("Cheat",tprintf("Idle Function call. Object #%i (%s) against object #%d (%s).", player, Name(player), target, Name(target))); */
    if (!WizRoy(Owner(player)))
	safe_tprintf_str(buff, bufc, "%s", "PERMISSION DENIED HOSER"); 
    else
...
{"BTGETCHARVALUE", fun_btgetcharvalue, 0, FN_VARARGS, CA_WIZARD},
...
{"BTSETCRITSLOT",  fun_btsetcritslot, 0, FN_VARARGS,	CA_WIZARD},
...
{"BTSTORES",    fun_btstores,   2, 0,		CA_WIZARD},
...
{"BTTECHTIME",	fun_bttechtime, 0, 0,		CA_PUBLIC},
...
{"BTLOADMAP", fun_btloadmap, 2, 0,		CA_WIZARD},
