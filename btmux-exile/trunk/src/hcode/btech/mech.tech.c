...
int tech_roll(dbref player, MECH * mech, int diff)
{
    ...
    s += diff;
    succ = r >= s;
    if ((Wizard(player)) || (mudconf.btech_showtech)) {
	notify(player, tprintf("Tech - BTH: %d(Base:%d, Mod:%d) Roll: %d",
		s, s - diff, diff, r));
	}
    if (succ)
	AccumulateTechXP(player, mech, BOUNDED(1, s - 7, MAX(5,
		    1 + diff)));
    return (r - s);
}
...
int tech_weapon_roll(dbref player, MECH * mech, int diff)
{
	...
	notify(player,
		...
		s - diff, diff, r));
    if (succ)
	AccumulateCustTechXP(player, NULL, BOUNDED(1, s - 7, MAX(5,
		    1 + diff)), skhill);
    return (r - s);
}
...
void tech_status(dbref player, time_t dat)
{
	...
	if (un >= MAX_TECHTIME)
	    strcat(buf, " and you're too tired to do more efficiently.");
	else {
	    un = MAX_TECHTIME - un;
	    strcat(buf, tprintf(" and you're ready to do at least %d more %s%s of work.", un, TECH_UNIT, un == 1 ? "" : "s"));
	}
	notify(player, buf);
    }
}

int tech_playertime(dbref player, int time)
{
time_t old;
...
if (olds) {
    ...
    old = mudstate.now;
old += time * TECH_TICK;
return ((old - mudstate.now) / TECH_TICK);
}
...
#if 0
    top = *pos + GetWeaponCrits(mech, t) - 1;
    for (l = *pos; l <= top; l++)
	if (PartIsDestroyed(mech, *loc, l))
	    break;
    if (l <= top)
	*pos = l;
#endif
