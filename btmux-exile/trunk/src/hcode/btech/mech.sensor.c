    ...
    float range)
{
	...
	bth2 =
	    1 + sensors[(int) MechSensor(mech)[1]].tohitbonus_func(mech,
	    target, flag, maplight);
	...
	bth1 =
	    sensors[(int) MechSensor(mech)[0]].tohitbonus_func(mech,
	    target, flag, maplight);
#ifdef SENSOR_BTH_DEBUG
    ...
    bth1 =
	sensors[(int) MechSensor(mech)[0]].tohitbonus_func(mech, target,
	flag, maplight);
    bth2 =
	1 + sensors[(int) MechSensor(mech)[1]].tohitbonus_func(mech,
	target, flag, maplight);
...
int Sensor_CanSee(MECH * mech, MECH * target, int *flag, int arc,
    float range, int mapvis, int maplight, short cloudbase)
{
    int i, j = 0, sn;
...
#define MAXVR(mech,sensor) MaxVis(mech,sensor) - MaxVar(mech, sensor)
	...
	for (i = 0; i < 2; i++) {
	    ...
	    /* No chance */
	    if (!sensors[sn].fullvision && !(arc & FORWARDARC) && !Sees360(mech))
		continue;
	    ...
	    if (!sensors[sn].cansee_func(mech, target, range, *flag))
		continue;
	    if (!sensors[sn].seechance_func(target, sn, range, mapvis,
		    maplight)) continue;
    ...
    if (!sensors[sn].cansee_func(mech, target, range, *flag))
	return 0;
    if (!sensors[sn].seechance_func(target, sn, range, mapvis, maplight))
	return 0;
    if (MaxVar(mech, sn) && (MAXVR(mech, sn)) < range)
	if (((MAXVR(mech, sn)) + MechVisMod(mech) * (MaxVar(mech,
			sn) + 1) / 100) < range)
...
/* Modified a hair to support Hidden rules on Bsuits more. Note : Standard HiddenUnits */
/* Rules are implementable........ DJ */
    ...
    int snum, int chance_divisor, int mapvis, int maplight)
{
    int chance = (Sensor_ArcBaseChance(MechType(mech), arc) * ((target &&
		MechTeam(mech) !=
		MechTeam(target)) ? Sensor_DriverBaseChance(mech) : 100)) /

	100;
    int ch2 = sensors[snum].seechance_func(target, snum, range, mapvis,
	maplight);

    if (!ch2 || !sensors[snum].cansee_func(mech, target, range, f))
	return 0;
    if (target && IsDS(target))
	chance = chance * 4;
    if (target && (MechCritStatus(target) & HIDDEN) && MechTeam(mech) != MechTeam(target)) {
        if ((MechStatus2(target) & NULLSIG_ACTIVE) || StealthAndECM(target)) {
	    if ((sensors[snum].sensorname[0] == 'B') && (sensors[snum].sensorname[1] == 'l'))
		    return 1;
	} else {
	    if (sensors[snum].sensorname[0] == 'B')
		    return 1;
	}
	if ((int) range > ((MechType(target) == CLASS_BSUIT || MechType(target) == CLASS_MW) ? 1 : 2))
	    return 0;
        chance = chance / 6;
    }
...
static int valid_to_notice(MECH * mech, MECH * targ)
{
    int bf = (mech->brief / 4);
    int foe = 2 * (MechTeam(mech) != MechTeam(targ));
	...
	return 4;
    case 2:
	if (MechTeam(mech) == MechTeam(targ))
	    return 0;
	return 1 + foe;
    case 3:
	if (MechTeam(mech) == MechTeam(targ))
...
void Sensor_DoWeSeeNow(MECH * mech, unsigned int *fl, float range, int x,
    int y, MECH * target, int mapvis, int maplight, short cloudbase,
    int seeanew, int wlf)
{
    int arc;
    float x1, y1;
	...
	if (!(f & (MECHLOSFLAG_SEESP | MECHLOSFLAG_SEESS))) {
	    ...
	    f &= ~MECHLOSFLAG_SEEN;
	    if (Started(target) && (st = valid_to_notice(mech, target)) &&
		seeanew < 3) {
		if (st & 2)
		    strcpy(buf, "%cy");
		else
		...
		if (st >= 4)
			...
			GetMechToMechID_base(mech, target,
			    wlf > 0 ? 2 : 1), GetArcID(mech, arc));
		else
		    sprintf(buf,
...
void update_LOSinfo(dbref obj, MAP * map)
{
	...
	if (!mech) {
		if(map->mechsOnMap[i] != -1) {
			snprintf(buffer, LBUF_SIZE, "Map #%d has invalid dbref #%d at location %d.", 
				map->mynum, map->mechsOnMap[i], i);
			SendError(buffer);
		}
	    continue;
	}
...
/* Commented too allow ECCM code to protect self */
		...
		if (MechStatus(mech) & ECM_ACTIVE)
		    if (range < ECMRange(mech))
			cause_ecm(mech, target);
/*		if (MechStatus2(mech) & ECCM_ACTIVE)
	...
	if (!Started(mech))
	    continue;
	for (j = 0; j <= i; j++)
/* < changed to <= to allow ECCM to protect self */
	    if (map->mechflags[i] || map->mechflags[j]) {
		target = FindObjectsData(map->mechsOnMap[j]);
		if (!target)
		    continue;
...
static void mech_sensorchange_event(EVENT * e)
{
    ...
    if (prim == SENSOR_BP || sec == SENSOR_BP ||
	prim == SENSOR_BL || sec == SENSOR_BL)
	MechStatus2(mech) |= BEAGLE_ACTIVE;
    else
	MechStatus2(mech) &= ~BEAGLE_ACTIVE;
}
...
int CanChangeTo(MECH * mech, int s)
{
    ...
    if (s == SENSOR_SE && (is_aero(mech) || IsDS(mech) || WaterBeast(mech) || MechType(mech) == CLASS_VTOL || MechMove(mech) == MOVE_HOVER || MechType(mech) == CLASS_MW || MechType(mech) == CLASS_BSUIT || MechType(mech) == CLASS_VEH_NAVAL)) {
	mech_notify(mech, MECHALL, tprintf("Sorry.  You cannot use such seismic."));
	return 0;
	}
    /* < 0, means you you don't have the sensors if you _do_ have the bit
       > 0, means you have the sensors if you have the bit */
    t = (sensors[s].min_light == -2 || sensors[s].max_light == -2);
    if ((i = sensors[s].required_special))
	if ((i > 0) == ((!(t ? MechSpecials2(mech) & abs(i) : MechSpecials(mech) & abs(i)))) != 0) {
	    mech_notify(mech, MECHALL, tprintf("You lack the %s sensors!",
...
static int set_sensor(MECH * mech, char *ps, char *ss)
{
	...
	if (sensors[i].sensorname[0] == toupper(ps[0]))
	    if (prim == -1 || (prim > -1 && sensors[i].sensorname[1] == tolower(ps[1])))
	        prim = i;
	if (sensors[i].sensorname[0] == toupper(ss[0]))
            if (sec == -1 || (sec > -1 && sensors[i].sensorname[1] == tolower(ss[1])))
		sec = i;
    }
    if (prim < 0 || sec < 0)
	return -1;
/* Changed to allow retro-sensor changes - DJ */
    if ((prim != MechSensor(mech)[0] || sec != MechSensor(mech)[1]) || SensorChange(mech)) {
...
void mech_sensor(dbref player, void *data, char *buffer)
{
    ...
    if (!mech)
	return;
    if (mech->mynum == player)
	{
	SendError(tprintf("Mech #%d : Sensor command sent by itself.. #%d. Crash avoided on event.",mech->mynum, player));
	return;
	}
...
/*	DOCHECK(set_sensor(mech, toupper(args[0][0]), toupper(args[1][0]))
	    < 0, "Invalid arguments!"); */
	DOCHECK(set_sensor(mech, args[0], args[1])
            < 0, "Invalid arguments!");
	show_sensor(player, mech, 0);
	break;
    }
...
void possibly_see_mech(MECH * mech)
{
	...
	if (i != num && (j = map->mechsOnMap[i]) >= 0) {
		...
		map->LOSinfo[i][num], (float) range, 0);
	    /* Then, we update the SEES* */
    ...
    char *inframsg, char *liteampmsg)
{
	...
	if (mech_map->mechsOnMap[i] != -1 &&
		...
		if (InLineOfSight(tempMech, mech, MechX(mech), MechY(mech),
			...
			continue;
		    if (sensors[(int)
			    MechSensor(tempMech)[0]].sensorname[0] == 'I'
			|| sensors[(int)
			    MechSensor(tempMech)
			    [0]].sensorname[1] == 'I') {
			if (chance)
			...
			mech_notify(tempMech, MECHALL, inframsg);
			    ...
			    MechSensor(tempMech)[0]].sensorname[0]
			== 'L' || sensors[(int)
			    MechSensor(tempMech)
			    [0]].sensorname[1]
			== 'L') {
