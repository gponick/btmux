...
void mech_scan(dbref player, void *data, char *buffer)
{
	...
	if (args[0][1]) {
	    ...
	    DOCHECK(!(MechCritStatus(mech) & OBSERVATORIC) && ((((MechStatus2(tempMech) & NULLSIG_ACTIVE) && Started(tempMech)) && (range > (MechScanRange(mech) / 2))) ||
		(MechTeam(mech) != MechTeam(tempMech) && ((MechStatus2(tempMech) & ECM_PROTECTED) ||
		(mech_map->LOSinfo[mech->mapnumber][tempMech->mapnumber] & MECHLOSFLAG_ECM)) && (range > (MechScanRange(mech) / 2)))),
		"You cannot get a clear scan due to some kind of disturbance!");
	...
	if (tempMech) {
		...
		doh = 1;
		    ...
		    find_mech_in_hex(mech, mech_map, mapx, mapy,
	1))) tempMech = (MECH *) NULL;
	}
	break;
    case 3:
	/* scan x, y b */
...
void PrintReport(dbref player, MECH * mech, MECH * tempMech, float range)
{
	...
	sprintf(buff, "      Vlcty: %.1f KPH\tAngle: %3d degrees", MechVelocity(mech), MechAngle(mech));
	notify(player, buff);
	sprintf(buff, "      Thrust: %.1f KPH", MechThrust(mech));
	notify(player, buff);
	}
    ...
    float range, int opt)
{
	...
	return;
    if (MechCritStatus(mymech) & CLAIRVOYANT)
	owner = 1;
...
void mech_bearing(dbref player, void *data, char *buffer)
{
	...
	if (argc == 0) {
		...
		z1 = (Elevation(mech_map, ix1, iy1) * ZSCALE);
		sprintf(buff, "Bearing to  %d,%d,%d is: ", ix1, iy1, (int) (z1 / ZSCALE));
	    ...
	    iz1 = atoi(args[2]);
            if (!(ix1 >= 0 && ix1 < mech_map->map_width && iy1 >= 0 &&
                ...
                z1 = (iz1 * ZSCALE);
                sprintf(buff, "Bearing to  %d,%d,%d is: ", ix1, iy1, iz1);
                MapCoordToRealCoord(ix1, iy1, &x1, &y1);
            }
	} else if (argc == 4) {
	    ix0 = atoi(args[0]);
		...
		z0 = (Elevation(mech_map, ix0, iy0) * ZSCALE);
		z1 = (Elevation(mech_map, ix1, iy1) * ZSCALE);
		sprintf(buff, "Bearing to %d,%d,%d from %d,%d,%d is: ", ix1, iy1, (int) (z0 / ZSCALE),
		    ix0, iy0, (int) (z1 / ZSCALE));
	...
	if (x1 != -1) {
	    ...
	    temp2 = FindZBearing(x0, y0, z0, x1, y1, z1);
	    sprintf(trash, "%.0f degrees mark %c%d.", temp, (z1 > z0 ? '+' : z1 < z0 ? '-' : ' '), temp2);
	    strcat(buff, trash);
	    notify(player, buff);
	}
    } else {
...
#define SHOW(part,phys,loc) \
  ...
  if (MechType(mech) == CLASS_MECH || MechType(mech) == CLASS_BSUIT || MechType(mech) == CLASS_MW) {
	...
	if (count > 0) {
		...
		if (PartIsNonfunctional(mech, loop, critical[ii]) || WeaponIsNonfunctional(mech, loop, critical[ii],
                GetWeaponCrits(mech, Weapon2I(GetPartType(mech, loop, critical[ii])))) > 0 ||
                PartIsDisabled(mech, loop, critical[ii])) {
		    strcat(weapbuff, "%ch%cx*****%c");
		} else {
		    if (weapdata[ii] || (GetPartMode(mech, loop, critical[ii]) & JAMMED_MODE)) {
			strcat(weapbuff, "-----");
		    } else {
...
void mech_view(dbref player, void *data, char *buffer)
{
	...
	if (!(MechCritStatus(mech) & OBSERVATORIC) && FaMechRange(mech, target) > MechScanRange(mech)) {
	    mech_notify(mech, MECHALL, "Where is your telescope?");
	    return;
	}
	if (!target ||
	    !InLineOfSight_NB(mech, target, MechX(target), MechY(target),
     FlMechRange(mech_map, mech, target))) {
	    mech_notify(mech, MECHPILOT, "That is not a valid target ID!");
	    return;
	}
