...
int auto_valid_progline(AUTO * a, int p)
{
	...
	i += (acom[a->commands[i]].argcount + 1)) if (i == p)
	    return 1;
    return 0;
}
...
int AutoPilotOn(AUTO * a)
{
	...
	if ((j = event_count_type_data(i, (void *) a)))
	    count += j;
    if (!count)
	return a->flags & (AUTOPILOT_AUTOGUN | AUTOPILOT_GUNZOMBIE | AUTOPILOT_PILZOMBIE);
    return count;
}
...
void StopAutoPilot(AUTO * a)
{
    ...
    a->flags &= ~(AUTOPILOT_AUTOGUN | AUTOPILOT_GUNZOMBIE | AUTOPILOT_PILZOMBIE);
    for (i = FIRST_AUTO_EVENT; i <= LAST_AUTO_EVENT; i++)
