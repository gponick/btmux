...
void listtic_sub(dbref player, MECH * mech, char *buffer)
{
    ...
    present_count = count;
    c =
	SelCol_FunStringMenuK(2, tprintf("TIC #%d listing for %s", ticnum,
...
void heat_cutoff(dbref player, void *data, char *buffer)
{
	...
	MECHEVENT(mech, EVENT_HEATCUTOFFCHANGING, heat_cutoff_event, 30, 0);
    } else {
	notify(player, "Engaging heat dissipation cutoff...");
	MECHEVENT(mech, EVENT_HEATCUTOFFCHANGING, heat_cutoff_event, 30, 1);
    }
}
