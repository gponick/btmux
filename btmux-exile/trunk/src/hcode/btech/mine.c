    ...
    char *otmsg, char *tomsg1, char *otmsg1, int dam, int heat, int nb)
{
	...
	otmsg1, MINE_TABLE, 2, 1, 1, 1, NULL, NULL);
}
...
{
    mapobj *o, *o2;
    int mdis = (MechRTons(mech) / 1024 - 20) / 10;
	...
	if (o->x == x && o->y == y) {
	    switch (o->datac) {
	    case MINE_TRIGGER:
		if (o->datas > (MechRTons(mech) / 1024))
		    continue;
		break;
	    case MINE_VIBRA:
		if (o->datai <= (MechRTons(mech) / 1024))
		...
		if (FindHexRange(x1, y1, x2, y2) > (o->datai + 1))
		    continue;
		make_mine_explode(mech, map, o, x, y, reason);
	    } else if (o->datai < (MechRTons(mech) / 1024)) {
		if (abs(o->x - x) <= mdis && abs(o->y - y) <= mdis) {
		    /* Possible remote explosion */
		    MapCoordToRealCoord(o->x, o->y, &x2, &y2);
		    if ((range = FindHexRange(x1, y1, x2, y2)) > ((MechRTons(mech) / 1024) - o->datai) / 10)
			continue;
		    make_mine_explode(mech, map, o, x, y, reason);
		}
	    }
...
static void add_mine_on_map(MAP * map, int x, int y, char type, int data)
{
	...
	for (x1 = x - data; x1 <= x + data; x1++)
		...
		if (FindHexRange(fx, fy, fx1, fy1) <= data)
