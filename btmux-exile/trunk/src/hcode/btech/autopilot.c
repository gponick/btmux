...
void gradually_load(MECH * mech, int loc, int percent)
{
	...
	if (i == NUM_ITEMS)
	    break;
	lastid = i;
	econ_change_items(loc, i, -1);
	econ_change_items(mech->mynum, i, 1); 
	pile[i]--;
	cnt--;
	SetCargoWeight(mech);
...
PSTART(a,mech);\
if (MechType(mech) == CLASS_MECH && Fallen(mech) && !(MechCritStatus(mech) & (NO_LEGS|LEG_DESTROYED))) { if (!Standing(mech)) mech_stand(a->mynum, mech, my2string("")); REDO(a, AUTOPILOT_NC_DELAY); return; };\
if (MechType(mech) == CLASS_VEH_VTOL && Landed(mech) && !SectIsDestroyed(mech, ROTOR)) { if (!TakingOff(mech)) aero_takeoff(a->mynum, mech, my2string("")); REDO(a, AUTOPILOT_NC_DELAY); return; }
...
if (MechAuto(mech) > 0) {
    if (!(a = FindObjectsData(MechAuto(mech))) || !Good_obj(MechAuto(mech)) || Location(MechAuto(mech)) != mech->mynum) {
	SendError(tprintf("Mech #%d thinks it has AutoP #%d on it but FindObj breaks.", mech->mynum, MechAuto(mech)));
	MechAuto(mech) = -1;
    } else
...
void autopilot_enterbase(MECH * mech, int dir)
{
    static char strng[2];
    switch(dir) {
    case 0: strcpy(strng, "n"); break;
    case 1: strcpy(strng, "e"); break;
    case 2: strcpy(strng, "s"); break;
    case 3: strcpy(strng, "w"); break;
    default: sprintf(strng, "%c", dir); break;
    }
...
void auto_com_event(EVENT * e)
{
    ...
    case GOAL_FOLLOW:
	GSTART(a, mech);
	AUTOEVENT(a, EVENT_AUTOFOLLOW, auto_follow_event, AUTOPILOT_FOLLOW_TICK, 0);
	return;
    case GOAL_DUMBFOLLOW:
	GSTART(a, mech);
	AUTOEVENT(a, EVENT_AUTOFOLLOW, auto_dumbfollow_event, AUTOPILOT_FOLLOW_TICK, 0);
	return;
    case GOAL_GOTO:
	GSTART(a, mech);
	AUTOEVENT(a, EVENT_AUTOGOTO, auto_goto_event, AUTOPILOT_GOTO_TICK, 0);
	return;
    case GOAL_ROAM:
	GSTART(a, mech);
    ...
    case GOAL_DUMBGOTO:
	GSTART(a, mech);
	AUTOEVENT(a, EVENT_AUTOGOTO, auto_dumbgoto_event, AUTOPILOT_GOTO_TICK, 0);
	return; 
    case GOAL_LEAVEBASE:
	GSTART(a, mech);
	a->mapindex = mech->mapindex;
	AUTOEVENT(a, EVENT_AUTOLEAVE, auto_leave_event, AUTOPILOT_LEAVE_TICK, 0);
	return;
    case COMMAND_LOAD: 
...
/*	mech_unloadcargo(GOD, mech, my2string(" * 9999")); */
        strcpy(buf, " * 9999");
	mech_unloadcargo(MechPilot(mech), mech, buf);
	ADVANCE_PG(a);
	break;
...
void auto_roam_event(EVENT * e)
{
    ...
    if ((tx == 0 && ty == 0) || e->data2 > 0 || (MechX(mech) == tx && MechY(mech) == ty && abs(MechSpeed(mech)) < 0.5)) {
	while (i) {
	    ...
	    range = FindRange(MechFX(mech), MechFY(mech), MechFZ(mech), dx, dy, ZSCALE * GetElev(map, tx, ty));
	    if ((InLineOfSight(mech, NULL, tx, ty, range) && t != WATER && t != HIGHWATER && t != MOUNTAINS ) || i > 5000) {
		i = 0;	
		} else {
		i++;
		}
