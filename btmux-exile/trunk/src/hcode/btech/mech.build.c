...
void FillDefaultCriticals(MECH * mech, int index)
{
	...
	MechSections(mech)[index].criticals[loop].mode = 0;
    }
...
int WeaponIndexFromString(char *string)
{
    ...
    if (find_matching_vlong_part(string, NULL, &id))
	if (IsWeapon(id))
	    return Weapon2I(id);
    return -1;
}
...
int FindSpecialItemCodeFromString(char *buffer)
{
    ...
    if (find_matching_vlong_part(buffer, NULL, &id))
	if (IsSpecial(id))
	    return Special2I(id);
    return -1;
}

int FindBombItemCodeFromString(char *buffer)
{
    int id;
