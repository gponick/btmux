...
int find_entrance(MAP * map, char dir, int *x, int *y)
{
	...
	tmp = next_mapobj(tmp)) if (!dir || tmp->datac == dir) {
	    *x = tmp->x;
	    *y = tmp->y;
	    return 1;
	}
...
char find_decorations(MAP * map, int x, int y)
{
    int i;
    mapobj *m;
    if (x < 1 || y < 1 || x >= (map->map_width - 1) || y >= (map->map_height - 1)) 
	return 0;
    ...
    int flaggo)
{
    ...
    if (x < 1 || y < 1 || x >= (map->map_width - 2) || y >= (map->map_height - 2))
        return;
...
void map_addsmoke(dbref player, void *data, char *buffer)
{
    ...
    if (strcmp(args[3], "h") == 0 || strcmp(args[3], "H") == 0)
        add_decoration(map, x, y, TYPE_SMOKE, HSMOKE, d);
    else
...
void map_add_mining(dbref player, void *data, char *buffer)
{
    char *args[4];
    int argc;
    int x, y, str, rng;
    ...
    if (!map) return;
    argc = mech_parseattributes(buffer, args, 4);
    DOCHECK(argc != 4, "Invalid arguements!");
    READINT(args[0], x);
    READINT(args[1], y);
    READINT(args[2], str);
    READINT(args[3], rng);
    ...
    foo.obj = player;
    foo.datac = rng;
    add_mapobj(map, &map->mapobj[TYPE_MINING], &foo, 1);
    notify(player, tprintf("Mineral concentration of strength %d and range %d added to %d,%d",
			str, rng, x, y));
}

int is_blocked_lz(MECH * mech, MAP * map, int x, int y, int check_dyn, int check3d)
{
    MECH *amech;
    mapobj *o;
    float fx, fy;
	...
	if (!check3d) {
	    ...
	    range = FindRange(fx, fy, MechFZ(mech), tx, ty, ZSCALE * Elevation(map, o->x, o->y));
	    if (range <= o->datai)
		return 1;
	}
    }
  ...
  if (check_dyn) {
	...
	if (map->mechsOnMap[i] != mech->mynum && map->mechsOnMap[i] != -1) {
	    amech = (MECH *) FindObjectsData(map->mechsOnMap[i]);
	    if (amech) {
		if (MechStatus2(amech) & CS_ON_LAND && IsDS(amech) && Landed(amech) && MechTeam(amech) != MechTeam(mech)) {
	    	    MapCoordToRealCoord(MechX(amech), MechY(amech), &tx, &ty);
		    if (!check3d) {
		        if (FindHexRange(fx, fy, tx, ty) <= DS_TO_DS_LZ_RANGE)
			    return 1;
		    } else {
		        range = FindRange(fx, fy, MechFZ(mech), tx, ty, ZSCALE * Elevation(map, MechX(amech), MechY(amech)));
		        if (range <= DS_TO_DS_LZ_RANGE)
			    return 1;
			}
		    }
		}
...
int hex_mineral_density(MAP * map, int x, int y)
{
float fx, fy, tx, ty, rng;
int dens = 0;
mapobj *o;
...
MapCoordToRealCoord(x, y, &fx, &fy);
for (o = first_mapobj(map, TYPE_MINING); o; o = next_mapobj(o)) {
    if (o->datai >= 0)
	continue;
    if (abs(x - o->x) > o->datac || abs(y - o->y) > o->datac)
	continue;
    MapCoordToRealCoord(o->x, o->y, &tx, &ty);
    if ((rng = FindHexRange(fx, fy, tx, ty)) <= o->datac) {
	dens += (((int) rng / o->datac) * o->datai);	
	}
    }
...
void hit_building(MECH * mech, int x, int y, int weapindx, int damage, int mode, int isglance)
{
    mapobj *o;
    MAP *map;
    MAP *nmap;
	...
	if (!IsMissile(weapindx)) {
	    damage = GunStat(weapindx, mode, GUN_DAMAGE);
	if (isglance) {
            damage = ((damage + 1) / 2);
            if (damage < 1)
                damage = 1;
	    ...
	    damage = num_missiles_hit * GunStat(weapindx, mode, GUN_DAMAGE); 
	    if (isglance) {
                damage = ((damage + 1) / 2);
                if (damage < 1)
                    damage = 1;
	...
	return;
    damage_cf(mech, o, i1, i2, pc_to_dam_conversion(NULL, weapindx, damage));
}
