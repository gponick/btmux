...
static void custom_info(coolmenu * c, char *str, MECH * mech, int loc, int pos)
{
    const char **locs = ProperSectionStringFromType(MechType(mech), MechMove(mech));

    cent(tprintf("%s for %s/%d:", str, locs[loc], pos + 1));
}

static void custom_do_weapon(coolmenu * c, MECH * mech, int loc, int pos, int req, int cant)
{
    int i;
...
static void custom_do_ammo(coolmenu * c, MECH * mech, int loc, int pos, int req, int cant)
{
    int i;

    custom_info(c, "Ammo selection", mech, loc, pos);
    for (i = 0; MechWeapons[i].name; i++)
        if (OkWeap(i, req, cant) && MechWeapons[i].ammoperton && !(MechWeapons[i].special & PCOMBAT))
            choice2(MechWeapons[i].name, i);
}
...
static void custom_do_special(coolmenu * c, MECH * mech, int loc, int pos, int spect)
{
    int i;
    ...
    for (i = 0; ok_specials[i][0] >= 0; i++)
        if (ok_specials[i][1] == spect)
            choice(get_parts_long_name(Special(ok_specials[i][0]), 0), ok_specials[i][0]);
}
...
int crit_weight(MECH * mech, int t)
{
    ...
    if (IsWeapon(t))
        return MechWeapons[Weapon2I(t)].weight * 1024 / 100 / GetWeaponCrits(mech, Weapon2I(t));
    ...
    switch (t) {
        ...
        case SWORD:
            ...
            return (int)((ceil(MechTons(mech) / 10.) * 512) / (ceil(MechTons(mech) / 15.)));
        case BEAGLE_PROBE:
...
int generate_change_list(MECH * from, MECH * to)
{
    ...
    for (i = 0; i < NUM_SECTIONS; i++) {
        if ((dif = (GetSectOArmor(to, i) - GetSectOArmor(from, i))))
            addchange(CHANGE_ARMOR, i, 0, dif, dif * 64, abs(dif) * CHANGE_ARMOR_TIME);

        if ((dif = (GetSectORArmor(to, i) - GetSectORArmor(from, i))))
            addchange(CHANGE_RARMOR, i, 0, dif, dif * 64, abs(dif) * CHANGE_ARMOR_TIME);
    }
    ...
    for (i = 0; i < NUM_SECTIONS; i++) {
        ...
        if ((num_weaps_f = FindWeapons_Advanced(from, i, weaparray_f, weapdata_f, critical_f, 1)) < 0) {
            addchange(CHANGE_ERROR1, i, 0, 0, 0, 0);
            continue;
        }
    
        if ((num_weaps_t = FindWeapons_Advanced(to, i, weaparray_t, weapdata_t, critical_t, 0)) < 0) {
        ...
        for (j = 0; j < crl; j++) {
            ...
            if (GetPartType(to, i, j) == ft) {
                if (IsAmmo(ft))
                    if ((GetPartAmmoMode(from, i, j) & CustomAmmoModes) != (GetPartAmmoMode(to, i, j) & CustomAmmoModes))
                        addchange(ALTER_AMMO, i, j, 0, (GetPartFireMode(from, i, j) & HALFTON_MODE) ? 512 : 0 - (GetPartFireMode(to, i, j) & HALFTON_MODE) ? 512 : 0, ALTER_AMMO_TIME);
            ...
            addchange(REMOVE_CRIT, i, j, 0, AmmoMod(from, i, j) * (0 - crit_weight(from, ft)), REMOVE_CRIT_TIME);
        }
        
        for (j = 0; j < num_weaps_f; j++) {
            found = 0;
            for (k = 0; k < num_weaps_t; k++)
                if (weaparray_f[j] == weaparray_t[k] && critical_f[j] == critical_t[k] && (!btechconf.btech_parts || GetPartBrand(from, i, critical_f[j]) == GetPartBrand(to, i, critical_t[k])))
                    found = 1;
        
            if (found) {
                if ((GetPartFireMode(from, i, critical_f[j]) & CustomWeaponModes) != (GetPartFireMode(to, i, critical_f[j]) & CustomWeaponModes)) {
                    cr = GetWeaponCrits(from, weaparray_f[j]);
                    /* Otherwise reorient the weapon */
                    addchange(REORIENT_WEAPON, i, critical_f[j], weaparray_f[j], 0, REORIENT_TIME * cr);
                }
            ...
            cr = GetWeaponCrits(from, weaparray_f[j]);
            addchange(REMOVE_WEAPON, i, critical_f[j], weaparray_f[j], 0 - MechWeapons[weaparray_f[j]].weight * 1024 / 100, REMOVE_WEAPON_TIME * cr);
        }
        ...
        for (j = 0; j < crl; j++) {
            ...
            addchange(DO_ADD_CRIT, i, j, ft, AmmoMod(to, i, j) * crit_weight(to, ft), ADD_CRIT_TIME);
        }
    
        for (j = 0; j < num_weaps_t; j++) {
            found = 0;
            for (k = 0; k < num_weaps_f; k++)
                if (weaparray_f[k] == weaparray_t[j] && critical_f[k] == critical_t[j] && (!btechconf.btech_parts || GetPartBrand(from, i, critical_f[k]) == GetPartBrand(to, i, critical_t[j])))
                    found = 1;
            ...
            addchange(DO_ADD_WEAPON, i, critical_t[j], weaparray_t[j], MechWeapons[weaparray_t[j]].weight * 1024 / 100, ADD_WEAPON_TIME * cr);
        }
    }
...
static void display_change_list(coolmenu * c, MECH * from, MECH * mech)
{
    ...
    for (i = 0; i < last_change; i++) {
        str = ShortArmorSectionString(MechType(mech), MechMove(mech), changes[i][1]);
        switch (foo_table[changes[i][0]].type) {
            case 1:
                sprintf(buf, foo_table[changes[i][0]].text, str, changes[i][3]);
                break;
            case 2:
                sprintf(buf, foo_table[changes[i][0]].text, str, changes[i][2] + 1, pos_part_name(changes[i][0] == REMOVE_CRIT ? from : mech, changes[i][1], changes[i][2]));
                if (changes[i][0] == REMOVE_CRIT && !PartIsDestroyed(from, changes[i][1], changes[i][2])) {
                    strcat(buf, "(*)");
                    warn++;
                }
                break;
            case 3:
                ...
                sprintf(buf, foo_table[changes[i][0]].text, str, t == f ? tprintf("%d", f) : tprintf("%d-%d", f, t), pos_part_name(changes[i][0] == REMOVE_WEAPON ? from : mech, changes[i][1], changes[i][2]));
                if (changes[i][0] == REMOVE_WEAPON && !PartIsDestroyed(from, changes[i][1], changes[i][2])) {
                    strcat(buf, "(*)");
                    warn++;
                }
                break;
        ...
        addmenu(buf);
        if (changes[i][5]) {
            sprintf(buf, "%.1f tons / %d mins", changes[i][4] / 1024.0, changes[i][5]);
    ...
    if (last_change > 1) {
        addmenu("Total");
        sprintf(buf, "%.1f tons / %d mins", total_tons / 1024.0, total_time);
        addmenu(buf);
    }
...
static void custom_do_main(coolmenu * c, MECH * newmech, dbref player)
{
    MECH *mech = static_cast <MECH *>(FindObjectsData(newmech->mynum));

    if (generate_change_list(mech, newmech)) {
        display_change_list(c, mech, newmech);
        addline();
    }
...
#define TB(v) ToggleBit(GetPartAmmoMode(mech, loc, pos), v)
    for (i = 0; ammo_mode_list[i][0]; i++)
        if ((m & ammo_mode_list[i][0]) && (ammo_mode_list[j][3] < 0 || (MechWeapons[idx].type == ammo_mode_list[j][3])))
            break;
    ...
    if (ammo_mode_list[i][0]) {
        ...
        for (j = (i + 1); ammo_mode_list[j][0]; j++)
            if ((ammo_mode_list[j][3] < 0 || (MechWeapons[idx].type == ammo_mode_list[j][3])) && (ammo_mode_list[j][1] == 0 || (!(MechWeapons[idx].special & ammo_mode_list[j][1]))) && (ammo_mode_list[j][2] == 0 || (MechWeapons[idx].special & ammo_mode_list[j][2])))
                break;
    ...
    int loc, int pos)
{
    if (GetPartType(mech, loc, pos)) {
        ...
        if (IsWeapon(GetPartType(mech, loc, pos))) {
            if (MechType(mech) == CLASS_MECH && (loc == CTORSO || loc == RTORSO || loc == LTORSO))
                choice(tprintf("Toggle rear (now: %s)", GetPartFireMode(mech, loc, pos) & REAR_MOUNT ? "Rear" : "Front"), TOGGLE_REAR);
        
            if (Wiz(player) || (custom_flag & 8))
                choice(tprintf("Toggle One-Shot (now: %s)", GetPartFireMode(mech, loc, pos) & OS_MODE ? "One-shot" : "Multi-shot"), TOGGLE_OS);
        
            if (Wiz(player) || (custom_flag & 4))
                choice(tprintf("Toggle TC (now: %s)", GetPartFireMode(mech, loc, pos) & ON_TC ? "On" : "Off"), TOGGLE_TC);
        } else 
            if (IsAmmo(GetPartType(mech, loc, pos))) {
                choice(tprintf("Change the ammo type (now: %s)", ammo_text(GetPartAmmoMode(mech, loc, pos))), TOGGLE_AMMO);
                if (Wiz(player) || (custom_flag & 16))
                    choice(tprintf("Toggle ammo size (full/half - now: %s)", GetPartFireMode(mech, loc, pos) & HALFTON_MODE ? "half" : " full"), TOGGLE_HALFAMMO);
            }
    } else {
...
static void custom_do_armor(coolmenu * c, MECH * newmech)
{
    int i;
    const char **locs =
    ProperSectionStringFromType(MechType(newmech), MechMove(newmech));
    char buf[MBUF_SIZE];
    MECH *mech = static_cast < MECH * >(FindObjectsData(newmech->mynum));
    ...
    for (i = 0; locs[i]; i++) {
        ...
        number2_l(locs[i], i, GetSectOArmor(newmech, i), SectMax(newmech, i) - GetSectORArmor(newmech, i));
        sprintf(buf, "%24s%s", " ", GetSectOArmor(mech, i) == GetSectOArmor(newmech, i) ? "" : tprintf("%%ch%3d%%cn   ->  ", GetSectOArmor(mech, i)));
        number2_r(buf, i, GetSectOArmor(newmech, i), 0);
    }
    ...
    for (i = 0; locs[i]; i++) {
        ...
        number2_l(tprintf("Rear: %s", locs[i]), i + 8, GetSectORArmor(newmech, i), SectMax(newmech, i) - GetSectOArmor(newmech, i));
        sprintf(buf, "%24s%s", " ", GetSectORArmor(mech, i) == GetSectORArmor(newmech, i) ? "" : tprintf("%%ch%3d%%cn   ->  ", GetSectORArmor(mech, i)));
        number2_r(buf, i + 8, GetSectORArmor(newmech, i), 0);
    }
}

static void custom_do_crit(coolmenu * c, MECH * newmech, int loc)
{
    int i, j, t, ok;
    const char **locs =
    ProperSectionStringFromType(MechType(newmech), MechMove(newmech));
    ...
    addline();
    for (i = 0; i < CritsInLoc(newmech, loc); i++) {
        ok = 1;
    
        if (IsSpecial((t = GetPartType(newmech, loc, i)))) {
            ok = 0;
        ...
        if (!ok)
            choiceg(tprintf("%2d - %s", i + 1, pos_part_name(newmech, loc, i)), i, CM_ONE | CM_NOTOG);
        else
            toggle(tprintf("%2d - %s", i + 1, pos_part_name(newmech, loc, i)), i);
    }
}

static void custom_do_brand(coolmenu * c, MECH * mech, int loc, int pos, int type)
{
    int i;
...
static coolmenu *custom_menu(dbref player)
{
    ...
    cu = static_cast < CUSTOM * >(FindObjectsData(Location(player)));
    if (!(!cu || cu->user < 0 || (!Wiz(player) && cu->user != player))) {    /* First criteria : Non-custom place / non-init'ed custom place -> no menu */
        mech = &cu->newmech;
        addline();
        if (cu->user != player) {
            cent(tprintf("%s customization in progress (by %s(#%d))", GetMechID(mech), Name(cu->user), cu->user));
            addline();
        }
                ...
                if (cu->state <= NUM_SECTIONS)
                    ...
                    switch (s) {
                        ...
                        case 128:
                            ...
                            break;
                        default:
                            custom_do_brand(c, mech, loc, pos, s / FIRST_UNUSED_BIT);
                            break;
                    }
                }
        }
...
void custom_edit(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(!Wiz(player) && cu->user != player && cu->user >= 0, "This booth is in use by another guy!");
    DOCHECK((it = match_thing(player, buffer)) <= 0, "Invalid target!");
    DOCHECK(!Wiz(player) && Location(it) != Location(cu->mynum), "Invalid target!");
    DOCHECK(!(mech = static_cast <MECH *>(FindObjectsData(it))), "Invalid target!");
    DOCHECK(figure_latest_tech_event(mech) > 0 && !Wiz(player), "That 'Mech is still under repairs!");
    cu->user = player;
    cu->state = STATE_MAIN;
    cu->submit = -1;
    memcpy(&cu->newmech, mech, sizeof(MECH));
    show_custom(cu, player);
}
...
void custom_finish(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(cu->user < 0, "You have to start the booth with 'EDIT #<num>'");
    DOCHECK(!Wiz(player) && cu->user != player && Location(cu->user) == cu->mynum && Connected(cu->user), "This booth is in use by another guy!");

    if (!buffer && Wiz(player))
        notify(player, "All changes have been saved and booth freed.");
    else
        notify(player, "All changes have been *dumped* and booth is free for anyone to use again.");
...
void custom_back(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(cu->user < 0, "You have to start the booth with 'EDIT #<num>'");
    DOCHECK(!Wiz(player) && cu->user != player, "This booth is in use by another guy!");
...
static int do_changes(CUSTOM * cu, dbref player, MECH * from, MECH * to)
{
    ...
    for (i = 0; i < last_change; i++) {
        ...
        switch (changes[i][0]) {
            ...
            case REMOVE_CRIT:
                ...
                break;
            case REMOVE_WEAPON:
                for (j = pos; j < (pos + GetWeaponCrits(from, changes[i][3])); j++) {
            ...
            case ALTER_AMMO:
                ...
                GetPartAmmoMode(from, loc, pos) |= (GetPartAmmoMode(to, loc, pos) & CustomAmmoModes);
                GetPartData(from, loc, pos) = 0;
                break;
            case REORIENT_WEAPON:
                for (j = pos; j < (pos + GetWeaponCrits(from, changes[i][3])); j++) {
                    GetPartFireMode(from, loc, j) &= CustomWeaponModes;
                    GetPartFireMode(from, loc, j) |= (GetPartFireMode(to, loc, pos) & CustomWeaponModes);
                }
                break;
            case DO_ADD_WEAPON:
                for (j = pos; j < (pos + GetWeaponCrits(from, changes[i][3])); j++) {
    ...
    SendCustom(tprintf("%s(#%d)'s request was granted by %s(#%d)", Name(cu->user), cu->user, buf, player));
...
static void custom_act_main(CUSTOM * cu, MECH * mech, dbref player, int choice)
{
    switch (choice) {
        case ALTER_CRIT:
            advance_state(cu, player, STATE_LIMB);
        ...
        case APPLY_FOR_APPROVAL:
            ...
            if (cu->submit >= 0)
                notify(player, "You have already submitted a request. Please stand by.");
            else {
                notify(player, "An approval request has been submitted. There is NO guaranteed response,");
                notify(player, "but if the design is reasonably IC and not munch, chances are that it might");
                notify(player, "even get through. Thanks for your co-operation.");
                SendCustom(tprintf("%s(#%d) requests customization approval in #%d for #%d.", Name(player), player, cu->mynum, mech->mynum));
                cu->submit = 0;
            }
            return;
        case DO_IT:
            if (do_changes(cu, player, static_cast <MECH *>(FindObjectsData(mech->mynum)), mech))
                custom_finish(player, cu, NULL);
    }
}

static void custom_act_ammo(CUSTOM * cu, MECH * mech, dbref player, int loc, int pos, int choice)
{
...
static void custom_act_brand(CUSTOM * cu, MECH * mech, dbref player, int loc, int pos, int weapindx, int choice)
{
...
static void custom_act_weapon(CUSTOM * cu, MECH * mech, dbref player, int loc, int pos, int choice)
{
    advance_state(cu, player, CALC_FROM(loc, pos, FIRST_UNUSED_BIT * (choice + 1)));
}

static void custom_act_special(CUSTOM * cu, MECH * mech, dbref player, int loc, int pos, int choice)
{
...
static void custom_act_edit(CUSTOM * cu, MECH * mech, dbref player, int loc, int pos, int choice)
{
    switch (choice) {
        case ADD_WEAPON:
            advance_state(cu, player, CALC_FROM(loc, pos, 1));
        ...
        case TOGGLE_AMMO:
            ...
            notify(player, tprintf("Ammo is now %s.", ammo_text(GetPartAmmoMode(mech, loc, pos))));
            break;
        case TOGGLE_HALFAMMO:
            ToggleBit(GetPartFireMode(mech, loc, pos), HALFTON_MODE);
            SetPartData(mech, loc, pos, FullAmmo(mech, loc, pos));
            notify(player, tprintf("Changed to %s ton of ammo.", GetPartFireMode(mech, loc, pos) & HALFTON_MODE ? "half" : "full"));
            break;
        case TOGGLE_TC:
...
static void custom_act_crit(CUSTOM * cu, MECH * mech, dbref player, int loc, int pos)
{
    advance_state(cu, player, CALC_FROM(loc, pos, 0));
}

static void /* Change */ custom_recalculate_menu(dbref player, coolmenu * c, coolmenu * d)
{
    CUSTOM *cu;
    MECH *mech;
    int loc, pos, s;
        ...
        default:
            if (cu->state <= NUM_SECTIONS)
                ...
                switch (s) {
                    ...
                    case 4:
                    case 16:
                        if (!btechconf.btech_parts)
                            custom_act_brand(cu, mech, player, loc, pos, s / FIRST_UNUSED_BIT, 0);
                        else
                    ...
                    case 128:
                        ...
                        break;
                    default:
                        custom_act_brand(cu, mech, player, loc, pos, s / FIRST_UNUSED_BIT, d->id);
                        break;
                }
            }
    }
...
void custom_weight2(dbref player, void *data, char *buffer)
{
    ...
    if (!(mech = static_cast <MECH *>(FindObjectsData(mech->mynum)))) {
...
{
    CUSTOM *newmech = static_cast < CUSTOM * >(*data);
