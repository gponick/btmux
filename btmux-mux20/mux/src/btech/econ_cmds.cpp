...
void SetCargoWeight(MECH * mech)
{
    ...
    if ((pattr = atr_str("ECONPARTS")) && pattr->number)
        t = atr_get(mech->mynum, pattr->number, &aowner, &aflags);
    else
        t = "";
...
int loading_bay_whine(dbref player, dbref cargobay, MECH * mech)
{
    ...
    if ((pattr = atr_str("MECHSKILLS")) && pattr->number)
        c = atr_get(cargobay, pattr->number, &aowner, &aflags);
    else
        c = "";
    ...
    if (c && *c)
        if (sscanf(c, "%d %d %d", &i1, &i2, &i3) >= 2)
            if (MechX(mech) != i1 || MechY(mech) != i2) {
                ...
                if (i3)
                    notify(player, tprintf("Try looking around %d,%d instead.", i1, i2));
...
void mech_Rfixstuff(dbref player, void *data, char *buffer)
{
    ...
    if ((pattr = atr_str("ECONPARTS")) && pattr->number)
        t = atr_get(loc, pattr->number, &aowner, &aflags);
    else
        t = "";
    ...
    int atr = mkattr(loc, "ECONPARTS");

    if (!atr)
        return;

    atr_pget_info(loc, atr, &aowner, &aflags);
    atr_add(loc, atr, "", aowner, aflags);
    ...
    nl = strlen(t);
    notify(player, tprintf("Fixing done. Original length: %d. New length: %d.", ol, nl));
    notify(player, tprintf("Items in new: %d. Unique items in new: %d.", items, kinds));
}
...
void list_matching(dbref player, char *header, dbref loc, char *buf)
{
    ...
    for (i = 0; i < object_count; i++) {
        ...
        if ((buf && (x = pile2[brand][id])) || ((!buf && (x = pile[brand][id])))) {
            sw = GetPartWeight(id);
            sprintf(tmpstr, "%s (%.1ft)", part_name_long(id, brand), (sw * x) / 1024.0);
            ch = tmpstr;
            if (!ch) {
                SendError(tprintf("#%d in %d encountered odd thing: %d %d/%d's.", player, loc, pile[brand][id], id, brand));
                continue;
            }
...
void mech_stores(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUAL);
    DOCHECK(Location(mech->mynum) != mech->mapindex || In_Character(Location(mech->mynum)), "You aren't inside a hangar!");
...
static void stuff_change_sub(dbref player, char *buffer, dbref loc1, dbref loc2, int mod, int mort)
{
    ...
    DOCHECK(!mort && count > 20 && player != GOD, tprintf("Wizards can't add more than 20 different objtypes at a time. ('%s' matches: %d)", args[0], count));
...
#define MY_ECON_MODIFY(loc,num) \
    ...
    while (sfun(args[0], &i, &id, &brand)) {
        ...
        if (count)
            switch (mort) {
                case 0:
                    notify(player, tprintf("You %s %d %s%s.", mod > 0 ? "add" : "remove", count, c, count > 1 ? "s" : ""));
                    break;
                case 1:
                    MY_ECON_MODIFY(loc2, (0 - mod) * count);
                    notify(player, tprintf("You %s %d %s%s.", mod > 0 ? "load" : "unload", count, c, count > 1 ? "s" : ""));
                    break;
            }
    }
...
void mech_Rresetstuff(dbref player, void *data, char *buffer)
{
    ...
    int atr = mkattr(Location(player), "ECONPARTS");
    ...
    atr_pget_info(Location(player), atr, &aowner, &aflags);
    atr_add(Location(player), atr, "", aowner, aflags);
