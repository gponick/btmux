...
void hudinfo_notify(DESC * d, const char *msgclass, const char *msgtype, const char *msg)
{
    char buf[LBUF_SIZE];
...
void init_hudinfo(void)
{
    ...
    for (cmd = hudinfo_cmds; cmd->cmd; cmd++)
        hashaddLEN(cmd->cmd, strlen(cmd->cmd), (int *) cmd, &hudinfo_cmdtab);
}
...
void do_hudinfo(DESC * d, char *arg)
{
    ...
    if (!*arg) {
        hudinfo_notify(d, NULL, NULL, "#HUD hudinfo version " HUD_PROTO_VERSION);
        return;
    }
    ...
    cmd = (HUDCMD *) hashfindLEN(subcmd, strlen(subcmd), &hudinfo_cmdtab);
    
    if (!cmd) {
        hudinfo_notify(d, "???", "E", tprintf("%s: subcommand not found", subcmd));
        return;
    }
    ...
    if (cmd->flag & HUDCMD_NEEDMECH) {
        ...
        if ((cmd->flag & HUDCMD_AWAKE) && (MechStatus(mech) & (BLINDED | UNCONSCIOUS))) {
            hudinfo_notify(d, cmd->msgclass, "E", "You are unconscious....zzzzzzz");
            return;
        }
    }
...
static void hud_generalstatus(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    if (MechHasTurret(mech))
        ...
        if ((MechType(mech) == CLASS_MECH && !MechIsQuad(mech)) || MechType(mech) == CLASS_MW)
            sprintf(tstat, "%d", MechStatus(mech) & TORSO_LEFT ? -60 : MechStatus(mech) & TORSO_RIGHT ? 60 : 0);
        else
            strcpy(tstat, "-");
    ...
    sprintf(response, "%s,%d,%d,%d,%d,%d,%.2f,%.2f,%d,%d,%s,%.2f,%.2f,%.3f,%d,%s,%s,%s,%s", 
                MechIDS(mech, 0), MechX(mech), MechY(mech), MechZ(mech), MechFacing(mech), 
                MechDesiredFacing(mech), MechSpeed(mech), MechDesiredSpeed(mech), 
                (int) (10 * MechPlusHeat(mech)), (int) (10. * MechActiveNumsinks(mech)), 
                fuel, MechVerticalSpeed(mech), MechVerticalSpeed(mech), rtc, btc, tstat, 
                getStatusString(mech, 2), jumpx, jumpy);
...
static void hud_weapons(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    for (sect = 0; sect < NUM_SECTIONS; sect++) {
        ...
        for (i = 0; i < weapcount; i++) {
            weapnum++;
            sprintf(response, "%d,%d,%d,%s%s,%s,%s,%s", weapnum, weaparray[i],
                        GetPartBrand(mech, sect, critical[i]),
                        ShortArmorSectionString(MechType(mech), MechMove(mech), sect), 
                        GetPartFireMode(mech, sect, critical[i]) & REAR_MOUNT ? "r" : "",
                        hud_getweaponstatus(mech, sect, critical[i], weapdata[i]),
                        hud_getfiremode(mech, sect, critical[i], weaparray[i]),
                        hud_getammomode(mech, GetPartAmmoMode(mech, sect, critical[i])));
...
static int get_weaponmodes(int weapindx, char *firemodes, char *ammomodes, char *damagetype)
{
...
static void hud_weaponlist(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    for (i = 0; i < num_def_weapons; i++) {
        ...
        w = &MechWeapons[i];
        sprintf(response, "%d,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%s,%s,%s,%d", i,
...
static void hud_limbstatus(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    if (MechType(mech) == CLASS_MECH) {
        for (; todo >= 0; todo--) {
            char *sect = ShortArmorSectionString(MechType(mech), MechMove(mech), locs[todo]);
            char status[10];

            if (SectIsDestroyed(mech, locs[todo]))
                strcpy(status, "*");
            else 
                if (MechSections(mech)[locs[todo]].recycle > 0)
                    sprintf(status, "%d", MechSections(mech)[locs[todo]].recycle);
                else
                    strcpy(status, "R");
...
static void hud_ammostatus(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    for (i = 0; i < ammonum; i++) {
        sprintf(response, "%d,%d,%s,%d,%d", i, weapnums[i], hud_getammomode(mech, ammomode[i]), curamm[i], maxamm[i]);
        hudinfo_notify(d, msgclass, "L", response);
    }
...
static char hud_typechar(MECH * mech)
{
        ...
        case CLASS_VEH_NAVAL:
            switch (MechMove(mech)) {
                ...
                case MOVE_SUB:
                    return 'U';
                default: 
                    SendError(tprintf("Unknown movement type on vehicle #%d: %d", mech->mynum, MechMove(mech)));
                    return '?';
            }
        case CLASS_VTOL:
            return 'V';
    ...
    SendError(tprintf("Unknown unit type on unit #%d: %d (move %d)", mech->mynum, MechType(mech), MechMove(mech)));
    return '?';
}
...
static void hud_templateinfo(DESC * d, MECH * mech, char *msgclass, char *args)
{
    char response[LBUF_SIZE];
    char fuel[20];
    ...
    sprintf(response, "%c,%s,%s,%.3f,%.3f,%.3f,%.3f,%s,%d,%s", hud_typechar(mech), MechType_Ref(mech), MechType_Name(mech),
                WalkingSpeed(MaxSettableSpeed(mech)), MaxSettableSpeed(mech), -WalkingSpeed(MaxSettableSpeed(mech)),
                MaxVerticalSpeed(mech), fuel, MechRealNumsinks(mech), hud_advtech(mech));
    
    hudinfo_notify(d, msgclass, "R", response);
}

static void hud_templatearmor(DESC * d, MECH * mech, char *msgclass, char *args)
{
    char response[LBUF_SIZE];
    int sect;
    ...
    for (sect = 0; sect < NUM_SECTIONS; sect++) {
        if (GetSectOInt(mech, sect)) {
            sprintf(response, "%s,%d,%d,%d", ShortArmorSectionString(MechType(mech), MechMove(mech), sect), GetSectOArmor(mech, sect), GetSectORArmor(mech, sect), GetSectOInt(mech, sect));
            hudinfo_notify(d, msgclass, "L", response);
        }
    }
...
static void hud_armorstatus(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    for (sect = 0; sect < NUM_SECTIONS; sect++) {
        if (GetSectOInt(mech, sect)) {
            sprintf(response, "%s,%d,%d,%d", ShortArmorSectionString(MechType(mech), MechMove(mech),
                        sect), GetSectArmor(mech, sect), GetSectRArmor(mech, sect), GetSectInt(mech, sect));
            hudinfo_notify(d, msgclass, "L", response);
        }
    }
...
static void hud_contacts(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    for (i = 0; i < map->first_free; i++) {
        ...
        bearing = FindBearing(MechFX(mech), MechFY(mech), MechFX(other), MechFY(other));
        ...
        if (!InLineOfSight_NB(mech, other, MechX(other), MechY(other), 0.0))
            ...
            if ((pattr = atr_str("MECHNAME")) && pattr->number)
                mechname = atr_get(other->mynum, pattr->number, &aowner, &aflags);
            else
                mechname = "";
        }   
        ...
        sprintf(response, "%s,%s,%s,%c,%s,%d,%d,%d,%.3f,%d,%.3f,%.3f,%d,%s,%.3f,%d,%d,%.0f,%s",
                    MechIDS(other, MechSeemsFriend(mech, other)), hud_arcstring(weaponarc), hud_sensorstring(mech, losflag),
                    hud_typechar(other), mechname, MechX(other), MechY(other), MechZ(other), range, bearing, MechSpeed(other),
                    MechVerticalSpeed(other), MechVFacing(other), jumph, rtc, btc, MechTons(other), MechHeat(other), constat);
...
static void hud_building_contacts(DESC * d, MECH * mech, char *msgclass, char *args)
{
    char response[LBUF_SIZE];
    MAP *map = getMap(mech->mapindex);
    MAP *building_map;
    ...
    for (building = first_mapobj(map, TYPE_BUILD); building; building = next_mapobj(building)) {
        MapCoordToRealCoord(building->x, building->y, &fx, &fy);
        z = Elevation(map, building->x, building->y) + 1;
        range = FindRange(MechFX(mech), MechFY(mech), MechFZ(mech), fx, fy, ZSCALE * z);
        ...
        if ((pattr = atr_str("MECHNAME")) && pattr->number)
            building_name = atr_get(building->obj, pattr->number, &aowner, &aflags);
        else
            building_name = "";
        ...
        sprintf(response, "%s,%s,%d,%d,%d,%f,%d,%d,%d,%s", hud_arcstring(weaponarc), building_name, building->x,
                    building->y, z, range, bearing, building_map->cf, building_map->cfmax, building_status(building_map, locked));
...
static MECH *hud_scantarget(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    if (!MechScanRange(mech)) {
        hudinfo_notify(d, msgclass, "E", "Your system seems to be inoperational");
        return NULL;
    }

    if (strlen(args) != 2 || !(targ = getMech(FindTargetDBREFFromMapNumber(mech, args)))) {
    ...
    if (MechType(targ) == CLASS_MW || !InLineOfSight_NB(mech, targ, MechX(targ), MechY(targ), range)) {
        hudinfo_notify(d, msgclass, "E", "Unable to scan target");
        return NULL;
    }

    if (!MechIsObservator(mech)) {
        mech_notify(targ, MECHSTARTED, tprintf("You are being scanned by %s", GetMechToMechID(targ, mech)));
        auto_reply(targ, tprintf("%s just scanned me.", GetMechToMechID(targ, mech)));
    }
...
static void hud_armorscan(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    for (sect = 0; sect < NUM_SECTIONS; sect++) {
        if (GetSectOInt(targ, sect)) {
            sprintf(response, "%s,%c,%c,%c", ShortArmorSectionString(MechType(targ), MechMove(targ),
                        sect), hud_damagechar(targ, sect, 1), hud_damagechar(targ, sect, 4), hud_damagechar(targ, sect, 2));
...
static void hud_weapscan(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    for (sect = 0; sect < NUM_SECTIONS; sect++) {
        ...
        for (i = 0; i < weapcount; i++) {
            weapnum++;
            sprintf(response, "%d,%d,%s,%s", weapnum, weaparray[i], ShortArmorSectionString(MechType(targ), MechMove(targ),
                        sect), hud_getweapscanstatus(targ, sect, critical[i], weapdata[i]));
...
static void hud_tactical(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    switch (argc) {
        ...
        case 3:
            ...
            if (!MechIsObservator(mech) && abs((int) range) > MechLRSRange(mech)) {
                hudinfo_notify(d, msgclass, "E", "Out of range");
                return;
            }
    ...
    if (lostactical || MapIsDark(map) || (MechType(mech) == CLASS_MW && btechconf.btech_mw_losmap))
        losmap = CalculateLOSMap(map, mech, sx, sy, ex - sx, ey - sy);

    if (!btechconf.hudinfo_show_mapinfo || (btechconf.hudinfo_show_mapinfo == 1 && In_Character(map->mynum))) {
        strcpy(mapid, "-1");
        strcpy(mapname, "-1");
    } else {
    ...
    sprintf(result, "%d,%d,%d,%d,%s,%s,-1", sx, sy, ex - 1, ey - 1, mapid, mapname);
...
static void hud_conditions(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    switch (map->maplight) {
        ...
        default:
            lt = '?';
            SendError(tprintf("Unknown light type %d on map #%d", map->maplight, map->mynum));
        break;
    }

    sprintf(res, "%c,%d,%d,%d,%s", lt, map->mapvis, map->grav, map->temp, hud_getmapflags(map));
