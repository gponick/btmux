...
#define Hardcode(a)     ((Flags3(a) & MARK_9) != 0) 
#define s_Hardcode(a)   s_Flags((a), FLAG_WORD3, Flags3(a) | MARK_9)
#define c_Hardcode(a)   s_Flags((a), FLAG_WORD3, Flags3(a) & ~MARK_9)

#define Zombie(a)       ((Flags3(a) & MARK_8) != 0)
#define s_Zombie(a)     s_Flags((a), FLAG_WORD3, Flags3(a) | MARK_8)
#define c_Zombie(a)     s_Flags((a), FLAG_WORD3, Flags3(a) & ~MARK_8)
...
typedef struct SpecialObjectStruct {
    ...
    void (*allocfreefunc) (int, void **, int);
    int updateTime;		/* Amount of time between updates */
    /* (secs) */
    void (*updatefunc) (int, void *);	/* called for every */
    /* object at every */
    /* update */
...
extern CF_HAND(cf_int);
extern CF_HAND(cf_bool);
extern CF_HAND(cf_string_dyn);

typedef struct btechdata BTECHDATA;
struct btechdata
{
    char *config_file;
    char *hcode_db;
...
extern BTECHDATA btechconf;

typedef struct confparm2 {
    char *pname;
...
void auto_newautopilot(dbref, void **, int);
void newturret(dbref, void **, int);
void newfreemech(dbref, void **, int);
// why isnt this being found from p.event.h
...
SpecialObjectStruct SpecialObjects[] = {
    LINE("MECH", mechcommands, MECH, newfreemech, HEAT_TICK, mech_update, POW_BUILDER),
    LINE_ND("DEBUG", debugcommands, POW_BUILDER),
    LINE_NU("MECHREP", mechrepcommands, struct mechrep_data, newfreemechrep, POW_BUILDER),
    LINE("MAP", mapcommands, MAP, newfreemap, LOS_TICK, map_update, POW_BUILDER),
    LINE_ND("CHARGEN", chargencommands, POW_BUILDER),
    LINE_NU("AUTOPILOT", autopilotcommands, AUTO, auto_newautopilot, POW_BUILDER),
    LINE_NU("TURRET", turretcommands, TURRET_T, newturret, POW_BUILDER),
    LINE_NU("CUSTOM", customcommands, struct custom_struct, newfreecustom, POW_BUILDER),
    LINE_NFS("SCEN", scencommands, SCEN, POW_BUILDER),
    LINE_NFS("SSIDE", sscommands, SSIDE, POW_BUILDER),
    LINE_NFS("SSOBJ", sscommands, SSOBJ, POW_BUILDER),
    LINE_NFS("SSINS", sscommands, SSINS, POW_BUILDER),
    LINE_NFS("SSEXT", sscommands, SSEXT, POW_BUILDER)
};
