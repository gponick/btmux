...
{
    struct mechrep_data *newmechrep = static_cast <mechrep_data *>(*data);

    switch (selector) {
        case SPECIAL_ALLOC:
            newmechrep->current_target = -1;
            break;
    }
}
...
void mechrep_Rsetradio(dbref player, void *data, char *buffer)
{
    ...
    notify(player, tprintf("Number of freqs: %d  Extra stuff: %d", MechRadioType(mech) % 16, (MechRadioType(mech) / 16) * 16));
    MechRadioRange(mech) = (int)(MechComputersRadioRange(mech));
    notify(player, tprintf("Radio range set to %d.", (int) MechRadioRange(mech)));
}
...
void mechrep_Rsettarget(dbref player, void *data, char *buffer)
{
    ...
    switch (mech_parseattributes(buffer, args, 2)) {
        case 1:
            ...
            DOCHECK(!(Good_obj(newmech) && Hardcode(newmech)), "That is not a BattleMech or Vehicle!");
...
void mechrep_Rsettype(dbref player, void *data, char *buffer)
{
    ...
    switch (mux_toupper(args[0][0])) {
        ...
        case 'B':
            ...
            break;
        default:
            notify(player, "Types are: MECH, GROUND, VTOL, NAVAL, AERO, DROPSHIP and SPHEROIDDROPSHIP");
            break;
    }
}
...
SETVALUE_FUNCTION_FLOAT(mechrep_Rsetspeed, MechMaxSpeed(mech), "Maxspeed", KPH_PER_MP);
SETVALUE_FUNCTION_FLOAT(mechrep_Rsetjumpspeed, MechJumpSpeed(mech), "Jumpspeed", KPH_PER_MP);
SETVALUE_FUNCTION_INT(mechrep_Rsetheatsinks, MechRealNumsinks(mech), "Heatsinks", 1);
...
SETVALUE_FUNCTION_INT(mechrep_Rsetscanrange, MechScanRange(mech), "SCANrange", 1);
SETVALUE_FUNCTION_INT(mechrep_Rsetradiorange, MechRadioRange(mech), "RADIOrange", 1);
...
void mechrep_Rsetmove(dbref player, void *data, char *buffer)
{
    ...
    switch (mux_toupper(args[0][0])) {
        ...
        case 'N':
            ...
            break;
        default:
            notify(player, "Types are: TRACK, WHEEL, VTOL, HOVER, HULL, FLY, SUB, FOIL and NONE");
            break;
    }
}
...
{
    struct tmpldirent const *p1 = static_cast < const tmpldirent * >(v1);
    struct tmpldirent const *p2 = static_cast < const tmpldirent * >(v2);
...
static int scan_template_dir(char const *dirname, char const *parent)
{
    ...
    while (1) {
        ...
        if (parent == NULL && S_ISDIR(sb.st_mode) && ent->d_name[0] != '.' && strlen(ent->d_name) <= CACHE_MAXNAME) {
            struct tmpldir *link;
...
redo:
    if (strchr(id, '/') == NULL && (tmpl_list != NULL ||
        ...
        ent = static_cast <tmpldirent *>(bsearch(&key, tmpl_list, tmpl_pos, sizeof tmpl_list[0], tmplcmp));
        if (ent == NULL) 
            return NULL;
...
int load_mechdata(MECH * mech, char *id)
{
    ...
    filename = mechref_path(id);
    TEMPLATE_GERR(filename == NULL, tprintf("No matching file for '%s'.", id));
    if (filename)
        fp = fopen(filename, "r");
    TEMPLATE_GERR(!fp, tprintf("Unable to open file %s (%s)!", filename, id));
    strncpy(MechType_Ref(mech), id, 15);
    MechType_Ref(mech)[14] = '\0';
    TEMPLATE_GERR(fscanf(fp, "%d %d %d %d %d %f %f %d\n", &i1, &i2, &i3, &i4, &i5, &MechMaxSpeed(mech), &MechJumpSpeed(mech), &i6) < 8, "Old template loading system: %s is invalid template file.", id);
    MechTons(mech) = i1;
...
#define DROP(a) \
    ...
    for (k = 0; k < NUM_SECTIONS; k++) {
        ...
        TEMPLATE_GERR(fscanf(fp, "%d %d %d %d\n", &i1, &i2, &i3, &i4) < 4, "Insufficient data reading section %d!", i);
        ...
        for (j = 0; j < NUM_CRITICALS; j++) {
            TEMPLATE_GERR(fscanf(fp, "%d %d %d\n", &i1, &i2, &i3) < 3, "Insufficient data reading critical %d/%d!", i, j);
            MechSections(mech)[i].criticals[j].type = i1;
            TEMPLATE_GERR(unable_to_find_proper_type(GetPartType(mech, i, j)), "Invalid datatype at %d/%d!", i, j);
            if (IsSpecial(i1))
...
void mechrep_Rrestore(dbref player, void *data, char *buffer)
{
    ...
    if ((pattr = atr_str("MECHREF")) && pattr->number)
        c = atr_get(mech->mynum, pattr->number, &aowner, &aflags);
    else
        c = "";
...
void mechrep_Rsavetemp(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(mech_parseattributes(buffer, args, 1) != 1, "You must specify a template name!");
    ...
    DOCHECK(!(fp = fopen(openfile, "w")), "Unable to open/create mech file! Sorry.");
    fprintf(fp, "%d %d %d %d %d %.2f %.2f %d\n", MechTons(mech), MechTacRange(mech), MechLRSRange(mech), MechScanRange(mech), MechRealNumsinks(mech), MechMaxSpeed(mech), MechJumpSpeed(mech), MechSpecials(mech));
    for (i = 0; i < NUM_SECTIONS; i++) {
        fprintf(fp, "%d %d %d %d\n", GetSectArmor(mech, i), GetSectInt(mech, i), GetSectRArmor(mech, i), MechSections(mech)[i].config);
        for (j = 0; j < NUM_CRITICALS; j++) 
            fprintf(fp, "%d %d %d\n", MechSections(mech)[i].criticals[j].type, MechSections(mech)[i].criticals[j].data, MechSections(mech)[i].criticals[j].firemode);
...
void mechrep_Rsavetemp2(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(save_template(player, mech, args[0], openfile) < 0, "Error saving the template file!");
    notify(player, "Saving complete!");
}
...
void mechrep_Raddweap(dbref player, void *data, char *buffer)
{
    ...
    if (args[argc - 1][0] == 'T' || args[argc - 1][0] == 't' || args[argc - 1][1] == 'T' || args[argc - 1][1] == 't')
        istc = 1;

    if (args[argc - 1][0] == 'R' || args[argc - 1][0] == 'r' || args[argc - 1][1] == 'R' || args[argc - 1][1] == 'r')
        isrear = 1;
...
void mechrep_Rreload(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(subsect < 0 || subsect >= CritsInLoc(mech, index), "Critslot out of range!");
        ...
        if (argc > 3)
            switch (mux_toupper(args[3][0])) {
                ...
                case 'S':
                    ...
                    break;
                case 'X':
                    MechSections(mech)[index].criticals[subsect].ammomode |= INARC_EXPLO_MODE;
                    break;
                case 'Y':
                    MechSections(mech)[index].criticals[subsect].ammomode |= INARC_HAYWIRE_MODE;
                    break;
                case 'E':
                ...
                case 'R':
                    ...
                    break;
                case 'F':
                    MechSections(mech)[index].criticals[subsect].ammomode |= AC_FLECHETTE_MODE;
                    break;
                case 'D':
                    MechSections(mech)[index].criticals[subsect].ammomode |= AC_INCENDIARY_MODE;
                    break;
                case 'P':
                    MechSections(mech)[index].criticals[subsect].ammomode |= AC_PRECISION_MODE;
                    break;
            }
...
void mechrep_Raddspecial(dbref player, void *data, char *buffer)
{
    ...
    MechSections(mech)[index].criticals[subsect].type = itemcode < 0 ? 0 : I2Special(itemcode);
    MechSections(mech)[index].criticals[subsect].data = newdata;

    switch (itemcode) {
        case CASE:
            MechSections(mech)[(MechType(mech) == CLASS_VEH_GROUND) ? BSIDE : index].config |= CASE_TECH;
...
{
    return const_cast <char *>((MechSpecials(mech) || MechSpecials2(mech)) ? BuildBitString2(specials, specials2, MechSpecials(mech), MechSpecials2(mech)) : "");
}
...
{
    return BuildBitString3(specials, specials2, infantry_specials, MechSpecials(mech), MechSpecials2(mech), MechInfantrySpecials(mech));
}
...
void mechrep_Rdeltech(dbref player, void *data, char *buffer)
{
    ...
    if (((nv < 0) && (nv2 < 0)) && (strcmp(buffer, "All") != 0) && (strcmp(buffer, "Case") != 0)) {
    ...
    if (((!nv) && (!nv2)) && (strcmp(buffer, "All") != 0) && (strcmp(buffer, "Case") != 0)) {
    ...
    if (strcmp(buffer, "All") == 0) {
        for (i = 0; i < NUM_SECTIONS; i++) {
            if ((MechSections(mech)[i].config & CASE_TECH) || (MechSpecials(mech) & TRIPLE_MYOMER_TECH) || (MechSpecials(mech) & MASC_TECH)) {
                for (j = 0; j < NUM_CRITICALS; j++) {
                    Type = MechSections(mech)[i].criticals[j].type;
                    if (Type == I2Special((CASE)) || Type == I2Special((TRIPLE_STRENGTH_MYOMER)) || Type == I2Special((MASC))) 
