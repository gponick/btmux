...
{
    int delay = MAX(ARTILLERY_MINIMUM_FLIGHT, (int)(FindHexRange(fx, fy, tx, ty) / ARTY_SPEED));
    ...
    int wmode, int ishit)
{
    ...
    s->map = mech->mapindex;
    MechLOSBroadcast(mech, tprintf("shoots %s towards %s!", artillery_type(s), artillery_direction(s)));
    MapCoordToRealCoord(s->from_x, s->from_y, &fx, &fy);
    MapCoordToRealCoord(s->to_x, s->to_y, &tx, &ty);
    muxevent_add(artillery_round_flight_time(fx, fy, tx, ty), 0, EVENT_DHIT, artillery_hit_event, (void *) s, NULL);
}
    ...
    for (loop = 0; loop < map->first_free; loop++)
        if (map->mechsOnMap[loop] >= 0) {
            ...
            while (damleft > 0) {
                ...
                DamageMech(tempMech, tempMech, 0, -1, hitloc, isrear, iscritical, ndam, 0, -1, 0, -1, 0, 0);
            }
    ...
    int safeup, int safedown, int isunderwater)
{
    ...
    blast_hit_hexf(map, dam, singlehitsize, heatdam, ffx, ffy, ftx, fty, tomsg, otmsg, table, safeup, safedown, isunderwater);
}
    ...
    int isunderwater, int doneighbors)
{
    ...
    dm = MAX(1, (int) t + 1);
    blast_hit_hexf(map, dam / dm, singlehitsize, heatdam / dm, fx, fy, ftx, fty, tomsg, otmsg, table, safeup, safedown, isunderwater);
    if (!doneighbors)
        return;
    ...
    for (x1 = (tx - doneighbors); x1 <= (tx + doneighbors); x1++)
        for (y1 = (ty - doneighbors); y1 <= (ty + doneighbors); y1++) {
            ...
            blast_hit_hexf(map, dam / dm, singlehitsize, heatdam / dm, hx, hy, ftx, fty, spot ? tomsg : tomsg1, spot ? otmsg : otmsg1, table, safeup, safedown, isunderwater);
    ...
    int mode, int dam, int tx, int ty, int isdirect)
{
    ...
    blast_hit_hex(map, dam, (mode & CLUSTER_MODE) ? 2 : 5, 0, tx, ty, tx, ty, buf2, buf1, (mode & CLUSTER_MODE) ? TABLE_PUNCH : TABLE_GEN, 10, 4, 0);
}
...
{
    artillery_hit_hex(map, hit_neighbors_s, hit_neighbors_type, hit_neighbors_mode, hit_neighbors_dam, x, y, 0);
}
    ...
    int mode, int dam, int tx, int ty)
{
    ...
    for (xd = 0; xd < 5; xd++)
        for (yd = 0; yd < 5; yd++)
            if ((d = targets[xd][yd]))
                artillery_hit_hex(map, s, type, mode, d * 2, xd + tx - 2, yd + ty - 2, 1);
}
...
void artillery_FriendlyAdjustment(dbref mechnum, MAP * map, int x, int y)
{
    ...
    if (!((MechTargX(mech) == x && MechTargY(mech) == y) || (spotter && (MechTargX(spotter) == x && MechTargY(spotter) == y))))
        return;
    ...
    if (spotter) {
        mech_notify(mech, MECHSTARTED, tprintf("%s sent you some trajectory-correction data.", GetMechToMechID(mech, tempMech)));
        mech_notify(tempMech, MECHSTARTED, tprintf("You provide %s with information about the miss.", GetMechToMechID(tempMech, mech)));
    }
...
static void artillery_hit(artillery_shot * s)
{
    ...
    if (!s->ishit) {
        ...
        original_x = s->to_x;
        original_y = s->to_y;
        s->to_x = s->to_x + (int)(dist * cos(dir));
        s->to_y = s->to_y + (int)(dist * sin(dir));
    ...
    if (!(s->mode & ARTILLERY_MODES))
        ...
        if (s->mode & CLUSTER_MODE)
            HexLOSBroadcast(map, s->to_x, s->to_y, "A rain of small bomblets hits $H's surroundings!");
        else 
            if (s->mode & MINE_MODE)
                HexLOSBroadcast(map, s->to_x, s->to_y, "A rain of small bomblets hits $H!");
            else 
                if (s->mode & SMOKE_MODE)
                    HexLOSBroadcast(map, s->to_x, s->to_y, tprintf("A %s %s hits $h, and smoke starts to billow!", &MechWeapons[s->type].name[3], &(artillery_type(s)[2])));
