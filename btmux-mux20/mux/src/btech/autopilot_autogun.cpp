...
int SearchLightInRange(MECH *mech, MAP *map) {
    ...
    for (i = 0; i < map->first_free; i++) {
        ...
        /* No units on the map */
        if (!(target = static_cast <MECH *> (FindObjectsData(map->mechsOnMap[i]))))
            continue;
...
void auto_sensor_event(EVENT *muxevent) {
...
weapon_node *auto_create_weapon_node(short weapon_number, short weapon_db_number,
    ...
    temp = static_cast <weapon_node *> (malloc(sizeof(weapon_node)));
...
target_node *auto_create_target_node(int target_score, dbref target_dbref) {
    ...
    temp = static_cast <target_node *> (malloc(sizeof(target_node)));
...
int auto_calc_weapon_score(int weapon_db_number, int range) {
    ...
    if (range < MechWeapons[weapon_db_number].min) {
        minrange_score = (int)(-12.5 * (float) ((MechWeapons[weapon_db_number].min - range) *
 ...
 */
void auto_update_profile_event(EVENT *muxevent) {
...
int auto_calc_target_score(AUTO *autopilot, MECH *mech, MECH *target, MAP *map) {
    ...
    speed_score = (int)(- 2 * target_speed + 300);
 ...
 */
void auto_gun_event(EVENT *muxevent) {
    ...
    if (autopilot->target == -1 || 
        ...
        if (autopilot->target > -1 && autopilot->target_score > 0) {
            ...
            if (autopilot->target != temp_target_node->target_dbref) {
                ...
                threshold_score = (int)(((100.0 + (float) autopilot->target_threshold) / 100.0) *
    ...
    if ((range < (float) AUTO_GUN_MAX_RANGE) && autopilot->profile[(int) range]) {
        ...
        accumulate_heat = (int)(MechWeapHeat(mech));
