...
static void mech_srec_event(EVENT * e)
{
    ...
    switch (vt) {
        case 0:
            ...
            if (!Destroyed(mech))
                mech_notify(mech, MECHALL, "Your tactical scanners are operational again.");
            break;
        case 1:
            MechLRSRange(mech) = val;
            if (!Destroyed(mech))
                mech_notify(mech, MECHALL, "Your long-range scanners are operational again.");
            break;
        case 2:
...
{
    MECHEVENT(mech, EVENT_MRECOVERY, mech_rrec_event, Number(30, Number(40, 200)), (int) MechRadioRange(mech));
...
void FailureRadioRange(MECH * mech, int weapnum, int weaptype, int section, int critical, int roll, int *modifier, int *type)
{
    ...
    MECHEVENT(mech, EVENT_MRECOVERY, mech_rrec_event, Number(30, Number(40, 200)), (int) mod);
...
void FailureComputerScanner(MECH * mech, int weapnum, int weaptype, int section, int critical, int roll, int *modifier, int *type)
{
    ...
    switch (tmp) {
        case 1:
            MECHEVENT(mech, EVENT_MRECOVERY, mech_srec_event, Number(30, Number(40, 200)), (int) MechTacRange(mech));
            MechTacRange(mech) = 0;
            break;
        case 2:
            MECHEVENT(mech, EVENT_MRECOVERY, mech_srec_event, Number(30, Number(40, 200)), MechLRSRange(mech) + 256);
            MechLRSRange(mech) = 0;
            break;
        case 4:
            MECHEVENT(mech, EVENT_MRECOVERY, mech_srec_event, Number(30, Number(40, 200)), MechScanRange(mech) + 512);
...
{
    SetPartTempNuke(mech, section, critical, failures[Conv(mech, section, critical) + roll].type);
    *type = CRAZY_MISSILES;
...
{
    *modifier = (int) (EGunRangeWithCheck(mech, section, weaptype) * (failures[Conv(mech, section, critical) + roll].data / 100.0));
    *type = RANGE;
}
...
{
    *modifier = (int) (MechWeapons[weaptype].damage * (failures[Conv(mech, section, critical) + roll].data / 100.0));
    *type = DAMAGE;
}

void FailureWeaponHeat(MECH * mech, int weapnum, int weaptype, int section, int critical, int roll, int *modifier, int *type)
{
    *modifier = (int)(MechWeapons[weaptype].heat * (failures[Conv(mech, section, critical) + roll].data / 100.0));
    *type = HEAT;
}
...
void CheckGenericFail(MECH * mech, int type, int *result, int *mod)
{
    ...
    switch (failures[in].flag) {
        ...
        case REQ_LRS:
            ...
            break;
        case REQ_SCANNERS:
            if (MechTacRange(mech) == 0 || MechLRSRange(mech) == 0 || MechScanRange(mech) == 0)
                return;
            break;
        case REQ_COMPUTER:
            /* */
...
void CheckWeaponFailed(MECH * mech, int weapnum, int weaptype, int section, int critical, int *modifier, int *type)
{
    ...
    *type = FAIL_NONE;
    ...
    failures[in].func(mech, weapnum, weaptype, section, critical, roll, modifier, type);
}
