...
int num_def_weapons = NUM_DEF_WEAPONS;
    ...
    if (MechType(mech) == CLASS_AERO)
        switch (index) {
            case AERO_COCKPIT:
                ...
                MechSections(mech)[index].criticals[4].type = I2Special(LIFE_SUPPORT);
                break;
            case AERO_ENGINE:
    ...
    if (MechType(mech) == CLASS_MECH)
        switch (index) {
            ...
            case RLEG:
                ...
                MechSections(mech)[index].criticals[3].type = I2Special(HAND_OR_FOOT_ACTUATOR);
                break;
        }
}
...
char *ShortArmorSectionString(char type, char mtype, int loc)
{
    ...
    for (i = 0; locs[loc][i]; i++)
        if (mux_isupper(locs[loc][i]) || mux_isdigit(locs[loc][i]))
            *(c++) = locs[loc][i];
...
int ArmorSectionFromString(char type, char mtype, char *string)
{
    ...
    for (i = 0; locs[i]; i++) {
        if (mux_toupper(string[0]) != locs[i][0])
            continue;
    
        for (j = (i + 1); locs[j]; j++)
            if (mux_toupper(string[0]) == locs[j][0])
                break;
        ...
        if (!string[1])
            continue;
        if (c && mux_toupper(string[1]) == *(++c))
            return i;
        if (d && mux_toupper(string[1]) == *(++d))
            return j;
    }
