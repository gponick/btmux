...
extern void KillText(char **mapt);
extern void ShowText(char **mapt, dbref player);
...
{
    if (m->value)
        sprintf(*c, " %s<%%cbX%%c%%ch>%%c", !(m->flags & CM_NO_HILITE) ? "%ch" : "");
    else
        sprintf(*c, " < >");
...
{
    if (m->value >= 0) 
        sprintf(*c, " %%cg%s%4s%%c", (m->value > 0 && !(m->flags & CM_NO_HILITE)) ? "%ch" : "", stringified_value(m->value));
    else
        sprintf(*c, " ____");
...
coolmenu *SelCol_Menu(int columns, char *heading, char **strings, int type, int max)
{
    coolmenu *c = NULL;
    int i, co = 0;
    char buf[LBUF_SIZE];
...
coolmenu *SelCol_FunStringMenuK(int columns, char *heading, char *(*fun) (int), int last)
{
    coolmenu *c = NULL;
    int i;
    char buf[LBUF_SIZE];
