...
static int get_arm_args(int *usinga, int *argc, char ***args, MECH * mech,
    int (*have_fn) (MECH * mech, int loc), char *weapon) {

    if (*argc != 0 && args[0][0][0] != '\0' && args[0][0][1] == '\0') {
        char arm = mux_toupper(args[0][0][0]);

        switch (arm) {
            case 'B':
                *usinga = P_LEFT | P_RIGHT;
                --*argc;
                ++*args;
                break;
            case 'L':
    ...
    switch (*usinga) {
        case P_LEFT:
            if (!have_fn(mech, LARM)) {
                mech_notify(mech, MECHALL, tprintf("You don't have %s in your left arm!", weapon));
                return 1;
            }
            break;
        case P_RIGHT:
            if (!have_fn(mech, RARM)) {
                mech_notify(mech, MECHALL, tprintf("You don't have %s in your right arm!", weapon));
                return 1;
            }
            break;
        case P_LEFT | P_RIGHT:
    ...
    if (punching & P_LEFT) {
        if (SectIsDestroyed(mech, LARM))
            ...
            if (!OkayCritSectS(LARM, 0, SHOULDER_OR_HIP))
                ...
                if (Fallen(mech)) {
                    DOCHECK(SectIsDestroyed(mech, RARM), "You need both arms functional to punch while prone.");
                    DOCHECK(SectHasBusyWeap(mech, RARM), "You have weapons recycling on your Right Arm.");
                    DOCHECK(MechSections(mech)[RARM].recycle, "Your Right Arm is still recovering from your last attack.");
                }
        
                DOCHECK(MechSections(mech)[RARM].specials & CARRYING_CLUB, "You're carrying a club in that arm.");
    ...
    if (punching & P_RIGHT) {
        if (SectIsDestroyed(mech, RARM))
            ...
            if (!OkayCritSectS(RARM, 0, SHOULDER_OR_HIP))
                ...
                if (Fallen(mech)) {
                    DOCHECK(SectIsDestroyed(mech, LARM), "You need both arms functional to punch while prone.");
                    DOCHECK(SectHasBusyWeap(mech, LARM), "You have weapons recycling on your Left Arm.");
                    DOCHECK(MechSections(mech)[LARM].recycle, "Your Left Arm is still recovering from your last attack.");
                }
        
                DOCHECK(MechSections(mech)[LARM].specials & CARRYING_CLUB, "You're carrying a club in that arm.");
...
void mech_club(dbref player, void *data, char *buffer)
{
    ...
    if (clubLoc == -1) {
        DOCHECKMA(MechRTerrain(mech) != HEAVY_FOREST && MechRTerrain(mech) != LIGHT_FOREST, "You can not seem to find any trees around to club with.");
        clubLoc = RARM;
    }

    argc = mech_parseattributes(buffer, args, 5);
    DOCHECKMA(SectIsDestroyed(mech, LARM), "Your left arm is destroyed, you can't club.");
    DOCHECKMA(SectIsDestroyed(mech, RARM), "Your right arm is destroyed, you can't club.");
    DOCHECKMA(!OkayCritSectS(RARM, 0, SHOULDER_OR_HIP), "You can't club anyone with a destroyed or missing right shoulder.");
    DOCHECKMA(!OkayCritSectS(LARM, 0, SHOULDER_OR_HIP), "You can't club anyone with a destroyed or missing left shoulder.");
    DOCHECKMA(!OkayCritSectS(RARM, 3, HAND_OR_FOOT_ACTUATOR), "You can't club anyone with a destroyed or missing right hand.");
    DOCHECKMA(!OkayCritSectS(LARM, 3, HAND_OR_FOOT_ACTUATOR), "You can't club anyone with a destroyed or missing left hand.");

    PhysicalAttack(mech, 5, (btechconf.phys_use_pskill ? FindPilotPiloting(mech) - 1 : 4) + 2 * MechSections(mech)[RARM].basetohit + 2 * MechSections(mech)[LARM].basetohit, PA_CLUB, argc, args, mech_map, RARM);
}
...
void mech_axe(dbref player, void *data, char *buffer)
{
    ...
    if (get_arm_args(&usinga, &argc, &args, mech, have_axe, "an axe")) 
        return;

    if (usinga & P_LEFT) {
        DOCHECK(SectIsDestroyed(mech, LARM), "Your left arm is destroyed, you can't axe with it.");
        DOCHECK(!OkayCritSectS(LARM, 0, SHOULDER_OR_HIP), "Your left shoulder is destroyed, you can't axe with that arm.");
        DOCHECK(!OkayCritSectS(LARM, 3, HAND_OR_FOOT_ACTUATOR), "Your left hand is destroyed, you can't axe with that arm.");

        PhysicalAttack(mech, 5, ltohit, PA_AXE, argc, args, mech_map, LARM);
    }

    if (usinga & P_RIGHT) {
        DOCHECK(SectIsDestroyed(mech, RARM), "Your right arm is destroyed, you can't axe with it.");
        DOCHECK(!OkayCritSectS(RARM, 0, SHOULDER_OR_HIP), "Your right shoulder is destroyed, you can't axe with that arm.");
        DOCHECK(!OkayCritSectS(RARM, 3, HAND_OR_FOOT_ACTUATOR), "Your right hand is destroyed, you can't axe with that arm.");

        PhysicalAttack(mech, 5, rtohit, PA_AXE, argc, args, mech_map, RARM);
    }

    DOCHECKMA(!usinga, "You may lack the axe, but not the will! Try punch/club until you find one.");
}
...
void mech_sword(dbref player, void *data, char *buffer)
{
    ...
    if (get_arm_args(&usinga, &argc, &args, mech, have_sword, "a sword")) 
        return;

    if (usinga & P_LEFT) {
        DOCHECK(SectIsDestroyed(mech, LARM), "Your left arm is destroyed, you can't use a sword with it.");
        DOCHECK(!OkayCritSectS(LARM, 0, SHOULDER_OR_HIP), "Your left shoulder is destroyed, you can't use a sword with that arm.");
        DOCHECK(!OkayCritSectS(LARM, 3, HAND_OR_FOOT_ACTUATOR), "Your left hand is destroyed, you can't use a sword with that arm.");

        PhysicalAttack(mech, 10, ltohit, PA_SWORD, argc, args, mech_map, LARM);
    }
    
    if (usinga & P_RIGHT) {
        DOCHECK(SectIsDestroyed(mech, RARM), "Your right arm is destroyed, you can't use a sword with it.");
        DOCHECK(!OkayCritSectS(RARM, 0, SHOULDER_OR_HIP), "Your right shoulder is destroyed, you can't use a sword with that arm.");
        DOCHECK(!OkayCritSectS(RARM, 3, HAND_OR_FOOT_ACTUATOR), "Your right hand is destroyed, you can't use a sword with that arm.");

        PhysicalAttack(mech, 10, rtohit, PA_SWORD, argc, args, mech_map, RARM);
    }
    
    DOCHECKMA(!usinga, "You have no sword to chop people with!");
}
...
void mech_kick(dbref player, void *data, char *buffer)
{
    ...
    if (get_arm_args(&usinga, &argc, &args, mech, have_punch, "")) 
        return;
    ...
    PhysicalAttack(mech, 5, (btechconf.phys_use_pskill ? FindPilotPiloting(mech) - 2 : 3), PA_KICK, argc, args, mech_map, leg);
}
...
void mech_charge(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALO);
    DOCHECK(Dodging(mech) || MoveModeLock(mech), "You cannot use physicals while using a special movement mode.");
    DOCHECK(MechType(mech) == CLASS_MW || MechType(mech) == CLASS_BSUIT, "You cannot charge without a 'mech!");
    DOCHECK(MechType(mech) != CLASS_MECH && (MechType(mech) != CLASS_VEH_GROUND || MechSpecials(mech) & SALVAGE_TECH), "You cannot charge with this vehicle!");

    if (MechType(mech) == CLASS_MECH) {
        /* set the number of dead legs we have */
        wcDeadLegs = CountDestroyedLegs(mech);

        DOCHECK(!MechIsQuad(mech) && (wcDeadLegs > 0), "With one leg? Are you kidding?");
        DOCHECK(!MechIsQuad(mech) && (wcDeadLegs > 1), "Without legs? Are you kidding?");
    ...
    switch (argc) {
        ...
        case 1:
            ...
            DOCHECKMA(!InLineOfSight_NB(mech, target, MechX(target), MechY(target), FaMechRange(mech, target)), "Target is not in line of sight!");
            ...
            mech_notify(mech, MECHALL, tprintf("%s target set to %s.", MechType(mech) == CLASS_MECH ? "Charge" : "Ram", GetMechToMechID(mech, target)));
            break;
        default:
    ...
    int AttackType, int argc, char **args, MAP * mech_map, int sect)
{
    ...
    DOCHECKMA(Fallen(mech) && (AttackType != PA_PUNCH), "You can't attack from a prone position.");
...
#define Check(num,okval,mod) \
  ...
  baseToHit += mod;
    ...
    if (AttackType == PA_KICK)
        DOCHECKMA((MechCritStatus(mech) & HIP_DAMAGED), "You can not kick if your have a destroyed hip.");
    ...
    switch (AttackType) {
        ...
        case PA_SWORD:
        case PA_AXE:
            DOCHECKMA(MechSections(mech)[LARM].recycle || MechSections(mech)[RARM].recycle, "You still have arms recovering from another attack.");
            DOCHECKMA(Fallen(mech), "You can't do this while fallen!");
        case PA_PUNCH:
            DOCHECKMA(MechSections(mech)[LLEG].recycle || MechSections(mech)[RLEG].recycle, "You're still recovering from another attack.");
            if (MechSections(mech)[LARM].recycle)
                DOCHECKMA(MechSections(mech)[LARM].config & AXED, "You are recovering from another attack.");

            if (MechSections(mech)[RARM].recycle)
                DOCHECKMA(MechSections(mech)[RARM].config & AXED, "You are recovering from another attack.");

            break;
        case PA_CLUB:
            DOCHECKMA(MechSections(mech)[LLEG].recycle || MechSections(mech)[RLEG].recycle, "You're still recovering from your kick.");
            /* Check Weapons recycling on LARM because we only checked RARM above. */
            DOCHECKMA(SectHasBusyWeap(mech, LARM), "You have weapons recycling on your Left Arm.");
            DOCHECKMA(Fallen(mech), "You can't do this while fallen!");
            break;
        case PA_KICK:
            DOCHECKMA(Fallen(mech), "You can't kick while fallen!");
            if (MechIsQuad(mech)) {
                DOCHECKMA(MechSections(mech)[LLEG].recycle || MechSections(mech)[RLEG].recycle, "Your rear legs are still recovering from your last attack.");
                DOCHECKMA(MechSections(mech)[RARM].recycle || MechSections(mech)[LARM].recycle, "Your front legs are not ready to attack again.");
            } else {
                DOCHECKMA(MechSections(mech)[LLEG].recycle || MechSections(mech)[RLEG].recycle, "Your legs are not ready to attack again.");
                DOCHECKMA(MechSections(mech)[RARM].recycle || MechSections(mech)[LARM].recycle, "Your arms are still recovering from your last attack.");
            }
            break;
    }
    ...
    if (MechSections(mech)[sect].recycle != 0) {
        ArmorStringFromIndex(sect, location, MechType(mech), MechMove(mech));
        mech_notify(mech, MECHALL, tprintf("Your %s is not ready to attack again.", location));
        return;
    }
    ...
    switch (argc) {
        ...
        case 0:
            ...
            DOCHECKMA((Fallen(mech) && (AttackType == PA_PUNCH)) && (MechType(target) != CLASS_VEH_GROUND), "You can only punch vehicles while you're fallen");
            range = FaMechRange(mech, target);
            DOCHECKMA(!InLineOfSight_NB(mech, target, MechX(target), MechY(target), range), "You are unable to hit that target at the moment.");
        ...
        default:
            ...
            DOCHECKMA(!InLineOfSight_NB(mech, target, MechX(target), MechY(target), range), "Target is not in line of sight!");
            DOCHECKMA(range >= maxRange, "Target out of range!");
    }

    DOCHECKMA((Fallen(mech) && (AttackType == PA_PUNCH)) && (MechType(target) != CLASS_VEH_GROUND), "You can only punch vehicles while you're fallen");
    DOCHECKMA(MechTeam(target) == MechTeam(mech) && MechNoFriendlyFire(mech), "You can't attack a teammate with FFSafeties on!");
    DOCHECKMA(MechType(target) == CLASS_MW && !MechPKiller(mech), "That's a living, breathing person! Switch off the safety first, " "if you really want to assassinate the target.");

    if (MechMove(target) != MOVE_VTOL && MechMove(target) != MOVE_FLY) {
        DOCHECKMA((AttackType == PA_PUNCH || AttackType == PA_AXE || AttackType == PA_SWORD) && (MechZ(mech) - 1) > MechZ(target), tprintf("The target is too low in elevation for you to %s.", AttackType == PA_PUNCH ? "punch at." : "axe it."));
        DOCHECKMA(AttackType == PA_KICK && MechZ(mech) < MechZ(target), "The target is too high in elevation for you to kick at.");
        DOCHECKMA(MechZ(mech) - MechZ(target) > 1 || MechZ(target) - MechZ(mech) > 1, "You can't attack, the elevation difference is too large.");
        DOCHECKMA( (AttackType == PA_KICK) && (MechZ(target) < MechZ(mech) && (((MechType(target) == CLASS_MECH) && Fallen(target)) || (MechType(target) == CLASS_VEH_GROUND) || (MechType(target) == CLASS_BSUIT) || (MechType(target) == CLASS_MW))), "The target is too low for you to kick at.")
    } else {
        DOCHECKMA((AttackType == PA_PUNCH || AttackType == PA_AXE || AttackType == PA_SWORD) && MechZ(target) - MechZ(mech) > 3, tprintf("The target is too far away for you to %s.", AttackType == PA_PUNCH ? "punch at" : "axe it"));
        DOCHECKMA(AttackType == PA_KICK && MechZ(mech) != MechZ(target), "The target is too far away for you to kick at.");
        DOCHECKMA(!(MechZ(target) - MechZ(mech) > -1 && MechZ(target) - MechZ(mech) < 4), "You can't attack, the elevation difference is too large.");
    }
    ...
    if (AttackType == PA_KICK) {
        ...
        if (AttackType == PA_CLUB) {
            ...
            if (sect == RARM) {
                DOCHECKMA(!((iwa & FORWARDARC) || (iwa & RSIDEARC)), "Target is not in your forward or right side arc!");
            } else {
                DOCHECKMA(!((iwa & FORWARDARC) || (iwa & LSIDEARC)), "Target is not in your forward or left side arc!");
            }
    }
    ...
    baseToHit += ((MechType(target) == CLASS_BSUIT) && (AttackType == PA_KICK)) ? 3 : 0;

    if (Dodging(target))
        baseToHit += 2;

    if ((AttackType == PA_PUNCH || AttackType == PA_AXE || AttackType == PA_SWORD) && MechType(target) == CLASS_BSUIT && MechSwarmTarget(target) > 0)
        baseToHit += (AttackType == PA_AXE || AttackType == PA_SWORD) ? 3 : 5;

    DOCHECKMA((AttackType != PA_PUNCH && AttackType != PA_AXE && AttackType != PA_SWORD) && MechType(target) == CLASS_BSUIT && MechSwarmTarget(target) > 0, "You can hit swarming 'suits only with punches (or axe/sword)!");
    roll = Roll();
    ...
    switch (AttackType) {
        case PA_PUNCH:
            ...
            if (MechZ(mech) >= MechZ(target))
                DOCHECKMA(((Fallen(target) && MechType(target) == CLASS_MECH) || ((MechType(target) != CLASS_MECH && (MechType(target) != CLASS_BSUIT || MechSwarmTarget(target) != mech->mynum) && !IsDS(target)) && !Fallen(mech))), "The target is too low to punch!");

            DOCHECKMA(Jumping(target) || (Jumping(mech) && MechType(target) == CLASS_MECH), "You cannot physically attack a jumping mech!");
            DOCHECKMA(Standing(mech), "You are still trying to stand up!");
            mech_notify(mech, MECHALL, tprintf("You try to punch the %s.  BTH:  %d,\tRoll:  %d", GetMechToMechID(mech, target), baseToHit, roll));
            mech_notify(target, MECHSTARTED, tprintf("%s tries to punch you!", GetMechToMechID(target, mech)));
             ...
             */
            SendAttacks(tprintf("#%i attacks #%i (punch) (%i/%i)", mech->mynum, target->mynum, baseToHit, roll));
            break;
        case PA_SWORD:
        case PA_AXE:
        case PA_CLUB:
            ...
            DOCHECKMA(Standing(mech), "You are still trying to stand up!");
            if (AttackType == PA_CLUB) {
                mech_notify(mech, MECHALL, tprintf("You try and club %s.  BaseToHit:  %d,\tRoll:  %d", GetMechToMechID(mech, target), baseToHit, roll));
                mech_notify(target, MECHSTARTED, tprintf("%s tries to club you!", GetMechToMechID(target, mech)));

                /* Switching to Exile method of tracking xp, where we split
                 * Attacking and Piloting xp into two different channels
                 * And since this is neither it goes to its own channel */
                SendAttacks(tprintf("#%i attacks #%i (club) (%i/%i)", mech->mynum, target->mynum, baseToHit, roll));
                /* SendXP(tprintf("#%i attacks #%i (club) (%i/%i)", mech->mynum, target->mynum, baseToHit, roll)); */
            } else {
                mech_notify(mech, MECHALL, tprintf("You try to swing your %s at %s.  BTH:  %d,\tRoll:  %d", AttackType == PA_SWORD ? "sword" : "axe", GetMechToMechID(mech, target), baseToHit, roll));
                mech_notify(target, MECHSTARTED, tprintf("%s tries to %s you!", GetMechToMechID(target, mech), AttackType == PA_SWORD ? "swing a sword at" : "axe"));
                /* Switching to Exile method of tracking xp, where we split
                 * Attacking and Piloting xp into two different channels
                 * And since this is neither it goes to its own channel */
                SendAttacks(tprintf("#%i attacks #%i (%s) (%i/%i)", mech->mynum, target->mynum, AttackType == PA_SWORD ? "sword" : "axe", baseToHit, roll));
            }
            break;
        case PA_KICK:
            DOCHECKMA(Jumping(target) || (Jumping(mech) && MechType(target) == CLASS_MECH), "You cannot physically attack a jumping mech!");
            DOCHECKMA(Standing(mech), "You are still trying to stand up!");
            mech_notify(mech, MECHALL, tprintf("You try and kick %s.  BaseToHit:  %d,\tRoll:  %d", GetMechToMechID(mech, target), baseToHit, roll));
            mech_notify(target, MECHSTARTED, tprintf("%s tries to kick you!", GetMechToMechID(target, mech)));

            /* Switching to Exile method of tracking xp, where we split
             * Attacking and Piloting xp into two different channels
             * And since this is neither it goes to its own channel */
            SendAttacks(tprintf("#%i attacks #%i (kick) (%i/%i)", mech->mynum, target->mynum, baseToHit, roll));
    }
    ...
    if (roll >= baseToHit) {    /*  hit the target */
        ...
        if (MechType(target) == CLASS_BSUIT && MechSwarmTarget(target) == mech->mynum) {
            if (!MadePilotSkillRoll(mech, 4)) {
                mech_notify(mech, MECHALL, "Uh oh. You miss the little buggers, but hit yourself!");
...
void PhysicalDamage(MECH * mech, MECH * target, int weightdmg, int AttackType, int sect)
{
            ...
            if (MechType(mech) == CLASS_MECH) {
                if (Fallen(mech)) {
                    if ((MechType(target) != CLASS_MECH) || (Fallen(target) && (MechElevation(mech) == MechElevation(target))))
                        ...
                        if (!Fallen(target) && (MechElevation(mech) > MechElevation(target)))
        ...
        case PA_KICK:
            ...
            if (Fallen(target) || MechType(target) != CLASS_MECH)
                ...
                if (MechElevation(mech) > MechElevation(target))
                    ...
                    hitloc = FindKickLocation(hitgroup);
                    if (!GetSectInt(target, hitloc) && GetSectInt(target, (i = BOUNDED(0, 5 + (6 - hitloc), NUM_SECTIONS - 1))))
                        hitloc = i;
                }
            }
            break;
    ...
    MyDamageMech(target, mech, 1, MechPilot(mech), hitloc, (hitgroup == BACK) ? 1 : 0, 0, damage, 0);

    if (MechType(target) == CLASS_BSUIT && MechSwarmTarget(target) > 0 && (AttackType == PA_PUNCH || AttackType == PA_AXE || AttackType == PA_SWORD))
...
int DeathFromAbove(MECH * mech, MECH * target)
{
    ...
    DOCHECKMA0(((MechTeam(mech) == MechTeam(target)) && (Started(target)) && (!Destroyed(target))), "Friendly units ? I dont Think so..");

    DOCHECKMA0(Dodging(mech) || MoveModeLock(mech), "You cannot use physicals while using a special movement mode.");

    DOCHECKMA0(MechSections(mech)[LLEG].recycle || MechSections(mech)[RLEG].recycle, "Your legs are still recovering from your last attack.");
    DOCHECKMA0(MechSections(mech)[RARM].recycle || MechSections(mech)[LARM].recycle, "Your arms are still recovering from your last attack.");
    DOCHECKMA0(Jumping(target), "Your target is jumping, you cannot land on it.");

    if ((MechType(target) == CLASS_VTOL) || (MechType(target) == CLASS_AERO) || (MechType(target) == CLASS_DS))
    ...
    DOCHECKMA0(baseToHit > 12, tprintf("DFA: BTH %d\tYou choose not to attack and land from your jump.", baseToHit));
    ...
    if (roll >= baseToHit) {
        /* OUCH */
        mech_notify(target, MECHSTARTED, tprintf("DEATH FROM ABOVE!!!\n%s lands on you from above!", GetMechToMechID(target, mech)));
        ...
        for (i = 0; i < spread; i++) {
            ...
            MyDamageMech(target, mech, 1, MechPilot(mech), hitloc, isrear, iscritical, 5, 0);
        }
        ...
        if (target_damage % 5) {
            ...
            MyDamageMech(target, mech, 1, MechPilot(mech), hitloc, isrear, iscritical, (target_damage % 5), 0);
        }
        ...
        if (!Fallen(mech)) {
            if (!MadePilotSkillRoll(mech, 4)) {
                mech_notify(mech, MECHALL, "Your piloting skill fails and you fall over!!");
                MechLOSBroadcast(mech, "stumbles and falls down!");
                MechFalls(mech, 1, 0);
            }
        
            if (MechType(target) == CLASS_MECH && !MadePilotSkillRoll(target, 2)) {
                mech_notify(target, MECHSTARTED, "Your piloting skill fails and you fall over!!");
                MechLOSBroadcast(target, "stumbles and falls down!");
                MechFalls(target, 1, 0);
            }
        }
        ...
        if (mech_damage % 5) {
            hitloc = FindHitLocation(mech, BACK, &iscritical, &tmpi);
            MyDamageMech2(mech, mech, 0, -1, hitloc, 1, iscritical, (mech_damage % 5), 0);
        }
...
void ChargeMech(MECH * mech, MECH * target)
{
    ...
    if (MechChargeTarget(target) == mech->mynum) {
        ...
        for (i = 0; i < CHARGE_SECTIONS && !done; i++) 
            if (SectHasBusyWeap(target, resect[i])) {
                ArmorStringFromIndex(resect[i], location, MechType(target), MechMove(target));
                mech_notify(target, MECHALL, tprintf("You have weapons recycling on your %s.", location));
                target_charge = 0;
                done = 1;
            }
        ...
        if (MechType(mech) == CLASS_MECH) {
            ...
            if (MechSections(mech)[LLEG].recycle || MechSections(mech)[RLEG].recycle) {
                mech_notify(mech, MECHALL, "Your legs are still recovering from your last attack.");
                mech_charge = 0;
            }
        
            /* Is the first unit's arms cycling */
            if (MechSections(mech)[RARM].recycle || MechSections(mech)[LARM].recycle) {
                mech_notify(mech, MECHALL, "Your arms are still recovering from your last attack.");
                mech_charge = 0;
            }
        } else {
            /* Is the first unit's front side cycling */
            if (MechSections(mech)[FSIDE].recycle) {
                mech_notify(mech, MECHALL, "You are still recovering from your last attack!");
                mech_charge = 0;
            }
        }
        ...
        if (MechType(target) == CLASS_MECH) {
            ...
            if (MechSections(target)[LLEG].recycle || MechSections(target)[RLEG].recycle) {
                mech_notify(target, MECHALL, "Your legs are still recovering from your last attack.");
                target_charge = 0;
            }
        
            /* Is the second unit's arms cycling */
            if (MechSections(target)[RARM].recycle || MechSections(target)[LARM].recycle) {
                mech_notify(target, MECHALL, "Your arms are still recovering from your last attack.");
                target_charge = 0;
            }
        } else {
            /* Is the second unit's front side cycling */
            if (MechSections(target)[FSIDE].recycle) {
                mech_notify(target, MECHALL, "You are still recovering from your last attack!");
                target_charge = 0;
            }
        }
        ...
        if (Jumping(target)) {
            mech_notify(mech, MECHALL, "Your target is jumping, you charge underneath it.");
        ...
        if (Jumping(mech)) {
            mech_notify(target, MECHALL, "Your target is jumping, you charge underneath it.");
        ...
        if (Fallen(mech) && (MechType(target) != CLASS_VEH_GROUND)) {
            mech_notify(target, MECHALL, "Your target's too low for you to charge it!");
            target_charge = 0;
        }
        ...
        if ((MechType(target) == CLASS_VEH_GROUND) && ((MechType(mech) != CLASS_MECH) && (MechType(mech) != CLASS_VEH_GROUND))) {
        ...
        if ((MechType(mech) == CLASS_VEH_GROUND) && ((MechType(target) != CLASS_MECH) && (MechType(target) != CLASS_VEH_GROUND))) {
        ...
        if (btechconf.btech_newcharge)
            target_damage = (int)((((((float) MechChargeDistance(mech)) * MP1) - MechSpeed(target) * cos((MechFacing(mech) - MechFacing(target)) * (M_PI / 180.))) * MP_PER_KPH) * (MechRealTons(mech) + 5) / 10);
        else
            target_damage = (int)(((MechSpeed(mech) - MechSpeed(target) * cos((MechFacing(mech) - MechFacing(target)) * (M_PI / 180.))) * MP_PER_KPH) * (MechRealTons(mech) + 5) / 10);
        ...
        if (btechconf.btech_newcharge)
            mech_damage = (int)((((((float) MechChargeDistance(target)) * MP1) - MechSpeed(mech) * cos((MechFacing(target) - MechFacing(mech)) * (M_PI / 180.))) * MP_PER_KPH) * (MechRealTons(target) + 5) / 10);
        else
            mech_damage = (int)(((MechSpeed(target) - MechSpeed(mech) * cos((MechFacing(target) - MechFacing(mech)) * (M_PI / 180.))) * MP_PER_KPH) * (MechRealTons(target) + 5) / 10);
        ...
        /* BTH for first unit */
        mech_baseToHit = 5;
        mech_baseToHit += FindPilotPiloting(mech) - FindSPilotPiloting(target);
        ...
        /* BTH for second unit */
        targ_baseToHit = 5;
        targ_baseToHit += FindPilotPiloting(target) - FindSPilotPiloting(mech);
        ...
        if (mech_charge)
            if (mech_baseToHit > 12) {
                mech_notify(mech, MECHALL, tprintf("Charge: BTH %d\tYou choose not to charge.", mech_baseToHit));
                mech_charge = 0;
            }

        if (target_charge)
            if (targ_baseToHit > 12) {
                mech_notify(target, MECHALL, tprintf("Charge: BTH %d\tYou choose not to charge.", targ_baseToHit));
                target_charge = 0;
            }
        ...
        if (mech_charge)
            mech_notify(mech, MECHALL, tprintf("Charge: BTH %d\tRoll: %d", mech_baseToHit, mech_roll));

        if (target_charge)
            mech_notify(target, MECHALL, tprintf("Charge: BTH %d\tRoll: %d", targ_baseToHit, targ_roll));

        /* Ok the first unit made its roll */
        if (mech_charge && mech_roll >= mech_baseToHit) {
            /* OUCH */
            mech_notify(target, MECHALL, tprintf("CRASH!!!\n%s charges into you!", GetMechToMechID(target, mech)));
             ...
             * charging */
            if (btechconf.btech_newcharge && btechconf.btech_tl3_charge)
                target_damage = (int)((((((float) MechChargeDistance(mech)) * MP1) - MechSpeed(target) * cos((MechFacing(mech) - MechFacing(target)) * (M_PI / 180.))) * MP_PER_KPH) * (MechRealTons(mech) + 5) / 20);
            else
            ...
            for (i = 0; i < spread; i++) 
                hitloc = FindHitLocation(mech, hitGroup, &iscritical, &isrear); MyDamageMech2(mech, mech, 0, -1, hitloc, isrear, iscritical, 5, 0);

            if (target_damage % 5) {
                hitloc = FindHitLocation(mech, hitGroup, &iscritical, &isrear);
                MyDamageMech2(mech, mech, 0, -1, hitloc, isrear, iscritical, (target_damage % 5), 0);
            }
            ...
            snprintf(emit_buff, LBUF_SIZE, "#%i charges #%i (%i/%i) Distance:" " %.2f DI: %i DR: %i", mech->mynum, target->mynum, mech_baseToHit, mech_roll, MechChargeDistance(mech), inflicted_damage, received_damage);
            SendDebug(emit_buff);
            ...
            if (MechType(mech) == CLASS_MECH && !MadePilotSkillRoll(target, 2)) {
                mech_notify(target, MECHALL, "Your piloting skill fails and you fall over!!");
        ...
        if (target_charge && targ_roll >= targ_baseToHit) {
            /* OUCH */
            mech_notify(mech, MECHALL, tprintf("CRASH!!!\n%s charges into you!", GetMechToMechID(mech, target)));
            ...
            for (i = 0; i < spread; i++) {
                hitloc = FindHitLocation(mech, hitGroup, &iscritical, &isrear);
                MyDamageMech(mech, target, 1, MechPilot(target), hitloc, isrear, iscritical, 5, 0);
            }

            if (mech_damage % 5) {
                hitloc = FindHitLocation(mech, hitGroup, &iscritical, &isrear);
                MyDamageMech(mech, target, 1, MechPilot(target), hitloc, isrear, iscritical, (mech_damage % 5), 0);
            }
             ...
             * charging */
            if (btechconf.btech_newcharge && btechconf.btech_tl3_charge)
                target_damage = (int)((((((float) MechChargeDistance(target)) * MP1) - MechSpeed(mech) * cos((MechFacing(target) - MechFacing(mech)) * (M_PI / 180.))) * MP_PER_KPH) * (MechRealTons(mech) + 5) / 20);
            else
            ...
            if (mech_damage % 5) {
                hitloc = FindHitLocation(target, hitGroup, &iscritical, &isrear);
                MyDamageMech2(target, target, 0, -1, hitloc, isrear, iscritical, (mech_damage % 5), 0);
            }
            ...
            /* Emit the damage for debugging purposes */
            snprintf(emit_buff, LBUF_SIZE, "#%i charges #%i (%i/%i) Distance:" " %.2f DI: %i DR: %i", target->mynum, mech->mynum, targ_baseToHit, targ_roll, MechChargeDistance(target), inflicted_damage, received_damage);
            SendDebug(emit_buff);
    ...
    if (MechType(mech) == CLASS_MECH) {
        DOCHECKMA(MechSections(mech)[LLEG].recycle || MechSections(mech)[RLEG].recycle, "Your legs are still recovering from your last attack.");
        DOCHECKMA(MechSections(mech)[RARM].recycle || MechSections(mech)[LARM].recycle, "Your arms are still recovering from your last attack.");
    } else 
        DOCHECKMA(MechSections(mech)[FSIDE].recycle, "You are still recovering from your last attack!");

    /* See if either the target or the attacker are jumping */
    DOCHECKMA(Jumping(target), "Your target is jumping, you charge underneath it.");
    DOCHECKMA(Jumping(mech), "You can't charge while jumping, try death from above.");

    /* If target is fallen make sure you in a tank */
    DOCHECKMA(Fallen(target) && (MechType(mech) != CLASS_VEH_GROUND), "Your target's too low for you to charge it!");

    /* Only mechs can charge mechs */
    DOCHECKMA((MechType(mech) == CLASS_MECH) && (MechType(target) != CLASS_MECH), "You can only charge mechs!");

    /* Only tanks can charge tanks and mechs */
    DOCHECKMA((MechType(mech) == CLASS_VEH_GROUND) && ((MechType(target) != CLASS_MECH) && (MechType(target) != CLASS_VEH_GROUND)), "You can only charge mechs and tanks!");
    ...
    iwa = InWeaponArc(mech, MechFX(target), MechFY(target));
    MechStatus(mech) |= ts;
    DOCHECKMA(!(iwa & FORWARDARC), "Your charge target is not in your forward arc and you are unable to charge it.");

    /* Damage inflicted by the charge */
    if (btechconf.btech_newcharge)
        target_damage = (int)((((((float) MechChargeDistance(mech)) * MP1) - MechSpeed(target) * cos((MechFacing(mech) - MechFacing(target)) * (M_PI / 180.))) * MP_PER_KPH) * (MechRealTons(mech) + 5) / 10 + 1);
    else
        target_damage = (int)(((MechSpeed(mech) - MechSpeed(target) * cos((MechFacing(mech) - MechFacing(target)) * (M_PI / 180.))) * MP_PER_KPH) * (MechRealTons(mech) + 5) / 10 + 1);

    if (HasBoolAdvantage(MechPilot(mech), "melee_specialist"))
        target_damage++;

    /* Not enough damage done so no charge */
    DOCHECKMP(target_damage <= 0, "Your target pulls away from you and you are unable to charge it.");
    ...
    DOCHECKMA(baseToHit > 12, tprintf("Charge: BTH %d\tYou choose not to charge.", baseToHit));

    /* Roll */
    roll = Roll();
    mech_notify(mech, MECHALL, tprintf("Charge: BTH %d\tRoll: %d", baseToHit, roll));

    /* Did the charge work ? */
    if (roll >= baseToHit) {
        /* OUCH */
        MechLOSBroadcasti(mech, target, tprintf("%ss %%s!", MechType(mech) == CLASS_MECH ? "charge" : "ram"));
        mech_notify(target, MECHSTARTED, tprintf("CRASH!!!\n%s %ss into you!", GetMechToMechID(target, mech), MechType(mech) == CLASS_MECH ? "charge" : "ram"));
        mech_notify(mech, MECHALL, "SMASH!!! You crash into your target!");
        hitGroup = FindAreaHitGroup(mech, target);
        ...
        if (btechconf.btech_newcharge && btechconf.btech_tl3_charge)
            mech_damage = (int)((((((float) MechChargeDistance(mech)) * MP1) - MechSpeed(target) * cos((MechFacing(mech) - MechFacing(target)) * (M_PI / 180.))) * MP_PER_KPH) * (MechRealTons(target) + 5) / 20);
        else
        ...
        if (mech_damage % 5) {
            hitloc = FindHitLocation(mech, hitGroup, &iscritical, &isrear);
            MyDamageMech2(mech, mech, 0, -1, hitloc, isrear, iscritical, (mech_damage % 5), 0);
        }
        ...
        /* Emit the damage for debugging purposes */
        snprintf(emit_buff, LBUF_SIZE, "#%i charges #%i (%i/%i) Distance:" " %.2f DI: %i DR: %i", mech->mynum, target->mynum, baseToHit, roll, MechChargeDistance(mech), inflicted_damage, received_damage);
        SendDebug(emit_buff);
    }
...
int checkGrabClubLocation(MECH * mech, int section, int emit)
{
    ...
    if (SectIsDestroyed(mech, section)) {
        ...
        if (!OkayCritSectS(section, 3, HAND_OR_FOOT_ACTUATOR)) {
            sprintf(buf, "Your %s's hand actuator is destroyed or missing.", location);
            tCanGrab = 0;
        } else 
            if (!OkayCritSectS(section, 0, SHOULDER_OR_HIP)) {
                sprintf(buf, "Your %s's shoulder actuator is destroyed or missing.", location);
                tCanGrab = 0;
            } else 
                if (SectHasBusyWeap(mech, section)) {
                    sprintf(buf, "Your %s is still recovering from it's last attack.", location);
                    tCanGrab = 0;
                }
...
void mech_grabclub(dbref player, void *data, char *buffer)
{
    ...
    if (wcArgs >= 1) 
        if (mux_toupper(args[0][0]) == '-') {
            if ((MechSections(mech)[LARM].specials & CARRYING_CLUB) || (MechSections(mech)[RARM].specials & CARRYING_CLUB)) 
                DropClub(mech);
            else 
    ...
    DOCHECKMA(Fallen(mech), "You can't grab a club while lying flat on your face.");
    ...
    DOCHECKMA(have_axe(mech, LARM) || have_axe(mech, RARM), "You can not grab a club if you carry an axe.");
    DOCHECKMA(have_sword(mech, LARM) || have_sword(mech, RARM), "You can not grab a club if you carry a sword.");
    DOCHECKMA(have_mace(mech, LARM) || have_mace(mech, RARM), "You can not grab a club if you carry an mace.");

    if (wcArgs == 0) {
        if (checkGrabClubLocation(mech, LARM, 0))
            location = LARM;
        else 
            if (checkGrabClubLocation(mech, RARM, 0))
                location = RARM;
            else {
                mech_notify(mech, MECHALL, "You don't have a free arm with a working hand actuator!");
                return;
            }
    } else {
        switch (mux_toupper(args[0][0])) {
            case 'R':
                location = RARM;
                break;
            case 'L':
    ...
    MechRTerrain(mech) != LIGHT_FOREST, "There don't appear to be any trees within grabbing distance.");
    ...
    mech_notify(mech, MECHALL, tprintf("You reach down and yank a tree out of the ground with your %s.", locname));
