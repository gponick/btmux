...
#define UnZombifyMech(mech) \
    do { AUTO *au; if (MechAuto(mech) > 0 && \
        (au=static_cast <AUTO *> (FindObjectsData(MechAuto(mech))))) UnZombifyAuto(au); } while (0)
...
void auto_com_event(EVENT *muxevent);
void auto_astar_goto_event(EVENT *muxevent);
void auto_astar_follow_event(EVENT *muxevent);
void auto_dumbgoto_event(EVENT *muxevent);
void auto_dumbfollow_event(EVENT *muxevent);
void auto_leave_event(EVENT *muxevent);
void auto_enter_event(EVENT *muxevent);
...
void auto_reply_event(EVENT *muxevent);
