...
void mech_standfail_event(EVENT * e)
{
    ...
    mech_notify(mech, MECHALL, "%cgYou have finally recovered from your attempt to stand.%c");
}
...
void mech_fall_event(EVENT * e)
{
    ...
    if (fallspeed <= 0 && (!Started(mech) || !(FlyingT(mech)) || ((AeroFuel(mech) <= 0) && !AeroFreeFuel(mech)) || ((MechType(mech) == CLASS_VTOL) && (SectIsDestroyed(mech, ROTOR)))))
...
void mech_lock_event(EVENT * e)
{
    ...
    if (MechTarget(mech) >= 0) {
        ...
        target = static_cast <MECH *>(FindObjectsData(MechTarget(mech)));
        
        if (!target)
            return;

        if (!InLineOfSight(mech, target, MechX(target), MechY(target), FlMechRange(map, mech, target)))
            return;
    
        mech_notify(mech, MECHALL, tprintf("The sensors acquire a stable lock on %s.", GetMechToMechID(mech, target)));
    } else 
        if (MechTargX(mech) >= 0 && MechTargY(mech) >= 0)
            mech_notify(mech, MECHALL, tprintf("The sensors acquire a stable lock on (%d,%d).", MechTargX(mech), MechTargY(mech)));
}
...
void mech_stabilizing_event(EVENT * e)
{
    ...
    mech_notify(mech, MECHSTARTED, "%cgYou have finally stabilized after your jump.%c");
}
...
void MaybeRecycle(MECH * mech, int wticks)
{
    ...
    if (nr < 0 || nr > ((wticks + 1) * WEAPON_TICK)) { 
        ...
        SendDebug(tprintf("%6d Recycle event for #%d set in %ds.", muxevent_tick, mech->mynum, dat));
#endif
    }
#ifdef WEAPON_RECYCLE_DEBUG
    else
        SendDebug(tprintf("%6d Recycle event for #%d exists at %d secs", muxevent_tick, mech->mynum, nr));
#endif
}
...
void mech_lateral_event(EVENT * e)
{
    ...
    tprintf("Lateral movement mode change to %s completed.", lateral_modes[latmode].full));
...
void mech_move_event(EVENT * e)
{
    ...
    if (MechType(mech) == CLASS_VEH_NAVAL && MechRTerrain(mech) != BRIDGE && MechRTerrain(mech) != ICE && MechRTerrain(mech) != WATER)
        return;

    if (MechSpeed(mech) || MechDesiredSpeed(mech) || MechDesiredFacing(mech) != MechFacing(mech) || ((MechType(mech) == CLASS_VTOL || MechMove(mech) == MOVE_SUB) && MechVerticalSpeed(mech)))
        MECHEVENT(mech, EVENT_MOVE, mech_move_event, MOVE_TICK, 0);
}
    ...
    for (i = 0; i < map->first_free; i++)
        if (map->mechsOnMap[i] > 0 && map->mechsOnMap[i] != mech->mynum)
            if (!(map->LOSinfo[mech->mapnumber][i] & MECHLOSFLAG_SEEN)) {
                target = static_cast <MECH *>(FindObjectsData(map->mechsOnMap[i]));
                if (!target)
                    continue;
...
void aero_move_event(EVENT * e)
{
    ...
    if (!Landed(mech)) {
        ...
        if (IsDS(mech) && MechZ(mech) <= (MechElevation(mech) + 5) && ((muxevent_tick / WEAPON_TICK) % 10) == 0)
            DS_BlastNearbyMechsAndTrees(mech, "You are hit by the DropShip's plasma exhaust!", "is hit directly by DropShip's exhaust!", "You are hit by the DropShip's plasma exhaust!", "is hit by DropShip's exhaust!", "light up and burn.", 8);
    
        MECHEVENT(mech, EVENT_MOVE, aero_move_event, MOVE_TICK, 0);
    } else 
        if (Landed(mech) && !Fallen(mech) && RollingT(mech)) {
            UpdateHeading(mech);
            UpdateSpeed(mech);
            move_mech(mech);
            if (fabs(MechSpeed(mech)) > 0.0 || fabs(MechDesiredSpeed(mech)) > 0.0 || MechDesiredFacing(mech) != MechFacing(mech))
                if (!FuelCheck(mech))
                    MECHEVENT(mech, EVENT_MOVE, aero_move_event, MOVE_TICK, 0);
        }
}
...
void unstun_crew_event(EVENT * e)
{
    ...
    mech_notify(mech, MECHALL, "Your head clears and you're able to control your vehicle again.");
...
void mech_unjam_ammo_event(EVENT * objEvent)
{
    ...
    ammoLeft = FindAmmoForWeapon_sub(objMech, wSect, wSlot, wWeapIdx, 0, &ammoLoc, &ammoCrit, 0, 0);

    if (!ammoLeft) {
        SetPartTempNuke(objMech, wSect, wSlot, 0);

        mech_notify(objMech, MECHALL, tprintf("You finish bouncing around and realize you nolonger have ammo for your %s!", get_parts_long_name(I2Weapon(wWeapIdx), 0)));
        return;
    }
    ...
    if (MechWeapons[wWeapStatus].special & RAC) {
        ...
        mech_notify(objMech, MECHPILOT, tprintf("Modified Gunnery Skill: BTH %d\tRoll: %d", wRollNeeded, wRoll));

        if (wRoll < wRollNeeded) {
            mech_notify(objMech, MECHALL, "Your attempt to remove the jammed slug fails. You'll need to try again to clear it.");
            return;
        }
    } else {
    ...
    SetPartTempNuke(objMech, wSect, wSlot, 0);
    mech_notify(objMech, MECHALL, tprintf("You manage to clear the jam on your %s!", get_parts_long_name(I2Weapon(wWeapIdx), 0)));
...
void check_stagger_event(EVENT * event)
{
    ...
    if ((StaggerLevel(mech) < 1) || Fallen(mech) || (MechType(mech) != CLASS_MECH)) {
    ...
    if (!MadePilotSkillRoll(mech, calcStaggerBTHMod(mech))) {
        mech_notify(mech, MECHALL, "You loose the battle with gravity and tumble over!!");
