...
SEEFUNC(electrom_see, MAX(r < 24 ? 2 : 0, (int)((60 - (r * 2))) / 2));
SEEFUNC(seismic_see, 50 - (r * 4));

SEEFUNC(radar_see, BOUNDED(10, (180 - (int)(r)), 90));
SEEFUNC(bap_see, 101);
...
TOHITFUNC(electrom_tohit, ((nwood_count(t, f) * 2) / 3) + ((f & MECHLOSFLAG_PARTIAL) ? 3 + (IsHulldown(t) ? 2 : 0) : 0) + WEIGHT_MODIFIER(MechTons(t)) + MOVE_MODIFIER((int)(MechSpeed(t))) + (MechStatus(t) & FIRED ? -1 : 0) + MNumber(m, 0, 1));

/* Seismic's hurt by enemy staying in place / light mechs */
TOHITFUNC(seismic_tohit, 2 + ((f & MECHLOSFLAG_PARTIAL) ? 3 + (IsHulldown(t) ? 2 : 0) : 0) + WEIGHT_MODIFIER(MechRealTons(t)) - MOVE_MODIFIER((int)(MechSpeed(t))) + MNumber(m, 0, 1));
