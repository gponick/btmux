...
void NormalizeArmActuatorCrits(MECH * objMech, int wLoc, int wCritType)
{
    switch (Special2I(wCritType)) {
        case SHOULDER_OR_HIP:        /* +4 to BTH with weapons in arm */
            MechSections(objMech)[wLoc].basetohit = 4;
            break;
        case UPPER_ACTUATOR:
        case LOWER_ACTUATOR:        /* +1 BTH */
...
void NormalizeLegActuatorCrits(MECH * objMech, int wLoc, int wCritType)
{
    switch (Special2I(wCritType)) {
        case SHOULDER_OR_HIP:
            /*
               speed cut in half, +2 to pskill rolls
        ...
        case HAND_OR_FOOT_ACTUATOR:
            /* -1 walking, +1 to pskill rolls, +1 to BTHs with this leg */
...
void NormalizeAllActuatorCrits(MECH * objMech)
{
    ...
    wMaxTemplateSpeed = (int)(((MechEngineSize(objMech) / MechTons(objMech)) * 1.5) * KPH_PER_MP);
    ...
    if (wLegsDestroyed > 0) 
        if (MechIsQuad(objMech)) {
            MechPilotSkillBase(objMech) += 2;    /* lose quad bonus */
    ...
    int critHit, int critType, int LOS)
{
    ...
    /* Find the first crit */
    wFirstCrit = FindFirstWeaponCrit(wounded, hitloc, critHit, 0, critType, wMaxCrits);

    /* See if the weapon is already destroyed */
    if (wFirstCrit != -1) 
        wWeapDestroyed = (PartIsNonfunctional(wounded, hitloc, wFirstCrit) || (PartTempNuke(wounded, hitloc, wFirstCrit) == FAIL_DESTROYED));
    ...
    if ((MechWeapons[Weapon2I(critType)].special & GAUSS) && !wWeapDestroyed) {
        ...
        mech_notify(wounded, MECHALL, tprintf("It explodes for %d points damage", MechWeapons[Weapon2I(critType)].explosiondamage));

        if (LOS && wounded != attacker && attacker)
            mech_notify(attacker, MECHALL, "Your target is covered with a large electrical discharge");

        DestroyWeapon(wounded, hitloc, critType, wFirstCrit, wMaxCrits, wMaxCrits);

        if (attacker) 
            DamageMech(wounded, attacker, 0, -1, hitloc, 0, 0, 0, MechWeapons[Weapon2I(critType)].explosiondamage, -1, 7, 1, 0, 1);

        return 1;
    } else 
        if (IsAMS(Weapon2I(critType))) {    /* Have to shut down AMS when its critted */
            mech_notify(wounded, MECHALL, tprintf("Your %s has been destroyed!!", &MechWeapons[Weapon2I(critType)].name[3]));
            MechStatus(wounded) &= ~AMS_ENABLED;
            MechSpecials(wounded) &= ~(IS_ANTI_MISSILE_TECH | CL_ANTI_MISSILE_TECH);
        } else 
            if (HotLoading(Weapon2I(critType), GetPartFireMode(wounded, hitloc, wFirstCrit)) && !wWeapDestroyed) {    /* And crit hotloaded LRMs */
                if (FindAmmoForWeapon(wounded, Weapon2I(critType), 0, &wAmmoSection, &wAmmoCritSlot) > 0) {
                    ...
                    mech_notify(wounded, MECHALL, tprintf("Your %s has been destroyed!!", &MechWeapons[Weapon2I(critType)].name[3]));
                    mech_notify(wounded, MECHALL, tprintf("%%ch%%crYour hotloaded launcher explodes for %d points of damage!%%cn", damage));
                    if (LOS && wounded != attacker && attacker)
                        MechLOSBroadcast(wounded, "loses a launcher in a brilliant explosion!");

                    DestroyWeapon(wounded, hitloc, critType, wFirstCrit, wMaxCrits, wMaxCrits);

                    if (attacker) 
                        DamageMech(wounded, attacker, 0, -1, hitloc, 0, 0, 0, damage, -1, 7, -1, 0, 1);

                    return 1;
                }
            } else 
                if ((GetPartAmmoMode(wounded, hitloc, wFirstCrit) & AC_INCENDIARY_MODE) && !wWeapDestroyed && WpnIsRecycling(wounded, hitloc, wFirstCrit))     /* Incendiary ACs blow up too */
                    if (FindAmmoForWeapon_sub(wounded, -1, -1, Weapon2I(critType), 0, &wAmmoSection, &wAmmoCritSlot, 0, AC_INCENDIARY_MODE) > 0) {
                        mech_notify(wounded, MECHALL, tprintf("Your %s has been destroyed!!", &MechWeapons[Weapon2I(critType)].name[3]));
                        mech_notify(wounded, MECHALL, tprintf("%%ch%%crThe incendiary ammunition in your launcher ignites for %d points of damage!%%cn", MechWeapons[Weapon2I(critType)].damage));

                        if (LOS && wounded != attacker && attacker) {
                            sprintf(msgbuf, "'s %s is engulfed by a brilliant blue flame!", locname);
                        ...
                        if (attacker) 
                            DamageMech(wounded, attacker, 0, -1, hitloc, 0, 0, 0, MechWeapons[Weapon2I(critType)].damage, -1, 7, -1, 0, 1);
...
void JamMainWeapon(MECH * mech)
{
    ...
    if (critfound) {
        ...
        mech_notify(mech, MECHALL, tprintf("%%ch%%crYour %s is jammed!%%c", &MechWeapons[maxtype].name[3]));
    }
}
    ...
    for (wIter = 0; wIter < MAX_WEAPS_SECTION; wIter++) 
        if (IsWeapon(GetPartType(objMech, wLoc, wIter))) 
            if (!PartIsBroken(objMech, wLoc, wIter)) 
                if (!wIgnoreJams || (wIgnoreJams && !PartTempNuke(objMech, wLoc, wIter))) {
...
void limitSpeedToCruise(MECH * objMech)
{
    ...
    if (MechMove(objMech) == MOVE_VTOL)
        wMaxSpeed = sqrt((float) wMaxSpeed * wMaxSpeed - MechVerticalSpeed(objMech) * MechVerticalSpeed(objMech));
...
void DoVehicleStablizerCrit(MECH * objMech, int wLoc)
{
    ...
    if (MechSections(objMech)[wLoc].config & STABILIZERS_DESTROYED)
        mech_notify(objMech, MECHALL, tprintf("The destroyed weapon stabilizers in your %s take another hit!", strLocName));
    else {
        mech_notify(objMech, MECHALL, tprintf("The weapon stabilizers in your %s have been destroyed!", strLocName));
...
void DoTurretLockCrit(MECH * objMech)
{
    ...
    if (MechTankCritStatus(objMech) & TURRET_LOCKED) {
        mech_notify(objMech, MECHALL, "The shot pierces your armor yet fails to hit a critical system!");
        return;
    }
    ...
    mech_notify(objMech, MECHALL, "%ch%crThe shot destroys your turret rotation mechanism!%cn");
}
...
void DoTurretJamCrit(MECH * objMech)
{
    ...
    MechTankCritStatus(objMech) |= TURRET_JAMMED;
    mech_notify(objMech, MECHALL, "%ch%crYour turret gets jammed on it's current facing!%cn");
}
    ...
    if (wWeapIdx >= 0) {
        switch (MechWeapons[wWeapIdx].type) {
            ...
            case TARTILLERY:
                ...
                mech_notify(objMech, MECHALL, tprintf("%%ch%%crThe shot causes your %s to temporarily short out!%%c", &MechWeapons[wWeapIdx].name[3]));
                break;
            case TAMMO:
                wCritType = FAIL_JAMMED;
                mech_notify(objMech, MECHALL, tprintf("%%ch%%crThe shot temporarily jams your %s!%%c", &MechWeapons[wWeapIdx].name[3]));
                break;
            default:
                wCritType = FAIL_SHORTED;
                mech_notify(objMech, MECHALL,
                tprintf("%%ch%%crThe shot causes your %s to temporarily short out!%%c", &MechWeapons[wWeapIdx].name[3]));
                break;
        }
...
void DoWeaponDestroyedCrit(MECH * objAttacker, MECH * objMech, int wLoc, int LOS)
{
    ...
    if (wWeapIdx >= 0) {
        ...
        mech_notify(objMech, MECHALL, tprintf("%%ch%%crYour %s is destroyed!%%c", &MechWeapons[wWeapIdx].name[3]));
    }
}
...
void DoTurretBlownOffCrit(MECH * objMech, MECH * objAttacker, int LOS)
{
    ...
    mech_notify(objMech, MECHALL, "%ch%crThe shot pops your turret clear off its housing!%cn");
    ...
    int LOS)
{
    ...
    for (wSecIter = 0; wSecIter <= 7; wSecIter++) {
        ...
        for (wSlotIter = CritsInLoc(objMech, wSecIter) - 1; wSlotIter >= 0; wSlotIter--) {
            wPartType = GetPartType(objMech, wSecIter, wSlotIter);
            wWeapIdx = Ammo2WeaponI(wPartType);

            if (IsAmmo(wPartType) && GetPartData(objMech, wSecIter, wSlotIter) && (!(MechWeapons[wWeapIdx].special & GAUSS))) {
                wTempDamage = (FindMaxAmmoDamage(Ammo2WeaponI(wPartType)) * GetPartData(objMech, wSecIter, wSlotIter));
    ...
    mech_notify(objMech, MECHALL, "%ch%crOne of your ammo bins is struck causing a cascading explosion!%cn");
    MechLOSBroadcast(objMech, "has an internal ammo explosion!");

    DamageMech(objMech, objAttacker, 0, -1, wLoc, 0, 0, 0, wTotalAmmoDamage, 0, 0, -1, 0, 1);
}
...
void DoVehicleEngineHit(MECH * objMech, MECH * objAttacker)
{
    ...
    if (Fallen(objMech)) {
        mech_notify(objMech, MECHALL, "Your destroyed engine takes another direct hit!");
        return;
    }
    ...
    if (MechType(objMech) == CLASS_VTOL) {
        if (!Landed(objMech)) {
            if (MechRTerrain(objMech) == GRASSLAND || MechRTerrain(objMech) == ROAD || MechRTerrain(objMech) == BUILDING) {
                if (MadePilotSkillRoll(objMech, MechZ(objMech) - MechElevation(objMech))) {
...
void DoVehicleFuelTankCrit(MECH * objMech, MECH * objAttacker)
{
    ...
    mech_notify(objMech, MECHALL, "%ch%crYour fuel tank explodes in a ball of fire!%cn");
...
void DoVehicleCrewStunnedCrit(MECH * objMech)
{
    ...
    mech_notify(objMech, MECHALL, "%ch%crThe shot resonates throughout the crew compartment temporarily stunning you!%cn");
...
void DoVehicleDriverCrit(MECH * objMech)
{
    ...
    mech_notify(objMech, MECHALL, "%ch%crYour vehicle's driver takes a piece of shrapnel, making it harder to control the vehicle!%cn");
    MechPilotSkillBase(objMech) += 2;
}
...
void DoVehicleCommanderHit(MECH * objMech)
{
    ...
    mech_notify(objMech, MECHALL, "%ch%crYour vehicle's commander takes a piece of shrapnel!%cn");
    MechPilotSkillBase(objMech) += 1;
    MechBTH(objMech) += 1;
...
void DoVehicleCrewKilledCrit(MECH * objMech, MECH * objAttacker)
{
    ...
    mech_notify(objMech, MECHALL, "%ch%crThe shot ricochets around the crew compartment, instantly killing everyone!%cn");
    DestroyMech(objMech, objAttacker, 0);
...
void DoVTOLCoPilotCrit(MECH * objMech)
{
    ...
    mech_notify(objMech, MECHALL, "%ch%crYour VTOL's pilot takes a piece of shrapnel, making it harder to aim your weapons!%cn");
    MechBTH(objMech) += 1;
}
...
{
    /* +2 pskill rolls
     * Must make sucessful pskill roll or fall 1 elevation. */

    mech_notify(objMech, MECHALL, "%ch%crYour VTOL's copilot takes a piece of shrapnel, making it harder to control the VTOL!%cn");
    MechPilotSkillBase(objMech) += 2;
...
void DoVTOLRotorDestroyedCrit(MECH * objMech, MECH * objAttacker, int LOS)
{
    ...
    mech_notify(objMech, MECHALL, "%ch%crThe shot hits your fragile rotor mechanism!%cn");
...
void DoVTOLTailRotorDamagedCrit(MECH * objMech)
{
    ...
    if (MechTankCritStatus(objMech) & TAIL_ROTOR_DESTROYED)
        mech_notify(objMech, MECHALL, "Your damaged tail rotor suffers more damage!");
    else {
        MechTankCritStatus(objMech) |= TAIL_ROTOR_DESTROYED;
        mech_notify(objMech, MECHALL, "%ch%crYour tail rotor is damaged slowing you down!%cn");
        limitSpeedToCruise(objMech);
    }
}
...
void StartVTOLCrash(MECH * objMech)
{
    if (!Fallen(objMech)) {
        ...
        if (!Landed(objMech)) {
            mech_notify(objMech, MECHALL, "You ponder F = ma, S = F/m, S = at^2 => S=agt^2 in relation to the ground.");
...
void HandleVTOLCrit(MECH * wounded, MECH * attacker, int LOS, int hitloc, int num)
{
    ...
    switch (random() % 6) {
        case 0:        /* Crew killed */
            ...
            KillMechContentsIfIC(wounded->mynum);
            break;
        case 1:        /* Weapon jams, set them recylcling maybe */
            JamMainWeapon(wounded);
            break;
        case 2:        /* Engine Hit */
            mech_notify(wounded, MECHALL, "Your engine takes a direct hit!!");
            if (!Landed(wounded)) {
                if (MechRTerrain(wounded) == GRASSLAND || MechRTerrain(wounded) == ROAD || MechRTerrain(wounded) == BUILDING)
                    if (MadePilotSkillRoll(wounded, MechZ(wounded) - MechElevation(wounded))) {
        ...
        case 4:        /* Fuel Tank Explodes */
            ...
            break;
        case 5:        /* Ammo/Power Plant Explodes */
...
void DestroyMainWeapon(MECH * mech)
{
    ...
    if (critfound) {
        ...
        DestroyWeapon(mech, maxloc, I2Weapon(maxtype), 1, firstCrit, GetWeaponCrits(mech, maxtype));
...
void HandleFasaVehicleCrit(MECH * wounded, MECH * attacker, int LOS, int hitloc, int num)
{
    ...
    switch (random() % 6) {
        case 0:        /* Crew stunned for one turn...treat like a head hit */
    ...
    if (hitloc == TURRET) {
        if (Number(1, 3) == 2) {
            if (!(MechTankCritStatus(wounded) & TURRET_LOCKED)) {
                ...
                mech_notify(wounded, MECHALL, "Your turret takes a direct hit and immobilizes!");
            }
            return;
        }
    } else
        switch (Number(1, 10)) {
            ...
            case 5:
                if (!Fallen(wounded)) {
                    ...
                    switch (MechMove(wounded)) {
                        case MOVE_TRACK:
                            mech_notify(wounded, MECHALL, "One of your tracks is destroyed, imobilizing your vehicle!!");
                            break;
                        case MOVE_WHEEL:
                            mech_notify(wounded, MECHALL, "One of your wheels is destroyed, imobilizing your vehicle!!");
                            break;
                        case MOVE_HOVER:
                            mech_notify(wounded, MECHALL, "Your lift fan is destroyed, imobilizing your vehicle!!");
                            break;
                        case MOVE_HULL:
                        case MOVE_SUB:
                        case MOVE_FOIL:
                            mech_notify(wounded, MECHALL, "You are halted in your tracks - literally.");
                    }
                    SetMaxSpeed(wounded, 0.0);
    ...
    int critHit, int critType, int critData)
{
    ...
    int tLocIsLeg = ((hitloc == LLEG || hitloc == RLEG) || ((hitloc == LARM || hitloc == RARM) && MechIsQuad(wounded)));
    char partBuf[100];

    MAP *map = static_cast <MAP *>(FindObjectsData(wounded->mapindex));
    ...
    if (PartIsBroken(wounded, hitloc, critHit) && IsWeapon(critType) && !PartIsDisabled(wounded, hitloc, critHit)) {
        while (--critHit && GetPartType(wounded, hitloc, critHit) == critType)
            if (PartIsDestroyed(wounded, hitloc, critHit))
                break;
    
        mech_notify(wounded, MECHALL, tprintf("Your destroyed %s is damaged some more!", &MechWeapons[Weapon2I(critType)].name[3]));
    ...
    if (PartIsNonfunctional(wounded, hitloc, critHit)) {
        ...
        mech_notify(wounded, MECHALL, tprintf("Part of your non-working %s has been hit!", partBuf));
        DestroyPart(wounded, hitloc, critHit);
        return 1;
    }
    ...
    if (IsSpecial(critType)) {
        ...
        switch (Special2I(critType)) {
            case LIFE_SUPPORT:
                ...
                break;
            case COCKPIT:    /* Destroy Mech for now, but later kill pilot as well */
                ...
                if (LOS && attacker)
                    mech_notify(attacker, MECHALL, "You destroy the cockpit!!  The pilot's blood splatters down the sides!!!");
                MechLOSBroadcast(wounded, "spasms for a second then remains oddly still.");
                MechPilot(wounded) = -1;
                KillMechContentsIfIC(wounded->mynum);
                break;
            case SENSORS:
            ...
            case HEAT_SINK:
                if (MechHasDHS(mech)) {
                    ...
                    DestroyWeapon(wounded, hitloc, critType, critHit, 1, HS_Size(mech));
                    destroycrit = 0;
                } else
            ...
            case JUMP_JET:
                if (!Destroyed(wounded) && Started(wounded)) {
                    sprintf(msgbuf, "'s %s flares as superheated plasma spews out!", locname);
                    MechLOSBroadcast(wounded, msgbuf);
                }
                ...
                if (attacker && MechJumpSpeed(wounded) < MP1 && Jumping(wounded)) {
                    mech_notify(wounded, MECHALL, "Losing your last Jump Jet you fall from the sky!!!!!");
                    MechLOSBroadcast(wounded, "falls from the sky!");
            ...
            case ENGINE:
                ...
                if (MechEngineHeat(wounded) < 10) {
                    ...
                    if (MechEngineHeat(wounded) < 15) {
                        ...
                        if (wounded != attacker && !(MechStatus(wounded) & DESTROYED) && attacker)
            ...
            case GYRO:
                if (!(MechCritStatus(wounded) & GYRO_DAMAGED)) {
                    if (!Destroyed(wounded) && Started(wounded)) {
                        sprintf(msgbuf, "emits a loud screech as it's gyro buckles under the impact!");
                        MechLOSBroadcast(wounded, msgbuf);
                    }
                    ...
                    if (!(MechCritStatus(wounded) & GYRO_DESTROYED)) {
            ...
            case SHOULDER_OR_HIP:
                ...
                if (tLocIsArm) {
                    ...
                    if (tLocIsLeg) {
                        ...
                        if (attacker && !Jumping(wounded) && !OODing(wounded) && !MadePilotSkillRoll(wounded, 0)) {
                            mech_notify(wounded, MECHALL, "You lose your balance and fall down!");
                            MechLOSBroadcast(wounded, "stumbles and falls down!");
                            MechFalls(wounded, 1, 0);
                        }
            ...
            case HAND_OR_FOOT_ACTUATOR:
                ...
                if (tLocIsArm) {
                    if (Special2I(critType) == HAND_OR_FOOT_ACTUATOR)
                        mech_notify(wounded, MECHALL, tprintf("Your %s hand actuator is destroyed!!", hitloc == LARM ? "left" : "right"));
                    else
                        mech_notify(wounded, MECHALL, tprintf("Your %s %s arm actuator is destroyed!!", hitloc == LARM ? "left" : "right", Special2I(critType) == LOWER_ACTUATOR ? "lower" : "upper"));

                    if ((Special2I(critType) == HAND_OR_FOOT_ACTUATOR) && (MechSections(mech)[hitloc].specials & CARRYING_CLUB))
                        DropClub(mech);
                    ...
                    if (tLocIsLeg) {
                        ...
                        if (OkayCritSectS(hitloc, 0, SHOULDER_OR_HIP)) {    /* don't need to bother with crits if we already have a hip crit here */
                            if (!Destroyed(wounded) && Started(wounded)) {
                                sprintf(msgbuf, "'s %s twists in an odd way!", locname);
                                MechLOSBroadcast(wounded, msgbuf);
                            }
            ...
            case C3_MASTER:
                ...
                if (temp == MechWorkingC3Masters(mech))
                    mech_notify(wounded, MECHALL, "Your destroyed C3 system takes another hit!");
                else {
                    ...
                    if (MechTotalC3Masters(mech))
                        mech_notify(wounded, MECHALL, "One of your C3 systems has been destroyed!");
                    else
            ...
            case BEAGLE_PROBE:
                ...
                mech_notify(wounded, MECHALL, "Your Beagle Active Probe has been destroyed!");
                if (((sensors[(short) MechSensor(wounded)[0]].required_special == BEAGLE_PROBE_TECH) && 
                        (sensors[(short) MechSensor(wounded)[0]].specials_set == 1)) || 
                        ((sensors[(short) MechSensor(wounded)[1]].required_special == BEAGLE_PROBE_TECH) && 
                    ...
                    if ((sensors[(short) MechSensor(wounded)[1]].required_special == BEAGLE_PROBE_TECH) &&
                            (sensors[(short) MechSensor(wounded)[1]].specials_set == 1))
            ...
            case BLOODHOUND_PROBE:
                ...
                if (((sensors[(short) MechSensor(wounded)[0]].required_special == BLOODHOUND_PROBE_TECH) &&
                        (sensors[(short) MechSensor(wounded)[0]].specials_set == 1)) ||
                        ((sensors[(short) MechSensor(wounded)[1]].required_special == BLOODHOUND_PROBE_TECH) &&
                        (sensors[(short) MechSensor(wounded)[1]].specials_set == 1))) {
                    ...
                    if ((sensors[(short) MechSensor(wounded)[1]].required_special == BLOODHOUND_PROBE_TECH) &&
                            (sensors[(short) MechSensor(wounded)[1]].specials_set == 1))
                        MechSensor(wounded)[1] = 0;
            ...
            case DS_AERODOOR:
                ...
                break;
            case DS_MECHDOOR:
                mech_notify(wounded, MECHALL, "One of the 'mech doors is destroyed useless!");
                break;
            case NULL_SIGNATURE_SYSTEM:
                mech_notify(wounded, MECHALL, "Your Null Signature System has been destroyed!");
...
void HandleCritical(MECH * wounded, MECH * attacker, int LOS, int hitloc, int num)
{
    ...
    if (MechType(wounded) == CLASS_MW && Number(1, 2) == 1)
        return;
    if (MechType(wounded) != CLASS_MECH && !btechconf.btech_vcrit)
        return;
    ...
    if (MechType(wounded) == CLASS_VEH_GROUND || MechType(wounded) == CLASS_VEH_NAVAL) {
        if (btechconf.btech_fasaadvvhlcrit) {
            for (i = 0; i < num; i++)
                HandleAdvFasaVehicleCrit(wounded, attacker, LOS, hitloc, num);

            return;
        } else 
            if (!btechconf.btech_fasacrit) {
                for (i = 0; i < num; i++)
    ...
    if (MechType(wounded) == CLASS_VTOL) {
        if (btechconf.btech_fasaadvvtolcrit) {
            for (i = 0; i < num; i++)
                HandleAdvFasaVehicleCrit(wounded, attacker, LOS, hitloc, num);
            return;
        } else {
    ...
    while (num > 0) {
        ...
        if (HandleMechCrit(wounded, attacker, LOS, hitloc, critHit, critType, critData))
            num--;
    }
}
