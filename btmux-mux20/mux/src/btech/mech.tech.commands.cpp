...
void tech_checkstatus(dbref player, void *data, char *buffer)
{
    ...
    notify(player, tprintf("The 'mech has approximately %s until done.", ms));
}
...
TECHCOMMANDH(tech_removegun)
{
    ...
    DOCHECK(SectIsDestroyed(mech, loc), "That part's blown off! You can assume the gun's gone too!");
    ...
    DOCHECK(SomeoneScrappingPart(mech, loc, part), "Someone's scrapping it already!");
    DOCHECK(!CanScrapPart(mech, loc, part), "Someone's tinkering with it already!");
    DOCHECK(SomeoneScrappingLoc(mech, loc), "Someone's scrapping that section - no additional removals are possible!");
    ...
    if (tech_weapon_roll(player, mech, REMOVEG_DIFFICULTY) < 0) {
        ...
        if (tech_weapon_roll(player, mech, REMOVEG_DIFFICULTY) < 0) {
            ...
            FAKEREPAIR(REMOVEG_TIME * ClanMod(GetWeaponCrits(mech, Weapon2I(GetPartType(mech, loc, part)))),
    ...
    STARTREPAIR(REMOVEG_TIME * ClanMod(GetWeaponCrits(mech, Weapon2I(GetPartType(mech, loc, part)))), mech, PACK_LOCPOS_E(loc, part, mod), muxevent_tickmech_removegun, EVENT_REPAIR_SCRG);
}
...
TECHCOMMANDH(tech_removepart)
{
    ...
    DOCHECK((t = GetPartType(mech, loc, part)) == EMPTY, "That location is empty!");
    DOCHECK(SectIsDestroyed(mech, loc), "That part's blown off! You can assume the part's gone too!");
    ...
    DOCHECK(t == Special(ENDO_STEEL) || t == Special(FERRO_FIBROUS) || t == Special(STEALTH_ARMOR) || t == Special(HVY_FERRO_FIBROUS) || t == Special(LT_FERRO_FIBROUS), "That type of item can't be removed!");
    DOCHECK(SomeoneScrappingPart(mech, loc, part), "Someone's scrapping it already!");
    DOCHECK(SomeoneScrappingLoc(mech, loc), "Someone's scrapping that section - no additional removals are possible!");
    DOCHECK(!CanScrapPart(mech, loc, part), "Someone's tinkering with it already!");
    /* Ok.. Everything's valid (we hope). */
    ...
    if (tech_roll(player, mech, REMOVEP_DIFFICULTY) < 0) {
        START("Ack! Your attempt is far from perfect, you try to recover the part..");
        if (tech_roll(player, mech, REMOVEP_DIFFICULTY) < 0) {
            START("No good. Consider the part gone.");
            mod = 3;
...
TECHCOMMANDH(tech_removesection)
{
    ...
    DOCHECK(Invalid_Scrap_Path(mech, loc), "You need to remove the outer sections first!");
    ...
    STARTREPAIR(REMOVES_TIME, mech, PACK_LOCPOS_E(loc, 0, mod), muxevent_tickmech_removesection, EVENT_REPAIR_SCRL);
}
...
TECHCOMMANDH(tech_replacegun)
{
    ...
    DOCHECK(SomeoneRepairing(mech, loc, part), "Someone's repairing that part already!");
    DOCHECK(!IsWeapon(GetPartType(mech, loc, part)), "That's no gun!");
    ...
    DOTECH_LOCPOS(REPLACE_DIFFICULTY + WEAPTYPE_DIFFICULTY(GetPartType(mech, loc, part)), replaceg_fail, replaceg_succ, replace_econ, REPLACEGUN_TIME * ClanMod(GetWeaponCrits(mech, Weapon2I(GetPartType(mech, loc, part)))), mech, PACK_LOCPOS_E(loc, part, brand), muxevent_tickmech_replacegun, EVENT_REPAIR_REPLG, "You start replacing the gun..", 1);
...
TECHCOMMANDH(tech_repairgun)
{
    ...
    if (PartIsDestroyed(mech, loc, part)) {
        if (GetWeaponCrits(mech, Weapon2I(GetPartType(mech, loc, part))) < 5 || PartIsDestroyed(mech, loc, part + 1)) {
        ...
        if (!PartTempNuke(mech, loc, part)) {
            notify(player, "That gun isn't hurtin'!");
            return;
        }

    DOTECH_LOCPOS(REPAIR_DIFFICULTY + WEAPTYPE_DIFFICULTY(GetPartType(mech, loc, part)) + extra_hard, repairg_fail, repairg_succ, repair_econ, REPAIRGUN_TIME, mech, PACK_LOCPOS(loc, part), muxevent_tickmech_repairgun, EVENT_REPAIR_REPAP, "You start repairing the weapon..", 1);
}
...
TECHCOMMANDH(tech_fixenhcrit)
{
    ...
    DOTECH_LOCPOS(ENHCRIT_DIFFICULTY, repairenhcrit_fail, repairenhcrit_succ, repairenhcrit_econ, REPAIRENHCRIT_TIME, mech, PACK_LOCPOS(loc, part), muxevent_tickmech_repairenhcrit, EVENT_REPAIR_REPENHCRIT, "You start repairing the weapon...", 1);
}
...
TECHCOMMANDH(tech_replacepart)
{
    ...
    DOCHECK(SomeoneRepairing(mech, loc, part), "Someone's repairing that part already!");
    DOCHECK(SomeoneScrappingLoc(mech, loc), "Someone's scrapping that section - no repairs are possible!");
    DOTECH_LOCPOS(REPLACE_DIFFICULTY + PARTTYPE_DIFFICULTY(GetPartType(mech, loc, part)), replacep_fail, replacep_succ, replace_econ, REPLACEPART_TIME, mech, PACK_LOCPOS(loc, part), muxevent_tickmech_repairpart, EVENT_REPAIR_REPL, "You start replacing the part..", 0);
}
...
TECHCOMMANDH(tech_repairpart)
{
    ...
    DOCHECK(SomeoneRepairing(mech, loc, part), "Someone's repairing that part already!");
    DOCHECK(SomeoneScrappingLoc(mech, loc), "Someone's scrapping that section - no repairs are possible!");
    DOTECH_LOCPOS(REPAIR_DIFFICULTY + PARTTYPE_DIFFICULTY(GetPartType(mech, loc, part)), repairp_fail, repairp_succ, repair_econ, REPAIRPART_TIME, mech, PACK_LOCPOS(loc, part), muxevent_tickmech_repairpart, EVENT_REPAIR_REPAP, "You start repairing the part..", 0);
}
...
TECHCOMMANDH(tech_toggletype)
{
    ...
    DOCHECK((!Wizard(player)) && In_Character(mech->mynum), "This command only works in simpods!");
    my_parsepart2(&loc, &part, &atype);
    DOCHECK(!IsAmmo((t = GetPartType(mech, loc, part))), "That's no ammo!");
    DOCHECK(PartIsNonfunctional(mech, loc, part) || PartIsDisabled(mech, loc, part), "The ammo compartment is nonfunctional!");
...
TECHCOMMANDH(tech_reload)
{
    ...
    my_parsepart2(&loc, &part, &atype);
    DOCHECK(!IsAmmo((t = GetPartType(mech, loc, part))), "That's no ammo!");
    DOCHECK(PartIsNonfunctional(mech, loc, part), "The ammo compartment is destroyed ; repair/replacepart it first.");
    ...
    change = 0;
    DOTECH_LOCPOS_VAL(RELOAD_DIFFICULTY, reload_fail, reload_succ, reload_econ, &change, RELOAD_TIME, mech, PACK_LOCPOS_E(loc, part, change), muxevent_tickmech_reload, EVENT_REPAIR_RELO, "You start reloading the ammo compartment..");
}
...
TECHCOMMANDH(tech_unload)
{
    ...
    DOCHECK(PartIsNonfunctional(mech, loc, part), "The ammo compartment is destroyed ; repair/replacepart it first.");
    DOCHECK(PartIsDisabled(mech, loc, part), "The ammo compartment is disabled ; repair/replacepart it first.");
    DOCHECK(!(now = GetPartData(mech, loc, part)), "That particular ammo compartment is empty already.");
    DOCHECK(SomeoneRepairing(mech, loc, part), "Someone's playing with that part already!");
    DOCHECK(SectIsDestroyed(mech, loc), "That part's blown off! Use reattach first!");
    DOCHECK(SectIsFlooded(mech, loc), "That location has been flooded! Use reseal first!");
    DOCHECK(SomeoneScrappingLoc(mech, loc), "Someone's scrapping that section - no repairs are possible!");
    ...
    STARTREPAIR(RELOAD_TIME, mech, PACK_LOCPOS_E(loc, part, change), muxevent_tickmech_reload, EVENT_REPAIR_RELO);
}
...
TECHCOMMANDH(tech_fixarmor)
{
    ...
    TECHCOMMANDD;
    DOCHECK(tech_parsepart_advanced(mech, buffer, &loc, NULL, NULL, 1) < 0, "Invalid section!");
    ...
    DOCHECK(SectIsDestroyed(mech, loc % 8), "That part's blown off! Use reattach first!");
    DOCHECK(SectIsFlooded(mech, loc % 8), "That location has been flooded! Use reseal first!");
    DOCHECK(SomeoneFixingA(mech, loc) || SomeoneFixingI(mech, loc % 8), "Someone's repairing that section already!");
    DOCHECK(GetSectInt(mech, loc % 8) != GetSectOInt(mech, loc % 8), "The internals need to be fixed first!");
    DOCHECK(SomeoneScrappingLoc(mech, loc), "Someone's scrapping that section - no repairs are possible!");
    ...
    DOTECH_LOC_VAL_S(FIXARMOR_DIFFICULTY, fixarmor_fail, fixarmor_succ, fixarmor_econ, &change, FIXARMOR_TIME * ochange, loc, EVENT_REPAIR_FIX, mech, "You start fixing the armor..");
    STARTIREPAIR(FIXARMOR_TIME * change, mech, (change * 16 + loc), muxevent_tickmech_repairarmor, EVENT_REPAIR_FIX, change);
}
...
TECHCOMMANDH(tech_fixinternal)
{
    ...
    DOCHECK(SectIsDestroyed(mech, loc), "That part's blown off! Use reattach first!");
    DOCHECK(SectIsFlooded(mech, loc), "That location has been flooded! Use reseal first!");
    DOCHECK(SomeoneFixing(mech, loc), "Someone's repairing that section already!");
    DOCHECK(SomeoneScrappingLoc(mech, loc), "Someone's scrapping that section - no repairs are possible!");
    ochange = change;
    DOTECH_LOC_VAL_S(FIXINTERNAL_DIFFICULTY, fixinternal_fail, fixinternal_succ, fixinternal_econ, &change, FIXINTERNAL_TIME * ochange, loc, EVENT_REPAIR_FIX, mech, "You start fixing the internals..");
    STARTIREPAIR(FIXINTERNAL_TIME * change, mech, (change * 16 + loc), muxevent_tickmech_repairinternal, EVENT_REPAIR_FIXI, change);
}
...
TECHCOMMANDH(tech_reattach)
{
    ...
    TECHCOMMANDC;
    my_parsepart(&loc, NULL);
    DOCHECK(MechType(mech) == CLASS_BSUIT, "You can't reattach a Battlesuit! Use 'replacesuit'!");
    DOCHECK(!SectIsDestroyed(mech, loc), "That section isn't destroyed!");
    DOCHECK(Invalid_Repair_Path(mech, loc), "You need to reattach adjacent locations first!");
    DOCHECK(SomeoneAttaching(mech, loc), "Someone's attaching that section already!");
    DOCHECK(!unit_is_fixable(mech), "You see nothing to reattach it to (read:unit is cored).");
    DOTECH_LOC(REATTACH_DIFFICULTY, reattach_fail, reattach_succ, reattach_econ, REATTACH_TIME, mech, loc, muxevent_tickmech_reattach, EVENT_REPAIR_REAT, "You start replacing the section..");
}
...
TECHCOMMANDH(tech_replacesuit)
{
    ...
    TECHCOMMANDC;
    my_parsepart(&loc, NULL);
    DOCHECK(MechType(mech) != CLASS_BSUIT, "You can only use 'replacesuit' on a battlesuit unit!");

    wSuits = CountBSuitMembers(mech);

    DOCHECK(MechMaxSuits(mech) <= wSuits, tprintf("This %s is already full! This %s only consists of %d suits!", GetLCaseBSuitName(mech), GetLCaseBSuitName(mech), MechMaxSuits(mech)));
    DOCHECK((loc >= MechMaxSuits(mech)) || (loc < 0), tprintf("Invalid suit! This %s only consists of %d suits!", GetLCaseBSuitName(mech), MechMaxSuits(mech)));

    DOCHECK(!SectIsDestroyed(mech, loc), "That suit isn't destroyed!");

    DOCHECK(SomeoneReplacingSuit(mech, loc), "Someone's already rebuilding that suit!");
    DOCHECK(wSuits <= 0, "You are unable to replace the suits here! None of the buggers are still alive!");
    DOTECH_LOC(REPLACESUIT_DIFFICULTY, replacesuit_fail, replacesuit_succ, replacesuit_econ, REPLACESUIT_TIME, mech, loc, muxevent_tickmech_replacesuit, EVENT_REPAIR_REPSUIT, "You start replacing the missing suit.");
}
...
TECHCOMMANDH(tech_reseal)
{
    ...
    DOCHECK(Invalid_Repair_Path(mech, loc), "You need to reattach adjacent locations first!");
    DOCHECK(SomeoneResealing(mech, loc), "Someone's sealing that section already!");
    DOTECH_LOC(RESEAL_DIFFICULTY, reseal_fail, reseal_succ, reseal_econ, RESEAL_TIME, mech, loc, muxevent_tickmech_reseal, EVENT_REPAIR_RESE, "You start resealing the section.");
}
