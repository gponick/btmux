...
void mech_scan(dbref player, void *data, char *buffer)
{
    ...
    switch (numargs) {
        case 1:
            ...
            if (args[0][1]) {
                ...
                DOCHECK(!InLineOfSight_NB(mech, tempMech, MechX(tempMech), MechY(tempMech), range), "That target isn't seen well enough by the scanners for scanning!");
                DOCHECK(!MechIsObservator(mech) && (int) range > MechScanRange(mech), "Target is out of scanner range.");
                break;
            } else {        /* Default target */
        ...
        case 0:
            ...
            if (tempMech) {
                ...
                if (!(MechStatus(mech) & LOCK_BUILDING))
                    DOCHECK(!FindTargetXY(mech, &enemyX, &enemyY, &enemyZ), "No default target set!");
                ...
                ValidCoordA(mech_map, mapx, mapy, "Those coordinates are out of scanner range.");
                DOCHECK(!MechIsObservator(mech) && (int) range > MechScanRange(mech), "Those coordinates are out of scanner range.");
                DOCHECK(!InLineOfSight_NB(mech, tempMech, mapx, mapy, range), "Target hex is not in line of sight!");
        ...
        case 3:
            ...
            fz = ZSCALE * Elevation(mech_map, mapx, mapy);
            range = FindRange(MechFX(mech), MechFY(mech), MechFZ(mech), fx, fy, fz);
            DOCHECK((int) range > MechScanRange(mech), "Those coordinates are out of scanner range.");
            DOCHECK(!InLineOfSight(mech, tempMech, mapx, mapy, range), "Coordinates are not in line of sight!");
            break;
        case 2:
            /* scan x, y */
            mapx = atoi(args[0]);
    ...
    if (tempMech) {
        DOCHECK(!InLineOfSight_NB(mech, tempMech, MechX(tempMech), MechY(tempMech), range), "That target isn't seen well enough by the scanners for report!");
        DOCHECK((MechType(tempMech) == CLASS_MW), "Your scanners cannot give you precise information on targets that small!");
        PrintEnemyStatus(player, mech, tempMech, range, options);
    
        if (!MechIsObservator(mech)) {
            mech_notify(tempMech, MECHSTARTED, tprintf("You are being scanned by %s", GetMechToMechID(tempMech, mech)));
            auto_reply(tempMech, tprintf("%s just scanned me.", GetMechToMechID(tempMech, mech)));
        }
...
void mech_report(dbref player, void *data, char *buffer)
{
    ...
    switch (numargs) {
        case 1:
            ...
            range = FaMechRange(mech, tempMech);
            DOCHECK(!InLineOfSight(mech, tempMech, MechX(tempMech), MechY(tempMech), range), "Target is not in line of sight!");
            DOCHECK(!InLineOfSight_NB(mech, tempMech, MechX(tempMech), MechY(tempMech), range), "That target isn't seen well enough by the scanners for a report!");
            break;
        case 2:
            /* report x, y */
            mapx = atoi(args[0]);
            ...
            DOCHECK((int) range > MechScanRange(mech), "Those coordinates are out of scanner range.");
            DOCHECK(!InLineOfSight(mech, tempMech, mapx, mapy, range), "Coordinates are not in line of sight!");
            DOCHECK(!InLineOfSight_NB(mech, tempMech, mapx, mapy, range), "That target isn't seen well enough by the scanners for a report!");
            fz = ZSCALE * Elevation(mech_map, mapx, mapy);
        ...
        case 0:
            ...
            if (tempMech) {
                ...
                DOCHECK(!InLineOfSight(mech, tempMech, MechX(tempMech), MechY(tempMech), range), "You don't see a thing.");
            }
    }
...
void ShowTurretFacing(dbref player, int spaces, MECH * mech)
{
    ...
    if (GetSectInt(mech, TURRET) && !(MechType(mech) == CLASS_MECH || MechType(mech) == CLASS_BSUIT || MechType(mech) == CLASS_MW) && !is_aero(mech)) {
        ...
        if (MechMove(mech) != MOVE_NONE)
            sprintf(buff, "      Turret Facing: %d degrees%s", j, i ? tprintf(" (%d offset from heading)", i) : "");
        else
...
void PrintReport(dbref player, MECH * mech, MECH * tempMech, float range)
{
    ...
    sprintf(buff, "[%s]  %-25.25s Tonnage: %d", MechIDS(tempMech, MechSeemsFriend(mech, tempMech)), mech_name, MechTons(tempMech));
    notify(player, buff);
    ...
    if (FlyingT(tempMech))
        notify(player, tprintf("      Vertical speed: %.1f KPH", MechVerticalSpeed(tempMech)));
    
    sprintf(buff, "      X, Y, Z: %3d, %3d, %3d\tHeat: %.0f deg C.", MechX(tempMech), MechY(tempMech), MechZ(tempMech), 10. * MechHeat(tempMech));
    notify(player, buff);
    
    if (MechLateral(tempMech))
        notify(player, tprintf("      Mech is moving laterally %s", LateralDesc(tempMech)));
    ...
    switch (MechMove(tempMech)) {
        ...
        case MOVE_VTOL:
            ...
            break;
        case MOVE_FLY:
            notify(player, tprintf("      Type: %-9s             Movement: FLIGHT", MechType(tempMech) == CLASS_AERO ? "AEROSPACE" : "DROPSHIP"));
            break;
        case MOVE_HULL:
    ...
    notify(player, tprintf("      In %s Weapons Arc", GetArcID(mech, weaponarc)));
    ...
    if (mech->mapindex != -1) {
        ...
        if (argc == 0) {
            ...
            if (argc == 2) {
                ...
                if (!(ix1 >= 0 && ix1 < mech_map->map_width && iy1 >= 0 && iy1 < mech_map->map_height)) {
                ...
                if (argc == 4) {
                    ...
                    if (!(ix1 >= 0 && ix1 < mech_map->map_width && iy1 >= 0 && y1 < mech_map->map_height && ix0 >= 0 && ix0 <= mech_map->map_width && iy0 >= 0 && iy0 < mech_map->map_height)) {
                        notify(player, "Invalid map coordinates!");
                        x1 = y1 = -1;
                    } else {
                        sprintf(buff, "Bearing to %d,%d from %d,%d is: ", ix1, iy1, ix0, iy0);
...
void mech_range(dbref player, void *data, char *buffer)
{
    ...
    if (mech->mapindex != -1) {
        ...
        if (argc == 0) {
            ...
            DOCHECK(!FindTargetXY(mech, &x1, &y1, &z1), "There is no default target!");
            ...
            if (argc == 2) {
                ...
                if (argc == 4) {
                    ...
                    if (!(ix1 >= 0 && ix1 < mech_map->map_width && iy1 >= 0 && iy1 < mech_map->map_height && ix0 >= 0 && ix0 <= mech_map->map_width && iy0 >= 0 && iy0 < mech_map->map_height)) {
                        notify(player, "Invalid map coordinates!");
                        x1 = y1 = -1;
                    } else {
                        sprintf(buff, "Range to %d,%d from %d,%d is: ", ix1, iy1, ix0, iy0);
                        MapCoordToRealCoord(ix1, iy1, &x1, &y1);
                        MapCoordToRealCoord(ix0, iy0, &x0, &y0);
...
void mech_view(dbref player, void *data, char *buffer)
{
    ...
    if (argc == 0) {        /* default target */
        ...
        if ((pattr = atr_str("MECHDESC")) && pattr->number)
            target_desc = atr_get(target->mynum, pattr->number, &aowner, &aflags);
        else
            target_desc = "";
        ...
        if (argc == 1) {    /* ID number */
            ...
            if (!target || !InLineOfSight(mech, target, MechX(target), MechY(target), FlMechRange(mech_map, mech, target))) {
                mech_notify(mech, MECHPILOT, "Target is not in line of sight!");
                return;
            }
