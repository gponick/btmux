...
void replicateC3iNetwork(MECH * mechSrc, MECH * mechDest)
{
    ...
    debugC3(tprintf("REPLICATE: %d's C3i network to %d", mechSrc->mynum, mechDest->mynum));
...
void addMechToC3iNetwork(MECH * mech, MECH * mechToAdd)
{
    ...
    debugC3(tprintf("ADD: %d to the C3i network of %d", mechToAdd->mynum, mech->mynum));
    ...
    mech_notify(mech, MECHALL, tprintf("%s connects to your C3i network.", GetMechToMechID(mech, mechToAdd)));
    ...
    for (i = 0; i < C3I_NETWORK_SIZE; i++) {
        ...
        if (otherRef != mechToAdd->mynum) {
            ...
            if (otherNotifyMech)
                mech_notify(otherNotifyMech, MECHALL, tprintf("%s connects to your C3i network.", GetMechToMechID(otherNotifyMech, mechToAdd)));
        }
...
void validateC3iNetwork(MECH * mech)
{
    ...
    for (i = 0; i < C3I_NETWORK_SIZE; i++) {
        ...
        debugC3(tprintf("VALIDATE INFO: %d is now in %d's C3i network", otherMech->mynum, mech->mynum));
    ...
    debugC3(tprintf("VALIDATE INFO: %d's C3i network is %d elements", mech->mynum, MechC3iNetworkSize(mech)));
}
...
void mech_c3i_join_leave(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(AnyECMDisturbed(mech), "Your C3i system is not currently operational!");
    ...
    DOCHECK(MechC3iNetworkSize(mech) > 0, "You are already in a C3i network!");
    ...
    DOCHECK(MechTeam(mech) != MechTeam(target), "You can't use the C3i network of unfriendly units!");
    ...
    DOCHECK(!HasC3i(target), "That unit does not appear to be equipped with C3i!");

    /* validate the network of our target */
    validateC3iNetwork(target);
    DOCHECK(MechC3iNetworkSize(target) >= C3I_NETWORK_SIZE, "That unit's C3i network is operating at maximum capacity!");

    /* Connect us up */
    mech_notify(mech, MECHALL, tprintf("You connect to %s's C3i network.", GetMechToMechID(mech, target)));
    addMechToC3iNetwork(target, mech);
}

void mech_c3i_message(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(MechC3iNetworkSize(mech) <= 0, "There are no other units in your C3i network!");
...
void mech_c3i_targets(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
...
void mech_c3i_network(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
