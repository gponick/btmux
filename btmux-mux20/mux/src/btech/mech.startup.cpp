...
static void mech_startup_event(EVENT * e)
{
    ...
    mech_notify(mech, MECHALL, is_aero(mech) ? aero_bootmsgs[timer] : MechType(mech) == CLASS_BSUIT ? bsuit_bootmsgs[timer] : MechMove(mech) == MOVE_HOVER ? hover_bootmsgs[timer] : MechMove(mech) == MOVE_TRACK ? track_bootmsgs[timer] : MechMove(mech) == MOVE_WHEEL ? wheel_bootmsgs[timer] : MechMove(mech) == MOVE_VTOL ? vtol_bootmsgs[timer] : bootmsgs[timer]);
    ...
    MechCommLast(mech) = 0;
    MechLastStartup(mech) = ltaNow.ReturnSeconds();
...
void mech_startup(dbref player, void *data, char *buffer)
{
    ...
    skipws(buffer);
    DOCHECK(!(Good_obj(player)), "That is not a valid player!");
    DOCHECK(MechType(mech) == CLASS_MW && Started(mech), "You're up and about already!");
    DOCHECK(Towed(mech), "You're being towed! Wait for drop-off before starting again!");
    ...
    DOCHECK(n, "This 'Mech is still under repairs (see checkstatus for more info)");
    ...
    MECHEVENT(mech, EVENT_STARTUP, mech_startup_event, (n || MechType(mech) == CLASS_MW) ? 1 : SSLEN,
...
void mech_shutdown(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK((!Started(mech) && !Starting(mech)), "The 'mech hasn't been started yet!");
    DOCHECK(MechType(mech) == CLASS_MW, "You snore for a while.. and then _start_ yourself back up.");
    DOCHECK(IsDS(mech) && !Landed(mech) && !Wiz(player), "No shutdowns in mid-air! Are you suicidal?");
    ...
    mech_notify(mech, MECHALL, tprintf("%s has been shutdown!", IsDS(mech) ? "Dropship" : is_aero(mech) ? "Fighter" : MechType(mech) == CLASS_BSUIT ? "Suit" : ((MechMove(mech) == MOVE_HOVER) || (MechMove(mech) == MOVE_TRACK) || (MechMove(mech) == MOVE_WHEEL)) ? "Vehicle" : MechMove(mech) == MOVE_VTOL ? "VTOL" : "Mech"));
    ...
    if ((MechType(mech) == CLASS_MECH && Jumping(mech)) || (MechType(mech) != CLASS_MECH && MechZ(mech) > MechUpperElevation(mech))) {
        ...
        if (MechSpeed(mech) > MP1) {
            ...
            if (MechType(mech) == CLASS_MECH)
                ...
                mech_notify(mech, MECHALL, "You tumble end over end and come to a crashing halt!");
                MechLOSBroadcast(mech, "tumbles end over end and comes to a crashing halt!");
            }
