...
int game_lag(void)
{
    ...
    return 100 * ltd.ReturnSeconds() / muxevent_tick - 100;
}
...
int tech_roll(dbref player, MECH * mech, int diff)
{
    ...
    if (Wizard(player)) {
        notify(player, tprintf("Tech - BTH: %d(Base:%d, Mod:%d) Roll: %d", s, s - diff, diff, r));
    } else {
...
int tech_weapon_roll(dbref player, MECH * mech, int diff)
{
    ...
    if (Wizard(player)) {
        notify(player, tprintf("Tech-W - BTH: %d(Base:%d, Mod:%d) Roll: %d", s, s - diff, diff, r));
    } else {
        notify(player, tprintf("BTH: %d Roll: %d", s, r));
    }
...
void tech_status(dbref player, CLinearTimeAbsolute dat)
{
    char buf[MBUF_SIZE];
    char *olds;
    int un;
    ...
    if (dat.ReturnSeconds() <= 0) {
        ...
        if ((pattr = atr_str("TECHTIME")) && pattr->number)
            olds = atr_get(player, pattr->number, &aowner, &aflags);
        else
            olds = "";
    ...
    if (dat <= ltaNow)
        ...
        ltd = dat - ltaNow;
        un = ltd.ReturnSeconds() / TECH_TICK;
        sprintf(buf, "You have %d %s%s of repairs pending", un, TECH_UNIT, un != 1 ? "s" : "");
        
        if (un >= MAX_TECHTIME)
            sprintf(buf + strlen(buf), " and you're too tired to do more efficiently.");
        else {
            un = MAX_TECHTIME - un;
            sprintf(buf + strlen(buf), " and you're ready to do at least %d more %s%s of work.", un, TECH_UNIT, un == 1 ? "" : "s");
        }
...
int tech_addtechtime(dbref player, int time)
{
    ...
    CLinearTimeAbsolute ltaNow;
    CLinearTimeDelta ltd, timeDelta;
    ...
    timeDelta.SetSeconds(time * TECH_TICK);
    old += timeDelta;

    int atr = mkattr(player, "TECHTIME");

    if (!atr)
        return false;

    atr_pget_info(player, atr, &aowner, &aflags);
    atr_add(player, atr, old.ReturnDateString(), GOD, aflags);

    tech_status(player, old);
    ltd = old - ltaNow;
    return (ltd.ReturnSeconds());
}
...
int tech_parsepart_advanced(MECH * mech, char *buffer, int *loc, int *pos, int *extra, int allowrear)
{
    ...
    if (!allowrear) {
        if ((!extra && argc != (1 + (pos != NULL))) || (extra && (argc < (1 + (pos != NULL)) || argc > (2 + (pos != NULL))))) 
            return -1;
    } else {
        if (argc == 2) {
            if (mux_toupper(args[1][0]) != 'R')
                return -1;
    ...
    if ((*loc = ArmorSectionFromString(MechType(mech), MechMove(mech), args[0])) < 0)
        return -1;
...
{
    CLinearTimeDelta ltd = e->tick - muxevent_tick;
    int ofs = ltd.ReturnSeconds();
