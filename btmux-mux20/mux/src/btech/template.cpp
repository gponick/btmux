...
#define CLCH(a) \
    do { if (temp_brand_flag) { if ((!btechconf.btech_parts) || (MechWeapons[a].special & CLAT)) return NULL; else if (MechWeapons[a].special & CLAT) isclan=1; }} while (0)
...
static char *part_figure_out_name_sub(int i, int j)
{
    ...
    if (IsWeapon(i) && i < I2Weapon(num_def_weapons)) {
        ...
        if (IsAmmo(i) && i < I2Ammo(num_def_weapons)) {
            CLCH(Ammo2WeaponI(i));
            if (MechWeapons[Ammo2WeaponI(i)].type != TBEAM && MechWeapons[Ammo2WeaponI(i)].type != THAND && !(MechWeapons[Ammo2WeaponI(i)].special & PCOMBAT)) {
                sprintf(buf, "Ammo_%s", &MechWeapons[Ammo2WeaponI(i)].name[(j && !isclan) ? 3 : 0]);
                return buf;
            }
        } else 
            if (!temp_brand_flag) {
                if (IsBomb(i)) {
                    ...
                    if (IsSpecial(i) && i < I2Special(internal_count))
...
char *part_figure_out_shname(int i)
{
    ...
    if (IsWeapon(i) && i < I2Weapon(num_def_weapons)) {
        ...
        if (IsAmmo(i) && i < I2Ammo(num_def_weapons)) {
            sprintf(buf, "Ammo_%s", &MechWeapons[Ammo2WeaponI(i)].name[CLCH(Ammo2WeaponI(i))]);
        } else 
            if (IsBomb(i))
                sprintf(buf, "Bomb_%s", bomb_name(Bomb2I(i)));
            else 
...
static int dump_item(FILE * fp, MECH * mech, int x, int y)
{
    ...
    if (IsWeapon(GetPartType(mech, x, y))) {
        ...
        if (!TCAble(GetPartType(mech, x, y)))
            flaggo = ON_TC;
        if (((y1 - y) + 1) > (z = GetWeaponCrits(mech, Weapon2I(GetPartType(mech, x, y)))))
            y1 = y + z - 1;
    }
    ...
    if (IsWeapon(GetPartType(mech, x, y)))
        fprintf(fp, "    %s          { %s - %s %s}\n", crit, get_parts_vlong_name(GetPartType(mech, x, y), 0), (wFireModes || wAmmoModes) ? BuildBitString2(crit_fire_modes, crit_ammo_modes, wFireModes, wAmmoModes) : "-", !btechconf.btech_parts ? "" : tprintf("%d ", GetPartBrand(mech, x, y)));
    else 
        if (IsAmmo(MechSections(mech)[x].criticals[y].type))
            fprintf(fp, "    %s          { %s %d %s - }\n", crit, get_parts_vlong_name(GetPartType(mech, x, y), 0), FullAmmo(mech, x, y), (MechSections(mech)[x].criticals[y].firemode || MechSections(mech)[x].criticals[y].  ammomode) ? BuildBitString2(crit_fire_modes, crit_ammo_modes, MechSections(mech)[x].criticals[y].firemode, MechSections(mech)[x].criticals[y].ammomode) : "-");
        else 
            if (IsBomb(MechSections(mech)[x].criticals[y].type))
                fprintf(fp, "    %s          { %s - - - }\n", crit, get_parts_vlong_name(GetPartType(mech, x, y), 0));
            else 
                fprintf(fp, "    %s          { %s - - %s}\n", crit, get_parts_vlong_name(GetPartType(mech, x, y), 0), !btechconf.btech_parts ? "" : tprintf("%d ", GetPartBrand(mech, x, y)));
...
void computer_conversion(MECH * mech)
{
    ...
    if (l) {
        MechComputer(mech) = l;
        MechScanRange(mech) = (int)(MechComputersScanRange(mech));
        MechTacRange(mech) = (int)(MechComputersTacRange(mech));
        MechLRSRange(mech) = (int)(MechComputersLRSRange(mech));
        MechRadioRange(mech) = (int)(MechComputersRadioRange(mech));
    }
}
        ...
        char *filename)
{
    ...
    ltaNow.GetUTC();
    char *c = ltaNow.ReturnDateString();
...
#define SILLY_UTTERANCE(ran,cran,dran,name) \
    ...
    SILLY_UTTERANCE(MechTacRange(mech), MechComputersTacRange(mech), DEFAULT_TACRANGE, "Tac_Range");
    SILLY_UTTERANCE(MechLRSRange(mech), MechComputersLRSRange(mech), DEFAULT_LRSRANGE, "LRS_Range");
    SILLY_UTTERANCE(MechScanRange(mech), MechComputersScanRange(mech), DEFAULT_SCANRANGE, "Scan_Range");
    SILLY_UTTERANCE(MechRadioRange(mech), MechComputersRadioRange(mech), DEFAULT_RADIORANGE, "Radio_Range");
...
#define SILLY_OUTPUT(def,now,name) \
    ...
    SILLY_OUTPUT((MechSpecials(mech) & ICE_TECH) ? 0 : DEFAULT_HEATSINKS, MechRealNumsinks(mech), "Heat_Sinks");
    SILLY_OUTPUT(generic_radio_type(MechRadio(mech), MechSpecials(mech) & CLAN_TECH), MechRadioType(mech), "RadioType");
    ...
    x &= ~(BEAGLE_PROBE_TECH | TRIPLE_MYOMER_TECH | MASC_TECH | ECM_TECH | C3_SLAVE_TECH | C3_MASTER_TECH | ARTEMIS_IV_TECH | ES_TECH | FF_TECH);

    if (MechType(mech) == CLASS_MECH)
        x &= ~(XL_TECH | XXL_TECH | CE_TECH | LE_TECH);

    /* Get rid of our specials2 */
    x2 &= ~(STEALTH_ARMOR_TECH | NULLSIGSYS_TECH | ANGEL_ECM_TECH | HVY_FF_ARMOR_TECH | LT_FF_ARMOR_TECH | TAG_TECH | C3I_TECH | BLOODHOUND_PROBE_TECH);
    ...
    if ((locs = ProperSectionStringFromType(MechType(mech), MechMove(mech)))) {
...
#define TECHU(item, name) \
    ...
    if ((MechSpecials(mech) & (XXL_TECH | XL_TECH | LE_TECH)) && (MechSpecials(mech) & CE_TECH))
        SendError(tprintf ("#%d apparently is very weird: Compact engine AND XL/XXL?", mech->mynum));
    if (tc_count)
        for (x = 0; x < NUM_SECTIONS; x++)
            for (y = 0; y < CritsInLoc(mech, x); y++)
...
int update_oweight(MECH * mech, int value)
{
    ...
    if (!btechconf.btech_dynspeed || (value == 1 && !Destroyed(mech)))
...
int load_template(dbref player, MECH * mech, char *filename)
{
    ...
    int atr = mkattr(mech->mynum, "MECHREF");
    ...
    atr_pget_info(mech->mynum, atr, &aowner, &aflags);
    atr_add(mech->mynum, atr, MechType_Ref(mech), GOD, aflags);
    ...
    while (fgets(line, 512, fp)) {
        ...
        if (!strncasecmp(cmd, "CRIT_", 5))
            ...
            if ((selection = compare_array(load_cmds, cmd)) == -1) {
                /* Initial premise: we will have a mech type before we get to this */
                section = find_section(cmd, MechType(mech), MechMove(mech));
                TEMPLATE_ERR(section == -1 && !ok_count, "New template loading system: %s is invalid template file.", filename);
                TEMPLATE_GERR(section == -1, "Error while loading: Section %s not found.", cmd);
        ...
        switch (selection) {
            case 0:        /* Reference */
                ...
                if (strcmp(tmpc, MechType_Ref(mech))) {
                    SendError(tprintf("Template %s has Reference <-> Filename mismatch : %s <-> %s - It is automatically fixed by saving again.", filename, tmpc, MechType_Ref(mech)));
                    tmpc = MechType_Ref(mech);
                }
                atr = mkattr(mech->mynum, "MECHREF");

                if (!atr)
                    return false;

                atr_pget_info(mech->mynum, atr, &aowner, &aflags);
                atr_add(mech->mynum, atr, tmpc, GOD, aflags);
            ...
            case 1:        /* Type */
                ...
                TEMPLATE_GERR(MechType(mech) == -1, "Error while loading: Type %s not found.", tmpc);
                AeroFuel(mech) = AeroFuelOrig(mech) = DefaultFuelByType(mech);
                break;
            case 2:        /* Movement Type */
                tmpc = read_desc(fp, ptr);
                MechMove(mech) = compare_array(move_types, tmpc);
                TEMPLATE_GERR(MechMove(mech) == -1, "Error while loading: Type %s not found.", tmpc);
                break;
            case 3:        /* Tons */
            ...
            case 10:        /* Specials */
                ...
                if (CheckSpecialsList(specials, specials2, tmpc)) {
                    ...
                    TEMPLATE_GERR(MechSpecials(mech) == -1, "Error while loading: Invalid specials - %s.", tmpc);
                break;
            case 11:        /* Armor */
            ...
            case 14:        /* Config */
                tmpc = read_desc(fp, ptr);
                MechSections(mech)[section].config = BuildBitVector(section_configs, tmpc) & ~(CASE_TECH | SECTION_DESTROYED);
                TEMPLATE_GERR(MechSections(mech)[section].config == -1, "Error while loading: Invalid location config: %s.", tmpc);
                break;
            case 9999:
                ...
                if (!strncasecmp(buf, "CL.", 3))
                    isClan = 1;
                TEMPLATE_GERR(!(find_matching_vlong_part(buf, NULL, &type, &brand)), "Unable to find %s", buf);
                ...
                if (IsWeapon(type)) {
                    ...
                    TEMPLATE_GERR(((wFireModes < 0) && (wAmmoModes < 0)), "Error while loading: Invalid crit modes for ammo: %s.", buf);
                    ...
                    if (GetPartData(mech, section, critical) != FullAmmo(mech, section, critical) && MechType(mech) != CLASS_MW && MechType(mech) != CLASS_BSUIT) {
                        SendError(tprintf("Invalid ammo crit for %s in #%d %s (%d/%d)", MechWeapons[Ammo2I(type)].name, mech->mynum, filename, GetPartData(mech, section, critical), FullAmmo(mech, section, critical)));
                        SetPartData(mech, section, critical, FullAmmo(mech, section, critical));
                    }
                } else {
...
#define Set(a,b) \
        ...
        for (x = 0; x < value; x++)
            for (y = 0; y < CritsInLoc(mech, x); y++)
                if ((t = GetPartType(mech, x, y))) {
                    ...
                    if (IsBomb(t))
                        continue;
                    SetPartBrand(mech, x, y, isClan ? DEFAULT_CLPART_LEVEL : DEFAULT_PART_LEVEL);
                }
    if (isClan) {
    ...
    if (!MechComputer(mech)) {
        ...
        Set(MechScanRange(mech), (int)(MechComputersScanRange(mech)));
        Set(MechLRSRange(mech), (int)(MechComputersLRSRange(mech)));
        Set(MechRadioRange(mech), (int)(MechComputersRadioRange(mech)));
        Set(MechTacRange(mech), (int)(MechComputersTacRange(mech)));
    }
    if (MechType(mech) == CLASS_MECH)
        if ((GetPartType(mech, LARM, 2) != Special(LOWER_ACTUATOR)) && (GetPartType(mech, RARM, 2) != Special(LOWER_ACTUATOR)) && (GetPartType(mech, LARM, 3) != Special(HAND_OR_FOOT_ACTUATOR)) && (GetPartType(mech, RARM, 3) != Special(HAND_OR_FOOT_ACTUATOR)))
    ...
    if (abs(x - y) > 40)
        SendError(tprintf("Error in %s template: %.1f tons of 'stuff', yet %d ton frame.", MechType_Ref(mech), x / 1024.0, y / 1024));
...
static char *dumpweapon_fun(int i)
{
    ...
    if (!i)
        ...
        i--;
        sprintf(buf, WDUMP_MASK, MechWeapons[i].name, MechWeapons[i].heat, MechWeapons[i].damage, MechWeapons[i].min, MechWeapons[i].shortrange, MechWeapons[i].medrange, GunRange(i), MechWeapons[i].vrt, MechWeapons[i].criticals, MechWeapons[i].ammoperton);
    }
    return buf;
}
...
void DumpWeapons(dbref player)
{
    ...
    c = SelCol_FunStringMenuK(1, "MechWeapons available", dumpweapon_fun, num_def_weapons + 1);
    ShowCoolMenu(player, c);
    KillCoolMenu(c);
}
...
char *techlist_func(MECH * mech)
{
    ...
    if (MechType(mech) == CLASS_MECH && MechMove(mech) != MOVE_QUAD) {
        if ((OkayCritSectS(RARM, 3, HAND_OR_FOOT_ACTUATOR) && OkayCritSectS(RARM, 0, SHOULDER_OR_HIP)) || (OkayCritSectS(LARM, 3, HAND_OR_FOOT_ACTUATOR) && OkayCritSectS(LARM, 0, SHOULDER_OR_HIP)) || MechSpecials(mech) & SALVAGE_TECH)
            strcat(buffer, " MTOW");
    } else {
...
char *payloadlist_func(MECH * mech)
{
    ...
    for (put_loop = 0; put_loop < (weap_count + ammo_count); put_loop++) {
        /* If its a weapon use this method of printing it out * Else use the part method */
        if (put_loop < weap_count) 
            sprintf(payloadbuff, "%s:%d", &MechWeapons[payload_items[put_loop]].name[0], payload_items_count[put_loop]);
        else 
            sprintf(payloadbuff, "%s:%d", partname_func(payload_items[put_loop], 'V'), payload_items_count[put_loop]);
