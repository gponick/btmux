...
void mech_findcenter(dbref player, void *data, char *buffer)
{
    ...
    notify(player, tprintf("Current hex: (%d,%d,%d)\tRange to center: %.2f\t" "Bearing to center: %d", x, y, MechZ(mech), FindHexRange(fx, fy, MechFX(mech), MechFY(mech)), FindBearing(MechFX(mech), MechFY(mech), fx, fy)));
}

static int parse_tacargs(dbref player, MECH * mech, char **args, int argc, int maxrange, short *x, short *y)
{
    int bearing;
    float range, fx, fy;
    MECH *tempMech;
    ...
    switch (argc) {
        case 2:
            bearing = atoi(args[0]);
            range = atof(args[1]);
            DOCHECK0(!MechIsObservator(mech) && abs((int) range) > maxrange, "Those coordinates are out of sensor range!");
...
static void set_colorscheme(dbref player)
{
    ...
    if ((pattr = atr_str("MAPCOLOR")) && pattr->number)
        str = atr_get(player, pattr->number, &aowner, &aflags);
    else
        str = "";
        ...
        for (i = 0; i < NUM_COLOR_IDX; i++) {
            switch (custom_color_str[i]) {
                ...
                case 'n':
                    ...
                    break;
                default:
                    notify(player, tprintf("Invalid character '%c' in MAPCOLOR " "attribute!", custom_color_str[i]));
...
void mech_navigate(dbref player, void *data, char *buffer)
{
    ...
    dolos = MapIsDark(mech_map) || (MechType(mech) == CLASS_MW && btechconf.btech_mw_losmap);

    DOCHECK(mech_map->map_width <= 0 || mech_map->map_height <= 0, "Nothing to see on this map, move along.");
    ...
    sprintf(mybuff[0], "              0                                          %.150s", maptext[0]);
    sprintf(mybuff[1], "         ___________                                     %.150s", maptext[1]);
    sprintf(mybuff[2], "        /           \\          Location:%4d,%4d, %3d   %.150s", MechX(mech), MechY(mech), MechZ(mech), maptext[2]);
    sprintf(mybuff[3], "  300  /             \\  60     Terrain: %14s   %.150s", GetTerrainName(mech_map, MechX(mech), MechY(mech)), maptext[3]);
    sprintf(mybuff[4], "      /               \\                                  %.150s", maptext[4]);
    sprintf(mybuff[5], "     /                 \\                                 %.150s", maptext[5]);
    sprintf(mybuff[6], "270 (                   )  90  Speed:           %6.1f   %.150s", MechSpeed(mech), maptext[6]);
    sprintf(mybuff[7], "     \\                 /       Vertical Speed:  %6.1f   %.150s", MechVerticalSpeed(mech), maptext[7]);
    sprintf(mybuff[8], "      \\               /        Heading:           %4d   %.150s", MechFacing(mech), maptext[8]);
    sprintf(mybuff[9], "  240  \\             /  120                              %.150s", maptext[9]);
    sprintf(mybuff[10], "        \\___________/                                    %.150s", maptext[10]);
    sprintf(mybuff[11], "                      ");
...
static char *add_color(char newc, char *prevc, char c)
{
    ...
    if (!newc || ((mux_isupper(*prevc)) && !mux_isupper(newc)) || (newc == 'H' && *prevc))
        strcpy(buf, "%cn");
    else 
        if (mux_isupper(newc) && !mux_isupper(*prevc))
         strcpy(buf, "%ch");
...
static char *get_lrshexstr(MECH * mech, MAP * map, int x, int y, char *prevc, int mode, MECH ** mechs, int lm, hexlosmap_info * losmap)
{
    ...
    if (((mode & LRS_TERRAINMODE) && !(losflag & MAPLOSHEX_SEETERRAIN)) || ((mode & LRS_ELEVMODE) && !(losflag & MAPLOSHEX_SEEELEV)))
    ...
    SendError(tprintf("Unknown LRS mode, mech #%d mode 0x%x.", mech->mynum, mode));

    return add_color('R', prevc, 'Y');

}

static void show_lrs_map(dbref player, MECH * mech, MAP * map, int x, int y, int displayHeight, int mode)
{
    ...
    if (mode & LRS_MECHMODE) {
        for (i = 0; i < map->first_free; i++) 
            if ((oMech = getMech(map->mechsOnMap[i]))) 
                if ((mech == oMech) || (MechY(oMech) >= b_height && MechY(oMech) <= e_height && MechX(oMech) >= b_width && MechX(oMech) <= e_width && InLineOfSight(mech, oMech, MechX(oMech), MechY(oMech), FlMechRange(map, mech, oMech))))
        ...
        for (i = 0; i < (last_mech - 1); i++)    /* Bubble-sort the list *  to y/x order */
            for (loop = (i + 1); loop < last_mech; loop++) {
                if (MechY(mechs[i]) > MechY(mechs[loop])) {
                    ...
                    if (MechY(mechs[i]) == MechY(mechs[loop]) && MechX(mechs[i]) > MechX(mechs[loop])) {
                        oMech = mechs[i];
                        mechs[i] = mechs[loop];
                        mechs[loop] = oMech;
                    }
    ...
    if (mode & LRS_LOSMODE)
        losmap = CalculateLOSMap(map, mech, b_width, b_height, e_width - b_width, e_height - b_height);
    ...
    for (loop = b_height; loop < e_height; loop++) {
        ...
        for (i = b_width; i < e_width; i += 2) {
            sprintf(topbuff + strlen(topbuff), oddcol ? "%s " : " %s", get_lrshexstr(mech, map, i + !oddcol, loop, &prevct, mode, mechs, last_mech, losmap));
            sprintf(botbuff + strlen(botbuff), oddcol ? " %s" : "%s ", get_lrshexstr(mech, map, i + oddcol, loop, &prevcb, mode, mechs, last_mech, losmap));
        }

        if (i == e_width && !oddcol) 
            sprintf(botbuff + strlen(botbuff), "%s", get_lrshexstr(mech, map, i, loop, &prevcb, mode, mechs, last_mech, losmap));
        else 
            if (i == e_width) {
                sprintf(topbuff + strlen(topbuff), "%s", get_lrshexstr(mech, map, i, loop, &prevct, mode, mechs, last_mech, losmap));
                strcat(botbuff, " ");
            }
...
void mech_lrsmap(dbref player, void *data, char *buffer)
{
    ...
    if (!parse_tacargs(player, mech, &args[1], argc - 1, MechLRSRange(mech), &x, &y))
        return;
    ...
    if (MapIsDark(map) || (MechType(mech) == CLASS_MW && btechconf.btech_mw_losmap))
        mode |= LRS_LOSMODE;
    ...
    if ((pattr = atr_str("LRSHEIGHT")) && pattr->number)
        str = atr_get(player, pattr->number, &aowner, &aflags);
    else
        str = "";
...
static inline void sketch_tac_row(char *pos, int left_offset, char const *src, int len)
{
    ...
    int left_offset, int docolour, int labels)
{
    ...
    for (i = 0; i < map->first_free; i++) {
        ...
        if (mech != player_mech && !InLineOfSight(player_mech, mech, MechX(mech), MechY(mech), FlMechRange(map, player_mech, mech))) 
            continue;
    ...
    int wy, int dispcols, int top_offset, int left_offset, int cliff_size)
{
    ...
    for (y = MAX(0, -sy); y < wy; y++) {
        ...
        for (x = MAX(0, -sx); x < wx; x++) {
            ...
            if (x != 0 && (y < wy - 1 || oddcolx) && abs(Elevation(map, tx - 1, ty + 1 - oddcolx) - elev) >= cliff_size) 
                base[dispcols - 1] = '|';
            ...
            if (x < wx - 1 && (y < wy - 1 || oddcolx) && abs(Elevation(map, tx + 1, ty + 1 - oddcolx) - elev) >= cliff_size) 
                base[dispcols + 2] = '!';
        }
    }
}
static void sketch_tac_dslz(char *buf, MAP * map, MECH * mech, int sx,
    ...
    int top_offset, int left_offset, int cliff_size, int docolour)
{
    char *pos = buf + top_offset * dispcols + left_offset;
    int y, x;
    int oddcol1 = is_oddcol(sx);
...
static char **colourize_tac_map(char const *sketch, int dispcols, int disprows)
{
...
char **MakeMapText(dbref player, MECH * mech, MAP * map, int cx, int cy, int wx, int wy, int labels, int dohexlos)
{
    ...
    sketch_tac_map(sketch_buf, map, mech, sx, sy, wx, wy, dispcols, top_offset, left_offset, docolour, dohexlos);
    ...
    if (labels & 8) {
        if (mech != NULL) 
            sketch_tac_ownmech(sketch_buf, map, mech, sx, sy, wx, wy, dispcols, top_offset, left_offset);
        sketch_tac_cliffs(sketch_buf, map, sx, sy, wx, wy, dispcols, top_offset, left_offset, 3);
    } else 
        if (labels & 16) {
            if (mech != NULL) 
                sketch_tac_ownmech(sketch_buf, map, mech, sx, sy, wx, wy, dispcols, top_offset, left_offset);
    
            sketch_tac_cliffs(sketch_buf, map, sx, sy, wx, wy, dispcols, top_offset, left_offset, 2);
        } else 
            if (labels & 32) {
                if (mech != NULL) 
                    sketch_tac_ownmech(sketch_buf, map, mech, sx, sy, wx, wy, dispcols, top_offset, left_offset);
                sketch_tac_dslz(sketch_buf, map, mech, sx, sy, wx, wy, dispcols, top_offset, left_offset, 2, docolour);
            } else 
                if (mech != NULL) 
                    sketch_tac_mechs(sketch_buf, map, mech, sx, sy, wx, wy, dispcols, top_offset, left_offset, docolour, labels);
    ...
    if (navigate) {
        ...
        if (oddcol1) /* * Don't need the last line in this case.  */
            disprows--;
    ...
    if (docolour) 
        /* If using colour then colourize the sketch map and return the result.  */
...
void mech_tacmap(dbref player, void *data, char *buffer)
{
    ...
    char **args = args_vec;
    MAP *mech_map;
    int displayHeight = MAP_DISPLAY_HEIGHT, displayWidth =
    MAP_DISPLAY_WIDTH;
    char *str;
    char **maptext;
    ...
    if (MapIsDark(mech_map) || (MechType(mech) == CLASS_MW && btechconf.btech_mw_losmap))
        dohexlos = 1;

    /* Check to see which type of tactical to display if they specified a particular one */
    if (argc > 0 && mux_isalpha((unsigned char) args[0][0]) && args[0][1] == '\0') {
        switch (mux_tolower((unsigned char) args[0][0])) {
            case 'c':
                flags |= 8;        /* Show cliffs */
                break;
            case 't':
    ...
    if ((pattr = atr_str("TACHEIGHT")) && pattr->number)
        str = atr_get(player, pattr->number, &aowner, &aflags);
    else
        str = "";
    ...
    if (!*str) {
        ...
        if (sscanf(str, "%d %d", &displayHeight, &displayWidth) != 2 || displayHeight > 24 || displayHeight < 5 || displayWidth > 40 || displayWidth < 5) {
            notify(player, "Illegal Tacsize attribute. Must be in format 'Height Width' . Height : 5-24 Width : 5-40");
            displayHeight = MAP_DISPLAY_HEIGHT;
            displayWidth = MAP_DISPLAY_WIDTH;
        }

    /* Everything worked but lets check the mech's tac range and the map size */
    displayHeight = (displayHeight <= 2 * MechTacRange(mech) ? displayHeight : 2 * MechTacRange(mech));
    displayWidth = (displayWidth <= 2 * MechTacRange(mech) ? displayWidth : 2 * MechTacRange(mech));

    displayHeight = (displayHeight <= mech_map->map_height) ? displayHeight : mech_map->map_height;
    displayWidth = (displayWidth <= mech_map->map_width) ? displayWidth : mech_map->map_width;

    set_colorscheme(player);

    /* Get the data to draw the map */
    maptext = MakeMapText(player, mech, mech_map, x, y, displayWidth, displayHeight, flags, dohexlos);
...
static void mech_enter_event(EVENT * e)
{
    ...
    if (!Started(mech) || Uncon(mech) || Jumping(mech) || (MechType(mech) == CLASS_MECH && (Fallen(mech) || Standing(mech))) || OODing(mech) || (fabs(MechSpeed(mech)) * 5 >= MMaxSpeed(mech) && fabs(MMaxSpeed(mech)) >= MP1) || (MechType(mech) == CLASS_VTOL && AeroFuel(mech) <= 0))
        return;
    ...
    if (!can_pass_lock(mech->mynum, newmap->mynum, A_LENTER) && (BuildIsSafe(newmap) || newmap->cf >= (newmap->cfmax / 2))) {
    ...
    mech_notify(mech, MECHALL, tprintf("You enter %s.", structure_name(mapo)));
    MechLOSBroadcast(mech, tprintf("has entered %s at %d,%d.", structure_name(mapo), MechX(mech), MechY(mech)));
...
void mech_enterbase(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(MechType(mech) == CLASS_MECH && (Fallen(mech) || Standing(mech)), "Crawl inside? I think not. Stand first.");
    DOCHECK(OODing(mech), "While in mid-flight? No way.");
    DOCHECK(MechType(mech) == CLASS_VTOL && AeroFuel(mech) <= 0, "You lack fuel to maneuver in!");
    DOCHECK(FlyingT(mech) && !Landed(mech), "You need to land before you can enter the hangar.");
    DOCHECK(IsDS(mech), "Heh, you're trying to be funny, right, a DropShip entering hangar?");
    DOCHECK(fabs(MechSpeed(mech)) * 5 >= MMaxSpeed(mech) && fabs(MMaxSpeed(mech)) >= MP1, "You are moving too fast to enter the hangar!");
    DOCHECK(!(mapo = find_entrance_by_xy(map, MechX(mech), MechY(mech))), "You see nothing to enter here!");

    /* Wow, *gasp*, we got something to enter */
    if (!(newmap = static_cast < MAP * >(FindObjectsData(mapo->obj)))) {
        mech_notify(mech, MECHALL, "You sense wrongness in fabric of space..");
        SendError(tprintf("Error: No map existing for mapindex #%d (@ %d,%d of #%d)", (int) mapo->obj, mapo->x, mapo->y, mech->mapindex));
        return;
    }
    
    if (!find_entrance(newmap, target, &x, &y)) {
        mech_notify(mech, MECHALL, "You sense wrongness in fabric of space..");
        SendError(tprintf("Error: No entrance existing for mapindex #%d (@ %d,%d of #%d)", (int) mapo->obj, mapo->x, mapo->y, mech->mapindex));
        return;
    }
    ...
    HexLOSBroadcast(map, MechX(mech), MechY(mech), "The doors at $h start to open..");
