...
{
    return const_cast <char *>((MechSpecials(mech) & CLAN_TECH) ? "Point" : "Squad");
}
...
void StopSwarming(MECH * mech, int intentional)
{
    ...
    if (intentional > 0) {
        mech_notify(mech, MECHALL, "You let your hold loosen and you drop from the 'mech!");
        mech_notify(target, MECHALL, tprintf("%s lets go of you!", GetMechToMechID(target, mech)));
        MechLOSBroadcasti(mech, target, "lets go of %s!");

        StartBSuitRecycle(mech, RECYCLE_INT_STOPSWARM);
    } else {
        if (MadePilotSkillRoll(mech, 4)) {
            mech_notify(mech, MECHALL, "The hold loosens and you drop from the 'mech!");
            MechLOSBroadcasti(mech, target, "jumps off of %s!");
            mech_notify(target, MECHALL, tprintf("%s jumps off!", GetMechToMechID(target, mech)));

            StartBSuitRecycle(mech, RECYCLE_UNINT_STOPSWARM);
        } else {
            mech_notify(mech, MECHALL, "You're suprised by the sudden action and find yourself rapidly approaching the ground!");
            MechLOSBroadcasti(mech, target, "falls off %s!");
            mech_notify(target, MECHALL, tprintf("%s falls off!", GetMechToMechID(target, mech)));

            DamageMech(mech, mech, 1, -1, Number(0, NUM_BSUIT_MEMBERS - 1), 0, 0, 11, 0, -1, 0, -1, 0, 1);
...
int IsMechSwarmed(MECH * mech)
{
    ...
    for (i = 0; i < map->first_free; i++)
        if ((j = map->mechsOnMap[i]) > 0 && i != mech->mapnumber) {
            if (!(t = static_cast <MECH *>(FindObjectsData(j))))
                continue;
...
int doBSuitCommonChecks(MECH * mech, dbref player)
{
    ...
    DOCHECK1(MechSwarmTarget(mech) > 0, "You are already busy with a special attack!");
    DOCHECK1(MoveModeLock(mech), "Unavailable when performing movement modes - deal.");
    for (i = 0; i < NUM_BSUIT_MEMBERS; i++) 
...
int FindBSuitTarget(dbref player, MECH * mech, MECH ** target, char *buffer)
{
    ...
    DOCHECK1((argc = mech_parseattributes(buffer, args, 3)) > 1, "Invalid arguments!");
    switch (argc) {
        case 0:
    ...
    DOCHECK1(!InLineOfSight_NB(mech, t, MechX(t), MechY(t), range), "Target is not in line of sight!");
...
int doJettisonChecks(MECH * mech)
{
    ...
    for (i = 0; i < NUM_BSUIT_MEMBERS; i++) 
        for (j = 0; j < NUM_CRITICALS; j++) 
            if ((GetPartFireMode(mech, i, j) & WILL_JETTISON_MODE) && (!(GetPartFireMode(mech, i, j) & IS_JETTISONED_MODE))) {
                mech_notify(mech, MECHALL, tprintf("Suit %d can not perform this feat before it jettisons its backpack!", i + 1));
                return 1;
            }
...
void bsuit_swarm(dbref player, void *data, char *buffer)
{
    ...
    if (MechTeam(target) == MechTeam(mech)) {
        ...
        if (!(MechInfantrySpecials(mech) & INF_SWARM_TECH)) {
            mech_notify(mech, MECHALL, "These battlesuits are not capable of mounting mechs!");
            return;
        }
        ...
        if (!(MechInfantrySpecials(mech) & INF_SWARM_TECH)) {
            mech_notify(mech, MECHALL, "These battlesuits are not capable of performing swarm attacks!");
            return;
        }
    }
    ...
    if (CountSwarmers(mech) > 0) {
        mech_notify(mech, MECHALL, "That target already have battlesuits crawling all over it! There's no room for you!");
        return;
    }
    ...
    if (MadePilotSkillRoll(mech, baseToHit)) {
        mech_notify(target, MECHALL, tprintf("%s %s you!", GetMechToMechID(target, mech), (tIsMount ? "mounts" : "swarms")));
        mech_notify(mech, MECHALL, tprintf("You %s %s!", (tIsMount ? "mount" : "swarm"), GetMechToMechID(mech, target)));
        ...
        mech_notify(target, MECHALL, tprintf("%s attempts to %s you!", GetMechToMechID(target, mech), (tIsMount ? "mount" : "swarm")));
        mech_notify(mech, MECHALL, tprintf("Nice try, but you don't succeed in your attempt at %s %s!", (tIsMount ? "mounting" : "swarming"), GetMechToMechID(mech, target)));
    }
...
void bsuit_attackleg(dbref player, void *data, char *buffer)
{
    ...
    if (!(MechInfantrySpecials(mech) & INF_ANTILEG_TECH)) {
        mech_notify(mech, MECHALL, "These battlesuits are not capable of performing leg attacks!");
        return;
    }
    ...
    DOCHECK((MechTeam(mech) == MechTeam(target)), "You can't attack the leg of a friendly mech!");
    ...
    mech_notify(mech, MECHALL, tprintf("You go for %s's %s, placing explosives in the joints!", GetMechToMechID(mech, target), strAttackLoc));

    if (MadePilotSkillRoll(mech, baseToHit)) {
        mech_notify(target, MECHALL, tprintf ("%s swarms your %s putting small packets of explosives all over it!", GetMechToMechID(target, mech), strAttackLoc));
        ...
        if (wCritRoll >= 8) {
            mech_notify(target, MECHALL, tprintf ("The explosives manage to rip into the internals of your %s!", strAttackLoc));
