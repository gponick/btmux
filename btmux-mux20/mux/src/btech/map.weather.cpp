...
int calcWeatherPilotEffects(MECH * mech)
{
    ...
    onTheGround = (!Jumping(mech) && (MechZ(mech) <= Elevation(map, MechX(mech), MechY(mech))));

    if (onTheGround) {
        if ((MapEffectHvyRain(map) || MapEffectHvySnow(map) || MapEffectHighWinds(map) || MapEffectBlizzard(map)))
            mod += 1;
...
void meltSnowAndIce(MAP * map, int x, int y, int depth, int emit, int makeSteam)
{
    ...
    if ((layers & HEXLAYER_SNOW) || (layers & HEXLAYER_DEEP_SNOW)) {
        ...
        if (emit) {
            ...
            if (!snowDone && !((layers & HEXLAYER_SNOW) || (layers & HEXLAYER_DEEP_SNOW))) {
                HexLOSBroadcast(map, x, y, "%ch%cgThe snow in $h melts to nothingness!%cn");
                snowDone = 1;
            }

            if (!snowDone && ((oldLayers & HEXLAYER_DEEP_SNOW) && (!(layers & HEXLAYER_DEEP_SNOW)))) {
                HexLOSBroadcast(map, x, y, "%ch%cgThe snow in $h visibly melts!%cn");
                snowDone = 1;
            }
        }   
    }
    ...
    if (IsIceHex(map, x, y)) {
        if (depth >= (Elevation(map, x, y) * 200)) {
            ...
            breakIceAndSplashUnits(map, NULL, x, y, "goes swimming as ice breaks!");
