...
void eliminate_empties(MAP * map)
{
    ...
    fprintf(stderr, "Map #%d contains empty entries ; removing %d (%d->%d)\n", map->mynum, oldcount - count, oldcount, count);
...
void remove_mech_from_map(MAP * map, MECH * mech)
{
    ...
    mech->mapindex = -1;
    if (map->first_free <= mech->mapnumber || map->mechsOnMap[mech->mapnumber] != mech->mynum) {
        SendError(tprintf("Map indexing error for mech #%d: Map index %d contains data for #%d instead.", mech->mynum, mech->mapnumber, map->mechsOnMap ? map->mechsOnMap[mech->mapnumber] : -1));
        if (map->mechsOnMap)
            for (loop = 0; (loop < map->first_free) && (map->mechsOnMap[loop] != mech->mynum); loop++);
    } else
        loop = mech->mapnumber;
    ...
    if (loop != (map->first_free)) {
        ...
        if (loop == (map->first_free - 1))
            map->first_free--;    /* Who cares about some lost memory? In realloc we'll gain it back anyway */
    }
...
void add_mech_to_map(MAP * newmap, MECH * mech)
{
    ...
    if (MechAuto(mech) > 0) {
        AUTO *a = static_cast <AUTO *>(FindObjectsData(MechAuto(mech)));
    ...
    if (Towed(mech)) {
        ...
        for (i = 0; i < newmap->first_free; i++)
            /* Release from towing if tow-guy ain't on same map already */
            if ((t = static_cast <MECH *>(FindObjectsData(newmap->mechsOnMap[i]))))
                if (MechCarrying(t) == mech->mynum)
                    break;
    ...
    if (IsDS(mech))
        SendDSInfo(tprintf("DS #%d has entered map #%d", mech->mynum, newmap->mynum));
}

int mech_size(MAP * map)
{
    return map->first_free * (sizeof(dbref) + sizeof(char) + sizeof(unsigned short *) + map->first_free * sizeof(unsigned short));
}
