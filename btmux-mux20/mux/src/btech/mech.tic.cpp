...
void cleartic_sub(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK((argc = mech_parseattributes(buffer, args, 3)) != 1, "Invalid number of arguments to function");
...
int addtic_sub_func(MECH * mech, dbref player, int low, int high)
{
    ...
    if (low != high)
        notify(player, tprintf("Weapons #%d - #%d added to TIC %d!", low, high, present_tic));
    else
        notify(player, tprintf("Weapon #%d added to TIC %d!", low, present_tic));
    return 0;
}


void addtic_sub(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    DOCHECK((argc = mech_parseattributes(buffer, args, 3)) != 2, "Invalid number of arguments to function!");
    ticnum = atoi(args[0]);
...
int deltic_sub_func(MECH * mech, dbref player, int low, int high)
{
    ...
    if (low != high)
        notify(player, tprintf("Weapons #%d - #%d removed from TIC %d!", low, high, present_tic));
    else
        notify(player, tprintf("Weapon #%d removed from TIC %d!", low, present_tic));
    
    return 0;
}


void deltic_sub(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
...
int firetic_sub_func(MECH * mech, dbref player, int low, int high)
{
    ...
    MAP *mech_map = static_cast <MAP *>(FindObjectsData(mech->mapindex));
    int f = Fallen(mech);
    ...
    for (i = low; i <= high; i++) {
            ...
            if (mech->tic[i][k])
                for (j = 0; j < SINGLE_TICLONG_SIZE; j++)
                    if (mech->tic[i][k] & (1 << j)) {
                        ...
                        FireWeaponNumber(player, mech, mech_map, weapnum, temp_argc, temp_args, 0);
                        
                        if (f != (Fallen(mech))) {
                            if (Started(mech))
                                mech_notify(mech, MECHALL, "That fall causes you to stop your fire!");
                            return 1;
        ...
        if (!count)
            notify(player, tprintf("*Click* (the tic contained no weapons)"));
    }
...
void firetic_sub(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast <MECH *>(data);
    ...
    DOCHECK((argc = mech_parseattributes(buffer, args, 5)) < 1, "Not enough arguments to function");
    ...
    for (j = 0; j < MAX_WEAPONS_PER_MECH; j++) {
        ...
        if (mech->tic[present_tic][k] & (1 << l)) {
            if (count == rtar) {
                ...
                sprintf(buf, "#%2d %3s %-16s %s", j, ShortArmorSectionString(MechType(mech), MechMove(mech), section), &MechWeapons[Weapon2I(GetPartType(mech, section, critical))].name[3], PartIsNonfunctional(mech, section, critical) ? "(*)" : "");
                return buf;
            }
            count++;
        }
...
void listtic_sub(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    DOCHECK((argc = mech_parseattributes(buffer, args, 2)) != 1, "Invalid number of arguments!");
    ticnum = atoi(args[0]);
    DOCHECK(!(ticnum >= 0 && ticnum < NUM_TICS), "TIC out of range!");
    present_mech = mech;
    present_tic = ticnum;
    ...
    present_count = count;
    c = SelCol_FunStringMenuK(2, tprintf("TIC #%d listing for %s", ticnum, GetMechID(mech)), listtic_fun, MAX(1, count));
...
void heat_cutoff(dbref player, void *data, char *buffer)
{
    ...
    if (HeatcutoffChanging(mech)) {
        notify(player, "You are already toggling heat cutoff status. Please be patient.");
        return;
    }
