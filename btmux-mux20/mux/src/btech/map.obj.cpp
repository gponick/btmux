...
extern void muxevent_remove_type_data2(int, void *);

static char *map_types[] =
    { "FIRE", "SMOKE", "DECO", "MINE", "BUILDING", "LEAVE", "ENTRA", "LINKED", "TBITS", "BLZ", NULL };

mapobjt *free_mapobjs = NULL;

#define MAPOBJSTART_MAGICNUM 27
#define MAPOBJEND_MAGICNUM 39

mapobjt *next_mapobj(mapobjt * m)
{
    return m->next;
}

mapobjt *first_mapobj(MAP * map, int type)
{
...
char *structure_name(mapobjt * mapo)
{
...
mapobjt *find_entrance_by_target(MAP * map, dbref target)
{
    mapobjt *tmp;
...
mapobjt *find_entrance_by_xy(MAP * map, int x, int y)
{
    mapobjt *tmp;
...
mapobjt *find_mapobj(MAP * map, int x, int y, int type)
{
    mapobjt *tmp;
    int i;
...
void del_mapobj(MAP * map, mapobjt * mapob, int type, int zap)
{
...
mapobjt *add_mapobj(MAP * map, mapobjt ** to, mapobjt * from, int flag)
{
    mapobjt *realto;
    ...
    if (!free_mapobjs) {
        Create(realto, mapobjt, 1);
    } else {
        realto = free_mapobjs;
        free_mapobjs = realto->next;
    }
    
    bcopy(from, realto, sizeof(mapobjt));
    *to = realto;
...
static void FindMyCoord(MAP * map, int tx, int ty, int i, int wdir, int *x, int *y)
{
    int dx, dy;
...
static void fire_spreading_event(EVENT * e)
{
    ...
    for (loop = 0; loop < 3; loop++) {
        ...
        FindMyCoord(map, x, y, loop, map->winddir, &new_smoke_hex_x[loop], &new_smoke_hex_y[loop]);
    }

    new_fire_hex_x[3] = -1;
    new_fire_hex_y[3] = -1;
    FindMyCoord(map, new_smoke_hex_x[0], new_smoke_hex_y[0], 0, map->winddir, &new_smoke_hex_x[3], &new_smoke_hex_y[3]);
#define Spr(n,ch) \
...
void add_decoration(MAP * map, int x, int y, int type, char data, int flaggo)
{
    ...
    if (flaggo) {
        ...
        if (type == TYPE_FIRE) {
            ...
            MAPEVENT(map, EVENT_DECORATION, fire_spreading_event, FIRESPEED(map), tmpo);
        }
    }
}
    ...
    for (i = 0; i < NUM_MAPOBJTYPES; i++)
        for (tmp = first_mapobj(map, i); tmp; tmp = next_mapobj(tmp)) {
            if (i == TYPE_BITS)
                ...
                notify(player, tprintf("%-3d %-3d %-5s %-5d %-4d %-6d %d", tmp->x, tmp->y, map_types[i], (int) tmp->obj, tmp->datac, tmp->datas, tmp->datai));
        }
...
void map_addfire(dbref player, void *data, char *buffer)
{
    ...
    notify(player, tprintf("Added: Fire at (%d,%d) with duration of %ds.", x, y, d));
}
...
void map_addsmoke(dbref player, void *data, char *buffer)
{
    ...
    notify(player, tprintf("Added: Smoke at (%d,%d) with duration of %ds.", x, y, d));
}
...
#define READINT(from,to) \
    ...
    notify(player, tprintf("Landingzone-block added to %d,%d (distance: %d)", x, y, str));
}
...
void map_delobj(dbref player, void *data, char *buffer)
{
    ...
    switch (mech_parseattributes(buffer, args, 3)) {
        ...
        case 2:
            ...
            notify(player, tprintf("%d objects at (%d,%d) deleted.", count, x, y));
            break;
        case 3:
            DOCHECK((tt = listmatch(map_types, args[0])) < 0, "Invalid type!");
            x = atoi(args[1]);
            ...
            notify(player, tprintf("%d %s at (%d,%d) deleted.", count, map_types[tt], x, y));
            break;
        default:
...
void add_links(dbref loc, MAP * map, char *data)
{
    ...
    if ((found = mech_parseattributes(buf, args, 500)) > 0)
        for (i = 0; i < found; i++) {
            ...
            if ((pattr = atr_str("BUILDCOORD")) && pattr->number)
                tmps = atr_get(targ, pattr->number, &aowner, &aflags);
            else
                tmps = "";
...
void recursively_updatelinks(dbref from, dbref loc)
{
    ...
    if (from >= 0) {
        ...
        if ((pattr = atr_str("BUILDENTRANCE")) && pattr->number)
            tmps = atr_get(loc, pattr->number, &aowner, &aflags);
        else
            tmps = "";

        if (tmps) 
            /* number number number number or x,y x,y x,y x,y */
    ...
    if ((pattr = atr_str("BUILDLINKS")) && pattr->number)
        tmps = atr_get(loc, pattr->number, &aowner, &aflags);
    else
        tmps = "";
...
void map_updatelinks(dbref player, void *data, char *buffer)
{
    ...
    notify(player, tprintf("Updated %d BUILD objs, %d LEAVE objs, %d ENTRANCE objs.", update_stats[0], update_stats[1], update_stats[2]));
}
...
static void building_regen_event(EVENT * e)
{
    ...
    if (cf != max)
        OBJEVENT(d, EVENT_BREGEN, building_regen_event, BUILDING_REPAIR_DELAY, NULL);
}
...
void possibly_start_building_regen(dbref obj)
{
    ...
    if (!f)
        OBJEVENT(obj, EVENT_BREBUILD, building_rebuild_event, BUILDING_DREBUILD_DELAY, NULL);
    else
        OBJEVENT(obj, EVENT_BREGEN, building_regen_event, BUILDING_REPAIR_DELAY, NULL);
}

static void damage_cf(MECH * mech, mapobjt * o, int from, int to, int damage)
{
    int destroy = 0;
    int start_regen = 0;
    ...
    scen_see_base(static_cast <MAP *>(FindObjectsData(mech->mapindex)), mech, o);
    
    if (destroy) {
        mech_notify(mech, MECHALL, tprintf("You hit %s for %d points of damage, destroying it!", structure_name(o), damage));
        notify_except(o->obj, NOTHING, o->obj, tprintf("%s is hit for %d more points of damage, destroying it!", MyToUpper(structure_name(o)), damage));
        MechLOSBroadcast(mech, tprintf("hits %s, destroying it!", structure_name(o), damage));
        scen_destroy_base(static_cast < MAP * >(FindObjectsData(mech->mapindex)), mech, o);
        start_regen = 2;
    } else {
        mech_notify(mech, MECHALL, tprintf("You hit %s for %d points of damage.", structure_name(o), damage));
        notify_except(o->obj, NOTHING, o->obj, tprintf("%s is hit for %d points of damage.", MyToUpper(structure_name(o)), damage));
        scen_damage_base(static_cast <MAP *>(FindObjectsData(mech->mapindex)), mech, o);
    }
...
void fire_hex(MECH * mech, int x, int y, int meant)
{
    ...
    if (meant) {
        ...
        mech_notify(mech, MECHALL, tprintf("You accidentally ignite %d,%d!", x, y));
    }
...
void steppable_base_check(MECH * mech, int x, int y)
{
    ...
    mech_notify(mech, MECHALL, tprintf("%s has CF of %d.", MyToUpper(structure_name(o)), nmap->cf));
...
void show_building_in_hex(MECH * mech, int x, int y)
{
    ...
    if (BuildIsInvis(nmap) || (BuildIsHidden(nmap) && !MadePerceptionRoll(mech, (int) (FindRange(MechX(mech), MechY(mech), MechZ(mech), x, y, 0) + 0.95)))) {
        mech_notify(mech, MECHALL, "The sensors detect no building in the hex!");
        return;
    }
    
    mech_notify(mech, MECHALL, tprintf("%s's CF is %d.", MyToUpper(structure_name(o)), nmap->cf));
    scen_see_base(map, mech, o);
}
...
int mech_underlying_terrain(MECH * mech)
{
    ...
    if (c = find_decorations(map, MechX(mech), MechY(mech)))
        return c;
