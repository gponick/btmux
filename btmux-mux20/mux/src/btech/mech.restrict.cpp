    ...
    for (i = 0; i < map->first_free; i++) {
        ...
        if (map->mechsOnMap[i] >= 0 && i != mech->mapnumber) {
            ...
            if ((MechStatus(mek) & LOCK_TARGET) && MechTarget(mek) == mech->mynum) {
                mech_notify(mek, MECHALL, "Weapon system reports the lock has been lost.");
                LoseLock(mek);
            }
        
            if ((map->LOSinfo[i][mech->mapnumber] & MECHLOSFLAG_SEEN) && MechTeam(mek) != MechTeam(mech))
...
void mech_Rsetxy(dbref player, void *data, char *buffer)
{
    ...
    x = atoi(args[0]);
    y = atoi(args[1]);
    DOCHECK(x >= mech_map->map_width || y >= mech_map->map_height || x < 0 || y < 0, "Invalid coordinates!");
    ...
    SendLoc(tprintf("#%d set #%d's pos to %d,%d,%d.", player, mech->mynum, x, y, z));
}
...
void mech_Rsetmapindex(dbref player, void *data, char *buffer)
{
    ...
    if (newindex == -1) {
        ...
        SendLoc(tprintf("#%d removed #%d from map #%d.", player, mech->mynum, oldmap->mynum));
        return;
    }
    ...
    if (nargs > 1 && strlen(args[1]) > 1) {
        ...
        if ((pattr = atr_str("MECHPREFID")) && pattr->number)
            tempstr = atr_get(mech->mynum, pattr->number, &aowner, &aflags);
        else
            tempstr = "";
    ...
    targ[0] = BOUNDED('A', mux_toupper(targ[0]), 'Z');
    targ[1] = BOUNDED('A', mux_toupper(targ[1]), 'Z');
    ...
    if (MechX(mech) > (newmap->map_width - 1) || MechY(mech) > (newmap->map_height - 1)) {
        MechX(mech) = 0;
        MechLastX(mech) = 0;
        MechY(mech) = 0;
        MechLastY(mech) = 0;
        ...
        notify(player, "You're current position is out of bounds, Pos changed to 0,0");
    }

    notify(player, tprintf("MapIndex changed to %d", newindex));
    notify(player, tprintf("Your ID: %c%c", MechID(mech)[0], MechID(mech)[1]));
    SendLoc(tprintf("#%d set #%d's mapindex to #%d.", player, mech->mynum, newindex));
    UnZombifyMech(mech);
}
...
{
    MECH *newmech = static_cast < MECH * >(*data);
    MAP *map;
    int i;
    ...
    switch (selector) {
        case SPECIAL_ALLOC:
            ...
            MechID(newmech)[0] = ' ';
            MechID(newmech)[1] = ' ';
            clear_mech(newmech, 1);

            for (i = 0; i < NUM_SECTIONS; i++)
                FillDefaultCriticals(newmech, i);
    
            break;
        case SPECIAL_FREE:
            if (newmech->mapindex != -1 && (map = getMap(newmech->mapindex)))
                remove_mech_from_map(map, newmech);
    }
}
