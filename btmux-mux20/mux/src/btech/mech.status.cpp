    ...
    if (MechTarget(mech) != -1) {
        ...
        if (tempMech) 
            if (InLineOfSight(mech, tempMech, MechX(tempMech), MechY(tempMech), FaMechRange(mech, tempMech))) {
                sprintf(buff, "\nTarget: %s\t   Range: %.1f hexes   Bearing: %d deg", GetMechToMechID(mech, tempMech), FaMechRange(mech, tempMech), FindBearing(MechFX(mech), MechFY(mech), MechFX(tempMech), MechFY(tempMech)));
                notify(player, buff);
                arc = InWeaponArc(mech, MechFX(tempMech), MechFY(tempMech));
                strcpy(buff, tprintf("Target in %s Weapons Arc", (arc & TURRETARC) ? "Turret" : GetArcID(mech, arc)));

                if (MechAim(mech) == NUM_SECTIONS || MechAimType(mech) != MechType(tempMech))
                    strcpy(location, "None");
                else
                    ArmorStringFromIndex(MechAim(mech), location, MechType(tempMech), MechMove(tempMech));
        ...
        if (MechTargX(mech) != -1 && MechTargY(mech) != -1) 
            if (MechStatus(mech) & LOCK_BUILDING)
                notify(player, tprintf("\nTarget: Building at %d %d", MechTargX(mech), MechTargY(mech)));
            else 
                if (MechStatus(mech) & LOCK_HEX)
                    notify(player, tprintf("\nTarget: Hex %d %d", MechTargX(mech), MechTargY(mech)));
                else
                    notify(player, tprintf("\nTarget: %d %d", MechTargX(mech), MechTargY(mech)));
    
    if (MechPKiller(mech))
        notify(player, "\nWeapon Safeties are %ch%crOFF%cn.");
    
    if (MechChargeTarget(mech) > 0 && btechconf.btech_newcharge) {
        tempMech = getMech(MechChargeTarget(mech));
        if (!tempMech)
            return;
    
        if (InLineOfSight(mech, tempMech, MechX(tempMech), MechY(tempMech), FaMechRange(mech, tempMech))) 
            notify(player, tprintf("\nChargeTarget: %s\t  ChargeTimer: %d", GetMechToMechID(mech, tempMech), MechChargeTimer(mech) / 2));
        else 
            notify(player, tprintf("\nChargeTarget: NOT in line of sight!\t Timer: %d", MechChargeTimer(mech) / 2));
    }
}
...
{
    MECH *tempMech = NULL;
    MAP *map = static_cast <MAP *>(FindObjectsData(mech->mapindex));
    char buff[100];
    char mech_name[100];
    char mech_ref[100];
    char move_type[50];
    ...
    strcpy(mech_name, usex ? MechType_Name(mech) : buf);

    if ((pattr = atr_str("MECHREF")) && pattr->number)
        buf = atr_get(mech->mynum, pattr->number, &aowner, &aflags);
    else
        buf = "";

    strcpy(mech_ref, usex ? MechType_Ref(mech) : buf);

    switch (MechType(mech)) {
        case CLASS_MW:
            notify(player, tprintf("MechWarrior: %-18.18s ID:[%s]", Name(player), MechIDS(mech, 0)));
            notify(player, tprintf("MaxSpeed: %3d", (int) MMaxSpeed(mech)));
            break;
        case CLASS_BSUIT:
            sprintf(buff, "%s Name: %-18.18s  ID:[%s]   %s Reference: %s", GetBSuitName(mech), mech_name, MechIDS(mech, 0), GetBSuitName(mech), mech_ref);
            notify(player, buff);
            notify(player, tprintf("MaxSpeed: %3d                  JumpRange: %d", (int) MMaxSpeed(mech), JumpSpeedMP(mech, map)));
            show_miscbrands(mech, player);
    
            if (MechPilot(mech) == -1)
                notify(player, "Leader: NONE");
            else {
                sprintf(buff, "%s Leader Name: %-16.16s %s Leader injury: %d", GetBSuitName(mech), Name(MechPilot(mech)), GetBSuitName(mech), MechPilotStatus(mech));
                notify(player, buff);
            }
            ...
            if (Jumping(mech)) {
                ...
                if ((MechStatus(mech) & DFA_ATTACK) && MechDFATarget(mech) != -1) {
                    ...
                    sprintf(buff + strlen(buff), "  Death From Above Target: %s", GetMechToMechID(mech, tempMech));
                }
            ...
            break;
        case CLASS_MECH:
            sprintf(buff, "Mech Name: %-18.18s  ID:[%s]   Mech Reference: %s", mech_name, MechIDS(mech, 0), mech_ref);
            notify(player, buff);
            notify(player, tprintf("Tonnage:   %3d     MaxSpeed: %3d       JumpRange: %d", MechTons(mech), (int) MMaxSpeed(mech), JumpSpeedMP(mech, map)));
            show_miscbrands(mech, player);
            
            if (MechPilot(mech) == -1)
                notify(player, "Pilot: NONE");
            else {
                sprintf(buff, "Pilot Name: %-28.28s Pilot Injury: %d", Name(MechPilot(mech)), MechPilotStatus(mech));
                notify(player, buff);
            }
            
            Mech_ShowFlags(player, mech, 0, 0);
            
            if (!Jumping(mech) && !Fallen(mech) && Started(mech) && (MechChargeTarget(mech) != -1)) {
                tempMech = getMech(MechChargeTarget(mech));
                if (tempMech) {
                    sprintf(buff, "CHARGING --> %s", GetMechToMechID(mech, tempMech));
                    notify(player, buff);
                }
            }
        ...
        case CLASS_SPHEROID_DS:
            ...
            if (MechMove(mech) != MOVE_NONE) {
                sprintf(buff, "Vehicle Name: %-15.15s  ID:[%s]   Vehicle Reference: %s", mech_name, MechIDS(mech, 0), mech_ref);
                notify(player, buff);
                sprintf(buff, "Tonnage:   %3d      %s: %3d       Movement Type: %s", MechTons(mech), is_aero(mech) ? "Max thrust" : "FlankSpeed", (int) MMaxSpeed(mech), move_type);
                notify(player, buff);
                show_miscbrands(mech, player);
                ...
                sprintf(buff, "Name: %-15.15s  ID:[%s]   Reference: %s", mech_name, MechIDS(mech, 0), mech_ref);
                notify(player, buff);
            }
...
void PrintShortInfo(dbref player, MECH * mech)
{
    ...
    switch (MechType(mech)) {
        case CLASS_VTOL:
            ...
            break;
        case CLASS_MECH:
            sprintf(typespecific, " HT: %3d/%3d ", (int) (10. * MechPlusHeat(mech)), (int) (10. * MechActiveNumsinks(mech)));
            break;
        case CLASS_AERO:
        case CLASS_DS:
        case CLASS_SPHEROID_DS:
            sprintf(typespecific, " VSPD: %3.1f  ANG: %2d  HT: %3d/%3d ", MechVerticalSpeed(mech), MechDesiredAngle(mech), (int) (10 * MechPlusHeat(mech)), (int) (10 * MechActiveNumsinks(mech)));
            break;
        case CLASS_VEH_NAVAL:
        ...
        case CLASS_VEH_GROUND:
            ...
            if (GetSectOInt(mech, TURRET)) {
                sprintf(typespecific, " TUR: %3d ", AcceptableDegree(MechTurretFacing(mech) + MechFacing(mech)));
                break;
            }
            /* FALLTHROUGH */
        default:
    ...
    snprintf(buff, 100, "LOC: %3d,%3d,%3d  HD: %3d/%3d  SP: %3.1f/%3.1f %s ST:%s", MechX(mech), MechY(mech), MechZ(mech), MechFacing(mech), MechDesiredFacing(mech), MechSpeed(mech), MechDesiredSpeed(mech), typespecific, getStatusString(mech, 2));
    buff[99] = '\0';
    notify(player, buff);
...
static char *MakeHeatScaleInfo(MECH * mech, char *fillchar)
{
    ...
    int counter = 0, heat = (int)(MechPlusHeat(mech)), minheat = (int)(MechMinusHeat(mech)), start = 0;
    char state = 1;
...
void PrintInfoStatus(dbref player, MECH * mech, int own)
{
    ...
    switch (MechType(mech)) {
        case CLASS_MECH:
            sprintf(buff, "X, Y, Z:%3d,%3d,%3d  Excess Heat:  %3d deg C.  Heat Production:  %3d deg C.", MechX(mech), MechY(mech), MechZ(mech), (int) (10. * MechHeat(mech)), (int) (10. * MechPlusHeat(mech)));
            notify(player, buff);
            sprintf(buff, "Speed:      %%ch%%cg%3d%%cn KPH  Heading:      %%ch%%cg%3d%%cn deg     Heat Sinks:       %3d", (int) (MechSpeed(mech)), MechFacing(mech), MechActiveNumsinks(mech));
            notify(player, buff);
            sprintf(buff, "Des. Speed: %3d KPH  Des. Heading: %3d deg     Heat Dissipation: %3d deg C.", (int) MechDesiredSpeed(mech), MechDesiredFacing(mech), (int) (10. * MechMinusHeat(mech)));
            notify(player, buff);
            ...
            if ((pattr = atr_str("HEATCHARS")) && pattr->number)
                tmpstr = atr_get(player, pattr->number, &aowner, &aflags);
            else
                tmpstr = "";
            ...
            if (MechLateral(mech))
                notify(player, tprintf("You are moving laterally %s", LateralDesc(mech)));
        ...
        case CLASS_DS:
        case CLASS_SPHEROID_DS:
            sprintf(buff, "X, Y, Z:%3d,%3d,%3d  Heat Sinks:          %3d       %s", MechX(mech), MechY(mech), MechZ(mech), MechActiveNumsinks(mech), is_aero(mech) ? tprintf("%s angle: %%ch%%cg%d%%cn", MechDesiredAngle(mech) >= 0 ? "Climbing" : "Diving", abs(MechDesiredAngle(mech))) : "");
            notify(player, buff);
            if (FlyingT(mech) || MechMove(mech) == MOVE_SUB) {
                sprintf(buff, "Speed:      %%ch%%cg%3d%%cn KPH  Vertical Speed:      %%ch%%cg%3d%%cn KPH   Des. Speed %3d KPH", (int) (MechSpeed(mech)), (int) (MechVerticalSpeed(mech)), (int) (MechDesiredSpeed(mech)));
                notify(player, buff);
                f = MAX(0, AeroFuel(mech));
        
                if (MechMove(mech) == MOVE_SUB) 
                    sprintf(buff, "Heading: %3d KPH  Des. Heading: %3d deg", (int) MechFacing(mech), MechDesiredFacing(mech));
                else 
                    if (AeroFreeFuel(mech)) 
                        sprintf(buff, "Heading:    %%ch%%cg%3d%%cn deg  Des. Heading:        %3d deg   Fuel: Unlimited", MechFacing(mech), MechDesiredFacing(mech));
                    else 
                        sprintf(buff, "Heading:    %%ch%%cg%3d%%cn deg  Des. Heading:        %3d deg   Fuel: %d (%.2f %%)", MechFacing(mech), MechDesiredFacing(mech), f, 100.0 * f / AeroFuelOrig(mech));
        
                notify(player, buff);
            } else 
                if (MechMove(mech) != MOVE_NONE) {
                    sprintf(buff, "Speed:      %%ch%%cg%3d%%cn KPH  Heading:      %%ch%%cg%3d%%cn deg", (int) (MechSpeed(mech)), MechFacing(mech));
                    notify(player, buff);
                    sprintf(buff, "Des. Speed: %3d KPH  Des. Heading: %3d deg", (int) MechDesiredSpeed(mech), MechDesiredFacing(mech));
                    notify(player, buff);
        ...
        case CLASS_MW:
        case CLASS_BSUIT:
            sprintf(buff, "X, Y, Z:%3d,%3d,%3d  Speed:      %%ch%%cg%3d%%cn KPH  Heading:      %%ch%%cg%3d%%cn deg", MechX(mech), MechY(mech), MechZ(mech), (int) (MechSpeed(mech)), MechFacing(mech));
            notify(player, buff);
            sprintf(buff, "                     Des. Speed: %3d KPH  Des. Heading: %3d deg", (int) MechDesiredSpeed(mech), MechDesiredFacing(mech));
            notify(player, buff);
            break;
    ...
    if (MechCarrying(mech) > 0)
        if ((tempMech = getMech(MechCarrying(mech))))
            notify(player, tprintf("Towing %s.", GetMechToMechID(mech, tempMech)));
}
...
void mech_status(dbref player, void *data, char *buffer)
{
    ...
    if (!buffer || !strlen(buffer))
        ...
        for (loop = 0; buffer[loop]; loop++) 
            switch (mux_toupper(buffer[loop])) {
                case 'R':
                    doweap = doinfo = doarmor = usex = 1;
                    break;
                case 'A':
                    if (mux_toupper(buffer[loop + 1]) == 'R')
                ...
                case 'I':
                    doinfo = 1;
                    if (mux_toupper(buffer[loop + 1]) == 'N')
                        while (buffer[loop + 1] && buffer[loop + 1] != ' ')
                            loop++;
                    break;
                case 'W':
                    doweap = 1;
                    if (mux_toupper(buffer[loop + 1]) == 'E')
                        while (buffer[loop + 1] && buffer[loop + 1] != ' ')
                            loop++;
                    break;
                case 'N':
    ...
    if (doweird) {
        sprintf(buf, "%s %s %d %d/%d/%d %d ", MechType_Ref(mech), MechType_Name(mech), MechTons(mech), (int) (MechMaxSpeed(mech) / MP1) * 2 / 3, (int) (MechMaxSpeed(mech) / MP1), (int) (MechJumpSpeed(mech) / MP1), MechActiveNumsinks(mech));
        weirdbuf = buf;
    } else 
        if (!doheat || (doarmor | doinfo | doweap))
            ...
            for (i = 0; i < NUM_SECTIONS; i++)
                if (GetSectOArmor(mech, i)) 
                    if (GetSectORArmor(mech, i))
                        sprintf(buf + strlen(buf), "%d|%d|%d ", GetSectOArmor(mech, i), GetSectOInt(mech, i), GetSectORArmor(mech, i));
                    else
                        sprintf(buf + strlen(buf), "%d|%d ", GetSectOArmor(mech, i), GetSectOInt(mech, i));
...
void mech_critstatus(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(mech_parseattributes(buffer, args, 1) != 1, "You must specify a section to list the criticals for!");
...
char *part_name(int type, int brand)
{
    ...
    if (!strcmp(c, "LifeSupport"))
        ...
        if (!strcmp(c, "TripleStrengthMyomer"))
...
char *pos_part_name(MECH * mech, int index, int loop)
{
    ...
    if (index < 0 || index >= NUM_SECTIONS || loop < 0 || loop >= NUM_CRITICALS) {
        SendError(tprintf("INVALID: For mech #%d, %d/%d was requested.", mech->mynum, index, loop));
        return "--?LocationBug?--";
    }
...
static char *wspec_fun(int i)
{
    ...
    if (!i)
        ...
        if (btechconf.btech_erange)
            sprintf(buf, WSDUMP_MASK_ER, MechWeapons[j].name, MechWeapons[j].heat, MechWeapons[j].damage, MechWeapons[j].min, MechWeapons[j].shortrange, MechWeapons[j].medrange, GunRange(j), EGunRange(j), MechWeapons[j].vrt);
        else
            sprintf(buf, WSDUMP_MASK_NOER, MechWeapons[j].name, MechWeapons[j].heat, MechWeapons[j].damage, MechWeapons[j].min, MechWeapons[j].shortrange, MechWeapons[j].medrange, GunRange(j), MechWeapons[j].vrt);
    }
    return buf;
}
...
/*   unsigned char weaps[8 * MAX_WEAPS_SECTION]; */
    ...
    if (strcmp(MechType_Name(mech), MechType_Ref(mech)))
        c = SelCol_FunStringMenuK(1, tprintf("Weapons statistics for %s: %s", MechType_Name(mech), MechType_Ref(mech)), wspec_fun, wspec_weapcount + 1);
    else
        c = SelCol_FunStringMenuK(1, tprintf("Weapons statistics for %s", MechType_Ref(mech)), wspec_fun, wspec_weapcount + 1);
...
char *critstatus_func(MECH * mech, char *arg)
{
    ...
    for (i = 0; i < max_crits; i++) {
        ...
        sprintf(buffer, "%s|%d", buffer, (PartIsNonfunctional(mech, index, i) && type != EMPTY && (!IsCrap(type) || SectIsDestroyed(mech, index))) ? -1 : PartTempNuke(mech, index, i));
        sprintf(buffer, "%s|%d", buffer, IsWeapon(type) ? 1 : IsAmmo(type) ? 2 : IsActuator(type) ? 3 : IsCargo(type) ? 4 : (IsCrap(type) || type == EMPTY) ? 5 : 0);
    }
...
char *weaponstatus_func(MECH * mech, char *arg)
{
    ...
    if (!arg)
        ...
        if (!*arg)
            ...
            if ((sect = ArmorSectionFromString(MechType(mech), MechMove(mech), arg)) == -1 || !GetSectOInt(mech, sect))
                return "#-1 INVALID SECTION";

    buffer[0] = '\0';
    
    for ((sect == -1) ? (loopsect = 0) : (loopsect = sect); (sect == -1) ? (loopsect < NUM_SECTIONS) : (loopsect < sect + 1); loopsect++) {
        ...
        for (i = 0; i < count; i++, totalcount++) {
            ...
            sprintf(buffer, "%s%d|%s|%d|%d|%d|%d|%d|%d", buffer, totalcount, get_parts_long_name(I2Weapon(type), GetPartBrand(mech, loopsect, criticals[i])), GetWeaponCrits(mech, type), GetPartBrand(mech, loopsect, criticals[i]), MechWeapons[type].vrt, weapdata[i], MechWeapons[type].type, PartIsNonfunctional(mech, loopsect, criticals[i]) ? 2 : PartTempNuke(mech, loopsect, criticals[i]) ? 1 : 0);
        }
    }
...
char *critslot_func(MECH * mech, char *buf_section, char *buf_critnum, char *buf_flag)
{
    ...
    if (!buf_flag)
        ...
        if (strcasecmp(buf_flag, "NAME") == 0)
            flag = 0;
        else 
            if (strcasecmp(buf_flag, "STATUS") == 0)
                flag = 1;
            else 
                if (strcasecmp(buf_flag, "DATA") == 0)
                    flag = 2;
                else 
                    if (strcasecmp(buf_flag, "MAXAMMO") == 0)
                        flag = 3;
                    else 
                        if (strcasecmp(buf_flag, "AMMOTYPE") == 0)
                            flag = 4;
                        else 
                            if (strcasecmp(buf_flag, "MODE") == 0)
                                flag = 5;
                            else
                                flag = 0;
                ...
                if (flag == 4) {
                    ...
                    if (flag == 5) {
                        ...
                        if (!IsWeapon(type))
                            ...
                            snprintf(buffer, MBUF_SIZE, "%c%c", GetWeaponFireModeLetter_Model_Mode(weapindex, GetPartFireMode(mech, index, crit)), GetWeaponAmmoModeLetter_Model_Mode(weapindex, GetPartAmmoMode(mech, index, crit)));
                            return buffer;
                        }
                    }
...
void CriticalStatus(dbref player, MECH * mech, int index)
{
    ...
    for (i = 0; i < max_crits; i++) {
        ...
        if (IsAmmo(type)) {
            ...
            strcat(buffer, GetAmmoDesc_Model_Mode(Ammo2WeaponI(type), GetPartAmmoMode(mech, index, loop)));
            strcat(buffer, " Ammo");
            ...
            if (IsWeapon(type) && (((wFireMode & OS_MODE) && (wFireMode & OS_USED)) || (wFireMode & ROCKET_FIRED)))
                strcat(buffer, " (Empty)");
        }

        if (PartIsBroken(mech, index, loop) && type != EMPTY && (!IsCrap(type) || SectIsDestroyed(mech, index)))
            strcat(buffer, PartIsDestroyed(mech, index, loop) ?  " (Destroyed)" : " (Broken)");
        else 
            if (PartIsDisabled(mech, index, loop) && type != EMPTY)
                strcat(buffer, " (Disabled)");
            else 
                if (PartIsDamaged(mech, index, loop) && type != EMPTY)
...
void PrintWeaponStatus(MECH * mech, dbref player)
{
    ...
    if ((MechSpecials(mech) & ECM_TECH) || (MechSpecials2(mech) & STEALTH_ARMOR_TECH) || (MechSpecials2(mech) & NULLSIGSYS_TECH) || (MechSpecials(mech) & SLITE_TECH) || HasC3(mech) || HasC3i(mech) || (MechSpecials(mech) & MASC_TECH) || (MechSpecials2(mech) & SUPERCHARGER_TECH) || (MechSpecials(mech) & TRIPLE_MYOMER_TECH) || (MechSpecials2(mech) & ANGEL_ECM_TECH) || HasTAG(mech) || (MechInfantrySpecials(mech) & FC_INFILTRATORII_STEALTH_TECH)) {
        strcpy(tempbuff, "AdvTech: ");

        if (MechSpecials(mech) & ECM_TECH) 
            sprintf(tempbuff + strlen(tempbuff), "ECM(%s)  ", (MechStatus2(mech) & ECM_DESTROYED) ? "%ch%crXX%cn" : ECMEnabled(mech) ? (ECMActive(mech) ? "%ch%cgECM%cn" : "%ch%crECM%cn") : ECCMEnabled(mech) ? "%ch%cgECCM%cn" : ECMCountered(mech) ? "%crOff%cn" : "%cgOff%cn");

        if (MechSpecials2(mech) & ANGEL_ECM_TECH) 
            sprintf(tempbuff + strlen(tempbuff), "AngelECM(%s)  ", (!HasWorkingAngelECMSuite(mech)) ? "%ch%crXX%cn" : AngelECMEnabled(mech) ? (AngelECMActive(mech) ?  "%ch%cgECM%cn" : "%ch%crECM%cn") : AngelECCMEnabled(mech) ? "%ch%cgECCM%cn" : ECMCountered(mech) ? "%crOff%cn" : "%cgOff%cn");

        if (MechInfantrySpecials(mech) & FC_INFILTRATORII_STEALTH_TECH) 
            sprintf(tempbuff + strlen(tempbuff), "PersonalECM(%s)  ", PerECMEnabled(mech) ? (PerECMActive(mech) ?  "%ch%cgECM%cn" : "%ch%crECM%cn") : PerECCMEnabled(mech) ? "%ch%cgECCM%cn" : ECMCountered(mech) ? "%crOff%cn" : "%cgOff%cn"); 

        if (MechSpecials2(mech) & STEALTH_ARMOR_TECH) 
            sprintf(tempbuff + strlen(tempbuff), "SthArmor(%s)  ", (MechStatus2(mech) & ECM_DESTROYED) ? "%ch%crXX%cn" : StealthArmorActive(mech) ? "%ch%cgOn%cn" : "%cgRdy%cn");

        if (MechSpecials2(mech) & NULLSIGSYS_TECH) 
            sprintf(tempbuff + strlen(tempbuff), "NullSigSys(%s)  ", NullSigSysDest(mech) ? "%ch%crXX%cn" : NullSigSysActive(mech) ? "%ch%cgOn%cn" : "%cgRdy%cn");

        if (MechSpecials(mech) & SLITE_TECH) 
            sprintf(tempbuff + strlen(tempbuff), "SLITE(%s)  ", (MechCritStatus(mech) & SLITE_DEST) ? "%cr%chXX%cn" : (MechStatus2(mech) & SLITE_ON) ? "%ch%cgOn%cn" : "%cgOff%cn");

        if (HasC3m(mech))
            sprintf(tempbuff + strlen(tempbuff), "%sC3M%%cn  ", C3Destroyed(mech) ? "%cr" : AnyECMDisturbed(mech) ? "%cy" : MechC3NetworkSize(mech) > 0 ? "%cg%ch" : "%cg");

        if (HasC3s(mech))
            sprintf(tempbuff + strlen(tempbuff), "%sC3S%%cn  ", C3Destroyed(mech) ? "%cr" : AnyECMDisturbed(mech) ? "%cy" : MechC3NetworkSize(mech) > 0 ? "%cg%ch" : "%cg");

        if (HasC3i(mech))
            sprintf(tempbuff + strlen(tempbuff), "%sC3i%%cn  ", C3iDestroyed(mech) ? "%cr" : AnyECMDisturbed(mech) ? "%cy" : MechC3iNetworkSize(mech) > 0 ? "%cg%ch" : "%cg");

        if (MechSpecials(mech) & TRIPLE_MYOMER_TECH)
            sprintf(tempbuff + strlen(tempbuff), "TSM(%s)  ", ((MechHeat(mech) >= 9.0) ? "%ch%cgOn%cn" : "%cgOff%cn"));

        if (HasTAG(mech)) 
            sprintf(tempbuff + strlen(tempbuff), "TAG(%s)  ", isTAGDestroyed(mech) ? "%cr%chXX%cn" : ((getMech(TAGTarget(mech)) <= 0) || (TaggedBy(getMech(TAGTarget(mech))) != mech->mynum)) ? (TagRecycling(mech) ?  "%cy%chNot Rdy%cn" : "%cgRdy%cn") : tprintf("%s%s%%cn", (TagRecycling(mech) ? "%cy%ch" : "%ch"), GetMechToMechID(mech, getMech(TAGTarget(mech)))));

        if (MechSpecials2(mech) & SUPERCHARGER_TECH)
            sprintf(tempbuff + strlen(tempbuff), "SCHARGE: %s%d%%cn (%s)", MechSChargeCounter(mech) > 3 ? "%ch%cr" : MechSChargeCounter(mech) > 0 ? "%ch%cy" : "%cg", MechSChargeCounter(mech), MechStatus(mech) & SCHARGE_ENABLED ? "On" : "Off");

        if (MechSpecials(mech) & MASC_TECH)
            sprintf(tempbuff + strlen(tempbuff), "MASC: %s%d%%cn (%s)", MechMASCCounter(mech) > 3 ? "%ch%cr" : MechMASCCounter(mech) > 0 ? "%ch%cy" : "%cg", MechMASCCounter(mech), MechStatus(mech) & MASC_ENABLED ? "On" : "Off");

        notify(player, tempbuff);
        tempbuff[0] = 0;
    }

    if ((MechSpecials(mech) & AA_TECH) || (MechSpecials(mech) & BEAGLE_PROBE_TECH) || (MechSpecials2(mech) & BLOODHOUND_PROBE_TECH)) {
    ...
    if ((MechInfantrySpecials(mech) & CS_PURIFIER_STEALTH_TECH) || (MechInfantrySpecials(mech) & DC_KAGE_STEALTH_TECH) || (MechInfantrySpecials(mech) & FWL_ACHILEUS_STEALTH_TECH) || (MechInfantrySpecials(mech) & FC_INFILTRATOR_STEALTH_TECH) || (MechInfantrySpecials(mech) & FC_INFILTRATORII_STEALTH_TECH)) {
    ...
    if ((MechInfantrySpecials(mech) & INF_SWARM_TECH) || (MechInfantrySpecials(mech) & INF_MOUNT_TECH) || (MechInfantrySpecials(mech) & INF_ANTILEG_TECH) || (MechInfantrySpecials(mech) & CAN_JETTISON_TECH)) {
    ...
    if (MechInfantrySpecials(mech) & MUST_JETTISON_TECH) {
        strcpy(tempbuff, "Requirements: Must jettison backpack before using special abilities or jumping");
        notify(player, tempbuff);
        tempbuff[0] = 0;
    }
...
#define SHOW(part,loc) \
        ...
        if (MechType(mech) == CLASS_BSUIT && !doweird) {
            ...
            if (((MechType(mech) == CLASS_VEH_GROUND) || (MechType(mech) == CLASS_VTOL)) && !doweird) {
                *tempbuff = 0;
    ...
    if (!doweird) {
        ...
        if (MechType(mech) == CLASS_BSUIT)
            notify(player, "------ Weapon --------- [##] Holder ------ Status ||--- Ammo Type ---- Rounds");
        else
    ...
    for (loop = 0; loop < NUM_SECTIONS; loop++) {
        ...
        for (ii = 0; ii < count; ii++) {
            if (IsAMS(weaparray[ii]))
                sprintf(weapbuff, " %-16.16s %c%c%c%c%c [%2d] ", &MechWeapons[weaparray[ii]].name[3], ' ', (MechStatus(mech) & AMS_ENABLED) ? ' ' : 'O', (MechStatus(mech) & AMS_ENABLED) ? 'O' : 'F', (MechStatus(mech) & AMS_ENABLED) ? 'N' : 'F', ' ', running_sum + ii);
            else {
                ...
                sprintf(weapbuff, " %-16.16s %c%c%c%c%c [%2d] ", tmpbuf, (GetPartFireMode(mech, loop, critical[ii]) & REAR_MOUNT) ? 'R' : ' ', (((GetPartFireMode(mech, loop, critical[ii]) & OS_USED) || (GetPartFireMode(mech, loop, critical[ii]) & ROCKET_FIRED)) ? '-' : (GetPartFireMode(mech, loop, critical[ii]) & OS_MODE) ? 'O' : ' '), GetWeaponAmmoModeLetter(mech, loop, critical[ii]), GetWeaponFireModeLetter(mech, loop, critical[ii]), ((GetPartFireMode(mech, loop, critical[ii]) & ON_TC) && (!(MechCritStatus(mech) & TC_DESTROYED))) ? 'T' : (GetPartFireMode(mech, loop, critical[ii]) & IS_JETTISONED_MODE) ? 'J' : (GetPartFireMode(mech, loop, critical[ii]) & WILL_JETTISON_MODE) ? 'P' : ' ', running_sum + ii);
            }
        
            if (doweird)
                sprintf(weirdbuf + strlen(weirdbuf), "%s|%s", &MechWeapons[weaparray[ii]].name[3], location);
        
            strcat(weapbuff, location);

            if (PartIsBroken(mech, loop, critical[ii]) || PartTempNuke(mech, loop, critical[ii]) == FAIL_DESTROYED)
                strcat(weapbuff, "%ch%cx*****%c  || ");
            else 
                if (PartIsDisabled(mech, loop, critical[ii]))
                    ...
                    if (PartTempNuke(mech, loop, critical[ii])) {
                        ...
                        if (GetPartFireMode(mech, loop, critical[ii]) & ROCKET_FIRED)
                            strcat(weapbuff, "%ch%cxEmpty%c  || ");
                        else 
                            if (weapdata[ii])
                                strcat(weapbuff, tprintf(" %2d    || ", weapdata[ii]));
                            else 
                                if (countDamagedSlotsFromCrit(mech, loop, critical[ii]))
                                    strcat(weapbuff, "%crDAMAGED%c|| ");
                                else
                                    strcat(weapbuff, "%cgReady%c  || ");

            if ((ii + running_sum) < ammoweapcount) {
                ammo_mode = GetWeaponAmmoModeLetter_Model_Mode(ammoweap[ii + running_sum], modearray[ii + running_sum]);
                sprintf(weapname, "%-16.16s %c", &MechWeapons[ammoweap[ii + running_sum]].name[3], ammo_mode);
                sprintf(tempbuff, "  %s%3d%s", evaluate_ammo_amount(ammo[ii + running_sum], ammomax[ii + running_sum]), ammo[ii + running_sum], "%cn");
                strcat(weapname, tempbuff);

                if (doweird) 
                    if (ammo_mode && ammo_mode != ' ')
                        sprintf(weirdbuf + strlen(weirdbuf), "|%s|%d|%c ", &MechWeapons[ammoweap[ii + running_sum]].name[3], ammo[ii + running_sum], ammo_mode);
                    else
                        sprintf(weirdbuf + strlen(weirdbuf), "|%s|%d ", &MechWeapons[ammoweap[ii + running_sum]].name[3], ammo[ii + running_sum]);
            } else {
                if (doweird)
                    strcat(weirdbuf, " ");
                sprintf(weapname, "   ");
    ...
    if (running_sum < ammoweapcount) {
        while (running_sum < ammoweapcount) {
            ...
            ammo_mode = GetWeaponAmmoModeLetter_Model_Mode(ammoweap[running_sum], modearray[running_sum]);
            sprintf(weapname, "%-16.16s %c", &MechWeapons[ammoweap[running_sum]].name[3], ammo_mode);
            sprintf(tempbuff, "  %s%3d%s", evaluate_ammo_amount(ammo[running_sum], ammomax[running_sum]), ammo[running_sum], "%cn");
...
char *show_armor(MECH * mech, int loc, int flag)
{
    ...
    if (!GetSectInt(mech, loc) && !(flag & 64))
        ...
        sprintf(foo, "%s%s%%c", PrintArmorDamageColor(mech, loc, flag), PrintArmorDamageString(mech, loc, flag));
...
{
    char *buf, *bufc, *statattr;

    dbref aowner;
    int aflags;
    ATTR *pattr;

    if ((pattr = atr_str("MECHSTATUS")) && pattr->number)
        statattr = atr_get(mech->mynum, pattr->number, &aowner, &aflags);
    else
        statattr = "";
    ...
    mux_exec(buf, &bufc, 0, player, mech->mynum, EV_STRIP_AROUND | EV_NO_COMPRESS | EV_NO_LOCATION | EV_NOFCHECK | EV_NOTRACE | EV_FIGNORE, &statattr, NULL, 0);
        ...
        switch (MechType(mech)) {
            ...
            case CLASS_MECH:
                if (MechIsQuad(mech))
                    ...
                    if (MechTons(mech) <= 35)
                        ...
                        if (MechTons(mech) > 35 && MechTons(mech) <= 55)
                            strcpy(tmpbuf, mediummechdesc);
                        else 
                            if (MechTons(mech) > 55 && MechTons(mech) <= 75)
            ...
            case CLASS_VEH_NAVAL:
                if (MechMove(mech) == MOVE_FOIL)
                    ...
                    if (MechMove(mech) == MOVE_HULL)
                        strcpy(tmpbuf, shipdesc);
                    else
                        strcpy(tmpbuf, subdesc);
                break;
            default:
                strcpy(tmpbuf, " This 'toy' is of unknown type to me. It has yet to be templated\n for status.\n0");
                break;
        }
    }
    ...
    while (p && *p != '0') {
        ...
        for (q = p + 1; *q; q++)
            if (*q == '@' && mux_isdigit(*(q + 1))) {
                ...
                if (*q == '!' && mux_isdigit(*(q + 1)) && mux_isdigit(*(q + 2))) {
                    q++;
        
                    if (GetSectInt(mech, (int) (*q - '0')) || GetSectInt(mech, (int) (*(q + 1) - '0')))
                        *r++ = *(q + 2);
                    else
                        *r++ = ' ';
                    ...
                    if (*q == '&' && (*(q + 1) == '+' || *(q + 1) == '-' || *(q + 1) == ':' || *(q + 1) == '(' || *(q + 1) == ')')) {
                        ...
                        strcpy(r, show_armor(mech, (int) (*(q + 1) - '0'), flag | gflag));
                        r += strlen(r);
    ...
    switch (wPhysType) {
        ...
        case PHY_SWORD:
            if (SectIsDestroyed(objMech, wLoc))
                ...
                if (!OkayCritSectS2(objMech, wLoc, 0, SHOULDER_OR_HIP))
                    tRet = 0;
                else 
                    if (!OkayCritSectS2(objMech, wLoc, 3, HAND_OR_FOOT_ACTUATOR))
                        tRet = 0;
            break;
        case PHY_MACE:
            if (SectIsDestroyed(objMech, LARM) || SectIsDestroyed(objMech, RARM))
                ...
                if ((!OkayCritSectS2(objMech, LARM, 0, SHOULDER_OR_HIP)) || (!OkayCritSectS2(objMech, RARM, 0, SHOULDER_OR_HIP)))
                    tRet = 0;
                else 
                    if ((!OkayCritSectS2(objMech, LARM, 3, HAND_OR_FOOT_ACTUATOR)) || (!OkayCritSectS2(objMech, RARM, 3, HAND_OR_FOOT_ACTUATOR)))
                        tRet = 0;
            break;
        default:
            tRet = 0;
