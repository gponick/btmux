    ...
    for (x = 0; x < NUM_SECTIONS; x++) {
        ...
        for (y = 0; y < CritsInLoc(mech, x); y++) {
            if ((t = GetPartType(mech, x, y))) 
                if (Special2I(t) == C3_MASTER) {
                    debugC3(tprintf("...found a C3Master slot at section %d, slot %d on %d.", x, y, mech->mynum));
                    wcSlots++;
            ...
            if (wcSlots == getC3MasterSize(mech)) {
                debugC3(tprintf ("...found enough slots for a C3Master for %d.", mech->mynum));
                wcSlots = 0;
                if (wcWorkingSlots == getC3MasterSize(mech)) {
                    debugC3(tprintf("...there is even enough working slots to make the computer work on %d.", mech->mynum));
                    wcMasters++;
                }
            }
        }
...
int countMaxC3Units(MECH * mech, dbref * myTempNetwork, int tempNetworkSize, MECH * targMech)
{
    dbref otherRef;
    MECH *otherMech;
    int i;
    ...
    for (i = 0; i < tempNetworkSize; i++) {
        ...
        debugC3(tprintf("...for %d, we add %d masters", otherMech->mynum, MechWorkingC3Masters(otherMech)));
    }
    ...
    debugC3(tprintf("...and after adding in my masters, we now have a max size of %d", maxC3Size));
...
void replicateC3Network(MECH * mechSrc, MECH * mechDest)
{
    ...
    debugC3(tprintf("C3 REPLICATE: %d's C3 network to %d", mechSrc->mynum, mechDest->mynum));
...
void addMechToC3Network(MECH * mech, MECH * mechToAdd)
{
    ...
    debugC3(tprintf("C3 ADD: %d to the C3 network of %d", mechToAdd->mynum, mech->mynum));
    ...
    debugC3(tprintf("C3 ADD: Position to add to %d's network is %d", mech->mynum, wPos));

    MechC3NetworkElem(mech, wPos) = mechToAdd->mynum;
    MechC3NetworkSize(mech) += 1;

    mech_notify(mech, MECHALL, tprintf("%s connects to your C3 network.", GetMechToMechID(mech, mechToAdd)));
    ...
    for (i = 0; i < C3_NETWORK_SIZE; i++) {
        ...
        if (otherRef != mechToAdd->mynum) {
            ...
            if (otherNotifyMech)
                mech_notify(otherNotifyMech, MECHALL, tprintf("%s connects to your C3 network.", GetMechToMechID(otherNotifyMech, mechToAdd)));
        }
...
void clearMechFromC3Network(dbref refToClear, MECH * mech)
{
    ...
    debugC3(tprintf("C3 CLEAR: %d from the C3 network of %d", refToClear, mech->mynum));
...
void validateC3Network(MECH * mech)
{
    ...
    for (i = 0; i < C3_NETWORK_SIZE; i++) {
        ...
        debugC3(tprintf("C3 VALIDATE INFO: %d is now in %d's C3 network", otherMech->mynum, mech->mynum));
    ...
    debugC3(tprintf("C3 VALIDATE INFO: (PreTrim) %d's C3 network is %d elements", mech->mynum, MechC3NetworkSize(mech)));
    networkSize = trimC3Network(mech, myTempNetwork, networkSize);
    debugC3(tprintf("C3 VALIDATE INFO: (PostTrim) %d's C3 network has been trimmed to %d elements", mech->mynum, networkSize));
...
void mech_c3_join_leave(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(mech_parseattributes(buffer, args, 2) != 1, "Invalid number of arguments to function!");

    DOCHECK(!HasC3(mech), "This unit is not equipped with C3!");
    DOCHECK(C3Destroyed(mech), "Your C3 system is destroyed!");
    DOCHECK(AnyECMDisturbed(mech), "Your C3 system is not currently operational!");
    ...
    DOCHECK((refTarget < 1) || !LOS, "That is not a valid targetID. Try again.");
    DOCHECK(MechTeam(mech) != MechTeam(target), "You can't use the C3 network of unfriendly units!");
    ...
    DOCHECK(!HasC3(target), "That unit does not appear to be equipped with C3!");
    ...
    maxC3Size = countMaxC3Units(mech, MechC3Network(target), MechC3NetworkSize(target), target);

    DOCHECK(maxC3Size < (MechC3NetworkSize(target) + 1), "That unit's C3 network is operating at maximum capacity!");

    /* Connect us up */
    mech_notify(mech, MECHALL, tprintf("You connect to %s's C3 network.", GetMechToMechID(mech, target)));
...
void mech_c3_message(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(MechC3NetworkSize(mech) <= 0, "There are no other units in your C3 network!");
...
void mech_c3_network(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
