...
static inline int MySeriousnessCheck(MECH * mech, int hitloc)
{
    ...
    if (!(orig = GetSectOArmor(mech, hitloc)))
        return 0;
    if (!(newd = GetSectArmor(mech, hitloc)))
        return 3;
    if (newd < orig / 4)
        return 2;
    if (newd < orig / 2)
...
static inline int MySeriousnessCheckR(MECH * mech, int hitloc)
{
    ...
    if (!(orig = GetSectORArmor(mech, hitloc)))
        return 0;
    if (!(newd = GetSectRArmor(mech, hitloc)))
        return 3;
    if (newd < orig / 4)
        return 2;
    if (newd < orig / 2)
...
int cause_armordamage(MECH * wounded, MECH * attacker, int LOS, int attackPilot, int isrear, int iscritical, int hitloc, int damage, int *crits, int wWeapIndx, int wAmmoMode)
{
    ...
    if (MechType(wounded) == CLASS_MECH && isrear && (hitloc == CTORSO || hitloc == RTORSO || hitloc == LTORSO)) {
        if ((GetSectRArmor(wounded, hitloc) - damage) >= 0)
            wPercentLeft = (((GetSectRArmor(wounded, hitloc) - damage) * 100) / GetSectORArmor(wounded, hitloc));
        ...
        if (intDamage > 0) {
            ...
            seriousness = (seriousness == MySeriousnessCheckR(wounded, hitloc)) ? 0 : MySeriousnessCheckR(wounded, hitloc);
        }
    } else {
        /* Silly stuff */
        /* SetSectArmor(wounded, hitloc, MAX(0, intDamage =  GetSectArmor(wounded, hitloc) - damage));
           intDamage = abs(intDamage); */
        if (GetSectOArmor(wounded, hitloc) && ((GetSectArmor(wounded, hitloc) - damage) >= 0))
            wPercentLeft = (((GetSectArmor(wounded, hitloc) - damage) * 100) / GetSectOArmor(wounded, hitloc));
        ...
        if (intDamage > 0) {
            ...
            seriousness = (seriousness == MySeriousnessCheck(wounded, hitloc)) ? 0 : MySeriousnessCheck(wounded, hitloc);
        }
    ...
    if (!iscritical && (wAmmoMode & AC_AP_MODE) && (intDamage <= 0) && (wPercentLeft < 50))
        tAPCritical = 1;
    ...
    if (iscritical || tAPCritical) {
        ...
        if (tAPCritical) {
            if (!strcmp(&MechWeapons[wWeapIndx].name[3], "AC/2"))
                ...
                if (!strcmp(&MechWeapons[wWeapIndx].name[3], "LightAC/2"))
                    r -= 4;
                else 
                    if (!strcmp(&MechWeapons[wWeapIndx].name[3], "AC/5"))
                        r -= 3;
                    else 
                        if (!strcmp(&MechWeapons[wWeapIndx].name[3], "LightAC/5"))
                            r -= 3;
                        else 
                            if (!strcmp(&MechWeapons[wWeapIndx].name[3], "AC/10"))
                                r -= 2;
                            else 
                                if (!strcmp(&MechWeapons[wWeapIndx].name[3], "AC/20"))
                                    r -= 1;
                                else
                                    r -= 10;
        }
    ...
    if (seriousness > 0 && MechArmorWarn(wounded))
        mech_notify(wounded, MECHALL, tprintf("%sWARNING: %s%s Armor %s", MySeriousColorStr(wounded, seriousness), ShortArmorSectionString(MechType(wounded), MechMove(wounded), hitloc), isrear ? " (Rear)" : "", MySeriousStr(wounded, seriousness)));
...
int cause_internaldamage(MECH * wounded, MECH * attacker, int LOS, int attackPilot, int isrear, int hitloc, int intDamage, int weapindx, int *crits)
{
    ...
    if ((MechSpecials(wounded) & REINFI_TECH) && intDamage > 0)
        ...
        if (MechSpecials(wounded) & COMPI_TECH)
    ...
    if (!(*crits))
        switch (r) {
            ...
            case 12:
                if ((MechType(wounded) == CLASS_MECH) || (MechType(wounded) == CLASS_MW)) {
                    switch (hitloc) {
                        ...
                        case HEAD:
                            ...
                            if (!Destroyed(wounded)) {
                                sprintf(msgbuf, "'s %s is blown off in a shower of sparks and smoke!", locname);
                ...
                break;
            default:
                break; /* No critical hit */
        }
    
    /* Hmm.. This should be interesting */
    if (MechType(wounded) == CLASS_MECH && intDamage && (hitloc == CTORSO) && GetSectInt(wounded, hitloc) == GetSectOInt(wounded, hitloc))
...
void DamageMech(MECH * wounded, MECH * attacker, int LOS, int attackPilot, int hitloc, int isrear, int iscritical, int damage, int intDamage, int cause, int bth, int wWeapIndx, int wAmmoMode, int tIgnoreSwarmers)
{
    ...
    /* if:
       damage = -1 && intDamage>0 - ammo expl
       damage = -2 && intDamage>0 - transferred ammo expl
       damage = n && intDamage = 0 - usual damage
       damage = n && intDamage = -1/-2 - usual damage + transfer/+red enable */
            ...
            if (!attacker || (attacker->mynum != mechSwarmer->mynum)) {
                ...
                if ((wSwarmerHitChance >= wRoll) && (GetSectArmor(wounded, hitloc))) {
                    if (attacker && (attacker->mynum != wounded->mynum)) 
                        mech_notify(attacker, MECHALL, "The battlesuits crawling all over your target absorb the damage!");

                    mech_notify(wounded, MECHALL, "The battlesuits crawling all over you absorb the damage!");
                    mech_notify(mechSwarmer, MECHALL, "You absorb the damage!");
                    hitloc = FindHitLocation(mechSwarmer, 0, &iscritical, &isrear);
                    DamageMech(mechSwarmer, attacker, LOS, attackPilot, hitloc, 0, 0, damage, 0, cause, bth, wWeapIndx, wAmmoMode, 0);
                    return;
                }
    ...
    if (damage > 0 && intDamage == 0) {
        /* If we're a VTOL and the hitloc is the rotor, we'll cut the damage by some value */
        if ((MechType(wounded) == CLASS_VTOL) && (hitloc == ROTOR)) {
            if (btechconf.btech_divrotordamage > 0)
                damage = damage / btechconf.btech_divrotordamage;
            if (damage < 1)
                damage = 1;
        }
        ...
        if (!global_physical_flag)
            ...
            if (global_physical_flag == 1)
                if (!Destroyed(wounded) && In_Character(wounded->mynum) && MechTeam(wounded) != MechTeam(attacker))
    ...
    if (isrear) {
        if (!(MechSpecials(wounded) & SALVAGE_TECH) && (Roll() <= 5) && (hitloc == CTORSO || hitloc == LTORSO || hitloc == RTORSO))
            tSnapTowLines = 1;

        if (MechType(wounded) == CLASS_MECH) {
            strcpy(rearMessage, "(Rear)");
            if (Dumping(wounded) && ((hitloc == CTORSO) || (hitloc == LTORSO) || (hitloc == RTORSO)) && (cause >= 0))
                if (Roll() <= 7)
                    tBlowDumpingAmmo = 1;
        } else {
            if (hitloc == FSIDE)
    ...
    while (((!is_aero(wounded) && !GetSectInt(wounded, hitloc)) || (is_aero(wounded) && !GetSectArmor(wounded, hitloc))) && !kill) 
        if (transfer && (hitloc = TransferTarget(wounded, hitloc)) >= 0 && (MechType(wounded) == CLASS_MECH || MechType(wounded) == CLASS_MW || MechType(wounded) == CLASS_BSUIT || is_aero(wounded))) {
            DamageMech(wounded, attacker, LOS, attackPilot, hitloc, isrear, iscritical, damage == -1 ? -2 : damage, transfer == 1 ? -2 : damage, cause, bth, wWeapIndx, wAmmoMode, tIgnoreSwarmers);
            return;
        } else 
            if (!((MechType(wounded) == CLASS_MECH || MechType(wounded) == CLASS_MW || MechType(wounded) == CLASS_BSUIT || is_aero(wounded)) && (hitloc = TransferTarget(wounded, hitloc)) >= 0)) 
                if (is_aero(wounded) && !Destroyed(wounded)) /* Hurt SI instead. */
    ...
    if (C_OODing(wounded) && Roll() > 8) {
        mech_ood_damage(wounded, attacker, damage + (intDamage < 0 ? 0 : intDamage));
        return;
    }

    if (hitloc != -1) {
        ArmorStringFromIndex(hitloc, locationBuff, MechType(wounded), MechMove(wounded));
        sprintf(notificationBuff, "for %d points of damage in the %s %s", damage + (intDamage < 0 ? 0 : intDamage), locationBuff, rearMessage);
    } else
        sprintf(notificationBuff, "for %d points of damage in the structure.", damage + (intDamage < 0 ? 0 : intDamage));

    /*  if (LOS && attackPilot != -1) */
    if (LOS) 
        if (!was_transfer)
            mech_notify(attacker, MECHALL, tprintf("%%cgYou hit %s%%c", notificationBuff));
        else
            mech_notify(attacker, MECHALL, tprintf("%%cgDamage transfer.. %s%%c", notificationBuff));
    ...
    if ((hitloc == HEAD && MechType(wounded) == CLASS_MECH) || (hitloc == AERO_COCKPIT && MechType(wounded) == CLASS_AERO)) 
        /*      mech_notify (wounded, MECHALL, "You take 10 points of Lethal damage!!"); */
        headhitmwdamage(wounded, 1);
    
    if (kill) {
        if (kill == 1) {
            mech_notify(wounded, MECHALL, "The blast causes last of your craft's structural integrity disappear, blowing");
    ...
    if (damage > 0) {
        if (MechType(wounded) == CLASS_MECH) 
            if (!isrear && (MechSpecials(wounded) & SLITE_TECH) && !(MechCritStatus(wounded) & SLITE_DEST) && (hitloc == LTORSO || hitloc == CTORSO || hitloc == RTORSO)) 
                ...
                if (Roll() > 6) 
                    if ((MechStatus2(wounded) & SLITE_ON) || (Roll() > 5)) {
                        ...
                        mech_notify(wounded, MECHALL, "%ch%cyYour searchlight is destroyed!%cn");
                    }
    
        if (MechType(wounded) == CLASS_VEH_GROUND) 
            if (!isrear && (MechSpecials(wounded) & SLITE_TECH) && !(MechCritStatus(wounded) & SLITE_DEST) && (hitloc == FSIDE)) 
                /* Possibly destroy the light */
                if (Roll() > 6) 
                    if ((MechStatus2(wounded) & SLITE_ON) || (Roll() > 5)) {
                        MechCritStatus(wounded) |= SLITE_DEST;
        ...
        intDamage += cause_armordamage(wounded, attacker, LOS, attackPilot, isrear, iscritical, hitloc, damage, &crits, wWeapIndx, wAmmoMode);
        if (intDamage >= 0)
            MechFloodsLoc(wounded, hitloc, MechZ(wounded));

        if (intDamage > 0 && !is_aero(wounded)) {
            intDamage = cause_internaldamage(wounded, attacker, LOS, attackPilot, isrear, hitloc, intDamage, cause, &crits);
        ...
        if (intDamage > 0 && transfer && (MechType(wounded) != CLASS_BSUIT)) {
            if ((hitloc = TransferTarget(wounded, hitloc)) >= 0)
                DamageMech(wounded, attacker, LOS, attackPilot, hitloc, isrear, iscritical, intDamage, -2, cause, bth, wWeapIndx, wAmmoMode, tIgnoreSwarmers);
            else {
                DestroyMech(wounded, attacker, 1);
                return;
            }
        ...
        if (intDamage > 0) {
            if (is_aero(wounded))
                intDamage = cause_armordamage(wounded, attacker, LOS, attackPilot, isrear, iscritical, hitloc, intDamage, &crits, wWeapIndx, wAmmoMode);
            else
                intDamage = cause_internaldamage(wounded, attacker, LOS, attackPilot, isrear, hitloc, intDamage, cause, &crits);
            ...
            if (intDamage > 0 && transfer && !((MechSections(wounded)[hitloc].config & CASE_TECH) || (MechSpecials(wounded) & CLAN_TECH))) 
                if ((hitloc = TransferTarget(wounded, hitloc)) >= 0) 
                    if (!is_aero(wounded))
                        DamageMech(wounded, attacker, LOS, attackPilot, hitloc, isrear, iscritical, -2, intDamage, cause, bth, wWeapIndx, wAmmoMode, tIgnoreSwarmers);
                    else
                        DamageMech(wounded, attacker, LOS, attackPilot, hitloc, isrear, iscritical, intDamage, -2, cause, bth, wWeapIndx, wAmmoMode, tIgnoreSwarmers);
                else {
                    DestroyMech(wounded, attacker, 1);
    ...
    if (tSnapTowLines && (MechCarrying(wounded) > 0)) 
        if ((towTarget = getMech(MechCarrying(wounded)))) {
            ...
            MechLOSBroadcast(wounded, "'s tow lines release and flap freely behind it!");
...
void LoseWeapon(MECH * mech, int hitloc)
{
    ...
    firstCrit = FindFirstWeaponCrit(mech, hitloc, -1, 0, I2Weapon(b), GetWeaponCrits(mech, b));

    DestroyWeapon(mech, hitloc, I2Weapon(b), firstCrit, 1, GetWeaponCrits(mech, b));
    mech_notify(mech, MECHALL, tprintf("%%ch%%crYour %s is destroyed!%%c", &MechWeapons[b].name[3]));
}
...
void DestroyHeatSink(MECH * mech, int hitloc)
{
    ...
    if (FindObj(mech, hitloc, i)) {
        ...
        mech_notify(mech, MECHALL, "The computer shows a heatsink died due to the impact.");
    }
}
...
void DestroySection(MECH * wounded, MECH * attacker, int LOS, int hitloc)
{
    ...
    int i, j;
    int tKillMech;
    int tIsLeg = ((hitloc == RLEG || hitloc == LLEG) || ((hitloc == RARM || hitloc == LARM) && (MechIsQuad(wounded))));
    ...
    if (attacker) {
        ...
        if (LOS >= 0)
            mech_notify(wounded, MECHALL, tprintf("Your %s has been destroyed!", locname));
    ...
    if (In_Character(wounded->mynum)) {
        ...
        if ((MechType(wounded) == CLASS_BSUIT) && (Number(1, 100) <= 25) && wounded_pilot)
        ...
        if (hitloc == LTORSO)
            ...
            if (hitloc == RTORSO)
                ...
                if (hitloc == CTORSO || hitloc == HEAD) {
                    ...
                    if (hitloc == HEAD || ((MechType(wounded) == CLASS_MW) && (hitloc == CTORSO))) 
                        if (In_Character(wounded->mynum)) {
                            for (j = 0; j < FREQS; j++) {
                                wounded->freq[j] = 0;
                                wounded->freqmodes[j] = 0;
...
int dodamage_func(dbref player, MECH * mech, int totaldam, int clustersize, int direction, int iscritical, char *mechmsg, char *mechbroadcast)
{
    ...
    while (totaldam) {
        ...
        DamageMech(mech, mech, 0, -1, hitloc, isrear, iscritical, this_time, 0, 0, 0, -1, 0, 1);
        totaldam -= this_time;
    }
...
void mech_damage(dbref player, void *data, char *buffer)
{
    ...
    Missile_Hit(mech, mech, -1, -1, isrear, iscritical, 0, -1, -1, clustersize, damage / clustersize, 1, 0, 0, 0);
}
