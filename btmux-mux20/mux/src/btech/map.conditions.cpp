...
void map_setconditions(dbref player, void *data, char *buffer)
{
    char *args[5];
    ...
    DOCHECK((argc = mech_parseattributes(buffer, args, 4)) < 2, "(At least) 2 options required (gravity + temperature)");
    DOCHECK(argc > 4, "Too many options! Command accepts only 4 at max (gravity + temperature + vacuum-flag + underground-flag)");
    DOCHECK(Readnum(grav, args[0]), "Invalid gravity (must be integer in range of 0 to 255)");
    DOCHECK(grav < 0 || grav > 255, "Invalid gravity (must be integer in range of 0 to 255)");
    DOCHECK(Readnum(temp, args[1]), "Invalid temperature (must be integer in range of -128 to 127");
    DOCHECK(temp < -128 || temp > 127, "Invalid temperature (must be integer in range of -128 to 127");
    if (argc > 2) {
        DOCHECK(Readnum(vacuum, args[2]), "Invalid vacuum flag (must be integer, 0 or 1)");
        DOCHECK(vacuum < 0 || vacuum > 1, "Invalid vacuum flag (must be integer, 0 or 1)");
    }
    if (argc > 3) {
        DOCHECK(Readnum(underground, args[3]), "Invalid underground flag (must be integer, 0 or 1)");
        DOCHECK(underground < 0 || underground > 1, "Invalid underground flag (must be integer, 0 or 1)");
    }
...
{
    if (btechconf.btech_stackpole && !doing_explode && (MechBoomStart(wounded) + MAX_BOOM_TIME) >= muxevent_tick 
        ...
        ScrambleInfraAndLiteAmp(wounded, 4, 0, "The searing blast of heat burns out your sensors!", 
                                    "The blinding flash of light overloads your sensors!");

        blast_hit_hexesf(map, dam, 3, MAX(MechTons(wounded) / 10, MechEngineSize(wounded) / 25), 
                            ...
                            btechconf.btech_engine > 1, 3, 5, 1, 2);
        MechZ(wounded) = z;
...
void DestroyParts(MECH * attacker, MECH * wounded, int hitloc, int breach, int IsDisable)
{
    ...
    if (!(MechType(wounded) == CLASS_MECH || MechType(wounded) == CLASS_MW || MechType(wounded) == CLASS_BSUIT)) {
        for (i = 0; i < CritsInLoc(wounded, hitloc); i++)
            if (GetPartType(wounded, hitloc, i) && !PartIsDestroyed(wounded, hitloc, i)) 
                if (IsDisable == 1)
    ...
    if (MechType(wounded) == CLASS_MECH || MechType(wounded) == CLASS_MW) {
        if (breach && hitloc == HEAD) {
            if (InVacuum(wounded))
                mech_notify(wounded, MECHALL, "You are exposed to the vacuum!");
            else
        ...
        if (tNormalizeAllCrits)
            ...
            if (tNormalizeLocCrits)
                NormalizeLocActuatorCrits(wounded, hitloc);
    
        if (tIsLeg && !Fallen(wounded) && !Jumping(wounded) && !OODing(wounded) && attacker) {
            if (tDoAutoFall) {
                mech_notify(wounded, MECHALL, "You realize remaining standing is no longer an option and crash to the ground!");
