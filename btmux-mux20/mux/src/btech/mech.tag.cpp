...
static void tag_recycle_event(EVENT * e)
{
    ...
    mech_notify(mech, MECHALL, "%cgYour TAG system has achieved a stable lock.%cn");
}
...
void mech_tag(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(refTarget < 1 || !LOS, "That is not a valid TAG targetID. Try again.");
    DOCHECK(MechTeam(mech) == MechTeam(target), "You can't TAG friendly units!");
    DOCHECK(mech == target, "You can't TAG yourself!");
    DOCHECK(range > TAG_LONG, tprintf("Out of range! TAG ranges are %d/%d/%d", TAG_SHORT, TAG_MED, TAG_LONG));

    /* * This should actually make a roll...  */

    /* if ( checkAllSections(mech,INARC_HAYWIRE_ATTACHED) )
       BTH += 1; */
    mech_notify(mech, MECHALL, tprintf("You light up %s with your TAG.", GetMechToMechID(mech, target)));
...
{
    if ((MechSpecials2(mech) & TAG_TECH) && (MechCritStatus(mech) & TAG_DESTROYED))
        return 1;
