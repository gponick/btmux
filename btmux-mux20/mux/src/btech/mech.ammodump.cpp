...
static void mech_dump_event(EVENT * ev)
{
    ...
    if (!arg) {
        ...
        if (e > 1)
            ...
            MechLOSBroadcast(mech, "no longer has ammo dumping from hatches on its back.");
        }
    ...
    if (arg < 256) {
        ...
        if (e > 1)
            ...
            if (e == 1 && Started(mech)) {
                ...
                mech_notify(mech, MECHALL, tprintf("All ammunition in %s dumped.", buf));
                MechLOSBroadcast(mech, "no longer has ammo dumping from hatches on its back.");
            }
    ...
    if (arg < 65536) {
        ...
        if (e > 1)
            ...
            mech_notify(mech, MECHALL, tprintf("Ammunition for %s dumped!", get_parts_long_name(I2Weapon(weapindx), 0)));
            MechLOSBroadcast(mech, "no longer has ammo dumping from hatches on its back.");
        }
    ...
    if (e > 1) 
        ...
        mech_notify(mech, MECHALL, tprintf("Ammunition in %s crit %i dumped!", buf, i + 1));
        MechLOSBroadcast(mech, "no longer has ammo dumping from hatches on its back.");
    }
}
...
void mech_dump(dbref player, void *data, char *buffer)
{
    ...
    DOCHECKMA(IsRunning(MechDesiredSpeed(mech), MMaxSpeed(mech)), "You can't dump ammo while running!");

    if (!strcasecmp(args[0], "stop")) {
        DOCHECKMA(!Dumping(mech), "You aren't dumping anything!");
        mech_notify(mech, MECHALL, "Ammo dumping halted.");
        StopDump(mech);
        MechLOSBroadcast(mech, "nolonger has ammo dumping from hatches on its back.");
        return;
    } else 
            ...
            if (!weapnum && strcmp(args[0], "0")) {
                ...
                if (args[1]) {
                    ...
                    if (i >= 0 && i < 12) {
                        ...
                        DOCHECKMA(!count, tprintf("There is no ammunition in %s crit %i!", buf, i + 1));
                        type = (((i + 1) << 8) | (loc + 1));
                        DOCHECKMA(type & ~0xFFFF, "Internal inconsistency, dump failed!");
                        type = type << 16;
                        mech_notify(mech, MECHALL, tprintf ("Starting dumping of ammunition in %s crit %i..", buf, i + 1));
                        MechLOSBroadcast(mech, "starts dumping ammo from hatches on its back.");
                ...
                type = loc + 1;
                mech_notify(mech, MECHALL, tprintf("Starting dumping of ammunition in %s..", buf));
                MechLOSBroadcast(mech, "starts dumping ammo from hatches on its back.");
                MECHEVENT(mech, EVENT_DUMP, mech_dump_event, DUMP_GRAD_TICK, type);
                return;
            }
    ...
    if (weapnum < 0)
        SendError(tprintf ("CHEATER: #%d tried to crash mux with command 'dump %d'!", (int) player, weapnum));

    DOCHECKMA(Dumping(mech), "You're already dumping some ammo!");
    DOCHECK(weapindx < 0, "Invalid weapon number!");
    FindWeaponNumberOnMech(mech, weapnum, &section, &critical);
    DOCHECK(MechWeapons[weapindx].type == TBEAM || MechWeapons[weapindx].type == THAND, "That weapon doesn't use ammunition!");
    DOCHECK(!FindAmmoForWeapon_sub(mech, -1, -1, weapindx, 0, &ammoLoc, &ammoCrit, 0, 0), "You don't have any ammunition for that weapon stored on this mech!");
    DOCHECK(GetPartData(mech, ammoLoc, ammoCrit) == 0, "You are out of ammunition for that weapon already!");
    type = 256 * (weapindx + 1);
    mech_notify(mech, MECHALL, tprintf("Starting dumping %s ammunition..", get_parts_long_name(I2Weapon(weapindx), 0)));
    MechLOSBroadcast(mech, "starts dumping ammo from hatches on its back.");
    MECHEVENT(mech, EVENT_DUMP, mech_dump_event, DUMP_GRAD_TICK, type);
}
...
void BlowDumpingAmmo(MECH * mech, MECH * attacker, int wHitLoc)
{
    ...
    if (!wEventData) {        /* Global ammo dump */
        for (wSecIter = 7; wSecIter >= 0; wSecIter--)
            for (wSlotIter = CritsInLoc(mech, wSecIter) - 1; wSlotIter >= 0; wSlotIter--) {
                wPartType = GetPartType(mech, wSecIter, wSlotIter);

                if (IsAmmo(wPartType))
                    if (GetPartData(mech, wSecIter, wSlotIter)) {
                        aobjAmmoItems[wcAmmoItems].wDamage = FindMaxAmmoDamage(Ammo2WeaponI(wPartType));
        ...
        if (wEventData < 256) {    /* Location specific ammo dump */
            wLoc = wEventData - 1;
            for (wSlotIter = 0; wSlotIter < CritsInLoc(mech, wLoc); wSlotIter++) {
                wPartType = GetPartType(mech, wLoc, wSlotIter);
                /*     wPartType = GetPartType(mech, wSecIter, wSlotIter); */
                if (IsAmmo(wPartType))
                    if (!PartIsNonfunctional(mech, wLoc, wSlotIter) && GetPartData(mech, wLoc, wSlotIter)) {
                        aobjAmmoItems[wcAmmoItems].wDamage = FindMaxAmmoDamage(Ammo2WeaponI(wPartType));
            ...
            if (wEventData < 65536) {    /* Weapon specific ammo dump */
    ...
    if (wcAmmoItems > 0) {
        ...
        if (wBlowDamage > 0) {
            MechLOSBroadcast(mech, "'s rear armor lights up as ammo being dumped ignites!");
            mech_notify(mech, MECHALL, tprintf("%%ch%%crSome of the %s ammo dumping out of your mech ignites!%%cn", get_parts_long_name(I2Weapon(wWeapIdx), 0)));
            DamageMech(mech, attacker, 0, -1, wHitLoc, 1, 0, wBlowDamage, -1, -1, 0, -1, 0, 1);
            /* * Decrement the ammo one round */
            wPartData = GetPartData(mech, wSecIter, wSlotIter);
            if (wPartData > 0)
                SetPartData(mech, wSecIter, wSlotIter, wPartData - 1);
    
            mech_notify(mech, MECHALL, "%ch%crAll ammo dumping operations have stopped!%cn");
            StopDump(mech);
        }
    }
}
