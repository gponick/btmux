    ...
    for (i = 0; i < NUM_SECTIONS; i++) {
        ...
        for (j = 0; j < NUM_CRITICALS; j++)
            if (IsBomb((k = GetPartType(mech, i, j)))) {
                ...
                if (!bc) 
                    vsi(tprintf("#  %-20s %-5s %-5s %s", "Location", "Weight", "Power", "Type"));

                vsi(tprintf("%-2d %-20s %5d %5d", bc + 1, location, bombs[k].weight / 10, bombs[k].aff, types[bombs[k].type]));
                bc++;
            }
    }
    if (!bc)
...
void bomb_aim(MECH * mech, dbref player)
{
    ...
    tprintf("Estimated bomb flight time %s, estimated landing hex %d,%d.", toi, x, y));
}

void bomb_hit_hexes(MAP * map, int x, int y, int hitnb, int iscluster,
    int aff_d, int aff_h, char *tomsg, char *otmsg, char *tomsg1, char *otmsg1)
{
    blast_hit_hexes(map, aff_d, iscluster ? 2 : 10, aff_h, x, y, tomsg, otmsg, tomsg1, otmsg1, 0, 4, 1, 1, hitnb);
}
...
static void bomb_hit(bomb_shot * s)
{
    switch (bombs[s->type].type) {
        case 0:
            ...
            break;
        case 1:
            HexLOSBroadcast(s->map, s->x, s->y, "A fiery blast occurs in $H, spraying flaming gel everywhere!");
            bomb_hit_hexes(s->map, s->x, s->y, 1, 0,
                bombs[s->type].type == 1 ? bombs[s->type].aff / 2 : bombs[s->type].aff, 
                bombs[s->type].type == 1 ? bombs[s->type].aff : 0,
                "You receive a direct hit!", "receives a direct hit!",
            ...
            break;
        case 2:
            HexLOSBroadcast(s->map, s->x, s->y, "A bomb drops rain of small bomblets in $H's surroundings!");
...
void bomb_drop(MECH * mech, int player, int bn)
{
    ...
    for (i = 0; i < NUM_SECTIONS; i++)
        for (j = 0; j < NUM_CRITICALS; j++)
            if (IsBomb((k = GetPartType(mech, i, j))) && !PartIsDestroyed(mech, i, j)) {
                if (bc == bn) {
                    lloc = i;
                    lpos = j;
                }
    ...
    DOCHECK(bn < 0 || bn >= bc, "No bomb with such number installed! (See BOMB LIST)");
    ...
    if (MadePilotSkillRoll(mech, 4 + ob) || t < 2.0)
        mech_notify(mech, MECHALL, "Despite the slight problems, you keep the craft stable enough to drop the bomb right on target..");
    else {
        mech_notify(mech, MECHALL, "The ship's lurches slightly, dropping the bomb off target!");
        ob = 6 * (1 + ob);    /* Max distance missed  */
        ob = MAX(1, (Number(1, ob)) / 2);
        di = Number(0, 359);
        dir = di * TWOPIOVER360;
        x = (short)(x + ob * cos(dir));
        y = (short)(y + ob * sin(dir));
    }
    ...
    s->map = map;
    SetCargoWeight(mech);
    muxevent_add(MAX(1, (int)t), 0, EVENT_DHIT, bomb_hit_event, (void *) s, NULL);
}
...
void mech_bomb(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALSO);
    DOCHECK(!(argc = mech_parseattributes(buffer, args, 3)), "(At least) one option required.");
