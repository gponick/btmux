...
static void GetAdjHex(int currx, int curry, int nexthex, int *x, int *y)
{
    ...
    const unsigned int SE = 2;
    const unsigned int S = 3;
    const unsigned int SW = 4;
    const unsigned int NW = 5;
...
int TraceLOS(MAP * map, int ax, int ay, int bx, int by, lostrace_info ** result)
{
    ...
    int nexthex;        /* potential next hex being examined */
    ...
    if (!((bx - ax) % 2) && ay == by) {
        ...
        while (currx != bx) {
            ...
            Store_BestOf(currx + 1 * i, by - currx % 2, currx + 1 * i, by - currx % 2 + 1);
    ...
    if (abs(by - ay) == (3 * abs(bx - ax) / 2) + abs((bx - ax) % 2) * abs((by < ay) ? (ax % 2) : (1 - ax % 2))) {
        ...
        while ((currx != bx) && (curry != by)) {
            Store_BestOf(currx, curry + ymul, currx + xmul, ymul == 1 ? curry + 1 - currx % 2 : curry - currx % 2);
...
/* First get the necessary constants set up */
    ...
    liney = acx * sinu + acy * cosu;    /* we could just as correctly use bx, by */
