...
static command_node *auto_create_command_node() {
    ...
    temp = static_cast <command_node *> (malloc(sizeof(command_node)));
    if (temp == NULL)
        return NULL;
...
void auto_delcommand(dbref player, void *data, char *buffer) {
    ...
    if (!*buffer) {
        ...
        notify(player, tprintf("Must be within the range 1 to %d or -1 for all\n", dllist_size(autopilot->commands)));
        return;
    }

    /* Make sure its a number */
    if (Readnum(p, buffer)) {
        notify(player, tprintf("Invalid Argument : Must be within the range"
                " 1 to %d or -1 for all\n", dllist_size(autopilot->commands)));
        return;
    }
    ...
    if (!remove_all_commands) {
        ...
        notify(player, tprintf("Command #%d Successfully Removed\n", p));
...
void auto_addcommand(dbref player, void *data, char *buffer) {
    ...
    notify(player, tprintf("Command Added: %s", auto_show_command(temp_command_node)));
...
void auto_eventstats(dbref player, void *data, char *buffer) {
    ...
    if (total) {
        notify(player, "-------------------------------");
        notify(player, tprintf("%d total", total));
    }
...
void auto_newautopilot(dbref key, void **data, int selector) {
    ...
    AUTO *autopilot = static_cast <AUTO *> (*data);
    command_node *temp;
    int i;
