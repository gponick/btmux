...
void mech_target(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(MechTarget(mech) < 0 || !(target = static_cast <MECH *>(FindObjectsData(MechTarget(mech)))), "Error: You need to be locked at something to target it's part!");
    type = MechType(target);
    move = MechMove(target);
    DOCHECK((index = ArmorSectionFromString(type, move, args[0])) < 0, "Invalid location!");
    MechAim(mech) = index;
...
void sixth_sense_check(MECH * mech, MECH * target)
{
    ...
    MECHEVENT(target, EVENT_SS, mech_ss_event, Number(1, 3), ((3 * (SSDistMod(r))) + (SSTonMod(d))));
}
...
void mech_settarget(dbref player, void *data, char *buffer)
{
    ...
    switch (argc) {
        case 1:
            ...
            if (target)
                LOS = InLineOfSight(mech, target, MechX(target), MechY(target), FlMechRange(mech_map, mech, target));
            else
                targetref = -1;
    
            DOCHECK(targetref == -1 || !LOS, "That is not a valid targetID. Try again.");

            if (MechSwarmTarget(mech) > 0) {
                if (MechSwarmTarget(mech) != target->mynum) {
                    mech_notify(mech, MECHALL, "You're a bit to busy holding on for dear life to lock a target!");
                    return;
                }
            }

            mech_notify(mech, MECHALL, tprintf("Target set to %s.", GetMechToMechID(mech, target)));
            StopLock(mech);
        ...
        case 2:
            ...
            notify(player, tprintf("Target coordinates set at (X,Y) %d, %d", newx, newy));
            StopLock(mech);
            MechStatus(mech) |= LOCK_TARGET;
#if LOCK_TICK > 0
            if (!arc_override)
        ...
        case 3:
            ...
            switch (mode) {
                case LOCK_HEX:
                    notify(player, tprintf("Target coordinates set to hex at (X,Y) %d, %d", newx, newy));
                    break;
                case LOCK_HEX_CLR:
                    notify(player, tprintf("Target coordinates set to clearing hex at (X,Y) %d, %d", newx, newy));
                    break;
                case LOCK_HEX_IGN:
                    notify(player, tprintf("Target coordinates set to igniting hex at (X,Y) %d, %d", newx, newy));
                    break;
                default:
                    notify(player, tprintf("Target coordinates set to building at (X,Y) %d, %d", newx, newy));
                    break;
            }
...
int FireWeaponNumber(dbref player, MECH * mech, MAP * mech_map, int weapnum, int argc, char **args, int sight)
{
    ...
    if (MechType(mech) == CLASS_BSUIT) {
        for (i = 0; i < NUM_BSUIT_MEMBERS; i++) {
            DOCHECK1(!SectIsDestroyed(mech, i) && MechSections(mech)[i].recycle, tprintf("Suit %d is still recovering from attack.", i + 1));
        }
    }

    if (!sight && (MechCritStatus(mech) & HIDDEN)) {
        mech_notify(mech, MECHALL, "You break out of your cover to initiate weapons fire!");
    ...
    if (!sight) {
        ...
        DOCHECK0((MechCritStatus(mech) & MECH_STUNNED), "You cannot take actions while stunned! That include finding the trigger.");
        DOCHECK0(PartTempNuke(mech, section, critical), "The weapons system chirps: 'That Weapon is still unusable - please stand by.'");
        DOCHECK0(weaptype == -3, "The weapon system chirps: 'That weapon is still reloading!'");
        DOCHECK0(weaptype == -4, "The weapon system chirps: 'That weapon is still recharging!'");

        /* New fancy message for when they try and fire a weapon and the section is busy */
        if (weaptype == -5) {

            /* Get the section name and print the message along with the time left */
            ArmorStringFromIndex(section, location, MechType(mech), MechMove(mech));
            notify(player, tprintf("%s%s is still recovering from a previous action!",
                    MechType(mech) == CLASS_BSUIT ? "" : "Your ", location));
            return 0;
        }

        DOCHECK0(MechSections(mech)[section].specials & CARRYING_CLUB, "You're carrying a club in that arm.");
        ...
        if (Fallen(mech) && MechType(mech) == CLASS_MECH) {
            ...
            if (!MechIsQuad(mech) || (MechIsQuad(mech) && wcDeadLegs > 0)) {
                ...
                switch (section) {
                    case RARM:
                        DOCHECK0(SectHasBusyWeap(mech, LARM) || MechSections(mech)[LARM].recycle || SectIsDestroyed(mech, LARM), "You currently can't use your Left Arm to prop yourself up.");
                        break;
                    case LARM:
                        DOCHECK0(SectHasBusyWeap(mech, RARM) || MechSections(mech)[RARM].recycle || SectIsDestroyed(mech, RARM), "Your currently can't use your Right Arm to prop yourself up.");
                        break;
                    default:
                        DOCHECK0((SectHasBusyWeap(mech, RARM) || MechSections(mech)[RARM].recycle || SectIsDestroyed(mech, RARM)) && (SectHasBusyWeap(mech, LARM) || MechSections(mech)[LARM].recycle || SectIsDestroyed(mech, LARM)), "You currently don't have any arms to spare to prop yourself up.");
                }
            }
        }
    }
    ...
    if (IsMechMounted(mech)) 
        DOCHECK0(((section == CTORSO) || (section == RTORSO) || (section == LTORSO)), "You can not fire torso mounted weapons while you have battlesuits on you!");

    DOCHECK0((MechDugIn(mech)) && section != TURRET, "Only turret weapons are available while in cover.");
    DOCHECK0(weaptype == -2 || (PartTempNuke(mech, section, critical) == FAIL_DESTROYED), "The weapons system chirps: 'That Weapon has been destroyed!'");
    DOCHECK0(IsAMS(weaptype), "That weapon is defensive only!");
    DOCHECK0(argc > 3, "Invalid number of arguments!");

    if ((MechWeapons[weaptype].special & IDF) && MechSpotter(mech) != -1 && MechTarget(mech) == -1 && MechTargY(mech) == -1 && MechTargX(mech) == -1)
        if (FireSpot(player, mech, mech_map, weapnum, weaptype, sight, section, critical))
            return 1;
            ...
            if (IsCoolant(weaptype) && (mode & HEAT_MODE)) {
                ...
                if (MechTarget(mech) != -1) {
                    ...
                    if (!(MechWeapons[weaptype].special & IDF)) {
                        ...
                        if (MapIsUnderground(mech_map)) {
                            DOCHECK0(!LOS, "That target is not in your direct line of sight, and you cannot fire your IDF weapons underground!");
                        }
                } else {
                    ...
                    if (mudconf.btech_idf_requires_spotter && (MechWeapons[weaptype].special & IDF) && (MechSpotter(mech) == -1))
                        DOCHECK0(!LOS, "That hex target is not in your direct line of sight and you do not have a spotter set!!");
                ...
                if (MechType(mech) != CLASS_BSUIT) {
                    ARCCHECK(mech, enemyX, enemyY, section, critical, "Default target is not in your weapons arc!");
                }
            }
            break;
        case 2:
            ...
            if (MechType(mech) != CLASS_BSUIT) {
                ARCCHECK(mech, enemyX, enemyY, section, critical, "That target is not in your weapons arc!");
            }
            break;
        case 3:
            /* Fire at the Map X Y */
            ...
            mapy = atoi(args[2]);
            ishex = 1;
            DOCHECK0(mapx < 0 || mapx >= mech_map->map_width || mapy < 0 || mapy >= mech_map->map_height, "Map coordinates out of range!");
            ...
            if (MechType(mech) != CLASS_BSUIT) {
                ARCCHECK(mech, enemyX, enemyY, section, critical, "That hex target is not in your weapons arc!");
            }
    ...
    if (tempMech) {
        DOCHECK0(IsArtillery(weaptype), "You can only target hexes with this kind of artillery.");
        DOCHECK0(MechSwarmTarget(tempMech) == mech->mynum, "You are unable to use your weapons against a 'swarmer!");
        DOCHECK0(StealthArmorActive(tempMech) && ((MechTarget(mech) != tempMech->mynum) || Locking(mech)), "You need a stable lock to fire on that target!");
        DOCHECK0(MechTeam(tempMech) == MechTeam(mech) && MechNoFriendlyFire(mech), "You can't fire on a teammate with FFSafeties on!");
        DOCHECK0(MechType(tempMech) == CLASS_MW && MechType(mech) != CLASS_MW && !MechPKiller(mech), "That's a living, breathing person! Switch off the safety first, if you really want to assassinate the target.");
    }

    FireWeapon(mech, mech_map, tempMech, LOS, weaptype, weapnum, section, critical, enemyX, enemyY, mapx, mapy, range, 1000, sight, ishex);
    return (1);
}
...
{
    CheckWeaponFailed(mech, *weapnum, *weapindx, *section, *critical, modifier, type);
    ...
    if (*type == WEAPON_JAMMED || *type == WEAPON_DUD) {
        ...
        decrement_ammunition(mech, *weapindx, *section, *critical, *ammoLoc, *ammoCrit, *ammoLoc1, *ammoCrit1, wGattlingShots);
        return 1;
    }
    
    if (*type == RANGE)
        if ((EGunRangeWithCheck(mech, *section, *weapindx) - *modifier) < range) {
            mech_notify(mech, MECHALL, "Due to weapons failure your shot fails short of its target!");
            *range_ok = 0;
        }
...
void sendC3TrackEmit(MECH * mech, dbref c3Ref, MECH * c3Mech)
{
    if (c3Mech && (c3Mech->mynum != mech->mynum)) {
        ...
        if ((pattr = atr_str("MECHNAME")) && pattr->number)
            buf = atr_get(c3Mech->mynum, pattr->number, &aowner, &aflags);
        else
            buf = "";

        mech_notify(mech, MECHALL, tprintf("Using range data from %s [%s]", buf, MechIDS(c3Mech, 1)));
    }
}
...
void FireWeapon(MECH * mech, MAP * mech_map, MECH * target, int LOS, int weapindx, int weapnum, int section, int critical, float enemyX, float enemyY, int mapx, int mapy, float range, int indirectFire, int sight, int ishex)
{
    ...
    DOCHECKMA((SectionUnderwater(mech, section) && (MechWeapons[weapindx].shortrange_water <= 0)), "This weapon may not be fired underwater.");
    ...
    DOCHECKMA((MechSwarmTarget(mech) > 0) && ((!target) || (MechSwarmTarget(mech) != target->mynum)), "You're too busy holding on for dear life!");
    ...
    if (!sight)
        if (!FindAndCheckAmmo(mech, weapindx, section, critical, &ammoLoc, &ammoCrit, &ammoLoc1, &ammoCrit1, &wGattlingShots))
            return;

/*** START: Calc BTH and Roll****/
    if (!isarty) {
        baseToHit = FindNormalBTH(mech, mech_map, section, critical, weapindx, range, target, indirectFire, &c3Ref);
        if (c3Ref) {
            c3Mech = getMech(c3Ref);
            if (c3Mech && ((MechTeam(c3Mech) != MechTeam(mech)) || (c3Ref == mech->mynum))) 
                c3Mech = NULL;
        }
    } else
    ...
    if ((MechWeapons[weapindx].special & DFM) || ((MechWeapons[weapindx].special & ELRM) && range < MechWeapons[weapindx].min)) {
        r1 = Number(1, 6);
        r2 = Number(1, 6);
        r3 = Number(1, 6);
...
/*** START: Do all the necessary emits for sighting and firing ***/
    if (target && !ishex) {
        ...
        strcpy(buf2, "");
        if (MechAim(mech) != NUM_SECTIONS && MechAimType(mech) == MechType(target) && !IsMissile(weapindx)) {
            ArmorStringFromIndex(MechAim(mech), buf3, MechType(target), MechMove(target));
            sprintf(buf2, "'s %s", buf3);
        }

        if (sight) {
            DOCHECKMA(baseToHit >= 900, tprintf("You aim %s at %s%s - Out of range.", &MechWeapons[weapindx].name[3], GetMechToMechID(mech, target), buf2));
            sendC3TrackEmit(mech, c3Ref, c3Mech);
            mech_notify(mech, MECHALL, tprintf("You aim %s at %s%s - BTH: %d %s", &MechWeapons[weapindx].name[3], GetMechToMechID(mech, target), buf2, baseToHit, MechStatus(target) & PARTIAL_COVER ?  "(Partial cover)" : ""));
            return;
        }
        
        if (baseToHit > 12) {
            DOCHECKMA(baseToHit >= 900, tprintf("Fire %s at %s%s - Out of range.", &MechWeapons[weapindx].name[3], GetMechToMechID(mech, target), buf2));
            mech_notify(mech, MECHALL, tprintf("Fire %s at %s%s - BTH: %d  Roll: Impossible! %s", &MechWeapons[weapindx].name[3], GetMechToMechID(mech, target), buf2, baseToHit, MechStatus(target) & PARTIAL_COVER ?  "(Partial cover)" : ""));
            return;
        }
    } else {
        ...
        if (sight) {
            if (baseToHit > 900)
                mech_notify(mech, MECHPILOT, tprintf("You aim your %s at (%d,%d) - Out of Range.", &MechWeapons[weapindx].name[3], mapx, mapy));
            else {
                sendC3TrackEmit(mech, c3Ref, c3Mech);
                mech_notify(mech, MECHPILOT, tprintf("You aim your %s at (%d,%d) - BTH: %d", &MechWeapons[weapindx].name[3], mapx, mapy, baseToHit));
            }
            return;
        }
    
        if (!isarty && baseToHit > 12) {
            mech_notify(mech, MECHALL, tprintf("Fire %s at (%d,%d) - BTH: %d  Roll: Impossible!", &MechWeapons[weapindx].name[3], mapx, mapy, baseToHit));
            return;
        }
    }
    ...
    if (target && !ishex) {
        sendC3TrackEmit(mech, c3Ref, c3Mech);
        mech_notify(mech, MECHALL, tprintf("You fire %s at %s%s - BTH: %d  %s%s", &MechWeapons[weapindx].name[3], GetMechToMechID(mech, target), buf2, baseToHit, buf, MechStatus(target) & PARTIAL_COVER ?  "(Partial cover)" : ""));
        SendAttacks(tprintf("#%i attacks #%i (weapon) (%i/%i)", mech->mynum, target->mynum, baseToHit, roll));

    /* If the target has the ATTACKEMIT_MECH flag on have it
     * output this info as well
     */
        if (MechStatus2(target) & ATTACKEMIT_MECH)
            SendAttackEmits(tprintf("#%i attacks #%i (weapon) (%i/%i)", mech->mynum, target->mynum, baseToHit, roll));
    } else {
        sendC3TrackEmit(mech, c3Ref, c3Mech);
        mech_notify(mech, MECHALL, tprintf("You fire %s %s (%d,%d) - BTH: %d  %s", &MechWeapons[weapindx].name[3], hex_target_id(mech), mapx, mapy, baseToHit, buf));
        SendAttacks(tprintf("#%i attacks %d,%d (%s) (weapon) (%i/%i)", mech->mynum, mapx, mapy, short_hextarget(mech), baseToHit, roll));
            ...
            for (foo = 0; foo < mech_map->first_free; foo++) {
                if (mech_map->mechsOnMap[foo] >= 0) {
                    ...
                    if (MechStatus2(tmpmech) & ATTACKEMIT_MECH)
                        SendAttackEmits(tprintf("#%i attacks %d,%d (%s) (weapon)" " (%i/%i)", mech->mynum, mapx, mapy, short_hextarget(mech), baseToHit, roll));
                }
            }
        }
    }
...
/*** END: Do all the necessary emits for sighting and firing ***/
    ...
    if (weapon_failure_stuff(mech, &weapnum, &weapindx, &section, &critical, &ammoLoc, &ammoCrit, &ammoLoc1, &ammoCrit1, &modifier, &type, range, &range_ok, wGattlingShots))
        return;
    ...
    if (GetPartFireMode(mech, section, critical) & RFAC_MODE) 
        if (roll == 2) {
            mech_notify(mech, MECHALL, tprintf("%%ch%%crA catastrophic misload on your %s destroys it and causes an internal explosion!%%cn", &(MechWeapons[weapindx].name[3])));
            firstCrit = FindFirstWeaponCrit(mech, section, -1, 0, I2Weapon(weapindx), GetWeaponCrits(mech, weapindx));
            DestroyWeapon(mech, section, I2Weapon(weapindx), firstCrit, GetWeaponCrits(mech, weapindx), GetWeaponCrits(mech, weapindx));
            MechLOSBroadcast(mech, "shudders from an internal explosion!");
            DamageMech(mech, mech, 0, -1, section, 0, 0, 0, MechWeapons[weapindx].damage, -1, 0, -1, 0, 1);
            decrement_ammunition(mech, weapindx, section, critical, ammoLoc, ammoCrit, ammoLoc1, ammoCrit1, wGattlingShots);
            return;
        } else 
            if (roll < 5) {
                mech_notify(mech, MECHALL, tprintf("%%ch%%crThe ammo loader mechanism jams on your %s!%%cn", &(MechWeapons[weapindx].name[3])));
    ...
    if (MechWeapons[weapindx].special & RAC) 
        if (((GetPartFireMode(mech, section, critical) & RAC_TWOSHOT_MODE) && (roll == 2)) || ((GetPartFireMode(mech, section, critical) & RAC_FOURSHOT_MODE) && (roll <= 3)) || ((GetPartFireMode(mech, section, critical) & RAC_SIXSHOT_MODE) && (roll <= 4))) {
            mech_notify(mech, MECHALL, tprintf("%%ch%%crThe ammo loader mechanism jams on your %s!%%cn", &(MechWeapons[weapindx].name[3])));
            SetPartTempNuke(mech, section, critical, FAIL_AMMOJAMMED);
            return;
        }
    ...
    if (GetPartFireMode(mech, section, critical) & ULTRA_MODE) 
        if (roll == 2) {
            mech_notify(mech, MECHALL, tprintf("The loader jams on your %s, destroying it!", &(MechWeapons[weapindx].name[3])));
            firstCrit = FindFirstWeaponCrit(mech, section, -1, 0, I2Weapon(weapindx), GetWeaponCrits(mech, weapindx));
            DestroyWeapon(mech, section, I2Weapon(weapindx), firstCrit, GetWeaponCrits(mech, weapindx), GetWeaponCrits(mech, weapindx));
            return;
        }
    ...
    if (canWeapExplodeFromDamage(mech, section, critical, roll)) {
        if (IsEnergy(weapindx)) 
            mech_notify(mech, MECHALL, tprintf("%%ch%%crThe damaged charging crystal on your %s overloads!%%cn", &(MechWeapons[weapindx].name[3])));
        else {
            mech_notify(mech, MECHALL, tprintf("%%ch%%crThe damaged ammo feed on your %s triggers an internal explosion!%%cn", &(MechWeapons[weapindx].name[3])));
            decrement_ammunition(mech, weapindx, section, critical, ammoLoc, ammoCrit, ammoLoc1, ammoCrit1, wGattlingShots);
        }
    ...
    if (OODing(mech)) 
        if (MechZ(mech) > MechElevation(mech)) 
            if (MechJumpSpeed(mech) >= MP1) {
                mech_notify(mech, MECHALL, "You initiate your jumpjets to compensate for the opened cocoon!");
                MechCocoon(mech) = -1;
            } else {
                mech_notify(mech, MECHALL, "Your action splits open the cocoon - have a nice fall!");
    ...
    if (!isarty)
        MechFireBroadcast(mech, ishex ? NULL : target, mapx, mapy, mech_map, &MechWeapons[weapindx].name[3], (roll >= baseToHit) && range_ok);

    /* Tell our target they were just shot at... */
    if (target) 
        if (InLineOfSight(target, mech, MechX(mech), MechY(mech), range))
            mech_notify(target, MECHALL, tprintf("%s has fired a %s at you!", GetMechToMechID(target, mech), &MechWeapons[weapindx].name[3]));
        else
            mech_notify(target, MECHALL, tprintf("Something has fired a %s at you from bearing %d!", &MechWeapons[weapindx].name[3], FindBearing(MechFX(target), MechFY(target), MechFX(mech), MechFY(mech))));

    /* Time to decide if we've really hit them and wot to do */
    MechStatus(mech) |= FIRED;

    if (isarty) 
        artillery_shoot(mech, mapx, mapy, weapindx, GetPartAmmoMode(mech, section, critical), baseToHit <= roll);
    else 
        if (range_ok) 
            if (IsMissile(weapindx)) 
                HitTarget(mech, weapindx, section, critical, target, mapx, mapy, LOS, type, modifier, roll >= baseToHit && range_ok, baseToHit, wGattlingShots, tIsSwarmAttack);
            else
                if (roll >= baseToHit)
                    HitTarget(mech, weapindx, section, critical, target, mapx, mapy, LOS, type, modifier, 1, baseToHit, wGattlingShots, tIsSwarmAttack);
                else {
                    int tTryClear = 1;
                    if (target) 
                        if ((MechType(target) == CLASS_BSUIT) && (MechSwarmTarget(target) > -1) && (altTarget = getMech(MechSwarmTarget(target)))) {

                            baseToHit = FindNormalBTH(mech, mech_map, section, critical, weapindx, range, altTarget, indirectFire, &c3Ref);

                            if (roll >= baseToHit) {
                                mech_notify(altTarget, MECHALL, "The shot hits you instead!");
                                MechLOSBroadcast(altTarget, "manages to get in the way of the shot instead!");
                                HitTarget(mech, weapindx, section, critical, altTarget, mapx, mapy, LOS, type, modifier, 1, baseToHit, wGattlingShots, tIsSwarmAttack);

                                tTryClear = 0;
                            } else 
                                if (Elevation(mech_map, MechX(target), MechY(target)) < (MechZ(target) - 2))
                                    tTryClear = 0;
                        }
                    if (tTryClear) {
                        int tempDamage = determineDamageFromHit(mech, section, critical, target, mapx, mapy, weapindx, wGattlingShots, MechWeapons[weapindx].damage, GetPartAmmoMode(mech, section, critical), type, modifier, 1);
                        possibly_ignite_or_clear(mech, weapindx, GetPartAmmoMode(mech, section, critical), tempDamage, mapx, mapy, 0);
                    }
                }
    ...
    if (GetPartFireMode(mech, section, critical) & GATTLING_MODE) 
        ...
        if (MechWeapons[weapindx].special & RAC) {
            if (GetPartFireMode(mech, section, critical) & RAC_TWOSHOT_MODE)
                ...
                if (GetPartFireMode(mech, section, critical) & RAC_FOURSHOT_MODE)
                    wRACHeat = 4;
                else 
                    if (GetPartFireMode(mech, section, critical) & RAC_SIXSHOT_MODE)
                        wRACHeat = 6;
                    else
                        wRACHeat = 1;
            ...
            if ((GetPartFireMode(mech, section, critical) & ULTRA_MODE) || (GetPartFireMode(mech, section, critical) & RFAC_MODE)) {
                if (type == HEAT)
                    MechWeapHeat(mech) += (float) modifier;
                MechWeapHeat(mech) += (float) MechWeapons[weapindx].heat;
            }
...
/*** END: Set the heat after firing ***/
    ...
    if ((MechWeapons[weapindx].special & HVYGAUSS) && (MechType(mech) == CLASS_MECH)) 
        if (abs((int)MechSpeed(mech)) > 0.0) {
            mech_notify(mech, MECHALL, "You realize that moving while firing this weapon may not be a good idea afterall.");
            if (MechTons(mech) <= 35)
                wHGRPSkillMod = 2;
            else 
                if (MechTons(mech) <= 55)
            ...
            if (!MadePilotSkillRoll(mech, wHGRPSkillMod)) {
                mech_notify(mech, MECHALL, "The weapon's recoil knocks you to the ground!");
                MechLOSBroadcast(mech, tprintf("topples over from the %s's recoil!", &MechWeapons[weapindx].name[3]));
                MechFalls(mech, 1, 0);
            }
        }
}
...
int determineDamageFromHit(MECH * mech, int wSection, int wCritSlot, MECH * hitMech, int hitX, int hitY, int weapindx, int wGattlingShots, int wBaseWeapDamage, int wAmmoMode, int type, int modifier, int isTempCalc)
{
    ...
    if (MechWeapons[weapindx].special & HVYGAUSS) 
        if (fRange > MechWeapons[weapindx].medrange)
            ...
            if (fRange > MechWeapons[weapindx].shortrange)
                wWeapDamage = 20;
    ...
    if (hitMech) 
        if (wAmmoMode & AC_FLECHETTE_MODE) {
            if (MechType(hitMech) == CLASS_MW) 
                ...
                if (MechType(hitMech) != CLASS_BSUIT)
                    wWeapDamage /= 2;
    ...
    if (btechconf.btech_moddamagewithrange && IsEnergy(weapindx)) 
        if (fRange <= 1.0)
            wWeapDamage++;
        else 
            if (SectionUnderwater(mech, wSection)) {
                if (fRange > MechWeapons[weapindx].longrange_water)
                    ...
                    if (fRange > MechWeapons[weapindx].medrange_water)
                        wWeapDamage--;
            } else 
    ...
    if (hitMech && !isTempCalc) 
        if (btechconf.btech_moddamagewithwoods && IsForestHex(mech_map, MechX(hitMech), MechY(hitMech)) && ((MechZ(hitMech) - 2) <= Elevation(mech_map, MechX(hitMech), MechY(hitMech)))) {
            wClearDamage = wWeapDamage;

            if (GetRTerrain(mech_map, MechX(hitMech), MechY(hitMech)) == LIGHT_FOREST)
                wWeapDamage -= 2;
            else 
                if (GetRTerrain(mech_map, MechX(hitMech), MechY(hitMech)) == HEAVY_FOREST)
                    wWeapDamage -= 4;
            ...
            possibly_ignite_or_clear(mech, weapindx, wAmmoMode, wClearDamage, MechX(hitMech), MechY(hitMech), 1);
        }
...
void HitTarget(MECH * mech, int weapindx, int wSection, int wCritSlot, MECH * hitMech, int hitX, int hitY, int LOS, int type, int modifier, int reallyhit, int bth, int wGattlingShots, int tIsSwarmAttack)
{
    ...
    int tFoundRACFake = 0;
    int tUsingTC = ((wFireMode & ON_TC) && !IsArtillery(weapindx) && !IsMissile(weapindx) && (!(MechCritStatus(mech) & TC_DESTROYED)) && ((MechAim(mech) != NUM_SECTIONS) && hitMech && (MechAimType(mech) == MechType(hitMech))));

    if (hitMech) 
        /* Check to see if we're aiming at a particular location. Swarm attacks can't aim. */
        if ((MechAim(mech) != NUM_SECTIONS) && hitMech && Immobile(hitMech) && !tIsSwarmAttack) {
            roll = Roll();
    ...
    if (!IsMissile(weapindx))
        wWeapDamage = determineDamageFromHit(mech, wSection, wCritSlot, hitMech, hitX, hitY, weapindx, wGattlingShots, wBaseWeapDamage, wAmmoMode, type, modifier, 0);

    /* * Ok, if we're not an artillery weapon or missile and we're not in
     * LBX, RAC, Ultra or RFAC mode...  */
    if (!IsArtillery(weapindx) && !IsMissile(weapindx) && !tIsUltra && !tIsLBX & !tIsRAC) {
        if (hitMech) {
            if ((MechWeapons[weapindx].special & CHEAT) && (GetPartFireMode(mech, wSection, wCritSlot) & HEAT_MODE)) {
                mech_notify(hitMech, MECHALL, "%cy%chThe flaming plasma sprays all over you!%cn");
                mech_notify(mech, MECHALL, "%cgYou cover your target in flaming plasma!%cn");
            ...
            DamageMech(hitMech, mech, LOS, GunPilot(mech), hitloc, isrear, iscritical, pc_to_dam_conversion(hitMech, weapindx, wWeapDamage), 0, weapindx, bth, weapindx, wAmmoMode, tIsSwarmAttack);
        } else {
    ...
    if (tIsRAC) {
        if (GetPartFireMode(mech, wSection, wCritSlot) & RAC_TWOSHOT_MODE)
            ...
            if (GetPartFireMode(mech, wSection, wCritSlot) & RAC_FOURSHOT_MODE)
                strcpy(strMissileFakeName, "IS.SRM-4");
            else 
                if (GetPartFireMode(mech, wSection, wCritSlot) & RAC_SIXSHOT_MODE)
    ...
    if (IsMissile(weapindx)) {
        if (tIsSwarm && hitMech)    /* No swarms on hex hits */
            SwarmHitTarget(mech, weapindx, wSection, wCritSlot, hitMech, LOS, bth, reallyhit ? bth + 1 : bth - 1, (type == CRAZY_MISSILES) ? MissileHitTable[loop].  num_missiles[10] * modifier / 100 : MissileHitTable[loop].num_missiles[10], (GetPartAmmoMode(mech, wSection, wCritSlot) & SWARM1_MODE), tIsSwarmAttack);
        else
            MissileHitTarget(mech, weapindx, wSection, wCritSlot, hitMech, hitX, hitY, LOS ? 1 : 0, bth, reallyhit ? bth + 1 : bth - 1, (type == CRAZY_MISSILES) ? MissileHitTable[loop].  num_missiles[10] * modifier / 100 : MissileHitTable[loop].num_missiles[10], tIsSwarmAttack);
        return;
    }
     ...
     */
    if (LOS)
        mech_notify(mech, MECHALL, tprintf("%%cgYou hit with %d %s%s!%%c", num_missiles_hit, (tIsUltra || tIsRAC ? "slug" : tIsLBX ? "pellet" : "missile"), (num_missiles_hit > 1 ? "s" : "")));

    if (tIsLBX)
        Missile_Hit(mech, hitMech, hitX, hitY, isrear, iscritical, weapindx, wFireMode, wAmmoMode, num_missiles_hit, tIsLBX ? 1 : wWeapDamage, Clustersize(weapindx), LOS, bth, tIsSwarmAttack);
    else {
...
/* intentional:
    if ignite attack hits, roll 2D6 and consult table:
        (Success Numbers) Weapon Type:
            Flamer 4+, Incendiary LRMs 5+, Energy Weapon (minus small lasers) 7+,
            Missile or Ballistic (minus GR or SRM2s) 9+

Modifiers for terrain to those bths are as follows:
    Woods/Light Buildings no bth mod, Med Bldg +1, Heavy Bldg +2, Hardened Building +3
...
void possibly_ignite(MECH * mech, MAP * map, int weapindx, int ammoMode, int x, int y, int intentional)
{
    ...
    if (!strcmp(&MechWeapons[weapindx].name[3], "Flamer") || !strcmp(&MechWeapons[weapindx].name[3], "HeavyFlamer"))
        bth = 4;
    else 
        if (IsMissile(weapindx) && (ammoMode & INFERNO_MODE))
            bth = 5;
        else 
            if (IsBallistic(weapindx) && (ammoMode & AC_FLECHETTE_MODE))
                bth = 5;
            else 
                if (IsEnergy(weapindx) && canWeaponIgnite(weapindx))
                    bth = 5;
                else 
                    if ((IsMissile(weapindx) || IsBallistic(weapindx)) && canWeaponIgnite(weapindx))
                        bth = 9;
...
void possibly_ignite_or_clear(MECH * mech, int weapindx, int ammoMode, int damage, int x, int y, int intentional)
{
    ...
    r = Roll();
    map = static_cast <MAP *>(FindObjectsData(mech->mapindex));
