    ...
    int opt, int max, int mode, char *tbuf)
{
    ...
    if (!tp) {
        if (!mode)
            ...
            strcpy(tbuf, tprintf("#-1 ERROR: NO UNITS WITH FLAG %d FOUND", types));
        return;
    }
    ...
    if (mode == 0) {
        addline();
        sim(tprintf("Mechs/Aeros/Whatever made for %s (%d tons)", side_names[table], otons), CM_ONE | CM_CENTER);
        addline();
    
        if (!count)
            vsi("No mechs created? (Too low tonnage?)");
        else {
            vsi(tprintf("%-4s %-10s %-20s %-6s %-s", "Tons", "Ref", "Name", "Chance", "Count"));
            for (i = 0; i < STACK_SIZE; i++)
                if (mech_stack[i][1]) {
                    d = find_mechname_by_mechref(Table(i, name));
                    vsi(tprintf("%4d %-10s %-20s %6.2f %d", Table(i, tons), Table(i, name), d ? d : "Unknown", (float) 100.0 * mech_stack[i][0] / tp, mech_stack[i][1]));
                }
        
            addline();
            vsi(tprintf("Avg weight: %.2f Total tons: %d", (float) ftons / count, ftons));
        }
...
void debug_makemechs(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK((table = compare_array(side_names_short, args[0])) < 0, "Invalid faction name!");
...
FUNCTION(fun_btmakemechs)
{
    ...
    FUNCHECK((table = compare_array(side_names_short, fargs[0])) < 0, "#-1 Invalid faction name!");
    ...
    if (nfargs > 2) {
        ...
        if (nfargs > 3) {
            ...
            if (nfargs > 4)
                FUNCHECK((max = atoi(fargs[4])) < 5, "#-1 Invalid MaxDifference!");
        }
    }

    mech_list_maker(executor, table, types, tons, opt, max, 1, buf);
