...
void muxevent_add(int time, int flags, int type, void (*func) (EVENT *), void *data, void *data2)
{
    EVENT *e;
    ...
    if (type > last_muxevent_type) {
        muxevent_first_in_type = static_cast <EVENT **>(realloc(muxevent_first_in_type, sizeof(EVENT *) * (type + 1)));
    ...
    ADD_TO_BIDIR_LIST_HEAD(muxevent_first_in_type[type], prev_in_type, next_in_type, e);
...
static void muxevent_delete(EVENT * e, int fp)
{
    ...
    REMOVE_FROM_BIDIR_LIST(muxevent_first_in_type[(int) e->type], prev_in_type, next_in_type, e);
