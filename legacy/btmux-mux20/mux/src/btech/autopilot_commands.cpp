...
void auto_cal_mapindex(MECH * mech) {
    ...
    if (MechAuto(mech) > 0) {
        if (!(autopilot = static_cast <AUTO *> (FindObjectsData(MechAuto(mech)))) || 
...
void auto_dumbfollow_event(EVENT *muxevent) {
    ...
    x = (int)(autopilot->ofsy * cos(TWOPIOVER360 * (270.0 + (h + autopilot->ofsx))));
    y = (int)(autopilot->ofsy * sin(TWOPIOVER360 * (270.0 + (h + autopilot->ofsx))));
    tx = MechX(leader) + x;
    ty = MechY(leader) + y;
