...
MECH *getOtherMechInNetwork(MECH * mech, int wIdx, int tCheckECM, int tCheckStarted, int tCheckUncon, int tIsC3)
{
    MECH *tempMech;
    dbref refOtherMech;
    int networkSize;
    ...
    refOtherMech = (tIsC3 ? MechC3NetworkElem(mech, wIdx) : MechC3iNetworkElem(mech, wIdx));
...
void buildTempNetwork(MECH * mech, dbref * myNetwork, int *networkSize, int tCheckECM, int tCheckStarted, int tCheckUncon, int tIsC3)
{
    ...
    *networkSize = 0;
    ...
    baseNetworkSize = (tIsC3 ? MechC3NetworkSize(mech) : MechC3iNetworkSize(mech));
    ...
    for (i = 0; i < baseNetworkSize; i++) {
        otherMech = getOtherMechInNetwork(mech, i, tCheckECM, tCheckStarted, tCheckUncon, tIsC3);
        if (!otherMech)
            continue;
...
void showNetworkTargets(dbref player, MECH * mech, int tIsC3)
{
    ...
    for (i = 0; i < objMap->first_free; i++) {
        ...
        losFlag = InLineOfSight(mech, otherMech, MechX(otherMech), MechY(otherMech), realRange);

        /* * If we do see them, let's make sure it's not just a 'something' */
        if (losFlag) {
            if (InLineOfSight_NB(mech, otherMech, MechX(otherMech), MechY(otherMech), 0.0))
        ...
        c3Range = findC3RangeWithNetwork(mech, otherMech, realRange, myNetwork, networkSize, &c3Ref);
        ...
        if ((wSeeTarget != TARG_LOS_CLEAR) && (wC3SeeTarget != TARG_LOS_CLEAR)) {
            ...
            if ((pattr = atr_str("MECHNAME")) && pattr->number)
                mech_name = atr_get(otherMech->mynum, pattr->number, &aowner, &aflags);
            else
                mech_name = "";
        }

        bearing = FindBearing(MechFX(mech), MechFY(mech), MechFX(otherMech), MechFY(otherMech));
        ...
        sprintf(buff, "%s%c%c%c[%s]%c %-11.11s x:%3d y:%3d z:%3d r:%4.1f c:%4.1f b:%3d s:%5.1f h:%3d S:%c%c%c%c%c%s",
                    otherMech->mynum == MechTarget(mech) ? "%ch%cr" : 
                    (tShowStatusInfo && !MechSeemsFriend(mech, otherMech)) ? "%ch%cy" : "", 
                    (losFlag & MECHLOSFLAG_SEESP) ? 'P' : ' ', (losFlag & MECHLOSFLAG_SEESS) ? 'S' : ' ', weaponarc, 
                    MechIDS(otherMech, MechSeemsFriend(mech, otherMech) || !tShowStatusInfo), 
                    move_type[0], mech_name, MechX(otherMech), MechY(otherMech), MechZ(otherMech), realRange, c3Range, 
                    bearing, MechSpeed(otherMech), MechVFacing(otherMech), cStatus1, cStatus2, cStatus3, cStatus4, cStatus5, 
                    (otherMech->mynum == MechTarget(mech) || !MechSeemsFriend(mech, otherMech)) ? "%c" : "");
...
void showNetworkData(dbref player, MECH * mech, int tIsC3)
{
    ...
    for (i = 0; i < networkSize; i++) {
        ...
        sprintf(buff, "%%ch%%cy[%s]%c %-12.12s x:%3d y:%3d z:%3d r:%4.1f b:%3d s:%5.1f h:%3d a: %3d i: %3d%%cn",
                    MechIDS(otherMech, 1), move_type[0], mech_name, MechX(otherMech), MechY(otherMech), MechZ(otherMech), range, bearing, 
                    MechSpeed(otherMech), MechVFacing(otherMech), getRemainingArmorPercent(otherMech), getRemainingInternalPercent(otherMech));
...
int mechSeenByNetwork(MECH * mech, MECH * mechTarget, int tIsC3)
{
    ...
    for (i = 0; i < networkSize; i++) {
        ...
        los = InLineOfSight(otherMech, mechTarget, MechX(mechTarget), MechY(mechTarget), range);

        if (los) {
            if (!InLineOfSight_NB(otherMech, mechTarget, MechX(mechTarget), MechY(mechTarget), range))
...
float findC3Range(MECH * mech, MECH * mechTarget, float realRange, dbref * c3Ref, int tIsC3)
{
    int networkSize;
    dbref myNetwork[C3_NETWORK_SIZE];
...
float findC3RangeWithNetwork(MECH * mech, MECH * mechTarget, float realRange, dbref * myNetwork, int networkSize, dbref * c3Ref)
{
    float c3Range = 0.0;
    float bestRange = 0.0;
    int i;
    ...
    *c3Ref = 0;
    ...
    for (i = 0; i < networkSize; i++) {
        ...
        if (mechTarget) {
            ...
            debugC3(tprintf("C3RANGE-NETWORK (mech): Finding range from %d to %d.", mech->mynum, mechTarget->mynum));

            c3Range = FaMechRange(otherMech, mechTarget);
            inLOS = InLineOfSight(otherMech, mechTarget, MechX(mechTarget), MechY(mechTarget), c3Range);
        } else 
            if ((MechTargX(mech) > 0) && (MechTargY(mech) > 0)) {
                ...
                debugC3(tprintf("C3RANGE-NETWORK (hex): Finding range from %d to %d %d.", mech->mynum, mapX, mapY));

                MechTargZ(mech) = Elevation(map, mapX, mapY);
                hexZ = ZSCALE * MechTargZ(mech);
                MapCoordToRealCoord(mapX, mapY, &hexX, &hexY);

                c3Range = FindRange(MechFX(otherMech), MechFY(otherMech), MechFZ(otherMech), hexX, hexY, hexZ);
