...
// why isnt it finding this in p.event.h via btmacros.h ?
...
/* Toggles ECM on / off */
void mech_ecm(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast <MECH *>(data);

    cch(MECH_USUALO);
    DOCHECK(MechCritStatus(mech) & ECM_DESTROYED, "Your Guardian ECM has been destroyed already!");
    TOGGLE_SPECIALS_MACRO_CHECK(ECM_TECH, ECM_ENABLED, ECCM_ENABLED, "You turn your ECM suite online (ECM mode).", "You turn your ECM suite offline.", "This unit isn't equipped with an ECM suite!");
    MarkForLOSUpdate(mech);
}

void mech_eccm(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast <MECH *>(data);

    cch(MECH_USUALO);
    DOCHECK(MechCritStatus(mech) & ECM_DESTROYED, "Your Guardian ECM has been destroyed already!");
    TOGGLE_SPECIALS_MACRO_CHECK(ECM_TECH, ECCM_ENABLED, ECM_ENABLED, "You turn your ECM suite online (ECCM mode).", "You turn your ECM suite offline.", "This unit isn't equipped with an ECM suite!");
    MarkForLOSUpdate(mech);
}

void mech_perecm(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast <MECH *>(data);

    cch(MECH_USUALO);
    TOGGLE_INFANTRY_MACRO_CHECK(FC_INFILTRATORII_STEALTH_TECH, PER_ECM_ENABLED, PER_ECCM_ENABLED, "You turn your Personal ECM suite online (ECM mode).", "You turn your Personal ECM suite offline.", "This unit isn't equipped with a Personal ECM suite!");
    MarkForLOSUpdate(mech);
}

void mech_pereccm(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast <MECH *>(data);

    cch(MECH_USUALO);
    TOGGLE_INFANTRY_MACRO_CHECK(FC_INFILTRATORII_STEALTH_TECH, PER_ECCM_ENABLED, PER_ECM_ENABLED, "You turn your Personal ECM suite online (ECCM mode).", "You turn your Personal ECM suite offline.", "This unit isn't equipped with a Personal ECM suite!");
    MarkForLOSUpdate(mech);
}

void mech_angelecm(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast <MECH *>(data);

    cch(MECH_USUALO);
    DOCHECK(MechCritStatus(mech) & ANGEL_ECM_DESTROYED, "Your Angel ECM has been destroyed already!");
    TOGGLE_SPECIALS_MACRO_CHECK2(ANGEL_ECM_TECH, ANGEL_ECM_ENABLED, ANGEL_ECCM_ENABLED, "You turn your Angel ECM suite online (ECM mode).", "You turn your Angel ECM suite offline.", "This unit isn't equipped with an Angel ECM suite!");
    MarkForLOSUpdate(mech);
}

void mech_angeleccm(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast <MECH *>(data);

    cch(MECH_USUALO);
    DOCHECK(MechCritStatus(mech) & ANGEL_ECM_DESTROYED, "Your Angel ECM has been destroyed already!");
    TOGGLE_SPECIALS_MACRO_CHECK2(ANGEL_ECM_TECH, ANGEL_ECCM_ENABLED, ANGEL_ECM_ENABLED, "You turn your Angel ECM suite online (ECCM mode).", "You turn your Angel ECM suite offline.", "This unit isn't equipped with an Angel ECM suite!");
    MarkForLOSUpdate(mech);
}
...
void mech_slite(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    DOCHECK(MechCritStatus(mech) & SLITE_DEST, "Your searchlight has been destroyed already!");

    if (SearchlightChanging(mech)) {
        if (MechStatus2(mech) & SLITE_ON)
            mech_notify(mech, MECHALL, "Your searchlight is already in the process of turning off.");
        else
            mech_notify(mech, MECHALL, "Your searchlight is already in the process of turning on.");
...
void mech_ams(dbref player, void *data, char *buffer)
{
    ...
    SILLY_TOGGLE_MACRO(IS_ANTI_MISSILE_TECH | CL_ANTI_MISSILE_TECH, AMS_ENABLED, "Anti-Missile System turned ON", "Anti-Missile System turned OFF", "This mech is not equipped with AMS");
}
...
void mech_fliparms(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALMO);
    DOCHECK(Fallen(mech), "You're using your arms to support yourself. Try flipping something else.");
    SILLY_TOGGLE_MACRO(FLIPABLE_ARMS, FLIPPED_ARMS, "Arms have been flipped to BACKWARD position", "Arms have been flipped to FORWARD position", "You cannot flip the arms in this mech");
}
...
static int mech_toggle_mode_sub_func(MECH * mech, dbref player, int index, int high)
{
    int section, critical, weaptype;

    weaptype = FindWeaponNumberOnMech_Advanced(mech, index, &section, &critical, 0);
    ...
    DOCHECK0(GetPartFireMode(mech, section, critical) & OS_MODE, "One-shot weapons' mode cannot be altered!");
    DOCHECK0(isWeapAmmoFeedLocked(mech, section, critical), "That weapon's ammo feed mechanism is damaged!");

    if (temp_nspec == -1)
    DOCHECK0(!FindArtemisForWeapon(mech, section, critical), "You have no Artemis system for that weapon.");

    weaptype = Weapon2I(GetPartType(mech, section, critical));

    DOCHECK0(MechWeapons[weaptype].special & ROCKET, "Rocket launchers' mode cannot be altered!");

    if ((temp_nspecisspec == 5 && (MechWeapons[weaptype].type == TAMMO) && !(MechWeapons[weaptype].special & temp_nspec)) ||
            (temp_nspecisspec == 4 && (MechWeapons[weaptype].type == TMISSILE) && !(MechWeapons[weaptype].type & (IDF | DAR))) ||
            (temp_nspecisspec == 2 && (MechWeapons[weaptype].special & IDF) && !(MechWeapons[weaptype].special & DAR)) ||
            (temp_nspecisspec == 1 && temp_nspec && (MechWeapons[weaptype].special & temp_nspec)) ||
            (temp_nspecisspec <= 0 && temp_nspec && (MechWeapons[weaptype].type == temp_nspec && !(MechWeapons[weaptype].special & NARC)))) {
        if (temp_nspecisspec == 0 && (temp_nspec & TARTILLERY))
            DOCHECK0((GetPartAmmoMode(mech, section, critical) & ARTILLERY_MODES) && !(GetPartAmmoMode(mech, section, critical) & temp_mode), "That weapon has already been set to fire special rounds!");
...
void mech_flamerheat(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALMO);
    mech_toggle_mode_sub(player, mech, buffer, 1, CHEAT, HEAT_MODE, 1, "Weapon %d has been set to HEAT mode", "Weapon %d has been set to normal mode", "That weapon cannot be set HEAT!");
}
...
void mech_ultra(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALMO);
    mech_toggle_mode_sub(player, mech, buffer, 1, ULTRA, ULTRA_MODE, 1, "Weapon %d has been set to ultra fire mode", "Weapon %d has been set to normal fire mode", "That weapon cannot be set ULTRA!");
}
...
void mech_inarc_ammo_toggle(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(wWeapType == -1, "The weapons system chirps: 'Illegal Weapon Number!'");
    DOCHECK(wWeapType == -2, "The weapons system chirps: 'That Weapon has been destroyed!'");
    DOCHECK(wWeapType == -3, "The weapon system chirps: 'That weapon is still reloading!'");
    DOCHECK(wWeapType == -4, "The weapon system chirps: 'That weapon is still recharging!'");
    DOCHECK(!(MechWeapons[wWeapType].special & INARC), "The weapon system chirps: 'That weapon is not an iNARC launcher!'");
    DOCHECK(isWeapAmmoFeedLocked(mech, wSection, wCritSlot), "That weapon's ammo feed mechanism is damaged!");
    ...
    mech_notify(mech, MECHALL, tprintf("Weapon %d set to fire iNARC %s pods.", wWeapNum, strMode));
}
...
void mech_rac(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(wWeapType == -3, "The weapon system chirps: 'That weapon is still reloading!'");
    DOCHECK(wWeapType == -4, "The weapon system chirps: 'That weapon is still recharging!'");
    DOCHECK(!(MechWeapons[wWeapType].special & RAC), "The weapon system chirps: 'That weapon is not an Rotary AC!'");

    /* Change our modes... */
    GetPartFireMode(mech, wSection, wCritSlot) &= ~FIRE_MODES;
    GetPartFireMode(mech, wSection, wCritSlot) &= ~(RAC_TWOSHOT_MODE | RAC_FOURSHOT_MODE | RAC_SIXSHOT_MODE);
    ...
    mech_notify(mech, MECHALL, tprintf("Weapon %d set to fire %s at a time.", wWeapNum, strMode));
}
...
static int mech_unjamammo_func(MECH * mech, dbref player, int index, int high)
{
    ...
    DOCHECK0(weaptype == -1, "The weapons system chirps: 'Illegal Weapon Number!'");
    DOCHECK0(weaptype == -2, "The weapons system chirps: 'That Weapon has been destroyed!'");
    DOCHECK0(PartTempNuke(mech, section, critical) != FAIL_AMMOJAMMED, "The ammo feed mechanism for that weapon is not jammed.");
    DOCHECK0(Jumping(mech), "You can't unjam the ammo feed while jumping!");
    DOCHECK0(IsRunning(MechDesiredSpeed(mech), MMaxSpeed(mech)), "You can't unjam the ammo feed while running!");
    ...
    mech_notify(mech, MECHALL, tprintf("You begin to shake the jammed ammo loose on weapon #%d", index));
    return 0;
}
...
void mech_artemis(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALMO);
    mech_toggle_mode_sub(player, mech, buffer, -1, TMISSILE, ARTEMIS_MODE, 0,
...
void mech_hotload(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(wWeapType == -3, "The weapon system chirps: 'That weapon is still reloading!'");
    DOCHECK(wWeapType == -4, "The weapon system chirps: 'That weapon is still recharging!'");
    DOCHECK(!(MechWeapons[wWeapType].special & IDF), "The weapon system chirps: 'That weapon can not be hotloaded!'");

    if (GetPartFireMode(mech, wSection, wCritSlot) & HOTLOAD_MODE) {
        mech_notify(mech, MECHALL, tprintf("Hotloading for weapon %d has been toggled off.", wWeapNum));
        GetPartFireMode(mech, wSection, wCritSlot) &= ~HOTLOAD_MODE;
    } else {
        mech_notify(mech, MECHALL, tprintf("Hotloading for weapon %d has been toggled on.", wWeapNum));
        GetPartFireMode(mech, wSection, wCritSlot) |= HOTLOAD_MODE;
    }
}
...
void mech_smoke(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALMO);
    mech_toggle_mode_sub(player, mech, buffer, 0, TARTILLERY, SMOKE_MODE, 0,
...
void mech_masc(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALMO);
    DOCHECK(!(MechSpecials(mech) & MASC_TECH), "Your toy ain't prepared for what you're askin' it!");
...
{
    MECH *mech = (MECH *) e->data;
    int needed = (2 * (1 + (MechMASCCounter(mech)++))) + ((MechStatus(mech) & MASC_ENABLED) ? 1 : 0) + (MechStatus2(mech) & SPRINTING ? 2 : 0);
    int roll = Roll();
    int j, count = 0;
    ...
    mech_notify(mech, MECHALL, tprintf("Supercharger: BTH %d, Roll: %d", needed + 1, roll));
        ...
        for (j = 0; count && j < CritsInLoc(mech, CTORSO); j++) {
            ...
            if (critType == Special(ENGINE) && !PartIsDestroyed(mech, CTORSO, j)) {
                ...
                if (MechEngineHeat(mech) < 10) {
                    ...
                    mech_notify(mech, MECHALL, "Your engine shielding takes a hit!  It's getting hotter in here!!");
                } else 
                    if (MechEngineHeat(mech) < 15) {
    ...
    if ((MechType(mech) == CLASS_VTOL) || (MechType(mech) == CLASS_VEH_GROUND)) {
        ...
        MechLOSBroadcast(mech, msgbuf);
        maxspeed = static_cast < int >(MechMaxSpeed(mech));
        newmaxspeed = static_cast < int >((maxspeed * .5));
...
void mech_scharge(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALMO);
    DOCHECK(!(MechSpecials2(mech) & SUPERCHARGER_TECH), "Your toy ain't prepared for what you're askin' it!");
    DOCHECK(MMaxSpeed(mech) < MP1, "Uh huh.");
...
static void mech_explode_event(EVENT * e)
{
    ...
    if ((--extra) % 256) {
        mech_notify(mech, MECHALL, tprintf("Self-destruction in %d second%s..", extra % 256, extra > 1 ? "s" : ""));
        ...
        if (extra >= 256) {
            ...
            doing_explode = 1;
            mech_notify(mech, MECHALL, "Suddenly you feel great heat overcoming your senses.. you faint.. (and die)");
            z = MechZ(mech);
            ...
            if (btechconf.btech_engine > 1)
                dam = MAX(MechTons(mech) / 5, MechEngineSize(mech) / 15);
            else
                dam = MAX(MechTons(mech) / 5, MechEngineSize(mech) / 10);
            
            blast_hit_hexesf(map, dam, 1, MAX(MechTons(mech) / 10, MechEngineSize(mech) / 25), MechFX(mech), MechFY(mech),
                    MechFX(mech), MechFY(mech), "%ch%crYou bear full brunt of the blast!%cn", "is hit badly by the blast!",
                    "%ch%cyYou receive some damage from the blast!%cn", "is hit by the blast!", btechconf.btech_engine > 1,
                    btechconf.btech_engine > 1 ? 5 : 3, 5, 1, 2);
...
void mech_explode(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(!btechconf.btech_engine, "The command has been disabled for now. Sorry.");

    if (!strcasecmp(buffer, "stop")) {
        DOCHECK(!Exploding(mech), "Your mech isn't undergoing a self-destruct sequence!");

        StopExploding(mech);
        mech_notify(mech, MECHALL, "Self-destruction sequence aborted.");
        SendDebug(tprintf("#%d in #%d stopped the self-destruction sequence.", player, mech->mynum));
        return;
    }
    
    DOCHECK(Exploding(mech), "Your mech is already undergoing a self-destruct sequence!");
    ...
    if (!strcasecmp(buffer, "ammo")) {
        ...
        time = 2;
        SendDebug(tprintf("#%d in #%d initiates the ammo explosion sequence.", player, mech->mynum));
        MECHEVENT(mech, EVENT_EXPLODE, mech_explode_event, 1, 256 + time);
    } else {
        DOCHECK(MechType(mech) != CLASS_MECH, "Only mechs can do the 'big boom' effect.");
        time = 10;
        SendDebug(tprintf("#%d in #%d initiates the reactor explosion sequence.", player, mech->mynum));
    ...
    mech_notify(mech, MECHALL, tprintf("%s in %d seconds.", ammo ? "The ammunition will explode" : "The reactor will blow up", time));
}
...
void mech_dig(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(MechRTerrain(mech) == ROAD || MechRTerrain(mech) == BRIDGE || MechRTerrain(mech) == BUILDING || MechRTerrain(mech) == WALL, "The surface is slightly too hard for you to dig in.");
    DOCHECK(MechRTerrain(mech) == WATER, "In water? Who are you kidding?");

    MechTankCritStatus(mech) |= DIGGING_IN; MECHEVENT(mech, EVENT_DIG, mech_dig_event, 20, 0);
...
void mech_fixturret(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALO);
    DOCHECK(MechTankCritStatus(mech) & TURRET_LOCKED, "Your turret is locked! You need a repairbay to fix it!");
    DOCHECK(!(MechTankCritStatus(mech) & TURRET_JAMMED), "Your turret is not jammed!");
    ...
    int high)
{
    ...
    DOCHECK0(weaptype == -2, "The weapons system chirps: 'That Weapon has been destroyed!'");
    weaptype = Weapon2I(GetPartType(mech, section, critical));
    DOCHECK0(!(MechWeapons[weaptype].special & GAUSS), "You can only disable Gauss weapons.");
...
void mech_stealtharmor(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    if (!(MechSpecials2(mech) & STEALTH_ARMOR_TECH)) {
        mech_notify(mech, MECHALL, "Your 'mech isn't equipped with a Stealth Armor system!");
        return;
    }

    if (!HasWorkingECMSuite(mech)) {
        mech_notify(mech, MECHALL, "Your 'mech doesn't have a working Guardian ECM suite!");
        return;
    }

    if (StealthArmorChanging(mech)) {
        mech_notify(mech, MECHALL, "You are already changing the status of your Stealth Armor system!");
        return;
    }

    if (!StealthArmorActive(mech)) {
        mech_notify(mech, MECHALL, "Your Stealth Armor system begins to come online.");
...
void mech_nullsig(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    if (!(MechSpecials2(mech) & NULLSIGSYS_TECH)) {
        mech_notify(mech, MECHALL, "Your 'mech isn't equipped with a Null Signature System!");
        return;
    }
    ...
    if (NullSigSysChanging(mech)) {
        mech_notify(mech, MECHALL, "You are already changing the status of your Null Signature System!");
        return;
    }

    if (!NullSigSysActive(mech)) {
        mech_notify(mech, MECHALL, "Your Null Signature System begins to come online.");
        MECHEVENT(mech, EVENT_NSS, changeNullSigSysEvent, 30, 1);
    } else {
        mech_notify(mech, MECHALL, "Your Null Signature System begins to shutdown.");
        MECHEVENT(mech, EVENT_NSS, changeNullSigSysEvent, 30, 0);
    }
}

void show_narc_pods(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    if (!(checkAllSections(mech, NARC_ATTACHED) || checkAllSections(mech, INARC_HOMING_ATTACHED) ||
            checkAllSections(mech, INARC_HAYWIRE_ATTACHED) || checkAllSections(mech, INARC_ECM_ATTACHED) || checkAllSections(mech, INARC_NEMESIS_ATTACHED))) {
        notify(player, "There are no NARC or iNARC pods attached to this unit.");
        return;
    }

    notify(player, "=========================Attached NARC and iNARC Pods========================");
    notify(player, "-- Location ---||- NARC -||- iHoming -||- iHaywire -||- iECM -||- iNemesis --");
    ...
    for (i = 0; i < NUM_SECTIONS; i++) 
        if (GetSectOInt(mech, i) > 0) {
            ...
            if (SectIsDestroyed(mech, i)) 
                notify(player, tprintf(" %-14.13s||********||***********||************||********||************* ", location));
            else 
                notify(player, tprintf(" %-14.13s||....%s...||.....%s.....||......%s.....||....%s...||......%s...... ", location, 
...
int findArmBTHMod(MECH * mech, int wSec)
{
    ...
    if (PartIsNonfunctional(mech, wSec, 1) || GetPartType(mech, wSec, 1) != I2Special(UPPER_ACTUATOR))
        wRet += 2;
    if (PartIsNonfunctional(mech, wSec, 2) || GetPartType(mech, wSec, 2) != I2Special(LOWER_ACTUATOR))
        wRet += 2;
    if (PartIsNonfunctional(mech, wSec, 3) || GetPartType(mech, wSec, 3) != I2Special(HAND_OR_FOOT_ACTUATOR))
        wRet += 1;

    return wRet;
}

void remove_inarc_pods_mech(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    DOCHECK(mech_parseattributes(buffer, args, 2) != 2, "Invalid number of arguments!");
    ...
    DOCHECK(!checkSectionForSpecial(mech, wPodType, wLoc), tprintf("There are no iNarc %s pods attached to your %s!", strPodType, strLocation));

    DOCHECK(((!GetSectInt(mech, RARM)) && (!GetSectInt(mech, LARM))), "You need atleast one functioning arm to remove iNarc pods!");

    if (wLoc == RARM) {
        DOCHECK(!GetSectInt(mech, LARM), "Your Left Arm needs to be intact to take iNarc pods off your right arm!");
    ...
    if (wLoc == LARM) {
        DOCHECK(!GetSectInt(mech, RARM), "Your Right Arm needs to be intact to take iNarc pods off your Left Arm!");
    ...
    if (wArmToUse == -1) {
        if (SectHasBusyWeap(mech, RARM) || MechSections(mech)[RARM].recycle || (!GetSectInt(mech, RARM)))
            wRAAvail = 0;

        if (SectHasBusyWeap(mech, LARM) || MechSections(mech)[LARM].recycle || (!GetSectInt(mech, LARM)))
            wLAAvail = 0;

        DOCHECK(!(wLAAvail || wRAAvail), "You need atleast one arm that is not recycling and does not have weapons recycling in it!");
    ...
    mech_notify(mech, MECHALL, tprintf("You try to swat at the iNarc pods attached to your %s with your %s.  BTH:  %d,\tRoll:  %d", strLocation, strPunchWith, wBTH, wRoll));

    /* Oops, we failed! */
    if (wRoll < wBTH) {
        mech_notify(mech, MECHALL, "Uh oh. You miss the pod and hit yourself!");
        MechLOSBroadcast(mech, "tries to swat off an iNarc pod, but misses and hits itself!");
        ...
        DamageMech(mech, mech, 1, MechPilot(mech), wLoc, 0, 0, wSelfDamage, 0, -1, 0, -1, 0, 0);
    } else {
        MechSections(mech)[wLoc].specials &= ~wPodType;
        mech_notify(mech, MECHALL, tprintf("You knock a %s pod off your %s!", strPodType, strLocation));
...
void removeiNarcPodsTank(EVENT * e)
{
    ...
    MechLOSBroadcast(mech, "'s crew climbs out and knocks off all the attached iNarc pods!");

    for (i = 0; i < NUM_SECTIONS; i++) 
        if (GetSectOInt(mech, i) > 0) 
            MechSections(mech)[i].specials &= ~(INARC_HOMING_ATTACHED | INARC_HAYWIRE_ATTACHED | INARC_ECM_ATTACHED | INARC_NEMESIS_ATTACHED);

}

void remove_inarc_pods_tank(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);

    cch(MECH_USUALSO);

    DOCHECK((MechDesiredSpeed(mech) > 0), "You can not be moving when attempting to remove iNarc pods!");
    DOCHECK((MechSpeed(mech) > 0), "You can not be moving when attempting to remove iNarc pods!");

    if (MechType(mech) == CLASS_VTOL)
        DOCHECK(!Landed(mech), "You must land before attempting to remove iNarc pods!");

    DOCHECK(CrewStunned(mech), "You're too stunned to remove iNarc pods!");
    DOCHECK(UnjammingTurret(mech), "You're too busy unjamming your turret to remove iNarc pods!");
    DOCHECK(UnJammingAmmo(mech), "You're too busy unjamming a weapon to remove iNarc pods!");

    if (!(checkAllSections(mech, INARC_HOMING_ATTACHED) || checkAllSections(mech, INARC_HAYWIRE_ATTACHED) ||
            checkAllSections(mech, INARC_ECM_ATTACHED) || checkAllSections(mech, INARC_NEMESIS_ATTACHED))) {
        mech_notify(mech, MECHALL, "There are no iNarc pods attached to this unit.");
        return;
    }

    mech_notify(mech, MECHALL, "You begin to systematically remove all the iNarc pods from your unit.");

    MECHEVENT(mech, EVENT_REMOVE_PODS, removeiNarcPodsTank, 60, 0);
}

void mech_auto_turret(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    mech_notify(mech, MECHALL, tprintf("Automatic turret turning is now %s", (MechStatus2(mech) & AUTOTURN_TURRET) ? "OFF" : "ON"));
...
void mech_usebin(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    DOCHECK(wWeapType == -1, "The weapons system chirps: 'Illegal Weapon Number!'");
    DOCHECK(wWeapType == -2, "The weapons system chirps: 'That Weapon has been destroyed!'");
    DOCHECK(wWeapType == -3, "The weapons system chirps: 'That weapon is still reloading!'");
    DOCHECK(wWeapType == -4, "The weapons system chirps: 'That weapon is still recharging!'");
    DOCHECK(IsEnergy(wWeapType), "Energy weapons do not use ammo!");

    if (args[1][0] == '-') {
        mech_notify(mech, MECHALL, tprintf("Prefered ammo source reset for weapon #%d", wWeapNum));
    ...
    DOCHECK(wCurLoc == wLoc, tprintf("Prefered ammo source already set to %s for weapon #%d", strLocation, wWeapNum));

    mech_notify(mech, MECHALL, tprintf("Prefered ammo source set to %s for weapon #%d", strLocation, wWeapNum));
...
void mech_safety(dbref player, void *data, char *buffer)
{
    ...
    mech_notify(mech, MECHPILOT, tprintf("Weapon safeties are %%ch%s%%cn", MechPKiller(mech) ? "%crOFF" : "%cgON"));
    return;
}
...
static char *display_mechpref(int i)
{
    ...
    if (((MechPrefs(target_mech) & info.bit) && (info.flags & MECHPREF_FLAG_INVERTED)) || (!(MechPrefs(target_mech) & info.bit) && !(info.flags & MECHPREF_FLAG_INVERTED))) 
...
void mech_mechprefs(dbref player, void *data, char *buffer)
{
    ...
    if (!nargs) {
        /* Show mechprefs */
        target_mech = mech;
        c = SelCol_FunStringMenuK(1, "Mech Preferences", display_mechpref, NUM_MECHPREFERENCES);
        ...
        if (((MechPrefs(mech) & info.bit) && (info.flags & MECHPREF_FLAG_INVERTED)) || (!(MechPrefs(mech) & info.bit) && !(info.flags & MECHPREF_FLAG_INVERTED))) 
            if (info.flags & MECHPREF_FLAG_NEGATIVE)
