...
void mech_ood_damage(MECH * wounded, MECH * attacker, int damage)
{
    ...
    tprintf("%%cgYou hit the cocoon for %d points of damage!%%cn", damage));
    mech_notify(wounded, MECHALL, tprintf("%%ch%%cyYour cocoon has been hit for %d points of damage!%%cn", damage));
    ...
    if (MechZ(wounded) > MechElevation(wounded)) {
        if (MechJumpSpeed(wounded) >= MP1) {
            mech_notify(wounded, MECHALL, "You initiate your jumpjets to compensate for the breached cocoon!");
            MechCocoon(wounded) = -1;
            return;
        }
        mech_notify(wounded, MECHALL, "Your cocoon has been destroyed - have a nice fall!");
        MechLOSBroadcast(wounded, "starts plummeting down, as the final blast blows the cocoon apart!");
        StopOOD(wounded);
...
void mech_ood_event(EVENT * e)
{
    ...
    if (!MadePilotSkillRoll(mech, 4)) {
        if (MechType(mech) == CLASS_MECH) {
            mech_notify(mech, MECHALL, "You are unable to control your momentum and fall on your face!");
            MechLOSBroadcast(mech, "touches down on the ground, twists, and falls down!");
        } else {
            mech_notify(mech, MECHALL, "You are unable to control your momentum and crash!");
...
void initiate_ood(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast <MECH *>(data);
    char *args[4];
    int x, y, z = ORBIT_Z, argc;

    DOCHECK((argc = mech_parseattributes(buffer, args, 3)) < 2, "Invalid attributes!");
