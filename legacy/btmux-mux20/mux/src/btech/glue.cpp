...
CHashTable SpecialCommandHash[NUM_SPECIAL_OBJECTS];
...
int HandledCommand_sub(dbref player, dbref location, char *command)
{
    ...
    int type;
    CommandsStruct *cmd;
    CHashTable *damnedhash;
    char *tmpc, *tmpchar;
    int ishelp;

    type = WhichSpecial(location);
    
    if (type < 0 || (SpecialObjects[type].datasize > 0 && !(n = FindNode(xcode_tree, location)))) {
    ...
    for (tmpchar = command; *tmpchar; tmpchar++)
        *tmpchar = mux_tolower(*tmpchar);

    cmd = (CommandsStruct *) hashfindLEN(command, strlen(command), &SpecialCommandHash[type]);

    if (tmpc)
        *tmpc = ' ';
    
    if (cmd && (type != GTYPE_MECH || (type == GTYPE_MECH && Can_Use_Command(((MECH *) (NodeData(n))), cmd->flag)))) {
#define SKIPSTUFF(a) while (*a && *a != ' ') a++;while (*a == ' ') a++
        if (cmd->helpmsg[0] != '@' || Have_MechPower(Owner(player), typeOfObject->power_needed)) {
            SKIPSTUFF(command);
        ...
        if (ishelp) {
            SKIPSTUFF(command);
            DoSpecialObjectHelp(player, typeOfObject->type, type, location, typeOfObject->power_needed, location, command);
            return 1;
        }
...
static int load_update2(Node * tmp)
{
    ...
    if (i == GTYPE_MECH)
        mech_map_consistency_check((MECH *) NodeData(tmp));
...
static int load_update1(Node * tmp)
{
    ...
    switch ((i = WhichType(tmp))) {
        ...
        case GTYPE_MECH:
            ...
            MechSpecials2(mech) &= ~(ECM_ENABLED | ECM_DISTURBANCE | ECM_PROTECTED | ECCM_ENABLED | ANGEL_ECM_ENABLED | ANGEL_ECCM_ENABLED | ANGEL_ECM_PROTECTED | ANGEL_ECM_DISTURBED);
...
static int load_autopilot_data(Node *tmp) {
    ...
    if (WhichType(tmp) == GTYPE_AUTO) {
        ...
        if (!autopilot->mymechnum || !(autopilot->mymech = getMech(autopilot->mymechnum))) 
            DoStopGun(autopilot);
        else 
...
static void load_xcode()
{
    ...
    initialize_colorize();
    fprintf(stderr, "LOADING: %s\n", btechconf.hcode_db);
    f = my_open_file(btechconf.hcode_db, "r", &filemode);
    if (!f) {
        fprintf(stderr, "ERROR: %s not found.\n", btechconf.hcode_db);
        return;
    }
    ...
    if (xcode_version != XCODE_VERSION) {
        fprintf(stderr, "LOADING: %s (skipped xcodetree - version difference: %d vs %d)\n", btechconf.hcode_db, (int) xcode_version, (int) XCODE_VERSION);
        return;
    }
...
static void save_econdb(char *target, int i)
{
    ...
    switch (i) {
        case DUMP_I_PANIC:
            sprintf(target, "%s.PANIC", btechconf.econ_db);
            break;
        default:            /* RESTART / normal */
            sprintf(target, "%s.tmp", btechconf.econ_db);
            break;
    }
    ...
    fclose(f);
    if (i == DUMP_I_RESTART || i == DUMP_I_NORMAL) {
        if (rename(target, btechconf.econ_db) < 0) {
...
void SaveSpecialObjects(int i)
{
    ...
    switch (i) {
        case DUMP_I_PANIC:
            sprintf(target, "%s.PANIC", btechconf.hcode_db);
            break;
        default:            /* RESTART / normal */
            sprintf(target, "%s.tmp", btechconf.hcode_db);
            break;
    }
    ...
    if (i == DUMP_I_RESTART || i == DUMP_I_NORMAL) {
        if (rename(btechconf.hcode_db, tprintf("%s.prev", btechconf.hcode_db)) < 0) 
            log_perror("SAV", "FAIL", "Renaming old hcode-save file ", target);
        if (rename(target, btechconf.hcode_db) < 0) 
...
void CreateNewSpecialObject(dbref player, dbref key)
{
    ...
    if ((pattr = atr_str("XTYPE")) && pattr->number) {
        str = atr_get(key, pattr->number, &aowner, &aflags);
    } else {
        notify(player, "You must first set the XTYPE using &xtype <object>=<type>");
        notify(player, "Valid XTYPEs include: MECH, MECHREP, MAP, DEBUG, AUTOPILOT, CUSTOM");
    ...
    if (type > -1) {
        ...
        if (typeOfObject->datasize) {
            newso = NewSpecialObject(key, type);
            if (!newso)
...
void DisposeSpecialObject(dbref player, dbref key)
{
    ...
    if (i < 0) {
        notify(player, "CRITICAL: Unable to free data, inconsistency somewhere. Please");
    ...
    if (typeOfObject->datasize > 0 && WhichSpecial(key) != i) {
        notify(player, "Semi-critical error has occured. For some reason the object's data differs\nfrom the data on the object. Please contact a wizard about this.");
        i = WhichSpecial(key);
    }
...
static void cut_apart_helpmsgs(coolmenu ** d, char *msg1, char *msg2, int len, int initial)
{
    ...
    msg2 = do_ugly_things(d, msg2, initial ? len : len - TAB, initial ? 0 : 0 - TAB);
    if (!msg1 && !msg2)
        return;
...
static void DoSpecialObjectHelp(dbref player, char *type, int id, int loc, int powerneeded, int objid, char *arg)
{
    ...
    if (id == GTYPE_MECH)
        ...
        for (i = 0; SpecialObjects[id].commands[i].name; i++) {
            if (!SpecialObjects[id].commands[i].func && (SpecialObjects[id].commands[i].helpmsg[0] != '@' || Have_MechPower(Owner(player), powerneeded)))
                if (id != GTYPE_MECH || Can_Use_Command(mech, SpecialObjects[id].commands[i].flag)) {
                    if (count)
      ...
      &SpecialObjects[id].commands[a].helpmsg[SpecialObjects[id].commands[a].helpmsg[0]=='@']
        for (i = 0; i < count; i++) {
            ...
            for (j = pos[i][0] + (count == 1 ? 0 : 1); j < pos[i][0] + pos[i][1]; j++)
                if (SpecialObjects[id].commands[j].helpmsg[0] != '@' || Have_MechPower(Owner(player), powerneeded))
                    if (id != GTYPE_MECH || Can_Use_Command(mech, SpecialObjects[id].commands[j].flag)) {
        ...
        if (dc > -2) {
            for (i = 0; i < count; i++)
                if (dc == -1 || i == dc) {
                    if (count > 1)
                        vsi(tprintf("%s%s%s", "%cg", center_string(HELPMSG(pos[i][0]), 70), "%c"));

                    for (j = pos[i][0] + (count == 1 ? 0 : 1); j < pos[i][0] + pos[i][1]; j++)
                        if (SpecialObjects[id].commands[j].helpmsg[0] != '@' || Have_MechPower(Owner(player), powerneeded))
                            if (id != GTYPE_MECH || Can_Use_Command(mech, SpecialObjects[id].commands[j].flag)) cut_apart_helpmsgs(&c, SpecialObjects[id].commands[j].name, HELPMSG(j), 37, 1);
                }
        }
    }
...
void InitSpecialHash(int which)
{
    ...
    for (i = 0; (tmp = SpecialObjects[which].commands[i].name); i++) {
        ...
        *tmpc = 0;
        ...
        hashaddLEN(buf, strlen(buf), (int *) &SpecialObjects[which].commands[i], &SpecialCommandHash[which]);
    }
}
        ...
        if (*p == '%' && *(p + 1) == 'c') {
            ...
            if (color_wanted && Ansi(player)) {
                *q = 0;
                ...
                cnt = 0;
                for (i = 0; color_table[i].string; i++)
                    if (color_wanted & color_table[i].bit && color_table[i].bit != color_table[i].negbit) {
                        if (cnt)
                            *q++ = ';';
