...
void map_view(dbref player, void *data, char *buffer)
{
    ...
    if (!*str) {
        ...
        if (sscanf(str, "%d %d", &displayHeight, &displayWidth) != 2 || displayHeight > 24 || displayHeight < 5 || displayWidth < 5 || displayWidth > 40) {
    ...
    /* Get the map data */
    maptext = MakeMapText(player, NULL, mech_map, x, y, displayWidth, displayHeight, 3, 0);
...
void map_addhex(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(!((x >= 0) && (x < map->map_width) && (y >= 0) && (y < map->map_height)), "X,Y out of range!");
...
int map_load(MAP * map, char *mapname)
{
    ...
    if (fscanf(fp, "%d %d\n", &height, &width) != 2 || height < 1 || height > MAPY || width < 1 || width > MAPX) {
    ...
    for (i = 0; i < height; i++) {
        if (feof(fp) || fgets(row, 2 * MAPX + 2, fp) == NULL || strlen(row) < (2 * width)) 
            break;
...
void map_savemap(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(!(fp = my_open_file(openfile, "w", &filemode)), "Unable to open the map file!");
        ...
        for (j = 0; j < map->map_width; j++) {
            ...
            switch (terrain) {
                case ' ':
                    terrain = '.';
                    break;
                case FIRE: /* check if we're burnin', if so, alter terrain type */
                    if ((mo = find_mapobj(map, j, i, TYPE_FIRE)))
                        terrain = TFIRE;
                    else 
                        if (!(map->flags & MAPFLAG_FIRES)) {
                            SetTerrain(map, j, i, ' ');
                            SendEvent(tprintf("[lost?] fire event noticed on map #%d (%s) at %d,%d", map->mynum, map->mapname, j, i));
                            terrain = '.';
                        }
                    break;
                case SMOKE:
                    ...
                    if (terrain == SMOKE) {
                        SetTerrain(map, j, i, ' ');
                        SendEvent(tprintf("[lost?] smoke event noticed on map #%d (%s) at %d,%d", map->mynum, map->mapname, j, i));
                        terrain = '.';
                    }
                    break;
            }
...
void map_setmapsize(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK((argc = mech_parseattributes(buffer, args, 4)) != 2, "Invalid number of arguments (X/Y expected)");
    x = atoi(args[0]);
    y = atoi(args[1]);
    DOCHECK(!((x >= 0) && (x <= MAPX) && (y >= 0) && (y <= MAPY)), "X,Y out of range!");
    ...
    for (i = 0; i < y1; i++)
        for (j = 0; j < x1; j++)
            SetMapB(map, j, i, GetTerrain(oldmap, j, i), GetElevation(oldmap, j, i));
...
void map_update(dbref obj, void *data)
{
    ...
    if (!(muxevent_tick % 60)) {
        ...
        if ((pattr = atr_str("MAPVIS")) && pattr->number)
            tmps = atr_get(obj, pattr->number, &aowner, &aflags);

        if (!(tmps) || sscanf(tmps, "%d %d %d %d %d %[^\n]", &ma, &ml, &wind, &wspeed, &cloudbase, changemsg) < 4) {
            ma = 30;
            ml = 2;
            wind = 0;
            wspeed = 0;
        ...
        if (ml != oldl || ma != oldv)
            for (i = 0; i < map->first_free; i++) {
                ...
                if (MechAuto(mech) > 0)
                    if ((au = static_cast <AUTO *>(FindObjectsData(MechAuto(mech)))))
                        if (Gunning(au))
...
void initialize_map_empty(MAP * newmap, dbref key)
{
    int i, j;

    newmap->mynum = key;
    newmap->map_width = DEFAULT_MAP_WIDTH;
    newmap->map_height = DEFAULT_MAP_HEIGHT;
    Create(newmap->map, unsigned char *, newmap->map_height);

    for (i = 0; i < newmap->map_height; i++)
        Create(newmap->map[i], unsigned char, newmap->map_width);

    for (i = 0; i < newmap->map_height; i++)
        for (j = 0; j < newmap->map_width; j++)
            SetMap(newmap, j, i, ' ', 0);
}
...
{
    MAP *newmap = static_cast < MAP * >(*data);
    int i;

    switch (selector) {
        case SPECIAL_ALLOC:
            initialize_map_empty(newmap, key);
            /* allocate default map space */
            for (i = 0; i < NUM_MAPOBJTYPES; i++)
                newmap->mapobj[i] = NULL;
            sprintf(newmap->mapname, "%s", "Default Map");
            break;
        case SPECIAL_FREE:
            del_mapobjs(newmap);
            if (newmap->map) {
                for (i = newmap->map_height - 1; i >= 0; i--)
                    if (newmap->map[i])
                        free((char *) (newmap->map[i]));
                free((char *) (newmap->map));
            }
            break;
    }
}
...
void map_listmechs(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(mech_parseattributes(buffer, args, 1) == 0, "Supply target type too!");
        ...
        case MECHS:
            ...
            for (i = 0; i < map->first_free; i++) 
                if (map->mechsOnMap[i] != -1) {
                    ...
                    notify(player, tprintf("Mech DB Number: %d : [%s]\t%s", map->mechsOnMap[i], ID, valid));
                    count++;
                }       
            ...
            if (count != map->first_free)
                notify(player, tprintf("%d is first free slot, according to db.", map->first_free));
            return;
        case OBJS:
...
void clear_hex(MECH * mech, int x, int y, int meant)
{
    ...
    if (meant) {
        ...
        mech_notify(mech, MECHALL, tprintf("You accidentally clear the %d,%d!", x, y));
    }
}
