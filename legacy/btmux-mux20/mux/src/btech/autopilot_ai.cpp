...
static int ai_crash(MAP * map, MECH * m, LOC * l)
{
    ...
    if (l->h != l->dh && !(MechCritStatus(m) & GYRO_DESTROYED)) {
        ...
        /* XXX Jumping */
        if (fabs(l->s) < 1.0)
            offset = (int)(3 * maxspeed * MP_PER_KPH * mw_mod);
        else {
            offset = (int)(2 * maxspeed * MP_PER_KPH * mw_mod);
            if ((abs(normangle) > offset) && (l->s > (maxspeed * 2 / 3)))
                offset -= (int)(offset / 2 * (3.0 * l->s / maxspeed - 2.0));
        }
    ...
    if (ch) {
        ...
        } else if (btechconf.btech_slowdown == 1) {
            if (l->h != l->dh)
...
int getEnemies(MECH * mech, MAP * map, int reset)
{
    ...
    for (i = 0; i < map->first_free; i++) {
        tempMech = static_cast <MECH *>(FindObjectsData(map->mechsOnMap[i]));
        if (!tempMech)
            continue;
        ...
        int *bd, int *bscore)
{
    ...
    for (i = 0; i < NORM_SAFE; i++) {
        ...
        for (k = 0; k < num_o; k++) {
            ...
            if ((l = (int)(FindXYRange(lo[k].fx, lo[k].fy, dx, dy))) < br[k])
                br[k] = l;
            ...
            if (opts[k][1] != SP_OPT_FASTER && MMaxSpeed(m) > 0.1) {
                sc = BOUNDED(0, (int)(100 * lo[k].ds / MMaxSpeed(m)), 100);
        ...
        if (gotenemy) {
            ...
            for (j = 0; j < enemy_c; j++) {
                ...
                for (k = 0; k < num_o; k++) {
                    ...
                    l = (int)(FindXYRange(lo[k].fx, lo[k].fy, dx, dy));
        ...
        float dely)
{
    ...
    o = ai_opponents(a, m);
    if (a->last_upd > ltaNow.ReturnSeconds() || (ltaNow.ReturnSeconds() - a->last_upd) > AUTO_GOET) {
        ...
        a->last_upd = ltaNow.ReturnSeconds();
    }
...
void mech_snipe(dbref player, void *data, char *buffer)
{
...
static astar_node *auto_create_astar_node(short x, short y, 
    ...
    astar_node *temp;
    temp = static_cast <astar_node *>(malloc(sizeof(astar_node)));
    if (temp == NULL)
        return NULL;
...
int auto_astar_generate_path(AUTO *autopilot, MECH *mech, short end_x, short end_y) {
    ...
    while (!found_path) {
        ...
        for (i = 0; i < 360; i += 60) {
            ...
            child_h_score = (int)(100 * FindHexRange(x2, y2, x1, y1));
    ...
    if (found_path) {
        ...
        temp_astar_node = static_cast <astar_node *> (rb_find(closed_list, &hexoffset));
        ...
        if (!(temp_astar_node->x == MechX(mech) &&
            ...
            while (1) {
                ...
                parent_astar_node = static_cast <astar_node *> (rb_find(closed_list, &hexoffset));
...
#ifdef DEBUG_ASTAR
    /* Log Message */
    fprintf(logfile, "Destroying the AI lists\n");
#endif
...
void auto_destroy_astar_path(AUTO *autopilot) {
    ...
    if (dllist_size(autopilot->astar_path) > 0) {
        ...
        while (dllist_size(autopilot->astar_path)) {
            temp_astar_node = static_cast <astar_node *> (dllist_remove_node_at_pos(autopilot->astar_path, 1));
