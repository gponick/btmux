...
char *MechIDS(MECH * mech, int islower)
{
    ...
    if (islower) {
        buf[0] = mux_tolower(buf[0]);
        buf[1] = mux_tolower(buf[1]);
    }
    ...
    if (MechType(mech) == CLASS_MECH)
        switch (index) {
            ...
            if (MechType(mech) == CLASS_MW)
                ...
                if (MechType(mech) == CLASS_AERO && index == AERO_COCKPIT)
                    return 5;
...
MAP *ValidMap(dbref player, dbref map)
{
    ...
    if ((pattr = atr_str("XTYPE")) && pattr->number)
        str = atr_get(map, pattr->number, &aowner, &aflags);
    else
        str = "";
...
dbref FindTargetDBREFFromMapNumber(MECH * mech, char *mapnum)
{
    ...
    if (!map) {
        SendError(tprintf("FTDBREFFMN:invalid map:Mech: %d  Index: %d", mech->mynum, mech->mapindex));
        mech->mapindex = -1;
        return -1;
    }
...
static int Leave_Hangar(MAP * map, MECH * mech)
{
    ...
    if (car)
        mech_Rsetmapindex(GOD, (void *) car, tprintf("%d", (int) map->mapobj[TYPE_LEAVE]->obj));

    map = getMap(mech->mapindex);

    if (mech->mapindex == mapob) {
        SendError(tprintf("#%d %s attempted to leave, but no target map?", mech->mynum, GetMechID(mech)));
        mech_notify(mech, MECHALL, "Exit of this map is.. fubared. Please contact a wizard");
        return 0;
    }

    if (!(mapo = find_entrance_by_target(map, mapob))) {
        SendError(tprintf("#%d %s attempted to leave, but no target place was found? setting the mech at 0,0 at %d.", mech->mynum, GetMechID(mech), mech->mapindex));
        mech_notify(mech, MECHALL, "Weird bug happened during leave. Please contact a wizard. ");
        return 1;
    }

    StopBSuitSwarmers(static_cast <MAP *>(FindObjectsData(mech->mapindex)), mech, 1);
    mech_notify(mech, MECHALL, tprintf("You have left %s.", structure_name(mapo)));
    ...
    MechLOSBroadcast(mech, tprintf("has left %s at %d,%d.", structure_name(mapo), MechX(mech), MechY(mech)));
...
void CheckEdgeOfMap(MECH * mech)
{
    ...
    if (!map) {
        ...
        SendError(tprintf("CheckEdgeofMap:invalid map:Mech: %d  Index: %d", mech->mynum, mech->mapindex));
        mech->mapindex = -1;
        return;
    }
    ...
    if (pinned > 0) {
        ...
        */
        if (map->onmap && IsMech(map->onmap)) {
            ...
            if (map->flags & MAPFLAG_MAPO && map->mapobj[TYPE_LEAVE])
...
if (Quiet(mech->mynum)) \
{ dbref aowner; int aflags; ATTR *pattr; \
if((pattr = atr_str("MECHSKILLS")) && pattr->number) \
str = atr_get(mech->mynum, pattr->number, &aowner, &aflags); \
else \
str = "";\
...
char *FindTechSkillName(MECH * mech)
{
    switch (MechType(mech)) {
        case CLASS_BSUIT:
            if (btechconf.btech_use_tech_bsuit) 
...
int MadePilotSkillRoll_Advanced(MECH * mech, int mods, int succeedWhenFallen)
{
    ...
    SendDebug(tprintf("Attempting to make pilot skill roll. SPilot: %d, mods: %d, MechPilot: %d, BTH: %d", FindSPilotPiloting(mech), mods, MechPilotSkillBase(mech), roll_needed));
    ...
    if (roll >= roll_needed) {
        if (roll_needed > 2)
            AccumulatePilXP(MechPilot(mech), mech, BOUNDED(1, roll_needed - 7, MAX(2, 1 + mods)), 1);
        return 1;
    }
...
void RealCoordToMapCoord(short *hex_x, short *hex_y, float cart_x, float cart_y)
{
    ...
    if (cart_x < ALPHA) {
        ...
        *hex_x = cart_x < 0 ? -1 : 0;
        *hex_y = int(floor(cart_y / SCALEMAP));
        return;
    }
    ...
    y_count = (int)(floor(cart_y / FULL_Y));
...
void navigate_sketch_mechs(MECH * mech, MAP * map, int x, int y, char buff[NAVIGATE_LINES][MBUF_SIZE])
{
    ...
    for (i = 0; i < map->first_free; i++) {
        if (map->mechsOnMap[i] < 0)
            continue;
        if (!(other = static_cast <MECH *>(FindObjectsData(map->mechsOnMap[i]))))
            continue;
        if (other == mech)
            continue;
        ...
        column = (int) (fx / NAV_COLUMN_WIDTH + NAV_X_OFFSET);

        fy = MechFY(other) - corner_fy;
        row = (int) (fy / NAV_ROW_HEIGHT + NAV_Y_OFFSET);
...
int FindLegHeatSinks(MECH * mech)
{
    ...
    for (loop = 0; loop < NUM_CRITICALS; loop++) {
        if (GetPartType(mech, LLEG, loop) == I2Special((HEAT_SINK)) && !PartIsNonfunctional(mech, LLEG, loop))
            heatsinks++;
        if (GetPartType(mech, RLEG, loop) == I2Special((HEAT_SINK)) && !PartIsNonfunctional(mech, RLEG, loop))
            heatsinks++;
    /*
     ...
     */
        ...
        if (MechIsQuad(mech)) {
            if (GetPartType(mech, LARM, loop) == I2Special((HEAT_SINK)) && !PartIsNonfunctional(mech, LARM, loop))
                heatsinks++;
            if (GetPartType(mech, RARM, loop) == I2Special((HEAT_SINK)) && !PartIsNonfunctional(mech, RARM, loop))
                heatsinks++;
        }
    }
        ...
        if (number < running_sum + num_weaps) {
            ...
            if (PartIsNonfunctional(mech, loop, critical[index])) {
                ...
                *crit = critical[index];
                ...
                if (weapdata[index] > 0 && !sight) {
                    *section = loop;
                    *crit = critical[index];
                    return (MechWeapons[weaparray[index]].type == TBEAM) ? TIC_NUM_RECYCLING : TIC_NUM_RELOADING;
                } else {
                    if (MechSections(mech)[loop].recycle && (MechType(mech) == CLASS_MECH || MechType(mech) == CLASS_VEH_GROUND || MechType(mech) == CLASS_VTOL) && !sight) {
                        *section = loop;
                        *crit = critical[index];
    ...
    for (loop = 0; loop < NUM_SECTIONS; loop++) {
        ...
        for (index = 0; index < num_weaps; index++)
            if (weaparray[index] == weapindx) {
                *section = loop;
                *crit = critical[index];
                if (!PartIsNonfunctional(mech, loop, index) && !WpnIsRecycling(mech, loop, index))
                    return 1;
...
int findAmmoInSection(MECH * mech, int section, int type, int nogof, int gof)
{
    ...
    for (wIter = 0; wIter < NUM_CRITICALS; wIter++) {
        if (GetPartType(mech, section, wIter) == type && !PartIsNonfunctional(mech, section, wIter) && (!nogof || !(GetPartAmmoMode(mech, section, wIter) & nogof)) && (!gof || (GetPartAmmoMode(mech, section, wIter) & gof))) {
            if (!PartIsNonfunctional(mech, section, wIter) && GetPartData(mech, section, wIter) > 0)
                return wIter;
        }
    }
    ...
    int weapindx, int start, int *section, int *critical, int nogof, int gof)
{
    ...
    if ((weapSection > -1) && (weapCritical > -1)) {
        ...
        wFirstCrit = FindFirstWeaponCrit(mech, weapSection, weapCritical, 0, wCritType, wWeapSize);
...
{
    return FindAmmoForWeapon_sub(mech, -1, -1, weapindx, start, section, critical, AMMO_MODES, 0);
}
...
int CountAmmoForWeapon(MECH * mech, int weapindx)
{
    ...
    for (wSecIter = 0; wSecIter < NUM_SECTIONS; wSecIter++) 
        for (wSlotIter = 0; wSlotIter < NUM_CRITICALS; wSlotIter++) 
            if ((GetPartType(mech, wSecIter, wSlotIter) == wAmmoIdx) && !PartIsNonfunctional(mech, wSecIter, wSlotIter) && (GetPartData(mech, wSecIter, wSlotIter) > 0))
...
int FindDestructiveAmmo(MECH * mech, int *section, int *critical)
{
    ...
    for (loop = 0; loop < NUM_SECTIONS; loop++)
        for (critloop = 0; critloop < NUM_CRITICALS; critloop++)
            if (IsAmmo(GetPartType(mech, loop, critloop)) && !PartIsDestroyed(mech, loop, critloop)) {
...
int FindRoundsForWeapon(MECH * mech, int weapindx)
{
    ...
    for (loop = 0; loop < NUM_SECTIONS; loop++)
        for (critloop = 0; critloop < NUM_CRITICALS; critloop++)
            if (GetPartType(mech, loop, critloop) == desired && !PartIsNonfunctional(mech, loop, critloop))
...
int IsInWeaponArc(MECH * mech, float x, float y, int section, int critical)
{
    ...
    if (MechType(mech) == CLASS_MECH && (section == LLEG || section == RLEG || (MechIsQuad(mech) && (section == LARM || section == RARM)))) {
    ...
    switch (MechType(mech)) {
        ...
        case CLASS_MW:
            if (GetPartFireMode(mech, section, critical) & REAR_MOUNT)
                ...
                if (section == LARM && (MechStatus(mech) & FLIPPED_ARMS))
                    wantarc = REARARC | LSIDEARC;
                else 
                    if (section == LARM)
                        wantarc = FORWARDARC | LSIDEARC;
                    else 
                        if (section == RARM && (MechStatus(mech) & FLIPPED_ARMS))
...
void do_sub_magic(MECH * mech, int loud)
{
    ...
    if (jjs > maxjjs) {
        if (loud)
            SendError(tprintf("Error in #%d (%s): %d JJs, yet %d maximum available (due to walk MPs)?", mech->mynum, MechType_Ref(mech), jjs, maxjjs));
        jjs = maxjjs;
    }
    ...
    if (loud)
        MechNumOsinks(mech) = wanths - MIN(MechRealNumsinks(mech), inthses * hs_eff);
    if (wanths != MechRealNumsinks(mech) && loud) 
        SendError(tprintf ("Error in #%d (%s): Set HS: %d. Existing HS: %d. Difference: %d. Please %s.", mech->mynum, MechType_Ref(mech), MechRealNumsinks(mech), wanths, MechRealNumsinks(mech) - wanths, wanths < MechRealNumsinks(mech) ? "add the extra HS critical(s)" : "fix the template"));
    else
...
void do_magic(MECH * mech)
{
    ...
    if (MechType(mech) != CLASS_MECH)
        tankCritMask = (TURRET_LOCKED | TURRET_JAMMED | TAIL_ROTOR_DESTROYED | CREW_STUNNED);
        ...
        if (MechType(mech) == CLASS_MECH) {
            for (j = 0; j < CritsInLoc(mech, i); j++) {
                if (PartIsDestroyed(mech, i, j)) {
                    ...
                    GetPartFireMode(mech, i, j) &= ~(OS_USED | ROCKET_FIRED | IS_JETTISONED_MODE);
                }
            }
        }
...
void mech_RepairPart(MECH * mech, int loc, int pos)
{
    ...
    if (IsWeapon(t) || IsAmmo(t)) {
        ...
        GetPartFireMode(mech, loc, pos) &= ~(OS_USED | IS_JETTISONED_MODE | ROCKET_FIRED);
    } else 
        if (IsSpecial(t)) {
            switch (Special2I(t)) {
...
void multi_weap_sel(MECH * mech, dbref player, char *buffer, int bitbybit, int (*foo) (MECH *, dbref, int, int))
{
    ...
    if (sscanf(buffer, "%d-%d", &i1, &i2) == 2) {
        DOCHECK(i1 < 0 || i1 >= MAX_WEAPONS_PER_MECH, tprintf("Invalid first number in range (%d)", i1));
        DOCHECK(i2 < 0 || i2 >= MAX_WEAPONS_PER_MECH, tprintf("Invalid second number in range (%d)", i2));
        if (i1 > i2) {
            i3 = i1;
            i1 = i2;
            i2 = i3;
        ...
        DOCHECK(i1 < 0 || i1 >= MAX_WEAPONS_PER_MECH, tprintf("Invalid weapon number: %d", i1));
        i2 = i1;
    }
    ...
    if (bitbybit / 2) {
        ...
        DOCHECK(!(FindWeaponNumberOnMech(mech, i2, &section, &critical) != -1), tprintf("Error: the mech doesn't HAVE %d weapons!", i2 + 1));
    }
...
{
    return (SectIsDestroyed(objMech, wLoc) || SectIsBreached(objMech, wLoc) || SectIsFlooded(objMech, wLoc));
}
...
int FindFirstWeaponCrit(MECH * objMech, int wLoc, int wSlot, int wStartSlot, int wCritType, int wMaxCrits)
{
     ...
     * we need to figure out what range it actually falls into.  */
        if ((wFirstCrit + wMaxCrits) <= wSlot)
            wFirstCrit = FindFirstWeaponCrit(objMech, wLoc, wSlot, (wFirstCrit + wMaxCrits), wCritType, wMaxCrits);
...
int getRemainingInternalPercent(MECH * mech)
{
    ...
    return (int) ((wRemaining / wMax) * 100);
}
...
MECH *find_mech_in_hex(MECH * mech, MAP * mech_map, int x, int y, int needlos)
{
    ...
    for (loop = 0; loop < mech_map->first_free; loop++)
        if (mech_map->mechsOnMap[loop] != mech->mynum && mech_map->mechsOnMap[loop] != -1) {
            ...
            if (needlos) {
                if (needlos & 1)
                    if (!InLineOfSight(mech, target, x, y, FlMechRange(mech_map, mech, target)))
                        continue;
                if (needlos & 2) {
...
int FindAndCheckAmmo(MECH * mech, int weapindx, int section, int critical, int *ammoLoc, int *ammoCrit, int *ammoLoc1, int *ammoCrit1, int *wGattlingShots)
{
    ...
    if (MechWeapons[weapindx].type == TBEAM || MechWeapons[weapindx].type == THAND)
        return 1;
    ...
    if (wWeapMode & OS_MODE) {
        DOCHECK0(GetPartFireMode(mech, section, critical) & OS_USED, "That weapon has already been used!");
        return 1;
    }
    ...
    if (!mod) {
        DOCHECK0(!FindAmmoForWeapon_sub(mech, section, critical, weapindx, section, ammoLoc, ammoCrit, AMMO_MODES, 0), "You don't have any ammo for that weapon stored on this mech!");
        DOCHECK0(!GetPartData(mech, *ammoLoc, *ammoCrit), "You are out of ammo for that weapon!");

        if (wRoundsToCheck > 1) {
            GetPartData(mech, *ammoLoc, *ammoCrit)--;

            if (FindAmmoForWeapon_sub(mech, section, critical, weapindx, section, ammoLoc1, ammoCrit1, AMMO_MODES, 0)) {
        ...
        DOCHECK0(!FindAmmoForWeapon_sub(mech, section, critical, weapindx, section, ammoLoc, ammoCrit, nmod, mod), "You don't have any ammo for that weapon stored on this mech!");

        DOCHECK0(!GetPartData(mech, *ammoLoc, *ammoCrit), "You are out of the special ammo type for that weapon!");

        if (wRoundsToCheck > 1) {
            GetPartData(mech, *ammoLoc, *ammoCrit)--;

            if (FindAmmoForWeapon_sub(mech, section, critical, weapindx, section, ammoLoc1, ammoCrit1, nmod, mod)) {
                if (!GetPartData(mech, *ammoLoc1, *ammoCrit1))
                    tResetMode = 1;
            } else
                tResetMode = 1;
...
void ChannelEmitKill(MECH * mech, MECH * attacker)
{
    ...
    SendDebug(tprintf("#%d has been killed by #%d", mech->mynum, attacker->mynum));
    
    if (IsDS(mech))
        SendDSInfo(tprintf("#%d has been killed by #%d", mech->mynum, attacker->mynum));
    
    if (mech->mynum > 0 && attacker->mynum > 0)
        did_it(attacker->mynum, mech->mynum, 0, NULL, 0, NULL, 0, (char **) NULL, 0);
}
...
{
    if (IsWeapon(part))
        return (int) (10.24 * MechWeapons[Weapon2I(part)].weight);
    else 
        if (IsAmmo(part))
            return 1024;
        else 
            if (IsBomb(part))
                ...
                if (IsSpecial(part))    /* && i <= I2Special(LAMEQUIP) */
...
#define ADDPRICE(desc, add) total += add;
...
long long int CalcFasaCost(MECH * mech)
{
    int ii, i, part;
    ...
    if (!(MechType(mech) == CLASS_MECH || MechType(mech) == CLASS_VEH_GROUND || MechType(mech) == CLASS_VEH_NAVAL || MechType(mech) == CLASS_VTOL) || is_aero(mech) || IsDS(mech))
        return 0;
...
/* Gyro */
        ...
        ADDPRICE("Control Components", (long long int)((float) 10000 * (float) ((float) 0.05 * (float) MechTons(mech))))

        if (MechSpecials(mech) & ICE_TECH)
            ADDPRICE("Power Amplifiers", (long long int)(20000 * (float) (((float) pamp / (float) 10) / (float) 1024)))

        ADDPRICE("Turret", (long long int)((float) 5000 * (float) (((float) turret / (float) 10) / (float) 1024)))

        if (MechMove(mech) == MOVE_HOVER || MechMove(mech) == MOVE_FOIL || MechMove(mech) == MOVE_SUB)
            ADDPRICE("Lift/Dive Equipment", (long long int)((float) 20000 * (float) ((float) 0.1 * (float) MechTons(mech))))

        if (MechMove(mech) == MOVE_VTOL)
            ADDPRICE("VTOL Equipment", (long long int)((float) 40000 * (float) ((float) 0.1 * (float) MechTons(mech))))
    }
...
/* Engine Math */
    ...
    i = (long long int)(MechJumpSpeed(mech) * MP_PER_KPH);
    if (i > 0)
    ...
    i = (MechSpecials(mech) & FF_TECH ? 20000 : MechSpecials(mech) & HARDA_TECH ? 15000 : MechSpecials2(mech) & LT_FF_ARMOR_TECH ? 15000 : MechSpecials2(mech) & HVY_FF_ARMOR_TECH ? 25000 : 10000);
#if COST_DEBUG
...
/* Parts */
    ...
    return (long long int)((float) total * (float) mod);
}
...
{
    int defbv = 0, offbv = 0, i, ii, temp, temp2;
    int deduct = 0, offweapbv = 0, defweapbv = 0, armor = 0, intern = 0, weapindx, mostheat = 0, tempheat = 0;
    int mechspec, mechspec2, type, move, pilskl = pilstat, gunskl = gunstat;
        ...
        for (ii = 0; ii < CritsInLoc(mech, i); ii++) {
            if (IsWeapon(temp = GetPartType(mech, i, ii))) {
                ...
                if (MechWeapons[weapindx].special & AMS) {
                    defweapbv += (debug1 = (int)((MechWeapons[weapindx].battlevalue * 100) * (float) (3000 / (MechWeapons[weapindx].vrt * 100))));
                } else {
                    offweapbv += (debug1 = (int)((MechWeapons[weapindx].battlevalue * (GetPartFireMode(mech, i, ii) & REAR_MOUNT ? 50 : 100)) * (float) ((float) 3000 / (float) (MechWeapons[weapindx].vrt * 100))));
                ...
                if (type == CLASS_MECH) 
                    if (!(GetPartFireMode(mech, i, ii) & REAR_MOUNT)) {
                        tempheat = (int)((MechWeapons[weapindx].heat * 100) * (float) ((float) 3000 / (float) (MechWeapons[weapindx].  vrt * 100)));
                ...
                if (IsAmmo(temp)) {
                    ...
                    mul = ((temp2 = GetPartAmmoMode(mech, i, ii)) & AC_AP_MODE ? 4 : temp2 & AC_PRECISION_MODE ? 6 : temp2 & (SWARM_MODE | SWARM1_MODE) ? 1.5 : 1);
                    mul = (mul * ((float) ((float) GetPartData(mech, i, ii) / (float) MechWeapons[weapindx = Ammo2WeaponI(temp)].ammoperton)));
                    if (MechWeapons[weapindx].special & AMS) {
                        defweapbv += (debug1 = (int)((((MechWeapons[weapindx].battlevalue / 10) * 100) * mul) * (3000.0 / (MechWeapons[weapindx].vrt * 100))));
                    } else {
                        offweapbv += (debug1 = (int)((((MechWeapons[weapindx].battlevalue / 10) * 100) * mul) * (3000.0 / (MechWeapons[weapindx].vrt * 100))));
                    }
                }
    ...
    if (type == CLASS_MECH) {
        mostheat += (MechJumpSpeed(mech) > 0 ? MAX((int)((MechJumpSpeed(mech) / MP1) * 100), 300) : 200);
    ...
    if (mechspec & (XL_TECH | XXL_TECH | LE_TECH)) 
        ...
        if (mechspec & ICE_TECH || MechType(mech) == CLASS_VEH_GROUND || MechType(mech) == CLASS_VEH_NAVAL) 
            mul = 0.5;
        else
            mul = 1.5;

    armor = (armor * (MechType(mech) == CLASS_MECH ? 2 : 1));
    intern = (int)(intern * mul);
    mul = 1.00;
    ...
    if (type != CLASS_MECH)
        defbv = (int)(((defbv * (move == MOVE_TRACK ? 0.8 : move == MOVE_WHEEL ? 0.7 : move == MOVE_HOVER ? 0.6 : move == MOVE_VTOL ? 0.4 : move == MOVE_FOIL || move == MOVE_SUB || move == MOVE_HULL ? 0.5 : 1.0)) - deduct));
...
*/
    ...
    offweapbv = (int)(offweapbv * mul);

    if (type != CLASS_AERO && type != CLASS_DS && MechJumpSpeed(mech) > 0)
        offweapbv += (int)(((MechJumpSpeed(mech) / MP1) * (100 * (MechTons(mech) / 5))));

    offbv = offweapbv;

    mul = (skillmul[LAZY_SKILLMUL(gunskl)][LAZY_SKILLMUL(pilskl)]);

    return (int)(((offbv + defbv) / 100) * mul);
}
...
int ProperInternal(MECH * mech) 
{
    ...
    if (btechconf.btech_complexrepair) {
...
int alias_part(MECH * mech, int t, int loc) 
{
    ...
    if (btechconf.btech_complexrepair) {
        ...
        part = (locmod && (t == Special(SHOULDER_OR_HIP) ||
            t == Special(UPPER_ACTUATOR)) ? (locmod == 1 ? Cargo(TON_ARMUPPER_FIRST + tonmod) : Cargo(TON_LEGUPPER_FIRST + tonmod)) : locmod &&
            t == Special(LOWER_ACTUATOR) ? (locmod == 1 ? Cargo(TON_ARMLOWER_FIRST + tonmod) : Cargo(TON_LEGLOWER_FIRST + tonmod)) : locmod &&
            t == Special(HAND_OR_FOOT_ACTUATOR) ? (locmod == 1 ? Cargo(TON_ARMHAND_FIRST + tonmod) : Cargo(TON_LEGFOOT_FIRST + tonmod)) : t == Special(ENGINE) &&
            MechSpecials(mech) & XL_TECH ? Cargo(TON_ENGINE_XL_FIRST + tonmod) : t == Special(ENGINE) &&
            MechSpecials(mech) & ICE_TECH ? Cargo(TON_ENGINE_ICE_FIRST + tonmod) : t == Special(ENGINE) &&
            MechSpecials(mech) & CE_TECH ? Cargo(TON_ENGINE_COMP_FIRST + tonmod) : t == Special(ENGINE) &&
            MechSpecials(mech) & XXL_TECH ? Cargo(TON_ENGINE_XXL_FIRST + tonmod) : t == Special(ENGINE) &&
            MechSpecials(mech) & LE_TECH ? Cargo(TON_ENGINE_LIGHT_FIRST + tonmod) : t == Special(ENGINE) ? Cargo(TON_ENGINE_FIRST + tonmod) : t == Special(HEAT_SINK) &&
            MechSpecials(mech) & (DOUBLE_HEAT_TECH | CLAN_TECH) ?  Cargo(DOUBLE_HEAT_SINK) : t == Special(HEAT_SINK) &&
            MechSpecials2(mech) & COMPACT_HS_TECH ? Cargo(COMPACT_HEAT_SINK) : t == Special(GYRO) &&
            MechSpecials2(mech) & XLGYRO_TECH ? Cargo(TON_XLGYRO_FIRST + tonmod) : t == Special(GYRO) && 
            MechSpecials2(mech) & HDGYRO_TECH ?  Cargo(TON_HDGYRO_FIRST + tonmod) : t == Special(GYRO) &&
            MechSpecials2(mech) & CGYRO_TECH ? Cargo(TON_CGYRO_FIRST + tonmod) : t == Special(GYRO) ? Cargo(TON_GYRO_FIRST + tonmod) : t == Special(SENSORS) ? Cargo(TON_SENSORS_FIRST + tonmod) : t == Special(JUMP_JET) ? Cargo(TON_JUMPJET_FIRST + tonmod) : t);
    } else 
        part = (IsActuator(t) ? Cargo(S_ACTUATOR) :
            t == Special(ENGINE) && MechSpecials(mech) & XL_TECH ? Cargo(XL_ENGINE) : t == Special(ENGINE) &&
            MechSpecials(mech) & ICE_TECH ? Cargo(IC_ENGINE) : t == Special(ENGINE) &&
            MechSpecials(mech) & CE_TECH ? Cargo(COMP_ENGINE) : t == Special(ENGINE) &&
            MechSpecials(mech) & XXL_TECH ? Cargo(XXL_ENGINE) : t == Special(ENGINE) &&
            MechSpecials(mech) & LE_TECH ? Cargo(LIGHT_ENGINE) : t == Special(HEAT_SINK) &&
            MechSpecials(mech) & (DOUBLE_HEAT_TECH | CLAN_TECH) ?  Cargo(DOUBLE_HEAT_SINK) : t == Special(HEAT_SINK) &&
            MechSpecials2(mech) & COMPACT_HS_TECH ?  Cargo(COMPACT_HEAT_SINK) : t == Special(GYRO) &&
            MechSpecials2(mech) & XLGYRO_TECH ? Cargo(XL_GYRO) : t == Special(GYRO) &&
            MechSpecials2(mech) & HDGYRO_TECH ? Cargo(HD_GYRO) : t == Special(GYRO) &&
            MechSpecials2(mech) & CGYRO_TECH ? Cargo(COMP_GYRO) : t);
...
int HeatFactor(MECH * mech) 
{
    ...
    factor = (int)((MechPlusHeat(mech) + (2 * (MechPlusHeat(mech) - MechMinusHeat(mech)))));
    return ((NullSigSysActive(mech) || HasWorkingECMSuite(mech) || StealthArmorActive(mech)) ? -1 : factor);
}
