...
/* 'nice' sensor stuff's in the mech.sensor.c ; nasty brute code lies here */
/* from here onwards.. black magic happens. Enter if you're sure of your peace of mind :-P */
/* -------------------------------------------------------------------- */
...
float ActualElevation(MAP * map, int x, int y, MECH * mech)
{
    ...
    if (MechType(mech) == CLASS_MECH && !Fallen(mech))
        ...
        if (MechMove(mech) == MOVE_NONE)
            ...
            if (IsDS(mech))
                return (float) MechZ(mech) + 2.5 + (MechType(mech) == CLASS_DS ? 0 : 2);
...
extern int TraceLOS(MAP * map, int ax, int ay, int bx, int by, lostrace_info ** result);
...
int CalculateLOSFlag(MECH * mech, MECH * target, MAP * map, int x, int y, int ff, float hexRange)
{
    ...
    if (hexRange > (((MechSpecials(mech) & AA_TECH) || (target && (MechSpecials(target) & AA_TECH))) ? 180 : map->maxvis))
        return new_flag + MECHLOSFLAG_BLOCK;
...
/* Definition of 'both worlds': According to FASA, if a mech is half
   ...
   'both worlds' variables test for. */
    ...
    bothworlds = IsWater(MechRTerrain(mech)) && (((MechType(mech) == CLASS_MECH) && (MechZ(mech) == -1)) || ((WaterBeast(mech)) && (MechZ(mech) == 0)) || ((MechMove(mech) == MOVE_HOVER) && (MechZ(mech) == 0)));
    ...
    if (target) {
        ...
        t_bothworlds = IsWater(MechRTerrain(target)) && (((MechType(target) == CLASS_MECH) && (MechZ(target) == -1)) || ((WaterBeast(target)) && (MechZ(target) == 0)) || ((MechMove(target) == MOVE_HOVER) && (MechZ(target) == 0)));
    ...
    if (((underwater) && !(t_underwater || t_bothworlds)) || ((t_underwater) && !(underwater || bothworlds))) 
        return new_flag + MECHLOSFLAG_BLOCK;
    ...
    if (coordcount > 0) {    /* not in same hex ; in same hex, you see always */
        for (i = 0; i < coordcount; i++) {
            ...
            if (coords[i].x < 0 || coords[i].x >= map->map_width || coords[i].y < 0 || coords[i].y >= map->map_height)
                continue;
        
            /* Should be possible to see into water.. perhaps. But not on vislight */
            ...
            if (underwater) {
                /* LoS hits sea floor */
                if (!(IsWater(terrain)) || (terrain != BRIDGE && height >= pos_z)) {
    ...
    if (coordcount >= 2)
        if (dopartials) {
            if (MechZ(target) >= MechZ(mech) && (Elevation(map, coords[coordcount - 2].x, coords[coordcount - 2].y) == (MechZ(target) + 1)))
...
int AddTerrainMod(MECH * mech, MECH * target, MAP * map, float hexRange, int wAmmoMode)
{
    ...
    if (mech && target) {
        ...
        return Sensor_ToHitBonus(mech, target, MechToMech_LOSFlag(map, mech, target), map->maplight, hexRange, wAmmoMode);
    }
...
int InLineOfSight(MECH * mech, MECH * target, int x, int y, float hexRange)
{
    ...
    if (!map) {
        ...
        SendError(tprintf("InLineOfSight:invalid map:Mech %d  Index %d", mech->mynum, mech->mapindex));
        mech->mapindex = -1;
        return 0;
    }
    
    if (x < 0 || y < 0 || y >= map->map_height || x >= map->map_width) 
        SendError(tprintf("x:%d y:%d out of bounds for #%d (LOS check)", x, y, mech ? mech->mynum : -1));
    ...
    if (mech && target) {
        if (MechToMech_LOSFlag(map, mech, target) & (MECHLOSFLAG_SEESP | MECHLOSFLAG_SEESS))
            return MechToMech_LOSFlag(map, mech, target) & (MECHLOSFLAG_SEESP | MECHLOSFLAG_SEESS | MECHLOSFLAG_BLOCK);
    
        return 0;
    }
    
    losflag = CalculateLOSFlag(mech, NULL, map, x, y, 0, hexRange);
    return Sensor_CanSee(mech, NULL, &losflag, arc, hexRange, map->mapvis, map->maplight, map->cloudbase);
}


void mech_losemit(dbref player, void *data, char *buffer)
{
