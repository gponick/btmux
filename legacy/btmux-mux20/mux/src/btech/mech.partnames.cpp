...
static CHashTable short_hash, vlong_hash;
...
#define DASH(fromval,tohash) \
  ...
  *tmpc2 = 0; \
  hashaddLEN(tmpbuf, strlen(tmpbuf), (int *) (i+1), &tohash);
...
extern bool wildcard_match(const char *, const char *);
...
int find_matching_vlong_part(char *wc, int *ind, int *id, int *brand)
{
    ...
    *tmpc2 = 0;
    if ((i = static_cast <int *>(hashfindLEN(tmpbuf, strlen(tmpbuf), &vlong_hash)))) {
...
int find_matching_short_part(char *wc, int *ind, int *id, int *brand)
{
    ...
    *tmpc2 = 0;
    ...
    if ((i = static_cast < int *>(hashfindLEN(tmpbuf, strlen(tmpbuf), &short_hash)))) 
        if ((p = short_sorted[((int) i) - 1])) {
            *ind = ((int) i);
            UNPACK_PART(p->index, *id, *brand);
            return 1;
...
void ListForms(dbref player, void *data, char *buffer)
{
    ...
    for (i = 0; i < object_count; i++)
        notify(player, tprintf("%3d %-20s %-25s %s", i, short_sorted[i]->shorty, short_sorted[i]->longy, short_sorted[i]->vlongy));
}
...
FUNCTION(fun_btpartmatch)
{
    ...
    FUNCHECK(!WizR(executor), "#-1 PERMISSION DENIED");
