...
void mech_createbays(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK((argc = mech_parseattributes(buffer, args, NUM_BAYS + 1)) == (NUM_BAYS + 1), "Invalid number of arguments!");
    for (i = 0; i < argc; i++) {
        ...
        map = static_cast <MAP *>(FindObjectsData(it));
        AeroBay(ds, i) = it;
        map->onmap = ds->mynum;
    }
...
int Find_DS_Bay_Number(MECH * ds, int dir)
{
    ...
    for (i = 0; i <= dir; i++) {
        for (j = 0; j < NUM_CRITICALS; j++)
            if (GetPartType(ds, dir2loc[i % 6], j) == I2Special(DS_MECHDOOR) || GetPartType(ds, dir2loc[i % 6], j) == I2Special(DS_AERODOOR))
                break;
...
int Find_DS_Bay_In_MechHex(MECH * seer, MECH * ds, int *bayn)
{
    ...
    for (i = t; i < (t + 6); i++) {
        if (((MechX(ds) + dirs[i % 6][0]) == MechX(seer)) && ((MechY(ds) + dirs[i % 6][1] + KLUDGE(MechX(ds), MechX(ds) + dirs[i % 6][0])) == MechY(seer))) {
...
static void mech_enterbay_event(EVENT * e)
{
    ...
    MechLOSBroadcast(mech, tprintf("has entered %s at %d,%d.", GetMechID(ds), MechX(mech), MechY(mech)));
        ...
        if (AeroBay(ds, i) > 0)
            if (AeroBay(ds, i) == bayref) {
                ...
                for (i = 0; i < NUM_CRITICALS; i++) {
                    if (((is_aero(mech) && GetPartType(ds, dir2loc[j], i) == I2Special(DS_AERODOOR)) || (!is_aero(mech) && GetPartType(ds, dir2loc[j], i) == I2Special(DS_MECHDOOR))) && !PartIsDestroyed(ds, dir2loc[j], i))
                        return 1;
                }
                return 0;
            }
...
static int DS_Bay_Is_EnterOK(MECH * mech, MECH * ds, dbref bayref)
{
    ...
    for (i = 0; i < NUM_BAYS; i++)
        if (AeroBay(ds, i) > 0)
            if (AeroBay(ds, i) == bayref)
                return muxevent_count_type_data2(EVENT_ENTER_HANGAR, (void *) bayref) > 0 ? 0 : 1;
...
void mech_enterbay(dbref player, void *data, char *buffer)
{
    ...
    int argc;
    dbref ref = -1, bayn = -1;
    MECH *mech = static_cast <MECH *>(data), *ds;
    MAP *map;
    ...
    DOCHECK((argc = mech_parseattributes(buffer, args, 2)) == 2, "Hmm, invalid number of arguments?");

    if (argc > 0)
        DOCHECK((ref = FindTargetDBREFFromMapNumber(mech, args[0])) <= 0, "Invalid target!");
    
    if (ref < 0) {
        DOCHECK(!Find_Single_DS_In_MechHex(mech, &ref, &bayn), "No DS bay found in your hex!");
        DOCHECK(ref < 0, "Multiple enterable things found ; use the id for specifying which you want.");
        DOCHECK(!(ds = getMech(ref)), "You sense wrongness in fabric of space.");
    } else {
        DOCHECK(!(ds = getMech(ref)), "You sense wrongness in fabric of space.");
        DOCHECK(!Find_DS_Bay_In_MechHex(mech, ds, &bayn), "You see no bays in your hex.");
    }
    
    DOCHECK(IsDS(mech) && !(MechSpecials2(mech) & CARRIER_TECH), "Your craft can't enter bays.");
    DOCHECK(!DS_Bay_Is_Open(mech, ds, AeroBay(ds, bayn)), "The door has been jammed!");
    DOCHECK(IsDS(mech), "Your unit is a bit too large to fit in there.");
    DOCHECK((fabs((float) (MechSpeed(mech) - MechSpeed(ds)))) > MP1, "Speed difference's too large to enter!");
    DOCHECK(MechZ(ds) != MechZ(mech), "Get to same elevation before thinking about entering!");
    DOCHECK(abs((int)(MechVerticalSpeed(mech) - MechVerticalSpeed(ds))) > 10, "Vertical speed difference is too great to enter safely!");
    DOCHECK(MechType(mech) == CLASS_MECH && !MechIsQuad(mech) && (IsMechLegLess(mech)), "Without legs? Are you kidding?");
    ...
    DOCHECK(!DS_Bay_Is_EnterOK(mech, ds, AeroBay(ds, bayn)), "Someone else is using the door at the moment.");
    DOCHECK(!(map = getMap(mech->mapindex)), "You sense a wrongness in fabric of space.");
    
    HexLOSBroadcast(map, MechX(mech), MechY(mech), "The bay doors at $h start to open..");
...
static int Leave_DS_Bay(MAP * map, MECH * ds, MECH * mech, dbref frombay)
{
    ...
    DOCHECKMA0(mech->mapindex == map->mynum, "Fatal error: Unable to find the map 'ship is on.");
...
int Leave_DS(MAP * map, MECH * mech)
{
    ...
    DOCHECKMA0(!DS_Bay_Is_Open(mech, car, map->mynum), "The door has been jammed!");
    DOCHECKMA0(!Landed(car) && !FlyingT(mech), "The 'ship is still airborne!");
    DOCHECKMA0(Zombie(car->mynum), "You don't feel leaving right now would be prudent..");
