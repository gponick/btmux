...
int Sensor_ToHitBonus(MECH * mech, MECH * target, int flag, int maplight, float range, int wAmmoMode)
{
    ...
    if (!(flag & MECHLOSFLAG_SEESP)) {
        bth2 = 1 + sensors[(int) MechSensor(mech)[1]].tohitbonus_func(mech, target, map, flag, maplight);
    ...
    if (!(flag & MECHLOSFLAG_SEESS) || (MechSensor(mech)[0] == MechSensor(mech)[1])) {
        bth1 = sensors[(int) MechSensor(mech)[0]].tohitbonus_func(mech, target, map, flag, maplight);
#ifdef SENSOR_BTH_DEBUG
...
int Sensor_CanSee(MECH * mech, MECH * target, int *flag, int arc, float range, int mapvis, int maplight, int cloudbase)
{
    int i, j = 0, sn;
    MAP *map = getMap(mech->mapindex);
...
#define MAXVR(mech,sensor) MaxVis(mech,sensor) - MaxVar(mech, sensor)
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (!sensors[sn].fullvision && !(arc & (FORWARDARC | TURRETARC)) && !Sees360(mech))
                continue;
            ...
            if (target) {
                if (cloudbase && sn < 3 && ((MechZ(mech) < cloudbase) ? (MechZ(target) >= cloudbase) : (MechZ(target) < cloudbase)))
                    continue;
            } else 
            ...
            if (MaxVar(mech, sn) && (MAXVR(mech, sn)) < range)
                if (((MAXVR(mech, sn)) + (MechVisMod(mech) * (MaxVar(mech, sn) + 1)) / 100) < range)
                    continue;
    ...
    if (cloudbase && target && sn < 3 && ((MechZ(mech) < cloudbase) ? (MechZ(target) >= cloudbase) : (MechZ(target) < cloudbase)))
        return 0;
...
int Sensor_Sees(MECH * mech, MECH * target, int f, int arc, float range, int snum, int chance_divisor, int mapvis, int maplight)
{
    MAP *map = getMap(mech->mapindex);
    int chance = (Sensor_ArcBaseChance(MechType(mech), arc) * ((target && MechTeam(mech) != MechTeam(target)) ?  Sensor_DriverBaseChance(mech) : 100)) / 100;
    int ch2 = sensors[snum].seechance_func(target, map, snum, range, mapvis, maplight);
    ...
    if (target && MechCritStatus(target) & HIDDEN && MechTeam(mech) != MechTeam(target)) {
        if (sensors[snum].matchletter[0] == 'B' && (MechInfantrySpecials(target) & STEALTH_TECH) && (!(MechInfantrySpecials(target) & CS_PURIFIER_STEALTH_TECH)))
            return 0;
    ...
    if (range < 3 || Number(1, 10000) < ((chance * ch2) / chance_divisor)) {
        if (target && In_Character(mech->mynum) && In_Character(target->mynum) && MechTeam(mech) != MechTeam(target))
            if (!Number(0, 5))
...
int Sensor_SeesNow(MECH * mech, MECH * target, int f, int arc, float range, int mapvis, int maplight)
{
    int i, sn;
...
static int valid_to_notice(MECH * mech, MECH * targ, int los)
{
    ...
    if ((los < 0 && MechSeemsFriend(mech, targ)) || (los > 0 && MechTeam(mech) == MechTeam(targ)))
        foe = 0;
    else
        foe = AUTOCON_WARN;
...
void Sensor_DoWeSeeNow(MECH * mech, unsigned short *fl, float range, int x, int y, MECH * target, int mapvis, int maplight, int cloudbase, int seeanew, int wlf)
{
    int arc;
    float x1, y1;
    int sc, sl, st;
    ...
    if (f & MECHLOSFLAG_SEEN) {
        if ((sl = Sensor_CanSee(mech, target, &f, arc, range, mapvis, maplight, cloudbase))) {
            if (sl & 1)
                f |= MECHLOSFLAG_SEESP;
        ...
        if (!(f & (MECHLOSFLAG_SEESP | MECHLOSFLAG_SEESS))) {
            ...
            if ((Started(target) || SeeWhenShutdown(target) || MechAutoconSD(mech)) && (st = valid_to_notice(mech, target, wlf)) && seeanew < 3) {
                if (st & AUTOCON_WARN)
                    strcpy(buf, "%cy");
                else
                    buf[0] = 0;
                ...
                if (st & AUTOCON_SHORT)
                    sprintf(buf + strlen(buf), "Lost: %s, %s arc.", GetMechToMechID_base(mech, target, wlf), GetArcID(mech, arc));
                else
                    sprintf(buf, "You have lost %s from your scanners. It was last in your %s arc.", GetMechToMechID_base(mech, target, wlf), GetArcID(mech, arc));
            ...
            if ((MechStatus(mech) & LOCK_TARGET) && target->mynum == MechTarget(mech)) {
                mech_notify(mech, MECHALL, "Weapon system reports the lock has been lost.");
                LoseLock(mech);
            }

#ifdef SENSOR_DEBUG
            sprintf(buf, "Notice: #%d lost #%d (Sensor: %d, Flag: %s)", mech->mynum, target->mynum, (f & (MECHLOSFLAG_SEESP | MECHLOSFLAG_SEESS)), my_dump_flag(f));
            SendSensor(buf);
#endif
            MechPNumSeen(mech)++;
        }
    ...
    if ((sc = Sensor_CanSee(mech, target, &f, arc, range, mapvis, maplight, cloudbase))) {
        if (!seeanew) {
        ...
        if ((f & (MECHLOSFLAG_SEESP | MECHLOSFLAG_SEESS))) {
            ...
            *fl = f;
            ...
            if ((Started(target) || SeeWhenShutdown(target) || MechAutoconSD(mech)) && (st = valid_to_notice(mech, target, -1)) && seeanew < 2) {
                if (st & AUTOCON_WARN)
                    strcpy(buf, "%cr");
                else
                    buf[0] = 0;
                ...
                if (st & AUTOCON_SHORT)
                    sprintf(buf + strlen(buf), "Seen: %s, %s arc.", GetMechToMechID(mech, target), GetArcID(mech, arc));
                else
                    sprintf(buf + strlen(buf), "You notice %s in your %s arc.", GetMechToMechID(mech, target), GetArcID(mech, arc));
            ...
            sprintf(buf, "Notice: #%d saw #%d (Sensor: %d, Flag: %s C:%d)", mech->mynum, target->mynum, (f & (MECHLOSFLAG_SEESP | MECHLOSFLAG_SEESS)), my_dump_flag(f), seeanew);
            SendSensor(buf);
#endif
        } else
            MechPNumSeen(mech)++;
    ...
    for (i = 0; i < map->first_free; i++) {
        ...
        for (j = i + 1; j < map->first_free; j++)
            if (map->mechflags[i] || map->mechflags[j]) {
                ...
                if (ECMEnabled(mech) || ECCMEnabled(mech) || AngelECMEnabled(mech) || AngelECCMEnabled(mech))
                    if (range < ECM_RANGE)
                        checkECM(target);
                ...
                wlf = !(map->LOSinfo[i][j] & MECHLOSFLAG_BLOCK) && ((map->LOSinfo[i][j] & MECHLOSFLAG_SEESP) || (map->LOSinfo[i][j] & MECHLOSFLAG_SEESS));
                map->LOSinfo[i][j] = fl = CalculateLOSFlag(mech, target, map, MechX(target), MechY(target), map->LOSinfo[i][j], (float) range);

                /* Then, we update the SEES* */
                Sensor_DoWeSeeNow(mech, &map->LOSinfo[i][j], range, -1, -1, target, mapvis, maplight, map->cloudbase, 0, wlf);
            }
    }
            ...
            if (map->mechflags[i] || map->mechflags[j]) {
                ...
                if (Started(target))
                    if (map->LOSinfo[j][i] & MECHLOSFLAG_BLOCK) {
                        wlf = !(map->LOSinfo[i][j] & MECHLOSFLAG_BLOCK) && ((map->LOSinfo[i][j] & MECHLOSFLAG_SEESP) || (map->LOSinfo[i][j] & MECHLOSFLAG_SEESS));
                        map->LOSinfo[i][j] = MECHLOSFLAG_BLOCK | (map->LOSinfo[i][j] & (MECHLOSFLAG_SEEN | MECHLOSFLAG_SEEC2));
                        Sensor_DoWeSeeNow(mech, &map->LOSinfo[i][j], range, -1, -1, target, mapvis, maplight, map->cloudbase, 0, wlf);
                        continue;
                    }
...
void add_sensor_info(char *buf, MECH * mech, int sn, int verbose)
{
    if (!verbose)
        ...
        sprintf(buf + strlen(buf), "\n\tRange:      %s\n\tBlocked by: %s", sensors[sn].range_desc, sensors[sn].block_desc);
        if (sensors[sn].special_desc)
            sprintf(buf + strlen(buf), "\n\tNotes:      %s", sensors[sn].special_desc);
    }
}
...
static char *sensor_mode_name(MECH * mech, int sn, int full, int verbose)
{
    ...
    if (sensors[sn].fullvision) {
        ...
        if (full || Sees360(mech))
            sprintf(buf, "%s in 360 degree scanning mode ", sensors[sn].sensorname);
        else
            sprintf(buf, "%s in 120 degree scanning mode (Forward arc) ", sensors[sn].sensorname);
...
static void sensor_mode(MECH * mech, char *msg, dbref player, int p, int s, int verbose)
{
    char buf[MBUF_SIZE];
    int i;
    ...
    if (p != s) {
        ...
        notify(player, msg);
        notify(player, buf);
        notify(player, tprintf("Primary:   %s", sensor_mode_name(mech, p, 0, verbose)));
        notify(player, tprintf("Secondary: %s", sensor_mode_name(mech, s, 0, verbose)));
    } else
        notify(player, tprintf("%s: %s", msg, sensor_mode_name(mech, p, 1, verbose)));
}
...
{
    tmp_found = 0;
    sensor_mode(mech, "Sensors", player, MechSensor(mech)[0], MechSensor(mech)[1], verbose);
...
int CanChangeTo(MECH * mech, int s)
{
    ...
    if ((i = sensors[s].required_special)) 
        ...
        if (sensors[s].specials_set == 1) 
            if ((i > 0) == ((!(MechSpecials(mech) & abs(i))) != 0)) {
                mech_notify(mech, MECHALL, tprintf("You lack the %s sensors!", sensors[s].sensorname));
                return 0;
            }
        else 
    ...
    if (sensors[s].min_light >= 0 && sensors[s].min_light > map->maplight) 
        if (!Destroyed(mech) && Started(mech)) {
            mech_notify(mech, MECHALL, tprintf("It's now too dark to use %s!", sensors[s].sensorname));
            return 0;
        }
    
    if (sensors[s].max_light >= 0 && sensors[s].max_light < map->maplight)
        if (!Destroyed(mech) && Started(mech)) {
            mech_notify(mech, MECHALL, tprintf("The light's kinda too bright now to use %s!", sensors[s].sensorname));
            return 0;
        }

    switch (sensors[s].attributeCheck) {
        case SENSOR_ATTR_SEISMIC:
            if ((MechType(mech) == CLASS_MW) || (MechType(mech) == CLASS_BSUIT) || (MechType(mech) == CLASS_VEH_NAVAL) || (MechMove(mech) == MOVE_HOVER)) {
                mech_notify(mech, MECHALL, tprintf("You lack the %s sensors!", sensors[s].sensorname));
                return 0;
            }
            break;
...
static int set_sensor(MECH * mech, char ps, char ss)
{
    ...
    if (prim != MechSensor(mech)[0] || sec != MechSensor(mech)[1]) {
        ...
        MECHEVENT(mech, EVENT_SCHANGE, mech_sensorchange_event, SCHANGE_TICK, ((prim * NUM_SENSORS) + sec));
    }
    return 0;
}
...
void mech_sensor(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(MechType(mech) == CLASS_MW, "You're using your eyes, and nothing you can do changes that!");
    ...
    switch (argc) {
        ...
        case 1:
            ...
            break;
        case 2:
            DOCHECK(set_sensor(mech, mux_toupper(args[0][0]), mux_toupper(args[1][0])) < 0, "Invalid arguments!");
            show_sensor(player, mech, 0);
            break;
    }
}
    ...
    for (i = 0; i < map->first_free; i++)
        if (i != num && (j = map->mechsOnMap[i]) >= 0) {
            ...
            if (seer->mapindex != map->mynum) {
                SendError(tprintf("Mech #%d was on map #%d but with " "incorrect mapindex (%d)", seer->mynum, map->mynum, seer->mapindex));
                map->mechsOnMap[i] = -1;
                continue;
            }
            
            range = FaMechRange(seer, mech);
            map->LOSinfo[i][num] = CalculateLOSFlag(seer, mech, map, MechX(mech), MechY(mech), map->LOSinfo[i][num], (float) range);
            /* Then, we update the SEES* */
            /* seeanew used to be 2 ; I want them to know they notice it first not to bug me 'bout it, though */
        
            Sensor_DoWeSeeNow(seer, &map->LOSinfo[i][num], range, -1, -1, mech, mapvis, maplight, map->cloudbase, 2, 0);
        }
}
...
void ScrambleInfraAndLiteAmp(MECH * mech, int time, int chance, char *inframsg, char *liteampmsg)
{
    ...
    for (i = 0; i < mech_map->first_free; i++)
        if (mech_map->mechsOnMap[i] != -1 && mech_map->mechsOnMap[i] != mech->mynum)
            if ((tempMech = getMech(mech_map->mechsOnMap[i])))
                if (InLineOfSight(tempMech, mech, MechX(mech), MechY(mech), FaMechRange(tempMech, mech))) {
                    if (Blinded(tempMech) || Uncon(tempMech))
                        continue;
            
                    if (sensors[(int)MechSensor(tempMech)[0]].matchletter[0] == 'I' || sensors[(int)MechSensor(tempMech)[0]].matchletter[1] == 'I') {
                        if (chance)
                        ...
                        if (sensors[(int)MechSensor(tempMech)[0]].matchletter[0] == 'L' || sensors[(int)MechSensor(tempMech)[0]].matchletter[1] == 'L') {
                            if (chance)
                                if (Number(1, 100) > chance)
                                    continue;
