...
void decrement_ammunition(MECH * mech, int weapindx, int section, int critical, int ammoLoc, int ammoCrit, int ammoLoc1, int ammoCrit1, int wGattlingShots)
{
    ...
    if (MechWeapons[weapindx].special == ROCKET) {
        ...
        firstCrit = FindFirstWeaponCrit(mech, section, critical, 0, GetPartType(mech, section, critical), weapSize);
    ...
    ammo_expedinture_check(mech, weapindx, MAX(wGattlingShots, ((GetPartFireMode(mech, section, critical) & ULTRA_MODE) || (GetPartFireMode(mech, section, critical) & RFAC_MODE))));

    if ((GetPartFireMode(mech, section, critical) & GATTLING_MODE) || (MechWeapons[weapindx].special & RAC)) {
        ...
        while (wShotsLeft > 0) {
            FindAmmoForWeapon_sub(mech, section, critical, weapindx, section, &wGatSec, &wGatCrit, AMMO_MODES, 0);
            wCurShots = GetPartData(mech, wGatSec, wGatCrit);

            if (wCurShots) 
                if (wCurShots >= wShotsLeft) {
                    SetPartData(mech, wGatSec, wGatCrit, wCurShots - wShotsLeft);
                    wShotsLeft = 0;
                } else {
...
void mech_ammowarn_event(EVENT * e)
{
    ...
    mech_notify(mech, MECHALL, tprintf("%sWARNING: Ammo for %s is running low.%%c", sev ? "%ch%cr" : "%ch%cy", get_parts_long_name(I2Weapon(weapindx), 0)));
}
...
void ammo_expedinture_check(MECH * mech, int weapindx, int ns)
{
    ...
    if ((cnt == (t + ns)) || (ns && cnt >= t && cnt < (t + ns)))
        ...
        if ((cnt == (t2 + ns)) || (ns && cnt >= t2 && cnt < (t2 + ns)))
            sev = 0;
        else
            return;

    /* Okay, we have case of warning here */
    MECHEVENT(mech, EVENT_AMMOWARN, mech_ammowarn_event, 1, (sev * 65536 + weapindx));
}

void heat_effect(MECH * mech, MECH * tempMech, int heatdam, int fromInferno)
{
    if (MechType(tempMech) != CLASS_MECH && MechType(tempMech) != CLASS_MW && MechType(tempMech) != CLASS_BSUIT && !IsDS(tempMech) && MechMove(tempMech) != MOVE_NONE) {
        if (((MechType(tempMech) == CLASS_VEH_GROUND) || (MechType(tempMech) == CLASS_VTOL)) && btechconf.btech_fasaadvvhlfire) {
            if (fromInferno)
            ...
            if (Roll() > 8) {
                ...
                mech_notify(tempMech, MECHALL, "The heat's too much for your vehicle! It blows up!");
                Destroy(tempMech);
...
//    global_kill_cheat = 1;
    ...
    if (!btechconf.btech_ic || btechconf.btech_xploss >= 1000)
