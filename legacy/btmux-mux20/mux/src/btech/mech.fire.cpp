...
static void inferno_end_event(EVENT * e)
{
    ...
    mech_notify(mech, MECHALL, "You feel suddenly far cooler as the fires finally die.");
}
...
static void vehicle_burn_event(EVENT * objEvent)
{
    ...
    mech_notify(objMech, MECHALL, tprintf("%%cr%%chYour %s takes damage from the fire!%%cn", strLocName));
    ...
    if ((wDamRoll > 1) && (GetSectInt(objMech, wLoc)))
        MECHEVENT(objMech, EVENT_VEHICLEBURN, vehicle_burn_event, VEHICLEBURN_TICK, wLoc);
    else {
        if (GetSectInt(objMech, wLoc))
            mech_notify(objMech, MECHALL, tprintf("The fire burning on your %s finally goes out.", strLocName));
...
void vehicle_start_burn(MECH * objMech, MECH * objAttacker)
{
    ...
    for (wIter = 0; wIter < NUM_SECTIONS; wIter++) {
        if (GetSectInt(objMech, wIter) && !BurningSide(objMech, wIter)) {
            wDamage = Number(1, 6);
            ArmorStringFromIndex(wIter, strLocName, MechType(objMech), MechMove(objMech));
            mech_notify(objMech, MECHALL, tprintf("Your %s catches on fire!", strLocName));

            DamageMech(objMech, objAttacker, 0, -1, wIter, 0, 0, wDamage, 0, 0, 0, -1, 0, 1);
            MECHEVENT(objMech, EVENT_VEHICLEBURN, vehicle_burn_event, VEHICLEBURN_TICK, wIter);
        }
    }
}
...
void vehicle_extinquish_fire(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);

    cch(MECH_USUALS);

    DOCHECK(Started(mech), "Your tank is started! You can not extinguish the flames while your tank is started!");
    ...
    MECHEVENT(mech, EVENT_VEHICLE_EXTINGUISH, vehicle_extinquish_fire_event, VEHICLE_EXTINGUISH_TICK, 0);
}
...
void water_extinguish_inferno(MECH * mech)
{
    ...
    if (!InWater(mech) || MechType(mech) != CLASS_MECH || !Jellied(mech) || (elev == -1 && !Fallen(mech)))
        return;
...
void checkVehicleInFire(MECH * objMech, int fromHexFire)
{
    ...
    if (fromHexFire)
        mech_notify(objMech, MECHALL, "%cr%chYou drive through a wall of searing flames!%cn");
    else
    ...
    switch (wRoll) {
        ...
        case 9:
            if (MechType(objMech) == CLASS_VTOL) {
                ...
                mech_notify(objMech, MECHALL, "%cr%chThe fire damages your motive system!%cn");
        ...
        case 11:
            ...
            mech_notify(objMech, MECHALL, "%cr%chThe fire sweeps across your unit damaging it!%cn");

            for (wIter = 0; wIter < NUM_SECTIONS; wIter++) {
                wDamage = Number(1, 6);

                if (GetSectInt(objMech, wIter))
                    DamageMech(objMech, objMech, 0, -1, wIter, 0, 0, wDamage, 0, 0, 0, -1, 0, 1);
            }
            break;
        default:
