...
MECH *mech = static_cast <MECH *> (FindObjectsData(tur->parent)); \
...
{
    TURRET_T *newturret = static_cast <TURRET_T *>(*data);
...
{
    TUR_BASE;
    DOCHECK(player != tur->gunner && Connected(tur->gunner) && Location(tur->gunner) == Location(player), tprintf("You need %s to leave or disconnect first.", Name(tur->gunner)));
    DOCHECK(player == tur->gunner, "You grap firmer hold on the joystick..");
    notify_except(tur->mynum, NOTHING, tur->mynum, tprintf("%s initialized as gunner.", Name(player)));
    tur->gunner = player;
}
...
void turret_deinitialize(dbref player, void *data, char *buffer)
{
    ...
    notify_except(tur->mynum, NOTHING, tur->mynum, tprintf("%s deinitialized as gunner.", Name(player)));
    tur->gunner = -1;
}
