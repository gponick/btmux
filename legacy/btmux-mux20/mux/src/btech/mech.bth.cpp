...
int FindNormalBTH(MECH * mech, MAP * mech_map, int section, int critical, int weapindx, float range, MECH * target, int indirectFire, dbref * c3Ref)
{
    MECH *spotter = NULL;
    int baseToHit;
    ...
    *c3Ref = -1;
    ...
    if (EGunRange(weapindx) < range) 
        ...
        if ((MechWeapons[weapindx].min >= range) && (MechWeapons[weapindx].min > 0)) {
            if (!HotLoading(weapindx, GetPartFireMode(mech, section, critical))) 
                /* if the target is in minimum range then the BTH is as good as it will get */
                rbth = (int)(MechWeapons[weapindx].min - range + 1);
            else 
                if (btechconf.btech_hotloadaddshalfbthmod) 
                    rbth = (int)((MechWeapons[weapindx].min - range + 2) / 2);
            BTHADD("MinRange", rbth);
        } else 
            if (HasC3(mech) && !C3Destroyed(mech) && !AnyECMDisturbed(mech) && (MechC3NetworkSize(mech) > 0)) {
                wRangeBracket = FindBTHByC3Range(mech, target, section, weapindx, range, findC3Range(mech, target, range, c3Ref, 1), wAmmoMode, &rbth);
                BTHADD("C3Range", rbth);
            } else 
                if (HasC3i(mech) && !C3iDestroyed(mech) && !AnyECMDisturbed(mech) && (MechC3iNetworkSize(mech) > 0)) {
                    wRangeBracket = FindBTHByC3Range(mech, target, section, weapindx, range, findC3Range(mech, target, range, c3Ref, 0), wAmmoMode, &rbth);
                    BTHADD("C3iRange", rbth);
                } else {
                    wRangeBracket = FindBTHByRange(mech, target, section, weapindx, range, wFireMode, &rbth);
                    BTHADD("Range", rbth);
                }
 ...
 * that putting it into FindBTH*().  */
    if (MechTargComp(mech) == TARGCOMP_SHORT || MechTargComp(mech) == TARGCOMP_LONG) {
        ...
        if (tmp_range > (SectionUnderwater(mech, section) ? GunWaterRange(weapindx) : GunRange(weapindx)))
            BTHADD("TargComp/Long", MechTargComp(mech) == TARGCOMP_LONG ? -1 : 1);
        else 
            if (tmp_range <= (SectionUnderwater(mech, section) ? MechWeapons[weapindx].  medrange_water : MechWeapons[weapindx].medrange))
        ...
        if (MechInfantrySpecials(target) & FWL_ACHILEUS_STEALTH_TECH) {
            if (wRangeBracket == RANGE_SHORT)
                ...
                if (wRangeBracket == RANGE_MED)
                    ...
                    if (wRangeBracket == RANGE_LONG)
                        BTHADD("FWLStealthBonus", 3);
        } else 
            if (MechInfantrySpecials(target) & DC_KAGE_STEALTH_TECH) {
                if (wRangeBracket == RANGE_MED)
                ...
                if (MechInfantrySpecials(target) & FC_INFILTRATOR_STEALTH_TECH) {
                    if (wRangeBracket == RANGE_MED)
                    ...
                    if (MechInfantrySpecials(target) & FC_INFILTRATORII_STEALTH_TECH) {
                        if (wRangeBracket == RANGE_SHORT)
    ...
    if (target && (wAmmoMode & NARC_MODE) && (!(MechWeapons[weapindx].special & NARC)) && checkAllSections(target, INARC_HOMING_ATTACHED))
        BTHADD("iNARC", -1);
    ...
    /* Unstable lock */
    if (!arc_override && (!spotter && target && ((MechTarget(mech) != target->mynum) || (Locking(mech) && MechTargComp(mech) != TARGCOMP_MULTI)))) 
    ...
    if (!target && (MechStatus(mech) & (LOCK_HEX | LOCK_BUILDING | LOCK_HEX_IGN | LOCK_HEX_CLR)))
        BTHADD("HexBonus", -4);
    ...
    if (target) {
        ...
        if (MechDugIn(target) && (!btechconf.btech_dig_only_fs && (FindAreaHitGroup(mech, target) == FRONT)) && (MechZ(target) >= MechZ(mech)))
            BTHADD("DugIn", btechconf.btech_digbonus);
        ...
        if (target && !IsMissile(weapindx) && (((MechAim(mech) == HEAD) && ((MechType(target) == CLASS_MECH) || (MechType(target) == CLASS_MW))) || ((MechAim(mech) == AERO_COCKPIT) && (MechType(target) == CLASS_AERO)))) 
            if (Immobile(target))
                BTHADD("HeadTarget", 7);
            else
                BTHADD("HeadTarget-Fake", 25);
        else 
            if ((GetPartFireMode(mech, section, critical) & ON_TC) && !(MechCritStatus(mech) & TC_DESTROYED) && !(wAmmoMode & LBX_MODE)) 
        ...
        if (btechconf.btech_moddamagewithwoods && IsForestHex(mech_map, MechX(target), MechY(target)) && ((MechZ(target) - 2) <= Elevation(mech_map, MechX(target), MechY(target)))) 
...
int FindArtilleryBTH(MECH * mech, int section, int weapindx, int indirect, float range)
{
    int baseToHit = 11;
    MECH *spotter;
    ...
    if (indirect) {
        ...
        if (indirect && (MechSpotter(mech) == -1 || MechSpotter(mech) == mech->mynum))
            baseToHit += 1;
    } else
        baseToHit -= 2;
    ...
    int weapindx, float frange, int mode, int *wBTH)
{
    ...
    if (target)
        wTargetStealth = (StealthArmorActive(target) || NullSigSysActive(target));
    ...
    int weapindx, float realRange, float c3Range, int mode, int *wBTH)
{
    ...
    if (!HotLoading(weapindx, mode)) {
        if (btechconf.btech_hotloadaddshalfbthmod)
            *wBTH = (int)((MechWeapons[weapindx].min - realRange + 1) / 2);
        else
            *wBTH = 0;
    ...
    *wBTH = (int)(MechWeapons[weapindx].min - realRange + 1);
    return RANGE_SHORT;
}
...
int AttackMovementMods(MECH * mech)
{
    ...
    if ((!MechIsQuad(mech) || (MechIsQuad(mech) && CountDestroyedLegs(mech) > 0)) && Fallen(mech) && !IsDS(mech))
        return 2;
...
int TargetMovementMods(MECH * mech, MECH * target, float range)
{
    ...
    int returnValue = 0;
    float m = 1.0;
    MAP *map = static_cast < MAP * >(FindObjectsData(target->mapindex));
    MECH *swarmTarget;
    ...
    if (is_aero(target)) {
        ...
        target_speed = (float) my_sqrt((double) MechSpeed(target) / m, (double) MechVerticalSpeed(target) / m);
    } else 
        if (Jumping(target)) 
    ...
    if (MechInfantrySpecials(target) & CS_PURIFIER_STEALTH_TECH) 
        if (target_speed == 0.0) /* Mech moved 0-2 hexes */
                returnValue = 3;
        else 
            if (target_speed <= MP1) /* Mech moved 3-4 hexes */
                returnValue = 2;
            else 
                if (target_speed <= MP2) /* Mech moved 5-6 hexes */
                    returnValue = 1;
                 else
                    returnValue = 0;
    else 
        if (target_speed <= MP2) /* Mech moved 0-2 hexes */
            returnValue = 0;
        else 
            if (target_speed <= MP4) /* Mech moved 3-4 hexes */
                returnValue = 1;
            else 
                if (target_speed <= MP6) /* Mech moved 5-6 hexes */
                    returnValue = 2;
                else 
                    if (target_speed <= MP9) /* Mech moved 7-9 hexes */
                        returnValue = 3;
                    else /* Moving more than 9 hexes */
                        if (btechconf.btech_extendedmovemod)
                            returnValue = (int)(4 + (target_speed - 10 * MP1) / MP4);
                        else
                            returnValue = 4;
    ...
    if (Fallen(target) && ((MechType(target) == CLASS_MECH) || (MechType(target) == CLASS_MW)))
