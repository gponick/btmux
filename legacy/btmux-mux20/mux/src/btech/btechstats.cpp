...
static int char_xp_bonus(PSTATS * s, int code)
{
    ...
    xp = s->xp[code];
    while (xp > (t * count)) {
    xp -= t * count * (btechconf.btech_ic ? 1 : 4);
    count++;
    }
    return count - 1;
#endif
...
CHashTable playervaluehash, playervaluehash2;
...
int char_getvaluecode(char *name)
{
    ...
    *tmpc2 = mux_tolower(*tmpc1);
    *tmpc2 = 0;
    if ((ip =
        (int *) hashfindLEN(tmpbuf, strlen(tmpbuf),
        &playervaluehash)) == NULL)
    ip = (int *) hashfindLEN(tmpbuf, strlen(tmpbuf),
        &playervaluehash2);
    free_sbuf(tmpbuf);
    return ((int) ip) - 1;
}
...
int char_gainxpbycode(dbref player, int code, int amount)
{
    ...
    if (!((ltaNow > (s->last_use[code] + time_30s)) ||
        (char_values[code].flag & SK_XP)))
    return 0;
    s->last_use[code] = ltaNow;
    s->xp[code] += amount;
    s->xp[code] =
...
int char_getskillsuccess(dbref player, char *name, int modifier, int loud)
{
    ...
    notify(player, tprintf("Modified skill BTH : %d Roll : %d", val,
        roll));
    }
...
void init_btechstats(void)
{
    ...
    for (tmpc1 = char_values[i].name, tmpc2 = tmpbuf; *tmpc1;
        ...
        *tmpc2 = mux_tolower(*tmpc1);
    *tmpc2 = '\0';
    hashaddLEN(tmpbuf, strlen(tmpbuf), (int *) (i + 1),
        &playervaluehash);
    tmpbuf[0] = '\0';
    tmpc1 = tmpbuf;
    for (j = 0; char_values[i].name[j]; j++) {
        if (!mux_isupper(char_values[i].name[j]))
        continue;
...
static void show_charstatus(dbref player, PSTATS * s, dbref thing)
{
    ...
    if ((pattr = atr_str("FACTION")) && pattr->number)
        p = atr_get(thing, pattr->number, &aowner, &aflags);
...
void do_charstatus(dbref player, dbref cause, int key, char *arg1)
{
    ...
    if (arg1 && *arg1) {
    thing = char_lookupplayer(player, cause, 0, arg1);
...
void do_charclear(dbref player, dbref cause, int key, char *arg1)
{
    ...
    atr = mkattr(thing, "ATTRS");
    if (!atr)
    return;
    atr_pget_info(thing, atr, &aowner, &aflags);
    atr_add(thing, atr, "", aowner, aflags);

    atr = mkattr(thing, "SKILLS");
    if (!atr)
    return;
    atr_pget_info(thing, atr, &aowner, &aflags);
    atr_add(thing, atr, "", aowner, aflags);

    atr = mkattr(thing, "ADVS");
    if (!atr)
    return;
    atr_pget_info(thing, atr, &aowner, &aflags);
    atr_add(thing, atr, "", aowner, aflags);

    atr = mkattr(thing, "HEALTH");
    if (!atr)
    return;
    atr_pget_info(thing, atr, &aowner, &aflags);
    atr_add(thing, atr, "", aowner, aflags);
...
#define PC_LOCS 4
    ...
    if ((pattr = atr_str("PCEQUIP")) && pattr->number)
    c = atr_get(player, pattr->number, &aowner, &aflags);
...
void AccumulatePilXP(dbref pilot, MECH * mech, int reason, int addanyway)
{
     ...
     */
    if (char_gainxp(pilot, skname, xp))
    SendPilotXP(tprintf("%s gained %d %s XP", Name(pilot), xp,
        skname));
...
void AccumulateArtyXP(dbref pilot, MECH * attacker, MECH * wounded)
{
    ...
    SendAttackXP(tprintf("%s gained %d artillery XP", Name(pilot),
        xp));
...
void AccumulateComputerXP(dbref pilot, MECH * mech, int reason)
{
    ...
    if (char_gainxp(pilot, "computer", MAX(1, reason)))
        SendXP(tprintf("%s gained %d computer XP (mech #%d)",
            Name(pilot), reason, mech ? mech->mynum : -1));
}
    ...
    int numOccurences, int multiplier, int weapindx, int bth)
{
    ...
    missilemod = 1;
    if (btechconf.btech_oldxpsystem) {
    AccumulateGunXPold(pilot, attacker, wounded, numOccurences, multiplier, weapindx, bth);
    return;
    }
    ...
    multiplier = multiplier * btechconf.btech_xp_modifier;
    if (btechconf.btech_xp_bthmod) {
    if (!(bth >= 3 && bth <= 12))
    ...
    if (btechconf.btech_xp_usePilotBVMod) {
    myPilotBVMod = getPilotBVMod(attacker, weapindx);
    theirPilotBVMod = getPilotBVMod(wounded, weapindx);

    my_BV = my_BV * (int)myPilotBVMod;
    th_BV = th_BV * (int)theirPilotBVMod;

    SendDebug(tprintf ("Using skill modified battle value for mechs %d and %d " "with skill mods of %2.2f and %2.2f", attacker->mynum, wounded->mynum, myPilotBVMod, theirPilotBVMod));
    }

    my_speed = NewMoveValue(attacker) + 1;
    th_speed = NewMoveValue(wounded) + 1;
    if (MechWeapons[weapindx].type == TMISSILE)
    missilemod = btechconf.btech_xp_missilemod;
    else if (MechWeapons[weapindx].type == TAMMO)
    missilemod = btechconf.btech_xp_ammomod;

    if (btechconf.btech_defaultweapdam > 1)
    damagemod = numOccurences;
    else
    damagemod = 1;

    if (btechconf.btech_xp_vrtmod)
    vrtmod = (MechWeapons[weapindx].vrt < 30 ? sqrt((double) MechWeapons[weapindx].vrt / 30.0) : 1);
    else
    vrtmod = 1.0;

    multiplier = (int)((vrtmod * missilemod * multiplier * sqrt((double) (th_BV + 1) * th_speed * btechconf.btech_defaultweapbv / btechconf.btech_defaultweapdam)) / (sqrt((double) (my_BV + 1) * my_speed * MechWeapons[weapindx].battlevalue / damagemod)));
     ...
     */
    if (char_gainxp(pilot, skname, xp))
    SendAttackXP(tprintf ("%s gained %d gun XP from feat of %d %% difficulty " "(%d occurences) against %s", Name(pilot), xp, multiplier, numOccurences, buf));
    ...
    int numOccurences, int multiplier, int weapindx, int bth)
{
    ...
    multiplier = multiplier * BOUNDED(50, 100 * TonValue(wounded) / TonValue(attacker), 150);
    else {
...
FUNCTION(fun_btgetcharvalue)
{
    ...
    FUNCHECK((target =
        char_lookupplayer(executor, caller, 0, fargs[0])) == NOTHING,
    "#-1 INVALID TARGET");
    FUNCHECK(!Wiz(executor), "#-1 PERMISSION DENIED!");
...
FUNCTION(fun_btsetcharvalue)
{
    ...
    if (flaggo == 1) {
        ...
        SendXP(tprintf("#%d added %d more %s XP to #%d", executor,
        ...
        SendXP(tprintf("#%d set #%d's %s XP to %d", executor,
            target, char_values[targetcode].name,
            targetvalue));
...
FUNCTION(fun_btcharlist)
{
    ...
    if (nfargs == 2) {
    target = char_lookupplayer(executor, caller, 0, fargs[1]);
    if (target == NOTHING) {
...
static void store_health(dbref player, PSTATS * s)
{
    ...
    atr_pget_info(player, atr, &aowner, &aflags);
    atr_add(player, atr, tprintf("%d,%d", char_gvalue(s, "Bruise"),
        char_gvalue(s, "Lethal")), aowner, aflags);
}
...
static void retrieve_health(dbref player, PSTATS * s)
{
    ...
    if ((pattr = atr_str("HEALTH")) && pattr->number)
    c = atr_get(player, pattr->number, &aowner, &aflags);
...
static void store_attrs(dbref player, PSTATS * s)
{
    ...
    atr_pget_info(player, atr, &aowner, &aflags);
    atr_add(player, atr, tprintf("%d,%d,%d,%d,%d", char_gvalue(s, "Build"),
        char_gvalue(s, "Reflexes"), char_gvalue(s, "Intuition"),
        char_gvalue(s, "Learn"), char_gvalue(s, "Charisma")), aowner,
    aflags);
}
...
static void retrieve_attrs(dbref player, PSTATS * s)
{
    ...
    if ((pattr = atr_str("ATTRS")) && pattr->number)
    c = atr_get(player, pattr->number, &aowner, &aflags);
...
static void generic_retrieve_stuff(dbref player, PSTATS * s, char *attr)
{
    dbref aowner;
    int aflags;
    char *c;
    ...
    if ((pattr = atr_str(attr)) && pattr->number)
    c = atr_get(player, pattr->number, &aowner, &aflags);
...
static void generic_store_stuff(dbref player, PSTATS * s, char *attr,
    int flag)
{
    char buf[LBUF_SIZE];
    int i;
    ...
    if (s->xp[i])
        ...
        s->xp[i], s->last_use[i].ReturnSeconds());
    else
        sprintf(c, "%s:%d/", char_values_short[i], s->values[i]);
    while (*(++c));
    }

    int atr = mkattr(player, attr);
    ...
    atr_pget_info(player, atr, &aowner, &aflags);
    if (*buf)
    atr_add(player, atr, buf, aowner, aflags);
    else
    atr_add(player, atr, "", aowner, aflags);
}

static void retrieve_skills(dbref player, PSTATS * s)
{
    generic_retrieve_stuff(player, s, "SKILLS");
}

static void retrieve_advs(dbref player, PSTATS * s)
{
    generic_retrieve_stuff(player, s, "ADVS");
}

static void store_skills(dbref player, PSTATS * s)
{
    generic_store_stuff(player, s, "SKILLS", 1);
}

static void store_advs(dbref player, PSTATS * s)
{
    generic_store_stuff(player, s, "ADVS", 0);
}
