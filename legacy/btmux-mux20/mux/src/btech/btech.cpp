...
extern CHashTable playervaluehash;
extern CHashTable playervaluehash2;
...
void list_fhashstats(dbref player)
{
    ...
    for (i = 0; factions[i].name; i++)
        if (factions[i].factnewshash)
            list_hashstat(player, tprintf("FNews:%s", factions[i].key), factions[i].factnewshash);
...
void do_show(dbref player, dbref cause, int key, char *arg1, char *arg2)
{
    int i;
    enum {
    CHAVA, CHVAL, CHSKI, CHADV, CHATT, MECHVALUES
    };
    ...
    if (!arg1 || !*arg1) {
        ...
        for (i = 0; cmds_help[i]; i++)
            sprintf(buf + strlen(buf), "%c %s", i > 0 ? ',' : ' ', cmds_help[i]);
        notify(player, buf);
        return;
    }
