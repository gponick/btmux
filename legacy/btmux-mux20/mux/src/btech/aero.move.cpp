...
land_data[] = {
    {CLASS_VTOL, 10, -60, -15, 15, 5, 0,
        ...
        "takes off!"}, 
    {CLASS_AERO, 10, -30, (int)(MIN_TAKEOFF_SPEED * MP1), 999, 20, 10,
        "You land your AeroFighter safely.", "lands safely.",
        "The Aerofighter launches into the air!",
        "launches into the air!"}, 
    {CLASS_DS, 10, -25, (int)(MIN_TAKEOFF_SPEED * MP1), 999, 20, 300,
...
static void aero_takeoff_event(EVENT * e)
{
    ...
    if (count > 0) {
        if (count > 5) {
            if (!(count % 10))
                mech_notify(mech, MECHALL, tprintf("Launch countdown: %d.", count));
        } else
            mech_notify(mech, MECHALL, tprintf("Launch countdown: %d.", count));
        ...
        if (i >= 0) {
            ...
            switch (count) {
                ...
                case 6:
                    ...
                    break;
                case 2:
                    mech_notify(mech, MECHALL, "The engines pulse out a stream of superheated plasma!");
                    DSSpam_O(mech, "'s engines send forth a tremendous stream of superheated plasma!");
    ...
    MechStartFZ(mech) = 0;
    if (IsDS(mech))
        SendDSInfo(tprintf("DS #%d has lifted off at %d %d " "on map #%d", mech->mynum, MechX(mech), MechY(mech), map->mynum));
...
void aero_takeoff(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUAL);
    DOCHECK(!FlyingT(mech), "Only VTOL, Aerospace fighters and Dropships can take off.");
    DOCHECK(!Landed(mech), "You haven't landed!");

    if (Fallen(mech) || (MMaxSpeed(mech) <= MP1) || (SectIsDestroyed(mech, ROTOR))) {
    ...
    if (!AeroFreeFuel(mech) && AeroFuel(mech) < 1) {
        ...
        notify(player, "Your craft's out of fuel! No taking off until it's refueled.");
        return;
    }
    
    DOCHECK(MechType(mech) == CLASS_AERO && MechSpeed(mech) < (MIN_TAKEOFF_SPEED * MP1), "You're moving too slowly to take off!");
    DOCHECK(MapIsUnderground(map), "Realize the ceiling in this grotto is a bit to low for that!");

    if (land_data[i].launchtime > 0)
        mech_notify(mech, MECHALL, "Launch sequence initiated.. type 'land' to abort it.");

    DSSpam(mech, "starts warming engines for liftoff!");
    if (IsDS(mech))
        SendDSInfo(tprintf("DS #%d has started takeoff at %d %d on map #%d", mech->mynum, MechX(mech), MechY(mech), map->mynum));
    ...
    StopHiding(mech);
    MECHEVENT(mech, EVENT_TAKEOFF, aero_takeoff_event, 1, (void *) j ? j : land_data[i].launchtime);
}
    ...
    char *nearhitmsg, char *nearhitmsg1, char *treehitmsg, int damage)
{
            ...
            switch (GetRTerrain(map, x1, y1)) {
                ...
                case HEAVY_FOREST:
                    if (!find_decorations(map, x1, y1)) {
                        HexLOSBroadcast(map, x1, y1, tprintf("%%ch%%crThe trees in $h %s%%cn", treehitmsg));
                        if ((damage / d) > 100) 
    ...
    MechZ(mech) = z + 6;
    blast_hit_hexesf(map, damage, 5, damage / 2, MechFX(mech), MechFY(mech), MechFX(mech), MechFY(mech), hitmsg, hitmsg1, nearhitmsg, nearhitmsg1, 0, 4, 4, 1, rng);
    MechZ(mech) = z;
}
...
void aero_land(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(MechType(mech) != CLASS_VTOL && MechType(mech) != CLASS_AERO && !IsDS(mech), "You can't land this type of vehicle.");
    DOCHECK(MechType(mech) == CLASS_VTOL && AeroFuel(mech) <= 0 && !AeroFreeFuel(mech), "You lack fuel to maneuver for landing!");
    ...
    DOCHECK((Fallen(mech)) && (MechType(mech) == CLASS_VTOL), "The rotor's dead!");
    DOCHECK((Fallen(mech)) && (MechType(mech) != CLASS_VTOL), "The engines are dead!");

    if (MechStatus(mech) & LANDED) {
        if (TakingOff(mech)) {
            mech_notify(mech, MECHALL, tprintf("Launch aborted by %s.", Name(player)));

            if (IsDS(mech))
                SendDSInfo(tprintf("DS #%d aborted takeoff at %d %d on map #%d", mech->mynum, MechX(mech), MechY(mech), map->mynum));
    ...
    DOCHECK(MechZ(mech) > MechElevation(mech) + 1, "You are too high to land here.");
    DOCHECK(((horiz = my_sqrtm((double) MechDesiredSpeed(mech), (double) MechVerticalSpeed(mech))) >= ((double) 1.0 + land_data[i].maxhoriz)), "You're moving too fast to land.");
    DOCHECK(horiz < land_data[i].minhoriz, "You're moving too slowly to land.");
    DOCHECK(((vert = (int)MechVerticalSpeed(mech)) > (vertmax = land_data[i].maxvertup)) || (MechVerticalSpeed(mech) < (vertmin = land_data[i].maxvertdown)), "You are moving too fast to land. ");
    ...
    t = MechRTerrain(mech);
    DOCHECK(!(t == GRASSLAND || t == ROAD || (MechType(mech) == CLASS_VTOL && t == BUILDING)), "You can't land on this type of terrain.");
    ...
    if (IsDS(mech))
        SendDSInfo(tprintf("DS #%d has landed at %d %d on map #%d", mech->mynum, MechX(mech), MechY(mech), map->mynum));
...
void ds_BridgeHit(MECH * mech)
{
    ...
    if (In_Character(mech->mynum))
        mech_notify(mech, MECHALL, "SHIT! The shot seems to be coming straight for the bridge!");
...
void aero_UpdateSpeed(MECH * mech)
{
    ...
    if (Spinning(mech)) {
        MechDesiredSpeed(mech) = BOUNDED(0, (int)(MechDesiredSpeed(mech) + Number(1, 10)), (int)(MMaxSpeed(mech)));
        MechDesiredAngle(mech) = MAX(-90, MechDesiredAngle(mech) - Number(1, 15));
        MechDesiredFacing(mech) = AcceptableDegree(MechDesiredFacing(mech) + Number(-3, 3));
    }
...
int FuelCheck(MECH * mech)
{
    ...
    if (fabs(MechSpeed(mech)) > MMaxSpeed(mech)) {
        if (MechZ(mech) < ATMO_Z)
            fuelcost = abs((int)(MechSpeed(mech) / MMaxSpeed(mech)));
    } else 
        if (fabs(MechSpeed(mech)) < MP1 && fabs(MechVerticalSpeed(mech)) < MP2)
            if (Number(0, 1) == 0)
    ...
    if (IsDS(mech)) {
        ...
        AeroFuel(mech)--;
        mech_notify(mech, MECHALL, "As the fuel runs out, the engines switch to backup power.");
        return 0;
    }
...
void aero_update(MECH * mech)
{
    ...
    if (!(muxevent_tick / 3 % 5)) {
        ...
        if (Landed(mech))
            return;
        if (MadePilotSkillRoll(mech, (MechStartSpin(mech) - muxevent_tick) / 15 + 8)) {
...
void aero_thrust(dbref player, void *data, char *arg)
{
    ...
    if (mech_parseattributes(arg, args, 1) != 1) {
        notify(player, tprintf("Your current thrust is %.2f.", MechDesiredSpeed(mech)));
        return;
    }
    
    newspeed = atof(args[0]);
    if (RollingT(mech))
        DOCHECK(newspeed < (MP1 * MIN_TAKEOFF_SPEED / ACCEL_MOD), tprintf("Minimum thrust you stay in air with is %.1f kph.", (float) MP1 * MIN_TAKEOFF_SPEED / ACCEL_MOD));
    ...
    DOCHECK(newspeed < 0, "Doh, thrust backwards.. where's your sense of adventure?");
...
void aero_vheading(dbref player, void *data, char *arg, int flag)
{
    ...
    if (mech_parseattributes(arg, args, 1) != 1) {
        notify(player, tprintf("Present angle: %d degrees.", MechDesiredAngle(mech)));
        return;
    }
    ...
    DOCHECK(abs(i) != 90 && MechZ(mech) < ATMO_Z && SpheroidDS(mech), tprintf("You can go only up / down at <%d z!", ATMO_Z));

    if (i >= 0)
        mech_notify(mech, MECHALL, tprintf("Climbing angle set to %d degrees.", i));
    else
        mech_notify(mech, MECHALL, tprintf("Diving angle set to %d degrees.", 0 - i));

    MechDesiredAngle(mech) = i;
}

void aero_climb(dbref player, void *data, char *arg)
{
...
void aero_dive(dbref player, void *data, char *arg)
{
    MECH *mech = static_cast < MECH * >(data);
...
void DS_LandWarning(MECH * mech, int serious)
{
    ...
    ilz--;
    mech_notify(mech, MECHALL, tprintf("%sWARNING: %s - %s%%cn", colorstr(serious), reasons[ilz], 
                serious == 1 ? "CLIMB UP NOW!!!" : serious == 0 ? 
                    "No further descent is advisable." :
                    "Please do not even consider landing here."));
}

void aero_checklz(dbref player, void *data, char *buffer)
{
    MECH *mech = static_cast < MECH * >(data);
    ...
    switch (argc) {
        case 2:
            ...
            if (!MechIsObservator(mech)) {
                ...
                DOCHECK(FindHexRange(MechFX(mech), MechFY(mech), fx, fy) > MechTacRange(mech), "Out of range!");
            }
            break;
    ...
    DOCHECKMA(!ilz, tprintf("The hex (%d,%d) looks good enough for a landing.", x, y));
    ilz--;
    mech_notify(mech, MECHALL, tprintf("The hex (%d,%d) doesn't look good for landing: %s.", x, y, reasons[ilz]));
}
