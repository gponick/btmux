...
const char *GetAmmoDesc_Model_Mode(int model, int mode)
{
    ...
    if (mode & ARTEMIS_MODE)
        return " Artemis IV";
    if (mode & NARC_MODE)
        return (MechWeapons[model].special & NARC) ? " Explosive" : " Narc compatible";
...
{
    return GetWeaponAmmoModeLetter_Model_Mode(Weapon2I(GetPartType(mech, loop, crit)), GetPartAmmoMode(mech, loop, crit));
}

char GetWeaponFireModeLetter(MECH * mech, int loop, int crit)
{
    return GetWeaponFireModeLetter_Model_Mode(Weapon2I(GetPartType(mech, loop, crit)), GetPartFireMode(mech, loop, crit));
}
...
struct temp_flag_info_structstruct {
    char *onmsg;
    char *offmsg;
    int flag;
    int infolvl;
...
{
    static char buf[20];
    int mechlike = (MechType(mech) == CLASS_MECH || MechType(mech) == CLASS_MW || MechType(mech) == CLASS_BSUIT);
...
const char *GetMechToMechID_base(MECH * see, MECH * mech, int inlos)
{
    ...
    if (!inlos)
        ...
        if ((pattr = atr_str("MECHNAME")) && pattr->number)
            mname = atr_get(mech->mynum, pattr->number, &aowner, &aflags);
        else
            mname = "";
    }

    snprintf(ids, SBUF_SIZE, "%s [%s]", mname, MechIDS(mech, inlos && MechTeam(see) == MechTeam(mech)));
    ids[SBUF_SIZE - 1] = '\0';
    return ids;
}
...
void mech_set_channelfreq(dbref player, void *data, char *buffer)
{
    ...
    if (freq > 0) 
        for (i = 0; i < map->first_free; i++) {
            if (!(t = static_cast <MECH *>(FindObjectsData(map->mechsOnMap[i]))))
                continue;
            ...
            for (j = 0; j < MFreqs(t); j++) 
                if (t->freq[j] == freq && !(t->freqmodes[j] & FREQ_SCAN))
                    SendFreqs(tprintf("ALERT: Possible abuse by #%d (Team %d)" " setting freq %d matching #%d (Team %d)!", mech->mynum, MechTeam(mech), freq, t->mynum, MechTeam(t)));
        }
}
...
void mech_set_channeltitle(dbref player, void *data, char *buffer)
{
    ...
    notify(player, tprintf("Channel %c title set to set to %s.", 'A' + chn, buffer));
}
...
static char *ccode(MECH * m, int i)
{
    ...
    sprintf(buf, "%%ch%%c%c", mux_tolower(radio_colorstr[t - 1]));
    return buf;
}
...
void mech_set_channelmode(dbref player, void *data, char *buffer)
{
    ...
    if (!buffer || !*buffer) {
        ...
        notify(player, tprintf("Channel %c <send> mode set to analog.", 'A' + chn));
        return;
    }
    ...
    while (buffer && *buffer) {
        switch (*buffer) {
            case 'D':
            case 'd':
                DOCHECK(MechRadioInfo(mech) & RADIO_NODIGITAL, "Your radio can't handle digital frequencies!");
                nm |= FREQ_DIGITAL;
                break;
            case 'I':
            case 'i':
                DOCHECK(!(MechRadioInfo(mech) & RADIO_INFO), "This unit is unable to use info functionality.");
                nm |= FREQ_INFO;
                break;
            case 'U':
            case 'u':
            ...
            case 'E':
            case 'e':
                DOCHECK(!(MechRadioInfo(mech) & RADIO_RELAY), "This unit is unable to relay.");
                nm |= FREQ_RELAY;
                break;
            case 'S':
            case 's':
                DOCHECK(!(MechRadioInfo(mech) & RADIO_SCAN), "This unit is unable to scan.");
                nm |= FREQ_SCAN;
                break;
            default:
    ...
    DOCHECK(!(nm & FREQ_DIGITAL) && (nm & FREQ_RELAY), "Error: Need digital transfer for relay to work.");
    DOCHECK(!(nm & FREQ_DIGITAL) && (nm & FREQ_INFO), "Error: Need digital transfer for transfer info to work.");
    ...
    buf[i] = 0;
    notify(player, tprintf("Channel %c <send> mode set to %s (flags:%s).", 'A' + chn, nm & FREQ_DIGITAL ? "digital" : "analog", buf));
}
...
void mech_list_freqs(dbref player, void *data, char *buffer)
{
    ...
    for (i = 0; i < MFreqs(mech); i++)
        notify(player, tprintf("%c    %c%c%c%c    %-9d    %s", 'A' + i, mech->freqmodes[i] & FREQ_DIGITAL ? 'D' : 'A', mech->freqmodes[i] & FREQ_RELAY ? 'R' : '-', mech->freqmodes[i] & FREQ_MUTE ? 'M' : '-', mech->freqmodes[i] & FREQ_SCAN ? 'S' : mech-> freqmodes[i] >= FREQ_REST ?  radio_colorstr[mech->freqmodes[i] / FREQ_REST - 1] : mech->freqmodes[i] & FREQ_INFO ? 'I' : '-', mech->freq[i], mech->chantitle[i]));
}
...
void mech_sendchannel(dbref player, void *data, char *buffer)
{
    ...
    cch(MECH_USUALS);
    DOCHECK(Destroyed(mech) || !MechRadioRange(mech), "Your communication gear is inoperative.");
    ...
    if (!fail)
        for (i = 0; args[2][i]; i++)
            DOCHECK((BOUNDED(32, args[2][i], 255)) != args[2][i], "Invalid: No control characters in radio messages, please.");
    
    DOCHECK(fail, "Invalid format! Usage: sendchannel <letter>=<string>");

    if (mech->freq[chn] == 0 && In_Character(mech->mapindex)) 
        send_channel("ZeroFrequencies", tprintf("Player #%d (%s) in mech #%d (channel %c) " "on map #%d 0-freqs \"%s\"", player, Name(player), mech->mynum, chn + 'A', mech->mapindex, args[2]));
...
void ScrambleMessage(char *buffo, int range, int sendrange, int recvrrange, char *handle, char *msg, int bth, int *isxp, int under_ecm, int digmode)
{
    int mr, i;
    char *header = NULL;
    char buf[LBUF_SIZE];
    ...
    *isxp = 0;
    ...
    if (digmode > 1 && comm_best > 1) {
        ...
        for (i = 1; i < comm_best; i++) {
            ...
            bearing = FindBearing(MechFX(comm_mech[comm_best_path[i]]), MechFY(comm_mech[comm_best_path[i]]), MechFX(comm_mech[comm_best_path[i - 1]]), MechFY(comm_mech[comm_best_path[i - 1]]));
            snprintf(buf + strlen(buf), LBUF_SIZE, "[%c%c]-h:%.3d", MechID(comm_mech[comm_best_path[i]])[0], MechID(comm_mech[comm_best_path[i]])[1], bearing);
        }
...
int common_checks(dbref player, MECH * mech, int flag)
{
    ...
    if (flag & MECH_PILOT_CON)
        DOCHECK0(Uncon(mech) && (!Started(mech) || player == MechPilot(mech)), "You are unconscious....zzzzzzz");

    if (flag & MECH_PILOTONLY)
        DOCHECK0(!Wizard(player) && In_Character(mech->mynum) && MechPilot(mech) != player, "Now now, only the pilot can push that button.");
...
void nonrecursive_commlink(int i)
{
    ...
    while (dep >= 0) {
        ...
        if (iter_c++ == 100000) {
            SendError(tprintf("#%d: Infinite loop in relay code (?) ; using backup recursive code (num_mechs:%d, maxdepth:%d, nowdepth:%d)", comm_mech[0]->mynum, comm_num_to_conn, maxdepth, dep));
            comm_best = 9999;
...
int findCommLink(MAP * map, MECH * from, MECH * to, int freq)
{
    ...
    for (i = 0; i < map->first_free; i++) {
        ...
        if ((MechMove(t) != MOVE_NONE && !Started(t)) || (MechMove(t) == MOVE_NONE && Destroyed(t)))
            continue;
...
/* Removed the Radio Failing stuff cause it annoys me - Dany
    ...
    for (loop = 0; loop < mech_map->first_free; loop++) {
        if (mech_map->mechsOnMap[loop] != -1) {
            ...
            range = (int)(FaMechRange(mech, tempMech));
            bearing = FindBearing(MechFX(tempMech), MechFY(tempMech), MechFX(mech), MechFY(mech));
    
            for (i = 0; i < MFreqs(tempMech); i++) {
                if (tempMech->freq[i] == mech->freq[freq]) {
                    if ((tempMech->freqmodes[i] & FREQ_MUTE) || ((mech->freqmodes[freq] & FREQ_DIGITAL) && (MechRadioInfo(tempMech) & RADIO_NODIGITAL)))
                        continue;
            ...
            if (MechIsObservator(tempMech)) {
                ...
                snprintf(buf, LBUF_SIZE, "%s<Team %d:%6d:%.3d> %s%s%%c", i < MFreqs(tempMech) ? ccode(tempMech, i) : "", MechTeam(mech), mech->freq[freq], bearing, buf3, msg);
                    ...
                    if ((MechRadioInfo(tempMech) & RADIO_SCAN) && mech->freq[freq]) {
                        ...
                        for (i = 0; i < MFreqs(tempMech); i++)
                            if (tempMech->freqmodes[i] & FREQ_SCAN) {
                                ...
                                if (!tnc++)
                                    mech_notify(tempMech, MECHALL, "You notice a " "unknown transmission your scanner.. ");
                                ...
                                pr = t * 100 / diff;
                                mech_notify(tempMech, MECHALL, tprintf("Your systems manage to zero on it %s on channel %c.", pr < 30 ? "somewhat" : pr < 60 ? "fairly well" : pr < 95 ? "precisely" : "exactly", i + 'A'));
            ...
            * then we give the radio commands to the AI */
            if (MechAuto(tempMech) > 0 && tempMech->freq[i]) { 
                ...
                if (!a) {
                    ...
                    if (a && Location(a->mynum) != tempMech->mynum) {
                        /* Check to see if the AI is still in the same mech */
                        snprintf(ai_buf, LBUF_SIZE, "Autopilot #%d (Location: #%d) reported on Mech #%d but not in the proper location", a->mynum, Location(a->mynum), tempMech->mynum);
                        SendAI(ai_buf);
                    } else 
                        if (a && !ECMDisturbed(tempMech)) {
            ...
            if (mech->freqmodes[freq] & FREQ_DIGITAL) {
                ...
                ScrambleMessage(buf3, range, MechRadioRange(mech), MechRadioRange(mech), mech->chantitle[freq], buf2, MechComm(tempMech), &isxp, 0, (tempMech->freqmodes[i] & FREQ_INFO) ? 2 : 1);

                if (comm_best >= 2)
                    bearing = FindBearing(MechFX(tempMech), MechFY(tempMech), MechFX(comm_mech[comm_best_path[comm_best - 1]]), MechFY(comm_mech[comm_best_path[comm_best - 1]]));

                snprintf(buf, LBUF_SIZE, "%s[%c:%.3d] %s%%c", ccode(tempMech, i), (char) ('A' + i), bearing, buf3);
            } else {
                ScrambleMessage(buf3, range, MechRadioRange(mech), MechRadioRange(tempMech), mech->chantitle[freq], buf2, MechComm(tempMech), &isxp, (AnyECMDisturbed(mech) || AnyECMDisturbed(tempMech)) && mech != tempMech, 0);
                snprintf(buf, LBUF_SIZE, "%s(%c:%.3d) %s%%c", ccode(tempMech, i), (char) ('A' + i), bearing, buf3);
            }
...
void mech_radio(dbref player, void *data, char *buffer)
{
    ...
    if (!fail && (!args[0] || args[0][0] == 0 || args[0][1] == 0 || args[0][2] != 0))
        fail = 1;
    
    if (!fail) {
        target = FindTargetDBREFFromMapNumber(mech, args[0]);
        tempMech = getMech(target);
        DOCHECK(!tempMech || !InLineOfSight(mech, tempMech, MechX(tempMech), MechY(tempMech), FlMechRange(map, mech, tempMech)), "Target is not in line of sight!");
        mech_notify(mech, MECHSTARTED, tprintf("You radio %s with, '%s'", GetMechToMechID(mech, tempMech), args[2]));
        mech_notify(tempMech, MECHSTARTED, tprintf("%s radios you with, '%s'", GetMechToMechID(tempMech, mech), args[2]));
        auto_reply(tempMech, tprintf("%s radio'ed me '%s'", GetMechToMechID(tempMech, mech), args[2]));
    }
...
void MechBroadcast(MECH * mech, MECH * target, MAP * mech_map, char *buffer)
{
    ...
    if (target) 
        for (loop = 0; loop < mech_map->first_free; loop++) 
            if (mech_map->mechsOnMap[loop] != mech->mynum && mech_map->mechsOnMap[loop] != -1 && mech_map->mechsOnMap[loop] != target->mynum) {
    ...
    for (i = 0; i < mech_map->first_free; i++)
        if (mech_map->mechsOnMap[i] != -1 && mech_map->mechsOnMap[i] != mech->mynum)
            if ((tempMech = getMech(mech_map->mechsOnMap[i])))
                if (InLineOfSight(tempMech, mech, MechX(mech), MechY(mech), FlMechRange(mech_map, tempMech, mech))) {
                    sprintf(buf, "%s%s%s", GetMechToMechID(tempMech, mech), *message != '\'' ? " " : "", message);
...
{
    return (InLineOfSight(mech, NULL, ix, iy, FindRange(MechFX(mech), MechFY(mech), MechFZ(mech), x, y, ZSCALE * Elevation(map, ix, iy))));
}
...
void MechLOSBroadcasti(MECH * mech, MECH * target, char *message)
{
    ...
    for (i = 0; i < mech_map->first_free; i++)
        if (mech_map->mechsOnMap[i] != -1 && mech_map->mechsOnMap[i] != mech->mynum && mech_map->mechsOnMap[i] != target->mynum)
            if ((tempMech = getMech(mech_map->mechsOnMap[i]))) {
                a = InLineOfSight(tempMech, mech, MechX(mech), MechY(mech), FlMechRange(mech_map, tempMech, mech));
                b = InLineOfSight(tempMech, target, MechX(target), MechY(target), FlMechRange(mech_map, tempMech, target));

                if (a || b) {
                    sprintf(oddbuff, message, b ? GetMechToMechID(tempMech, target) : "someone");
                    sprintf(oddbuff2, "%s%s%s", a ? GetMechToMechID(tempMech, mech) : "Someone", *oddbuff != '\'' ? " " : "", oddbuff);
...
void MechFireBroadcast(MECH * mech, MECH * target, int x, int y, MAP * mech_map, char *weapname, int IsHit)
{
    ...
    if (target) {
        ...
        for (loop = 0; loop < mech_map->first_free; loop++)
            if (mech_map->mechsOnMap[loop] != mech->mynum && mech_map->mechsOnMap[loop] != -1 && mech_map->mechsOnMap[loop] != target->mynum) {
                ...
                if (target) {
                    if (InLineOfSight(tempMech, target, mapx, mapy, FlMechRange(mech_map, tempMech, target)))
                        defender = 1;
                } else 
                    if (InLineOfSight(tempMech, target, mapx, mapy, FindRange(MechFX(tempMech), MechFY(tempMech), MechFZ(tempMech), fx, fy, fz)))
                        defender = 1;
                ...
                if (attacker) {
                    if (defender)
                        mech_notify(tempMech, MECHSTARTED, tprintf("%s %s %s with a %s", GetMechToMechID(tempMech, mech), IsHit ? "hits" : "misses", buff, weapname));
                    else
                        mech_notify(tempMech, MECHSTARTED, tprintf("%s fires a %s at something!", GetMechToMechID(tempMech, mech), weapname));
                } else
                    mech_notify(tempMech, MECHSTARTED, tprintf("Something %s %s with a %s", IsHit ? "hits" : "misses", buff, weapname));
            }
    } else {
        mapx = x;
        mapy = y;
        ...
        for (loop = 0; loop < mech_map->first_free; loop++)
            if (mech_map->mechsOnMap[loop] != mech->mynum && mech_map->mechsOnMap[loop] != -1) {
                ...
                if (attacker) {
                    if (defender)    /* att + def */
                        mech_notify(tempMech, MECHSTARTED, tprintf("%s fires a %s at %s", GetMechToMechID(tempMech, mech), weapname, buff));
                    else    /* att */
                        mech_notify(tempMech, MECHSTARTED, tprintf("%s fires a %s at something!", GetMechToMechID(tempMech, mech), weapname));
                } else        /* def */
                    mech_notify(tempMech, MECHSTARTED, tprintf("Something fires a %s at %s", weapname, buff));
            }
    }
}
...
void mech_notify(MECH * mech, int type, char *buffer)
{
    ...
    if (type == MECHPILOT) {
        ...
        if ((type == MECHALL && !Destroyed(mech)) || (type == MECHSTARTED && Started(mech))) {
            notify_except(mech->mynum, NOTHING, mech->mynum, buffer);

            if (arc_override)
                for (i = 0; i < NUM_TURRETS; i++)
                    if (AeroTurret(mech, i) > 0)
                        notify_except(AeroTurret(mech, i), NOTHING, AeroTurret(mech, i), buffer);
        }
}
