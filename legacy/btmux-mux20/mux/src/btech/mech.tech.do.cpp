...
int FindAmmoType(MECH * mech, int loc, int part)
{
    ...
    if (strstr(MechWeapons[t].name, "StreakSRM"))
        ...
        if (strstr(MechWeapons[t].name, "StreakLRM"))
            base = SLRM_AMMO;
        else 
            if (strstr(MechWeapons[t].name, "ELRM"))
                base = ELRM_AMMO;
            else 
                if (strstr(MechWeapons[t].name, "LR_DFM"))
                    base = LR_DFM_AMMO;
                else 
                    if (strstr(MechWeapons[t].name, "SR_DFM"))
                        base = SR_DFM_AMMO;
                    else 
                        if (strstr(MechWeapons[t].name, "LRM"))
                            base = LRM_AMMO;
                        else 
                            if (strstr(MechWeapons[t].name, "SRM"))
                                base = SRM_AMMO;
                            else 
                                if (strstr(MechWeapons[t].name, "MRM"))
                                    base = MRM_AMMO;
    ...
    if (m & LBX_MODE) {
        if (strstr(MechWeapons[t].name, "LB20"))
           ...
           if (strstr(MechWeapons[t].name, "LB10"))
                base = LBX10_AMMO;
           else 
               if (strstr(MechWeapons[t].name, "LB5"))
                    base = LBX5_AMMO;
                else 
                    if (strstr(MechWeapons[t].name, "LB2"))
                        base = LBX2_AMMO;
...
TFUNC_LOCPOS(replace_econ)
{
    ...
    PARTCHECK(mech, GetPartType(mech, loc, part), GetPartBrand(mech, loc, part), 1);
    return 0;
}
...
TFUNC_LOCPOS(repair_econ)
{
    ...
    PARTCHECKTWO(mech, Cargo(S_ELECTRONIC), 0, PartIsDestroyed(mech, loc, part) ? 3 : 1, ProperInternal(mech), 0, PartIsDestroyed(mech, loc, part) ? 3 : 1);
    return 0;
}
...
TFUNC_LOC(reattach_econ)
{
    if (btechconf.btech_complexrepair) {
        if (MechType(mech) == CLASS_MECH) {
            PARTCHECKTWO(mech, ProperInternal(mech), 0, GetSectOInt(mech, loc), ProperMyomer(mech), 0, 1);
        } else {
            PARTCHECK(mech, ProperInternal(mech), 0, GetSectOInt(mech, loc));
        }
    } else {
        PARTCHECKTWO(mech, ProperInternal(mech), 0, GetSectOInt(mech, loc), Cargo(S_ELECTRONIC), 0, GetSectOInt(mech, loc));
    }
...
{
    PARTCHECKFOUR(mech, ProperInternal(mech), 0, BSUIT_REPAIR_INTERNAL_NEEDED, Cargo(BSUIT_SENSOR), 0, BSUIT_REPAIR_SENSORS_NEEDED, Cargo(BSUIT_LIFESUPPORT), 0, BSUIT_REPAIR_LIFESUPPORT_NEEDED, Cargo(BSUIT_ELECTRONIC), 0, BSUIT_REPAIR_ELECTRONICS_NEEDED);
    return 0;
}
...
TFUNC_LOCPOS(replaceg_fail)
{
    ...
    if (tech_roll(player, mech, REPLACE_DIFFICULTY) < 0) {
        notify(player, tprintf("You muck around, wasting the %s in the progress.", w ? "weapon" : "part"));
        return -1;
    }
    
    notify(player, tprintf("Despite messing the repair, you manage not to waste the %s.", w ? "weapon" : "part"));
    AddPartsM(mech, loc, FindAmmoType(mech, loc, part), GetPartBrand(mech, loc, part), 1);
    return -1;
}
...
TFUNC_LOC_VAL(fixarmor_fail)
{
    ...
    notify(player, tprintf("Your armor patching isn't exactly perfect.. " "You managed to fix %d out of %d.", tot, should));
    *val = tot;
    return 0;
}
...
TFUNC_LOC_VAL(fixinternal_fail)
{
    ...
    notify(player, tprintf("Your internal patching isn't exactly perfect.. You managed to fix %d out of %d.", tot, should));
    *val = tot;
    return 0;
}
...
TFUNC_LOC(reattach_fail)
{
    ...
    tot = random() % 90 + 5;
    notify(player, tprintf("Despite your disastrous failure, you recover %d%% of the materials.", tot));
    ...
    if (btechconf.btech_complexrepair && MechType(mech) == CLASS_MECH)
...
TFUNC_LOC(replacesuit_fail)
{
    ...
    wRand = random() % 90 + 5;
    notify(player, tprintf("Despite your disastrous failure, you recover %d%% of the materials.", wRand));
    AddPartsM(mech, loc, Cargo(BSUIT_SENSOR), 0, MAX(((BSUIT_REPAIR_SENSORS_NEEDED * wRand) / 100), 1));
    AddPartsM(mech, loc, Cargo(BSUIT_LIFESUPPORT), 0, ((BSUIT_REPAIR_LIFESUPPORT_NEEDED * wRand) / 100));
    AddPartsM(mech, loc, Cargo(BSUIT_ELECTRONIC), 0, ((BSUIT_REPAIR_ELECTRONICS_NEEDED * wRand) / 100));
    AddPartsM(mech, loc, ProperInternal(mech), 0, MAX(((BSUIT_REPAIR_INTERNAL_NEEDED * wRand) / 100), 1));
    return -1;
}
...
TFUNC_LOC_RESEAL(reseal_fail)
{
    ...
    tot = random() % 90 + 5;
    notify(player, tprintf("You don't manage to get all the water out and seal the section, though you recover %d%% of the materials.", tot));
    tot = (tot * GetSectOInt(mech, loc)) / 100;
