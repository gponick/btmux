...
static int engine_weight(MECH * mech)
{
    ...
    for (i = 0; engine_data[i][0] >= 0; i++)
        if (s == engine_data[i][0]) {
            ...
            if (MechType(mech) == CLASS_VEH_GROUND || MechType(mech) == CLASS_VTOL || MechType(mech) == CLASS_VEH_NAVAL)
    ...
    SendError(tprintf("Error in #%d (%s) : No engine found!", mech->mynum, Name(mech->mynum)));
    return 0;
}
        ...
        if (!SectIsDestroyed(mech, i))
            for (j = 0; j < CritsInLoc(mech, i); j++)
                if (IsAmmo((t = GetPartType(mech, i, j))))
                    w += GetPartData(mech, i, j) * 1024 / MechWeapons[Ammo2I(GetPartType(mech, i, j))].ammoperton;
...
#define ADDENTRY_C(text,count,weight) \
  if (weight) { if (interactive>0) { addmenu(text);addmenu(tprintf("%5d %6.1f", count, (float) (weight) / 1024.0)); }; total += weight; }
    sprintf(buf, "%-12s(%d rating)", MechSpecials(mech) & XL_TECH ? "Engine (XL)" : MechSpecials(mech) & XXL_TECH ? "Engine (XXL)" : MechSpecials(mech) & CE_TECH ?  "Engine (Compact)" : MechSpecials(mech) & LE_TECH ?  "Engine (Light)" : "Engine", MechEngineSize(mech));
    PLOC(CTORSO)
    ...
    ADDENTRY(const_cast <char *>(MechSpecials(mech) & REINFI_TECH ? "Internals (Reinforced)" : MechSpecials(mech) & COMPI_TECH ? "Internals (Composite)" : MechSpecials(mech) & ES_TECH ? "Internals (ES)" : "Internals"), round_to_halfton(MechTons(mech) * 1024 * (interactive >= 0 ? ints_tot : ints_c) / 5 / ints_tot / (MechSpecials(mech) & REINFI_TECH ? 1 : (MechSpecials(mech) & (ES_TECH | COMPI_TECH)) ? 4 : 2)));
    armor_o = armor;
    
    if (MechSpecials(mech) & FF_TECH)
        armor = armor * 50 / (cl ? 60 : 56);
    else 
        if (MechSpecials2(mech) & HVY_FF_ARMOR_TECH)
            armor = armor * 50 / 62;
        else 
            if (MechSpecials2(mech) & LT_FF_ARMOR_TECH)
                armor = armor * 50 / 53;

    ADDENTRY_C(const_cast <char *>(MechSpecials2(mech) & STEALTH_ARMOR_TECH ?  "Armor (Stealth)" : MechSpecials2(mech) & HVY_FF_ARMOR_TECH ?  "Armor (Hvy FF)" : MechSpecials2(mech) & LT_FF_ARMOR_TECH ?  "Armor (Lt FF)" : MechSpecials(mech) & HARDA_TECH ?  "Armor (Hardened)" : MechSpecials(mech) & FF_TECH ?  "Armor (FF)" : "Armor"), armor_o, (int)(ceil(armor / (8. * (MechSpecials(mech) & HARDA_TECH ? 2 : 1))) * 512));

    if (MyMechNumOsinks(mech)) 
        pile[Special(HEAT_SINK)] = MAX(0, MyMechNumOsinks(mech) * shs_size / hs_eff - (MechSpecials(mech) & ICE_TECH ? 0 : 10) * shs_size);
    else 
        if (interactive > 0)
    ...
    if (CargoSpace(mech))
        ADDENTRY(tprintf("CargoSpace (%.2ft)", (float) CargoSpace(mech) / 100), (int) (((float) CargoSpace(mech) / (MechSpecials2(mech) & CARRIER_TECH ? 1000 : MechSpecials(mech) & CARGO_TECH ? 100 : 500)) * 1024));

    if (interactive > 0) {
        addline();
        vsi(tprintf("%%cgTotal: %s%.1f tons (offset: %.1f)%%cn", (total / 1024) > MechTons(mech) ? "%ch%cr" : "", (float) (total) / 1024.0, MechTons(mech) - (float) (total) / 1024.0));
        addline();
...
int mech_weight_sub_veh(dbref player, MECH * mech, int interactive)
{
    ...
    for (i = 0; i < NUM_SECTIONS; i++) {
        ...
        for (j = 0; j < CritsInLoc(mech, i); j++) {
            ...
            if (interactive >= 0 || !SectIsDestroyed(mech, i)) {
                ...
                if (i == TURRET && (MechType(mech) == CLASS_VEH_GROUND || MechType(mech) == CLASS_VEH_NAVAL))
                    if (IsWeapon(t))
    ...
    if (es)
        sprintf(buf, "%-12s(%d->%d eff/wt rat)", MechSpecials(mech) & LE_TECH ? "Engine (Light)" : MechSpecials(mech) & CE_TECH ? "Engine (Compact)" : MechSpecials(mech) & XXL_TECH ? "Engine (XXL)" : MechSpecials(mech) & XL_TECH ? "Engine (XL)" : MechSpecials(mech) & ICE_TECH ? "Engine (ICE)" : "Engine", MechEngineSize(mech), MechEngineSize(mech) - susp_factor(mech));
    else
        sprintf(buf, "%-12s(%d rating)", MechSpecials(mech) & LE_TECH ? "Engine (Light)" : MechSpecials(mech) & CE_TECH ? "Engine (Compact)" : MechSpecials(mech) & XXL_TECH ? "Engine (XXL)" : MechSpecials(mech) & XL_TECH ? "Engine (XL)" : MechSpecials(mech) & ICE_TECH ? "Engine (ICE)" : "Engine", MechEngineSize(mech));
    
    if (!tank_in_pieces(mech)) {
        ADDENTRY(buf, (es = engine_weight(mech)));
        if (MechMove(mech) == MOVE_HOVER && es < (MechTons(mech) * 1024 / 5))
            ADDENTRY("Engine size fix (-> 1/5 hover wt.)", MechTons(mech) * 1024 / 5 - es);

        ADDENTRY("Cockpit", round_to_halfton(MechTons(mech) * 1024 / 20));
        if (MechType(mech) == CLASS_VTOL || MechMove(mech) == MOVE_HOVER || MechMove(mech) == MOVE_HULL || MechMove(mech) == MOVE_SUB)
            ADDENTRY("SpecialComponents", round_to_halfton(MechTons(mech) * 1024 / 10));
    }
    ...
    ADDENTRY_C(const_cast <char *>(MechSpecials2(mech) & STEALTH_ARMOR_TECH ?  "Armor (Stealth)" : MechSpecials2(mech) & HVY_FF_ARMOR_TECH ?  "Armor (Hvy FF)" : MechSpecials2(mech) & LT_FF_ARMOR_TECH ?  "Armor (Lt FF)" : MechSpecials(mech) & HARDA_TECH ?  "Armor (Hardened)" : MechSpecials(mech) & FF_TECH ?  "Armor (FF)" : "Armor"), armor_o, round_to_halfton(armor * 1024 / 16));

    pile[Special(HEAT_SINK)] = MAX(0, MechRealNumsinks(mech) * shs_size / hs_eff - (MechSpecials(mech) & ICE_TECH ? 0 : 10) * shs_size);
    for (i = 1; i < NUM_ITEMS_M; i++)
        if (pile[i]) 
            if (IsWeapon(i)) {
                id = Weapon2I(i);
...
void vehicle_int_check(MECH * mech, int noisy)
{
    ...
    for (i = 0; i < NUM_SECTIONS; i++)
        if (GetSectOInt(mech, i) && GetSectOInt(mech, i) != j) {
            if (noisy)
                SendError(tprintf ("Template %s / mech #%d: Invalid internals in loc %d (should be %d, are %d)", MechType_Ref(mech), mech->mynum, i, j, GetSectOInt(mech, i)));
...
void mech_int_check(MECH * mech, int noisy)
{
    ...
    if (int_data[i][0] < 0) {
        if (noisy)
            SendError(tprintf("VERY odd tonnage for #%d: %d.", mech->mynum, MechTons(mech)));
        return;
    }
