...
int fiery_death(MECH * mech)
{
    if (MechTerrain(mech) == FIRE) {
        ...
        /* Cause various things MWs _die_ */
...
{
    if (DSLastMsg(mech) > muxevent_tick || (muxevent_tick - DSLastMsg(mech)) >= DS_SPAM_TIME) {
...
void move_mech(MECH * mech)
{
    ...
    if (MechX(mech) < 0 || MechX(mech) >= mech_map->map_width ||
        ...
        mech_notify(mech, MECHALL, "You are at an invalid map location! Map index reset!");
        MechCocoon(mech) = 0;
        if (Jumping(mech))
            mech_land(MechPilot(mech), (void *) mech, "");
    ...
    if (btechconf.btech_newcharge && MechChargeTarget(mech) > 0)
    ...
    switch (MechMove(mech)) {
        ...
        case MOVE_QUAD:
            if (Jumping(mech)) {
                ...
                FindComponents(JumpSpeed(mech, mech_map) * MOVE_MOD * MAPMOVEMOD(mech_map), MechJumpHeading(mech), &newx, &newy);
                MechFX(mech) += newx;
                MechFY(mech) += newy;
                jump_pos = my_sqrt((MechFX(mech) - MechStartFX(mech)), (MechFY(mech) - MechStartFY(mech)));
                ...
                if (MechJumpTop(mech) >= (1 + JumpSpeedMP(mech, mech_map)))
                    ...
                    midmod = (1.0 - (midmod * midmod * midmod * midmod)) * MechJumpTop(mech);
                
                MechFZ(mech) = (rjump_pos * MechStartFZ(mech) + jump_pos * MechEndFZ(mech)) / MechJumpLength(mech) + midmod * ZSCALE;
                MechZ(mech) = (int) (MechFZ(mech) / ZSCALE + 0.5);
#ifdef JUMPDEBUG
                SendDebug(tprintf("#%d: %d,%d,%d (%d,%d,%d)", mech->mynum, MechX(mech), MechY(mech), MechZ(mech), (int) MechFX(mech), (int) MechFY(mech), (int) MechFZ(mech)));
#endif
                if (MechRTerrain(mech) == BRIDGE && collision_check(mech, JUMP, 0, 0) && MechZ(mech) > 0) {
                    mech_notify(mech, MECHALL, "CRASH!! You crash at the bridge!");
                ...
                if ((MechX(mech) == MechGoingX(mech)) && (MechY(mech) == MechGoingY(mech))) {
                    ...
                    if (my_sqrt(dax - MechStartFX(mech), day - MechStartFY(mech)) <= my_sqrt(MechFX(mech) - MechStartFX(mech), MechFY(mech) - MechStartFY(mech))) {
                        LandMech(mech);
                ...
                if (fabs(MechSpeed(mech)) > 0.0) {
                    FindComponents(MechSpeed(mech) * MOVE_MOD * MAPMOVEMOD(mech_map), MechLateral(mech) + MechFacing(mech), &newx, &newy);
                    MechFX(mech) += newx;
                    MechFY(mech) += newy;
                    upd_z = 1;
        ...
        case MOVE_SUB:
            ...
            MechZ(mech) = (int)(MechFZ(mech) / ZSCALE);
            break;
        case MOVE_FLY:
            if (!Landed(mech)) {
                MarkForLOSUpdate(mech);
                ...
                if (IsDS(mech)) {
                    if (MechZ(mech) < 10 && oz >= 10)
                        ...
                        if (MechZ(mech) < 50 && oz >= 50)
                            DS_LandWarning(mech, 0);
                        else 
                            if (MechZ(mech) < 100 && oz >= 100) {
    ...
    if (Jumping(mech) && MechLastX(mech) == MechGoingX(mech) && MechLastY(mech) == MechGoingY(mech) && (MechX(mech) != MechLastX(mech) || MechY(mech) != MechLastY(mech))) {
        LandMech(mech);
        MechFX(mech) -= newx;
        MechFY(mech) -= newy;
    ...
    if (oi != mech->mapindex || MechLastX(mech) != MechX(mech) || MechLastY(mech) != MechY(mech)) {
        ...
        if (upd_z) {
            ...
            if (MechType(mech) == CLASS_MECH && MechRTerrain(mech) == ICE && oz == -1 && MechZ(mech) == -1) {
                MechZ(mech) = 0;
...
void CheckNavalHeight(MECH * mech, int oz)
{
    if (MechRTerrain(mech) != WATER && MechRTerrain(mech) != ICE && MechRTerrain(mech) != BRIDGE) {
        MechSpeed(mech) = 0.0;
    ...
    if (MechFZ(mech) > 0.0) {
        if (MechVerticalSpeed(mech) > 0 && !MechZ(mech) && oz < 0) {
            mech_notify(mech, MECHALL, "Your sub has reached surface and stops rising.");
            MechLOSBroadcast(mech, tprintf("surfaces at %d,%d!", MechX(mech), MechY(mech)));
    ...
    if (MechZ(mech) <= (MechLowerElevation(mech))) {
        ...
        if (MechElevation(mech) > 0) 
            SendError(tprintf ("Oddity: #%d managed to wind up on '%c' (%d elev.)", mech->mynum, MechTerrain(mech), MechElev(mech)));
        
        MechFZ(mech) = ((5.0 * MechZ(mech) - 4) * ZSCALE) / 5.0;
        if (MechMove(mech) == MOVE_SUB) {
            if (MechVerticalSpeed(mech) < 0) {
                MechVerticalSpeed(mech) = 0;
                mech_notify(mech, MECHALL, "The sub has reached bottom and stops diving.");
            }
        }
    }
}
...
void CheckVTOLHeight(MECH * mech)
{
    ...
    MechFalls(mech, 1 + (int)(fabs(MechVerticalSpeed(mech) / MP1)), 0);
...
#define FASA_TURN_MOD 3/2
        if (Jumping(mech))
            ...
            if (ts > maxspeed || maxspeed < 0.1)    /* kludge */
                ...
                offset = (int)(SHO2FSIM(1) * 2 * 360 * FASA_TURN_MOD / 60 * (maxspeed - ts) * (omaxspeed / maxspeed) * mw_mod * MP_PER_KPH / 6);    /* hmm. */
            }
        }
    } else {
        if (Jumping(mech)) {
            mech_map = static_cast <MAP *>(FindObjectsData(mech->mapindex));
            offset = SHO2FSIM(1) * 6 * JumpSpeedMP(mech, mech_map) * mw_mod;
        } else 
            if (fabs(MechSpeed(mech)) < 1.0)
                offset = (int)(SHO2FSIM(1) * 3 * maxspeed * MP_PER_KPH * mw_mod);
            else {
                offset = (int)(SHO2FSIM(1) * 2 * maxspeed * MP_PER_KPH * mw_mod);
                if ((SHO2FSIM(abs(normangle)) > offset) && IsRunning(MechSpeed(mech), maxspeed)) {
                    if (MechSpeed(mech) > maxspeed)
                        offset -= (int)(offset / 2 * maxspeed / MechSpeed(mech));
                    else
                        offset -= (int)(offset / 2 * (3.0 * MechSpeed(mech) / maxspeed - 2.0));
                }
            }
    }
...
float terrain_speed(MECH * mech, float tempspeed, float maxspeed, int terrain, int elev)
{
    switch (terrain) {
        ...
        case ROAD:
            ...
            if (!(MechStatus2(mech) & SPRINTING) && (MechMove(mech) == MOVE_TRACK || MechMove(mech) == MOVE_WHEEL))
                INCREASE_OLD(MP1);
        case ICE:
            if (MechZ(mech) >= 0)
                break;
...
int OverheatMods(MECH * mech)
{
    ...
    if (MechHeat(mech) >= 24.) /* +4 to fire... */
        returnValue = 4;
    else 
        if (MechHeat(mech) >= 17.) /* +3 to fire... */
            returnValue = 3;
        else 
            if (MechHeat(mech) >= 13.) /* +2 to fire... */
                returnValue = 2;
            else 
                if (MechHeat(mech) >= 8.) /* +1 to fire... */
                    returnValue = 1;
                else
                    returnValue = 0;
        ...
        int ammocritnum, int damage)
{
    ...
    if (MechType(mech) == CLASS_BSUIT)
        DamageMech(mech, attacker, 0, -1, ammoloc, 0, 0, damage, 0, -1, 0, -1, 0, 0);
    else
        DamageMech(mech, attacker, 0, -1, ammoloc, 0, 0, -1, damage, -1, 0, -1, 0, 0);
...
void HandleOverheat(MECH * mech)
{
    ...
    if (MechHeat(mech) >= 19.) {
        ...
        if (!(avoided)) {
            ...
            if (damage) /* BOOM! That's going to hurt... */
...
void UpdateHeat(MECH * mech)
{
    ...
    if (Jumping(mech))
        MechPlusHeat(mech) += (MechJumpSpeed(mech) * MP_PER_KPH > 3.) ? MechJumpSpeed(mech) * MP_PER_KPH : 3.;
    ...
    if (InWater(mech) && MechZ(mech) <= -1) {
        ...
        if (MechZ(mech) == -1 && !Fallen(mech)) 
            MechMinusHeat(mech) = MIN(2 * MechActiveNumsinks(mech), legsinks + MechActiveNumsinks(mech));
        else 
            MechMinusHeat(mech) = MIN(2 * MechActiveNumsinks(mech), 6 + MechActiveNumsinks(mech));
    } else 
    ...
    if ((muxevent_tick % TURN) == 0)
        if (MechCritStatus(mech) & LIFE_SUPPORT_DESTROYED || (MechHeat(mech) > 30. && Number(0, 1) == 0)) 
            if (MechHeat(mech) > 25.) {
                mech_notify(mech, MECHPILOT, "You take personal injury from heat!!");
                headhitmwdamage(mech, MechCritStatus(mech) & LIFE_SUPPORT_DESTROYED ? 2 : 1);
            } else 
                if (MechHeat(mech) >= 15.) {
                    mech_notify(mech, MECHPILOT, "You take personal injury from heat!!");
                    headhitmwdamage(mech, 1);
...
int recycle_weaponry(MECH * mech)
{
            ...
            if (WpnIsRecycling(mech, loop, crit[i])) {
                if (diff >= GetPartData(mech, loop, crit[i])) {
                    ...
                    mech_notify(mech, MECHSTARTED,
                            tprintf(MechType(mech) == CLASS_MW ?  "%%cgYou are ready to attack again with %s.%%c" : PartTempNuke(mech, loop, crit[i]) != 0 ? "%%cg%s is operational again.%%c" : (GetPartFireMode(mech, loop, crit[i]) & ROCKET_FIRED) ? "" : "%%cg%s finished recycling.%%c", &MechWeapons[weaptype[i]].name[3]));
    ...
    /* Cycle a section */
        if (MechSections(mech)[loop].recycle && ((MechType(mech) == CLASS_MECH) || (MechType(mech) == CLASS_BSUIT) || (MechType(mech) == CLASS_VEH_GROUND) || (MechType(mech) == CLASS_VTOL))) {
            ...
            if (diff >= MechSections(mech)[loop].recycle && !SectIsDestroyed(mech, loop)) {
                ...
                mech_notify(mech, MECHSTARTED, tprintf("%%cg%s%s has finished its previous action.%%c", MechType(mech) == CLASS_BSUIT ? "" : "Your ", location));
            } else {
...
void NewHexEntered(MECH * mech, MAP * mech_map, float deltax, float deltay, int last_z)
{
    ...
    if ((MechMove(mech) == MOVE_HOVER) && (oldterrain == WATER || oldterrain == ICE || ((oldterrain == BRIDGE) && (last_z == 0)))) 
...
#define MOVE_BACK \
            ...
            if (collision_check(mech, WALK_WALL, lastelevation, oldterrain)) {
                ...
                if (MechPilot(mech) == -1 || (!btechconf.btech_skidcliff && MadePilotSkillRoll(mech, (int) (fabs((MechSpeed(mech)) + MP1) / MP1) / 3)) || (btechconf.btech_skidcliff && MadePilotSkillRoll(mech, SkidMod(fabs(MechSpeed(mech)) / MP1)))) {
                ...
                if (collision_check(mech, WALK_DROP, lastelevation, oldterrain)) {
                    mech_notify(mech, MECHALL, "You notice a large drop in front of you");
                    avoidbth = btechconf.btech_skidcliff ?  SkidMod((int)(fabs(MechSpeed(mech)) / MP1)) : (int)((fabs((MechSpeed(mech)) + MP1) / MP1) / 3);
                    if (MechPilot(mech) == -1 || (!MechAutoFall(mech) && MadePilotSkillRoll(mech, avoidbth))) {
            ...
            if (MechType(mech) == CLASS_BSUIT) {
                if ((MechRTerrain(mech) == WATER || (MechRTerrain(mech) == BRIDGE && (lastelevation < (elevation - 1)))) && elevation < 0) {
                    ...
                    if (MechPilot(mech) == -1 || MadePilotSkillRoll(mech, (int) (fabs((MechSpeed(mech)) + MP1) / MP1) / 3)) {
                        ...
                        mech_notify(mech, MECHALL, "You trip at the edge of the water and plunge in...");
                        MechFloods(mech);
                        return;
                    }
                ...
                if (((MechRTerrain(mech) == WATER && MechZ(mech) < 0) || (MechRTerrain(mech) == BRIDGE && MechZ(mech) < 0) || (MechRTerrain(mech) == ICE && MechZ(mech) < 0) || MechRTerrain(mech) == HIGHWATER) && MechType(mech) != CLASS_MW) {
                    int skillmod, dammod;

                    MechDesiredSpeed(mech) = MIN((int)(MechDesiredSpeed(mech)), (int)(WalkingSpeed(MMaxSpeed(mech))));
                    ...
                    if (IsRunning(MechSpeed(mech), MMaxSpeed(mech))) {
                        mech_notify(mech, MECHPILOT, "You struggle to keep control as you run into the water!");
                        skillmod = 2;
                        dammod = 2;
                    } else {
                    ...
                    skillmod += (MechRTerrain(mech) == HIGHWATER ? -2 : MechRTerrain(mech) == BRIDGE ? bridge_w_elevation(mech) : MechElev(mech) > 3 ? 1 : (MechElev(mech) - 2));
        ...
        case MOVE_TRACK:
            ...
            if ((MechRTerrain(mech) == WATER || (MechRTerrain(mech) == BRIDGE && (lastelevation < (elevation - 1)))) && elevation < 0) {
                ...
                if (MechPilot(mech) == -1 || MadePilotSkillRoll(mech, (int) (fabs((MechSpeed(mech)) + MP1) / MP1) / 3)) {
                    mech_notify(mech, MECHALL, "You manage to stop before falling in.");
                    MechLOSBroadcast(mech, "stops suddenly to avoid driving into the water!");
                } else {
                    mech_notify(mech, MECHALL, "You drive into the water and your vehicle becomes inoperable.");
                    Destroy(mech);
                    return;
        ...
        case MOVE_WHEEL:
            if (collision_check(mech, WALK_WALL, lastelevation, oldterrain)) {
                ...
                if (collision_check(mech, WALK_DROP, lastelevation, oldterrain)) {
                    ...
                    if (MechPilot(mech) == -1 || (!MechAutoFall(mech) && MadePilotSkillRoll(mech, avoidbth))) {
                        mech_notify(mech, MECHALL, "You manage to stop before falling off.");
                        MechLOSBroadcast(mech, "stops suddenly to avoid driving off a cliff!");
                    } else {
                        mech_notify(mech, MECHALL, "You drive off the cliff and fall to the ground below.");
                        MechLOSBroadcast(mech, "drives off a cliff and falls to the ground below.");
                        MechFalls(mech, lastelevation - elevation, 0);
            ...
            if ((MechRTerrain(mech) == WATER || (MechRTerrain(mech) == BRIDGE && (lastelevation < (elevation - 1)))) && elevation < 0) {
                ...
                if (MechPilot(mech) == -1 || MadePilotSkillRoll(mech, (int) (fabs((MechSpeed(mech)) + MP1) / MP1) / 3)) {
                    mech_notify(mech, MECHALL, "You manage to stop before falling in.");
                    MechLOSBroadcast(mech, "stops suddenly to driving into the water!");
                } else {
                    mech_notify(mech, MECHALL, "You drive into the water and your vehicle becomes inoperable.");
                    Destroy(mech);
                    return;
            ...
            if (btechconf.btech_newterrain) {
                tt = MechRTerrain(mech);
                if ((tt == HEAVY_FOREST || tt == LIGHT_FOREST) && fabs(MechSpeed(mech)) > MP1) {
                    mech_notify(mech, MECHALL, "You try to dodge the larger trees..");
                    if (MechPilot(mech) == -1 || MadePilotSkillRoll(mech, (tt == HEAVY_FOREST ? 3 : 0) + (int)(fabs(MechSpeed(mech)) / MP1 / 6)))
                        mech_notify(mech, MECHALL, "You manage to dodge 'em!");
                    else {
                        mech_notify(mech, MECHALL, "You swerve, but not enough! This'll hurt!");
                        MechLOSBroadcast(mech, "cruises headlong at a tree!");
                    ...
                    if ((tt == ROUGH) && fabs(MechSpeed(mech)) > MP1) {
        ...
        case MOVE_FOIL:
        case MOVE_SUB:
            if ((MechRTerrain(mech) != WATER && MechRTerrain(mech) != BRIDGE) || abs(MechElev(mech)) < (abs(MechZ(mech)) + (MechMove(mech) == MOVE_FOIL ? -1 : 0))) {
                /* Run aground */
                MechElev(mech) = le;
                MechTerrain(mech) = ot;
                ...
                if (MechPilot(mech) == -1 || MadePilotSkillRoll(mech, (int) (fabs((MechSpeed(mech)) + MP1) / MP1) / 3)) {
                    mech_notify(mech, MECHALL, "You manage to stop before crashing.");
                    MechLOSBroadcast(mech, "stops suddenly to avoid running aground!");
                    MOVE_BACK;
                } else {
        ...
        case MOVE_HOVER:
            if (collision_check(mech, WALK_WALL, lastelevation, oldterrain)) {
                ...
                if (collision_check(mech, WALK_DROP, lastelevation, oldterrain)) {
                    ...
                    if (collision_check(mech, HIT_UNDER_BRIDGE, lastelevation, oldterrain)) {
                        mech_notify(mech, MECHALL, "You notice the underside of the bridge in front of you!");
                        if (MechPilot(mech) == -1 || (!btechconf.btech_skidcliff && MadePilotSkillRoll(mech, (int) (fabs((MechSpeed(mech)) + MP1) / MP1) / 3)) || (btechconf.btech_skidcliff && MadePilotSkillRoll(mech, SkidMod(fabs(MechSpeed(mech)) / MP1)))) {
                            mech_notify(mech, MECHALL, "You manage to stop before slamming into the bridge.");
                            MechLOSBroadcast(mech, "stops suddenly to avoid slamming in the bridge!");
                        } else {
                            mech_notify(mech, MECHALL, "You drive right into the underside of the bridge.");
                            MechLOSBroadcast(mech, "drives right into the underside of the bridge.");
                            MechFalls(mech, 1, 0);
                        }
                        MOVE_BACK;
                        MechDesiredSpeed(mech) = 0;
        ...
        case MOVE_VTOL:
        case MOVE_FLY:
            if (Landed(mech) && MechRTerrain(mech) != ROAD && MechRTerrain(mech) != BRIDGE && MechRTerrain(mech) != GRASSLAND && MechRTerrain(mech) != BUILDING) {
            ...
            if (collision_check(mech, JUMP, 0, 0)) {
                ...
                MechElev(mech) = le;
                MechTerrain(mech) = ot;
                mech_notify(mech, MECHALL, "You attempt to fly over elevation that is too high!!");
                if (MechPilot(mech) == -1 || (MadePilotSkillRoll(mech, (int) (MechFZ(mech) / ZSCALE / 3)) && (ot == GRASSLAND || ot == ROAD || ot == BUILDING))) {
                    mech_notify(mech, MECHALL, "You land safely.");
                    MechStatus(mech) |= LANDED;
                    MechSpeed(mech) = 0.0;
                    MechVerticalSpeed(mech) = 0.0;
                } else {
                    mech_notify(mech, MECHALL, "You crash into the obstacle and fall from the sky!!!!!");
                    MechLOSBroadcast(mech, "crashes into an obstacle and falls from the sky!");
    ...
    if (!done) {
        ...
        if (btechconf.btech_fasaadvvhlfire && (MechType(mech) == CLASS_VEH_GROUND) && (MechTerrain(mech) == FIRE))
...
void CheckDamage(MECH * wounded)
{
    ...
    if (btechconf.btech_newstagger) {
        ...
        SendDebug(tprintf("For %d. StaggerDamage: %d. StaggerLevel: %d.", wounded->mynum, StaggerDamage(wounded), StaggerLevel(wounded)));
        ...
        if (LastStaggerNotify(wounded) < staggerLevel) {
            ...
            if (Jumping(wounded) || OODing(wounded)) {
                switch (staggerLevel) {
                    case 1:
                        mech_notify(wounded, MECHALL, "%cy%chThe damage causes you to spin a little in your flight path.%cn");
                        headingChange = 15;
                        break;
                    case 2:
                        mech_notify(wounded, MECHALL, "%crThe damage spins you but you maintain your flight!%cn");
                        MechLOSBroadcast(wounded, "turns slightly from the damage!");
                        headingChange = 45;
                        break;
                    default:
                        mech_notify(wounded, MECHALL, "%cr%chThe damage causes you to spin completely around!%cn");
                        MechLOSBroadcast(wounded, "spins around from the damage!");
                        headingChange = 180;
                        break;
                }
                SetFacing(wounded, AcceptableDegree(MechFacing(wounded) + headingChange) * ((Roll() >= 6) ? 1 : -1));
            } else {
                switch (staggerLevel) {
                    case 1:
                        mech_notify(wounded, MECHALL, "%cy%chThe damage causes you to stagger a little.%cn");
                        break;
                    case 2:
                        mech_notify(wounded, MECHALL, "%crThe damage causes you to stagger even more!%cn");
                        MechLOSBroadcast(wounded, "starts to stagger from the damage!");
                        break;
                    default:
                        mech_notify(wounded, MECHALL, "%cr%chThe damage causes you to stagger violently while attempting to keep your footing!%cn");
                        MechLOSBroadcast(wounded, "staggers back and forth attempting to keep its footing!");
                        break;
                }
            }
        }
    } else {
        if (!IsDS(wounded) && MechTurnDamage(wounded) >= 20 && (!MechStaggeredLastTurn(wounded) || MechStaggerStamp(wounded) == now)) {
        ...
        if ((MechStaggeredLastTurn(wounded) && MechStaggerStamp(wounded) == now) || (!MechStaggeredLastTurn(wounded) && !now)) {
            MechTurnDamage(wounded) = 0;
...
void UpdatePilotSkillRolls(MECH * mech)
{
    ...
    if (((muxevent_tick % TURN) == 0) && !Fallen(mech) && !Jumping(mech) && !OODing(mech)) {
        ...
        if (InSpecial(mech) && InGravity(mech) && !MoveModeChange(mech))
            if (MechSpeed(mech) > MechMaxSpeed(mech) && MechType(mech) == CLASS_MECH) {
                grav = 1;
                makeroll = 1;
            }
        
        if (IsRunning(MechSpeed(mech), maxspeed) && ((MechCritStatus(mech) & GYRO_DAMAGED) || (MechCritStatus(mech) & HIP_DAMAGED)))
            makeroll = 1;

        if (makeroll) {
            if (!MadePilotSkillRoll(mech, (makeroll - 1))) {
                if (grav) {
                    int dam = (int)((MechSpeed(mech) - MechMaxSpeed(mech)) / MP1 + 1);
...
void updateAutoturnTurret(MECH * mech)
{
    ...
    if ((MechTankCritStatus(mech) & TURRET_JAMMED) || (MechTankCritStatus(mech) & TURRET_LOCKED))
        return;

    if (!GetSectInt(mech, TURRET))
        return;

    if (MechTarget(mech) == -1 && (MechTargY(mech) == -1 || MechTargX(mech) == -1))
        return;
    ...
    bearing = AcceptableDegree(FindBearing(MechFX(mech), MechFY(mech), fx, fy) - MechFacing(mech));
