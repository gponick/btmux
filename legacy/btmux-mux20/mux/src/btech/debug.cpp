...
static int debug_check_stuff(Node * tmp)
{
    ...
    if (cheat_player > 0 && osize != size)
        notify(cheat_player, tprintf("#%d: %s (%d bytes)", NodeKey(tmp), SpecialObjects[t].type, size));
    ...
    for (i = 0; i < global_specials; i++) {
        if (number[i]) {
            if (smallest[i] == largest[i])
                notify(player, tprintf("%4d %-20s: %d bytes total, %d each", number[i], SpecialObjects[i].type, total[i], total[i] / number[i]));
            else
                notify(player, tprintf("%4d %-20s: %d bytes total, %d avg, %d/%d small/large", number[i], SpecialObjects[i].type, total[i], total[i] / number[i], smallest[i], largest[i]));
        }
...
void debug_setvrt(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(!find_matching_vlong_part(args[0], NULL, &id, &brand), "That is no weapon!");
    DOCHECK(!IsWeapon(id), "That is no weapon!");
    
    MechWeapons[Weapon2I(id)].vrt = vrt;
    notify(player, tprintf("VRT for %s set to %d.", MechWeapons[Weapon2I(id)].name, vrt));
    
    STARTLOG(LOG_ALWAYS, "WIZ", "CHANGE") {
        log_text(tprintf("VRT for %s set to %d by ", MechWeapons[Weapon2I(id)].name, vrt));
        log_name(player);
    ENDLOG;
}}
