...
{
    if (x < losmap->startx || x > losmap->startx + losmap->xsize || y < losmap->starty || y > losmap->starty + losmap->ysize) {
 ...
 * sees all terrain (e.g. 'satelite downlink' sensor) */
...
static int MechSeesRange(MECH * mech, MAP * map, int x, int y, int z, int sensor)
{
    ...
    /* XXX HACK: code duplication. this should be replaced with sensor functions */
    ...
    if (sn == 0 && maxvis && range >= maxvis && (losmap.flags & MAPLOS_FLAG_SLITE))
        return -1;
...
static void trace_slitelos(MAP * map, MECH * mech, int index, float start_height)
{
    ...
    float trace_a;
    int trace_coordnum = TraceLOS(map, MechX(mech), MechY(mech), INDEX2X(index), INDEX2Y(index), &trace_coords);
...
static void litemark_map(MAP * map)
{
    ...
    for (i = 0; i < map->first_free; i++) {
        ...
        mech = static_cast < MECH * >(FindObjectsData(map->mechsOnMap[i]));
        ...
        if (Jellied(mech)) {
            ...
            visit_neighbor_hexes(map, MechX(mech), MechY(mech), litemark_callback);
        }
...
static void trace_maphexlos(MAP * map, MECH * mech, int index, int tracew, float start_height)
{
    ...
    for (; trace_range < trace_coordnum; trace_range++) {
        ...
        for (nsensor = 0; nsensor < NUMSENSORS(mech); nsensor++) {
            ...
            seestate = MechSeesRange(mech, map, trace_x, trace_y, trace_height, nsensor);
            ...
            if (!newwoods) {
                if (trace_a < minangle[nsensor] || (seestate < 0 && !hexlit(trace_x, trace_y))) 
                    set_hexlosinfo(trace_x, trace_y, MAPLOSHEX_NOLOS);
                else {
            ...
            if (blockangle[nsensor] < trace_a) {
                ...
                if (!MechSeesThroughWoods(mech, map, woodcount[nsensor] + newwoods, nsensor)) {
            ...
            if (trace_terrain == WATER) {
                if (trace_water[nsensor])
                    watercount[nsensor]++;
                if (!trace_water[nsensor] || !MechSeesThroughWater(mech, map, watercount[nsensor], nsensor)) 
                    if (seestate < 0 && !hexlit(trace_x, trace_y))
                        set_hexlosinfo(trace_x, trace_y, MAPLOSHEX_NOLOS);
                    else
                        set_hexlosinfo(trace_x, trace_y, MAPLOSHEX_SEETERRAIN);
            } else 
                if (seestate < 0 && !hexlit(trace_x, trace_y))
                    set_hexlosinfo(trace_x, trace_y, MAPLOSHEX_NOLOS);
                else
...
hexinfluence:
            if (trace_terrain == WATER && !MechSeesThroughWater(mech, map, 1, nsensor)) {
                set_hexlosinfo(trace_x, trace_y, MAPLOSHEX_NOLOS);
                minangle[nsensor] = blockangle[nsensor] = 1000;
                continue;
            }
            ...
            trace_water[nsensor] = 0;
            if (trace_terrain == MOUNTAINS && !MechSeesOverMountain(mech, map, nsensor)) {
                set_hexlosinfo(trace_x, trace_y, MAPLOSHEX_NOLOS);
                minangle[nsensor] = blockangle[nsensor] = 1000;
                continue;
            }
...
hexlosmap_info *CalculateLOSMap(MAP * map, MECH * mech, int sx, int sy, int xsz, int ysz)
{
    ...
    if (xsz > MAPLOS_MAXX || ysz > MAPLOS_MAXY) {
        SendError(tprintf("xsize (%d vs %d) or ysize (%d vs %d) " "to CalculateLOSMap too large, for mech #%d", xsz, MAPLOS_MAXX, ysz, MAPLOS_MAXY, mech->mynum));
        return NULL;
    }
    ...
    if (IsWater(MechRTerrain(mech)) && ((MechType(mech) == CLASS_MECH && MechZ(mech) == -1) || ((WaterBeast(mech) || MechMove(mech) == MOVE_HOVER) && MechZ(mech) == 0))) 
        bothworlds = 1;
    else
        bothworlds = 0;
    ...
    for (index = 0; index < xsz; index++) {
        if (losmap.map[index] & MAPLOSHEX_SEEN)
            continue;
        trace_maphexlos(map, mech, index, underterrain || bothworlds, start_height);
    }
