...
static void swim_except(MAP * map, MECH * mech, int x, int y, char *msg, int isbridge)
{
    ...
    for (i = 0; i < map->first_free; i++) {
        ...
        MechTerrain(t) = WATER;
        if ((!isbridge && (MechZ(t) == 0) && (MechMove(t) != MOVE_HOVER)) || (isbridge && MechZ(t) == MechElev(t))) {
...
void drop_thru_ice(MECH * mech)
{
    ...
    if (MechElev(mech) > 0 && MechType(mech) == CLASS_VEH_GROUND && !Destroyed(mech)) {
...
{
    if ((MechMove(mech) == MOVE_HOVER) || (MechMove(mech) == MOVE_SUB) || (MechType(mech) == CLASS_BSUIT))
        return 0;
...
static void growable_callback(MAP * map, int x, int y)
{
    ...
    if ((IsWater(terrain) && terrain != ICE) || GetRTerrain(map, x, y) == TMP_TERR)
        watercount++;
}
...
void map_addice(dbref player, void *data, char *buffer)
{
    char *args[2];
    int num;
...
void map_delice(dbref player, void *data, char *buffer)
{
    char *args[2];
    int num;
    MAP *map = static_cast <MAP *>(data);
...
void possibly_blow_bridge(MECH * mech, int weapindx, int x, int y)
{
    ...
    if (Number(1, 10 * (1 + GetElevation(map, x, y))) > MechWeapons[weapindx].damage) {
