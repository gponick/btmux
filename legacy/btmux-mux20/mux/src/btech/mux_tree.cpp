        ...
        void *data)
{
    ...
    foo->type = type;
    foo->size = size;
    tree_add(tree, reinterpret_cast < fp > (NodeCompare), foo,
            reinterpret_cast < fp2 > (NodeDelete));
}
...
Node *FindNode(Tree tree, muxkey_t key)
{
    ...
    foo.key = key;
    return (Node *) tree_srch(&tree, reinterpret_cast < fp > (NodeCompare),
            &foo);
}
...
void DeleteEntry(Tree * tree, muxkey_t key)
{
    ...
    if (FindNode(*tree, key)) {
        foo.key = key;
        tree_delete(tree, (fp) NodeCompare, &foo, (fp2) NodeDelete);
    }
}
...
static void MyLoadTree(FILE * f, Tree * tree, int (*sizefunc) (int))
{
    ...
    while (key >= 0 && !feof(f)) {
        ...
        if (nsize) {
            ...
            if (nsize > osize)
                memset((u_char *) data + osize, 0, nsize - osize);
        } else
            data = NULL;
