...
int *TraceLOS(MAP * map, int ax, int ay, int bx, int by, int *result)
{
	...
	*result = found_count;
	...
	while (currx != bx) {
	    ...
	    DoBestOf(currx + 1 * i, by - currx % 2, currx + 1 * i,
		by - currx % 2 + 1);

	    if (currx != bx)
		DoStuffWithHex(currx + 2 * i, by);
...
/* 	DoStuffLastHex(bx,by); */
	*result = found_count;
	...
	while ((currx != bx) && (curry != by)) {
	    ...
	    DoBestOf(currx, curry + ymul, currx + xmul,
	    ...
	    DoStuffWithHex(currx, curry);
	}
	DoStuffLastHex(currx, curry);
	*result = found_count;
	return found_coords;
    }
