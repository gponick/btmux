...
static int load_update1(Node * tmp)
{
	...
	MechCocoon(mech) = 0;
	MechStatus(mech) &=
	    ~(BLINDED | UNCONSCIOUS | JUMPING | TOWED | ECM_ACTIVE | 
	    ECM_DISTURBANCE | JELLIED);
	MechStatus2(mech) &=
	    ~(ECCM_ACTIVE | ECM_PROTECTED);
	MechCritStatus(mech) &=
	    ~(LOAD_OK | OWEIGHT_OK | SPEED_OK);
...
void CreateNewSpecialObject(dbref player, dbref key)
{
    ...
    if (!(str && *str)) {
	notify(player,
	    "You must @defattr here/XTYPE first, with the type");
