    ...
    int spect)
{
	...
	if (ok_specials[i][1] == spect)
	    choice(get_parts_long_name(Special(ok_specials[i][0])),
		ok_specials[i][0]);
}
...
int crit_weight(MECH * mech, int t)
{
    ...
    if (IsWeapon(t))
	return MechWeapons[Weapon2I(t)].weight * 1024 / 10 /
    ...
    if (t == HEAT_SINK) {
	return (MechSpecials2(mech) & COMPACT_HEAT_TECH) ? 3072 : (1024 /
	    (MechSpecials(mech) & CLAN_TECH ? 2 : MechSpecials(mech) &
	    DOUBLE_HEAT_TECH ? 3 : 1));
    }
    if (t == TARGETING_COMPUTER)
	return 1024;
    if (t == AXE || t == ARTEMIS_IV || t == MASC ||
	t == C3_SLAVE || t == C3_MASTER || t == TAG || t == LAMEQUIP)
	return 1024;
    if (t == C3I)
	return (1024 * 5) / 4;
/* Mace rules originate in TacHandbook. (bleh) Should this be heavier than normal Axe? - DJ */
    if (t == MACE)
	return 1024;
    if (t == SWORD)
	return 1024 * 3 / 4;
...
/*	return 1024 * 2 / 3; */
	...
	return 1024;
    if (t == SUPERCHARGER)
/*	return (engine_weight(mech) / 10); */
	return (((engine_weight(mech) + (engine_weight(mech) % 5120 ? 5120 : 0)) / 5120) * 512);
    if (t == ANGEL_ECM) 
	return (2048 / 2);
    if (t == BLOODHOUND_PROBE)
	return (2048 / 3);
    if (t == BOMB_RACK)
	return 1024;
    return 0;
...
#define CustomWeaponModes (REAR_MOUNT|ON_TC|OS_MODE)
#define CustomAmmoModes (ARTEMIS_MODE|NARC_MODE|SWARM_MODE|SWARM1_MODE|INFERNO_MODE|LBX_MODE|HALFTON_MODE|CASELESS_MODE|PIERCE_MODE|PRECISION_MODE|TRACER_MODE)
...
int generate_change_list(MECH * from, MECH * to)
{
	...
	for (j = 0; j < crl; j++) {
		...
		if (IsAmmo(ft))
		    if ((GetPartMode(from, i,
				j) & CustomAmmoModes) != (GetPartMode(to,
				i, j) & CustomAmmoModes))
			addchange(ALTER_AMMO, i, j, 0, (GetPartMode(from,
				    i, j) & (HALFTON_MODE))
			    ? 512 : 0 - (GetPartMode(to, i,
	...
	for (j = 0; j < num_weaps_f; j++) {
		...
		0 - MechWeapons[weaparray_f[j]].weight * 1024 / 10,
	...
	for (j = 0; j < num_weaps_t; j++) {
		...
		MechWeapons[weaparray_t[j]].weight * 1024 / 10,
		ADD_WEAPON_TIME * cr);
	}
    }
    global_silence = 0;
...
static void display_change_list(coolmenu * c, MECH * from, MECH * mech)
{
	...
	cent
	    ("If you do not want to lose parts, remove them first with the");
	cent("tech commands.");
    }
}
...
#define TB(v) ToggleBit(GetPartMode(mech, loc, pos), v)
...
((MechType(mech) == CLASS_VEH_VTOL && s == ROTOR) ? 2 : 99) : \
(s == HEAD ? 9 : \
...
static coolmenu *custom_menu(dbref player)
{
	...
	default:
		...
		case 1:
		    custom_do_weapon(c, mech, loc, pos, 0,
			NEWT | CLAT | MUN);
		    break;
		case 2:
		    custom_do_ammo(c, mech, loc, pos, 0,
			NEWT | CLAT | MUN);
		    break;
		case 4:
		    custom_do_weapon(c, mech, loc, pos, NEWT, 0);
		    break;
		case 8:
		    custom_do_ammo(c, mech, loc, pos, NEWT, 0);
		    break;
		case 16:
...
static int do_changes(CUSTOM * cu, dbref player, MECH * from, MECH * to)
{
	...
	case REMOVE_WEAPON:
		...
		SetPartMode(from, loc, j, 0);
		SetPartData(from, loc, j, 0);
	    }
	    break;
	case DO_ADD_CRIT:
	    SetPartType(from, loc, pos, GetPartType(to, loc, pos));
	    SetPartMode(from, loc, pos, GetPartMode(to, loc, pos));
	    SetPartData(from, loc, pos, GetPartData(to, loc, pos));
	    if (!Wiz(cu->user))
		DestroyPart(from, loc, pos);
	    break;
	case ALTER_AMMO:
	    GetPartMode(from, loc, pos) &= CustomAmmoModes;
	    GetPartMode(from, loc, pos) |=
		(GetPartMode(to, loc, pos) & CustomAmmoModes);
	    GetPartData(from, loc, pos) = 0;
	    break;
	case REORIENT_WEAPON:
	    for (j = pos; j < (pos + GetWeaponCrits(from, changes[i][3]));
		j++) {
		GetPartMode(from, loc, j) &= CustomWeaponModes;
		GetPartMode(from, loc, j) |=
		    (GetPartMode(to, loc, pos) & CustomWeaponModes);
	    }
	    break;
	case DO_ADD_WEAPON:
	    for (j = pos; j < (pos + GetWeaponCrits(from, changes[i][3]));
		...
		SetPartMode(from, loc, j, GetPartMode(to, loc, j));
    ...
    int loc, int pos, int choice)
{
    ...
    SetPartMode(mech, loc, pos, 0);
    SetPartData(mech, loc, pos, FullAmmo(mech, loc, pos));
    advance_state(cu, player, loc + 1);
}

static void custom_act_brand(CUSTOM * cu, MECH * mech, dbref player, int loc, int pos, int weapindx)
{
    ...
    int loc, int pos, int choice)
{
	...
	return;
    case TOGGLE_REAR:
	ToggleBit(GetPartMode(mech, loc, pos), REAR_MOUNT);
	notify(player, "Toggled!");
	break;
    case TOGGLE_OS:
	ToggleBit(GetPartMode(mech, loc, pos), OS_MODE);
	notify(player, "Toggled!");
	break;
    case TOGGLE_AMMO:
	ToggleAmmo(player, mech, loc, pos);
	notify(player, tprintf("Ammo is now %s.",
		ammo_text(GetPartMode(mech, loc, pos))));
	break;
    case TOGGLE_HALFAMMO:
	ToggleBit(GetPartMode(mech, loc, pos), HALFTON_MODE);
	SetPartData(mech, loc, pos, FullAmmo(mech, loc, pos));
	...
	break;
    case TOGGLE_TC:
	ToggleBit(GetPartMode(mech, loc, pos), ON_TC);
	notify(player, "Toggled!");
	break;
    case REMOVE:
    ...
    coolmenu * c, coolmenu * d)
{
	...
	if (cu->state <= NUM_SECTIONS)
	    ...
	    case 4:
	    case 16:
		custom_act_brand(cu, mech, player, loc, pos, s / FIRST_UNUSED_BIT);
		break;
	    case 2:
	    case 8:
	    case 32:
