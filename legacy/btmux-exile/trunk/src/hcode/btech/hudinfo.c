...
{
    if (mudconf.btech_enablehudinfo)
	notify(player, "HUDINFO does not work from scode or macros.");
    else
	notify(player, "HUDINFO is currently disabled.");
}
...
void do_hudinfo(DESC * d, char *arg)
{
    ...
    if (!*arg) {
	hudinfo_notify(d, NULL, NULL, tprintf("#HUD hudinfo version %s", HUD_PROTO_VERSION));
	return;
    }
...
static char *hud_getweaponstatus(MECH * mech, int sect, int crit, int data)
{
    ...
    int tempstat;
    int tmp = WeaponIsNonfunctional(mech, sect, crit, GetWeaponCrits(mech, Weapon2I(GetPartType(mech, sect, crit))));
...
{
    int mode = GetPartMode(mech, sect, crit);
...
*/
	...
	*p++ = 'h';
    ...
    if (mode & (OS_USED))
	*p++ = 'o';
    else if ((mode & OS_MODE) || (MechWeapons[type].special & OS_WEAP))
	*p++ = 'O';

    if (MechWeapons[type].special & MUN && MechWeapons[type].special & NARC)
	*p++ = 'I';
...
static void hud_weapons(DESC * d, MECH * mech, char *msgclass, char *args)
{
	...
	for (i = 0; i < weapcount; i++) {
		...
		hud_getammomode(mech, GetPartMode(mech, sect,
			critical[i])));
    ...
    char *args)
{
	...
	sprintf(response,
	    ...
	    w->min, w->shortrange, w->medrange,
	    w->longrange, w->criticals, w->weight, w->damage, w->vrt,
...
static char *hud_advtech(MECH * mech)
{
    ...
    char *p = advtech;
    int spec = MechSpecials(mech), spec2 = MechSpecials2(mech);
	...
	*p++ = 't';
    ...
    if (spec2 & ANGEL_ECM_TECH)
	*p++ = 'A';
    if (spec & NULLSIG_TECH)
	*p++ = 'N';
    if (spec2 & BLOODHOUND_PROBE_TECH)
	*p++ = 'b';
    if (spec & STEALTHARMOR_TECH)
	*p++ = 's';
/*
...
/*
	...
	*p++ = 'j';
*/
    if (spec & CS_STEALTH_TECH)
	*p++ = 'p';

    if (spec & C3_SLAVE_TECH || spec & C3_MASTER_TECH)
	*p++ = 'C';
    if (HasFuncTAG(mech))
	*p++ = 'T';
    if (spec & C3_SLAVE_TECH)
	*p++ = 'c';
    if (spec & C3_MASTER_TECH)
	*p++ = 'm';
    ...
    char *args)
{
    ...
    if (!(MechCritStatus(mech) & OBSERVATORIC) &&
...
static void hud_tactical(DESC * d, MECH * mech, char *msgclass, char *args)
{
    ...
    float tfx, tfy, trange;
/*    hexlosmap_info *losmap = NULL; */

    if (!MechLRSRange(mech) || !MechTacRange(mech)) {
	hudinfo_notify(d, msgclass, "E",
	    "Your system seems to be inoperational");
	return;
    }
    ...
    case 3:{
	    ...
	    if (!(MechCritStatus(mech) & OBSERVATORIC) &&
		abs((int) range) > (MapIsUnderground(map) ? MechTacRange(mech) : MechLRSRange(mech))) {
		hudinfo_notify(d, msgclass, "E", "Out of range");
		return;
	    }
    ...
    height = MIN(height, 2 * (MapIsUnderground(map) ? MechTacRange(mech) : MechLRSRange(mech)));
...
/*	losmap = CalculateLOSMap(map, mech, sx, sy, ex - sx, ey - sy); */
	...
	for (x = sx; x < ex; x++) {
	    MapCoordToRealCoord(x, y, &tfx, &tfy);
	    trange = FindRange(MechFX(mech), MechFY(mech), MechFZ(mech), tfx, tfy, GetElev(map, x, y) * ZSCALE); 
	    if (MapIsUnderground(map) || lostactical) {
		if (trange > (float) MechTacRange(mech))
		    losflag = 0;
		else
		    losflag = !MySeeHex(mech, map, x, y, trange);
	    } else {
		if (trange > (float) MechLRSRange(mech) || MechZ(mech) > ORBIT_Z + 50)
		    losflag = 0;
		else
		    losflag = 1;
		}
