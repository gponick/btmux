...
void mech_createbays(dbref player, void *data, char *buffer)
{
	...
	NUM_BAYS + 1)) == (NUM_BAYS + 1), "Invalid number of arguments!");
    for (i = 0; i < argc; i++) {
...
static int dir2loc[6] =
    { DS_NOSE, DS_RWING, -1, DS_AFT, -1, DS_LWING };
...
static void mech_enterbay_event(EVENT * e)
{
	...
	(MechType(mech) == CLASS_MECH && (Fallen(mech) || Standing(mech))) || OODing(mech) ||
...
static int DS_Bay_Is_Open(MECH * mech, MECH * ds, dbref bayref)
{
	...
	if (AeroBay(ds, i) > 0)
		...
		for (i = 0; i < NUM_CRITICALS; i++) {
		    if (((is_aero(mech) && GetPartType(ds, dir2loc[j], i) == I2Special(DS_AERODOOR)) ||
			 (!is_aero(mech) && GetPartType(ds, dir2loc[j], i) == I2Special(DS_MECHDOOR))) &&
...
int DS_Bay_Is_EnterOK(MECH * ds, dbref bayref)
{
    int i;
	...
	if (AeroBay(ds, i) > 0)
	    if (AeroBay(ds, i) == bayref)
		return event_count_type_data2(EVENT_MISC,
		    (void *) bayref) > 0 ? 0 : 1;
    return 0;
}
...
void mech_enterbay(dbref player, void *data, char *buffer)
{
    ...
    DOCHECK(Fallen(mech) || Standing(mech), "Get up! Stand up! Stand up for your rights!");
    DOCHECK(Jumping(mech), "While in mid-jump? No way.");
    DOCHECK(OODing(mech), "While in mid-jump? No way.");
    DOCHECK(Stabilizing(mech), "Wait till you regain your balance.");
    DOCHECK((argc =
...
*/
    ...
    DOCHECK(MechZ(ds) < MechZ(mech) - 4 || MechZ(ds) > MechZ(mech) + 4,
	...
	"Shut up already!");
    DOCHECK(!can_pass_lock(mech->mynum, ref, A_LENTER),
	"You are unable to enter the bay!");
    DOCHECK(!DS_Bay_Is_EnterOK(ds, AeroBay(ds, bayn)),
	"Someone else is using the door at the moment.");
    DOCHECK(!(map =
	    getMap(mech->mapindex)),
	"You sense a wrongness in fabric of space.");
    if (MechZ(mech) > ORBIT_Z || MechZ(ds) > ORBIT_Z) {
	MechLOSBroadcast(mech, tprintf("begins entering the bay at %d,%d...", MechX(mech), MechY(mech)));
	mech_notify(mech, MECHALL, "You begin docking maneuvers....");
    } else {
	HexLOSBroadcast(map, MechX(mech), MechY(mech), "The bay doors at $h start to open..");
	}
    MECHEVENT(mech, EVENT_MISC, mech_enterbay_event, 45, ref);
}
...
static void DS_Place(MECH * ds, MECH * mech, int frombay)
{
    ...
    ny =
	dirs[(DSBearMod(ds) + i) % 6][1] + MechY(ds) + KLUDGE(MechX(ds),
	nx);
...
int Leave_DS(MAP * map, MECH * mech)
{
	...
	"The door has been jammed!");
    DOCHECKMA0(!Landed(car) && (!FlyingT(mech) || (FlyingT(mech) && Landed(mech))),
	"The 'ship is still airborne!");
    DOCHECKMA0(Zombie(car->mynum),
	"You don't feel leaving right now would be prudent..");
    DOCHECKMA0(MechZ(car) < ORBIT_Z && ImproperLZ(car, 0), "You cannot leave while inside a non-dynamic (landed DS) LZ blocker.");
