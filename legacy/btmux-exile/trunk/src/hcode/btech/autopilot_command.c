...
ACMD(auto_hide)
{
    if ((MechSpecials2(mech) & HIDDEN_TECH) ? 0 : MechType(mech) != CLASS_BSUIT && MechType(mech) != CLASS_MW) {
	return "!Last I checked I was kind of big for that";
	}

    if (!(MechRTerrain(mech) == HEAVY_FOREST || MechRTerrain(mech) == LIGHT_FOREST || MechRTerrain(mech) == WATER ||
		MechRTerrain(mech) == HIGHWATER || (MechType(mech) == CLASS_BSUIT ? MechRTerrain(mech) == BUILDING : 0))) {
...
ACMD(auto_report)
{
    ...
    if (MechTarget(mech) != -1) {
        ...
        if (tempMech)
            strcat(buf, tprintf(", targetting %s %s", GetMechToMechID(mech, tempMech),
...
ACMD(auto_enterbase)
{
    mech_enterbase(a->mynum, mech, argc ? tprintf("%s", args[0]) : tprintf(""));
    mech_enterbay(a->mynum, mech, tprintf(""));
    return argc ? tprintf("entering base (%s side)", args[0]) : "entering base";
}
...
ACMD(auto_help)
{
    auto_replyA(mech, "The following commands are possible:  "
    		"cmode dfollow follow freq dgoto goto heading nogun notarget pickup dropoff"
    		" position rally drally sensor sweight stop target report swarm attackleg"
    		" chanfreq chanmode startup prone stand shutdown enterbase help embark speed"
		" jumpjet chasetarg swarmcharge swarmmode leavebase hide");
    return NULL;
}
...
void auto_reply(MECH * mech, char *buf)
{
    ...
    if (!(FindObjectsData(MechAuto(mech))) || !Good_obj(MechAuto(mech)) || Location(MechAuto(mech)) != mech->mynum) {
	MechAuto(mech) = -1;
	free_lbuf(buf);
	return;
    }
