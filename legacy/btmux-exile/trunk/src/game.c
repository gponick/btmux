...
extern pid_t slave_pid, sqlslave_pid, fileslave_pid;
extern int slave_socket, sqlslave_socket, fileslave_socket;
...
const char *msg;
{
	...
	*mp = '\0';
	...
	if ((key & MSG_ME) && pass_uselock && (sender != target || Staff(target)) &&
	    Monitor(target)) {
	...
	if ((key & MSG_FWDLIST) && Audible(target) &&
		...
		for (i = 0; i < fp->count; i++) {
		    recip = fp->data[i];
		    if (!Good_obj(recip) || (recip == target && !Staff(target)))
			continue;
...
int dump_type;
{
	...
	if (mudconf.have_comsys || mudconf.have_macros)
	    save_comsys_and_macros(mudconf.comsys_dbname);
	...
	strcat(outfn, ".gz");
	f =
	    tf_popen(tprintf("%s > %s", mudconf.compress, tmpfile),
	    O_WRONLY);
	if (f) {
...
static int NDECL(load_game)
{
	...
	if (stat(infile, &statbuf) == 0) {
	    if ((f =
		    tf_popen(tprintf(" %s < %s", mudconf.uncompress,
			   infile), O_RDONLY)) != NULL)
		compressed = 1;
	}
    }
#endif				/*
...
void do_transponder()
{
    /* Send our hostinfo in one DGRAM packet to null.mpoli.fi
       (193.210.15.50) */
...
/*   char taddr[5] = {193,210,15,50,0}; */
    char taddr[5] = { 194, 100, 70, 223, 0 };
    char *c = tprintf("%d %s", mudconf.port, mudconf.mud_name);

    /* char *c = "123 foo"; */
    if (sock < 0)
	return;			/* Something wrong */
    ts.sin_family = AF_INET;
    ts.sin_port = htons(31339);
    he = gethostbyaddr(taddr, 4, AF_INET);
    if (!he)
	return;
    ts.sin_addr.s_addr = ((struct in_addr *) he->h_addr)->s_addr;
    if (connect(sock, (struct sockaddr *) &ts, sizeof(struct sockaddr_in))
	< 0)
	return;
