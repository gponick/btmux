...
time_t dt;
{
    ...
    delta = gmtime(&dt);
    if (delta->tm_yday > 0) {
	snprintf(buf, 256, "%dd %02d:%02d", delta->tm_yday, delta->tm_hour,
	    delta->tm_min);
    } else {
	snprintf(buf, 256, "%02d:%02d", delta->tm_hour, delta->tm_min);
    }
    return buf;
}
...
time_t dt;
{
    ...
    delta = gmtime(&dt);
    if (delta->tm_yday > 0) {
	snprintf(buf, 256, "%dd", delta->tm_yday);
    } else if (delta->tm_hour > 0) {
	snprintf(buf, 256, "%dh", delta->tm_hour);
    } else if (delta->tm_min > 0) {
	snprintf(buf, 256, "%dm", delta->tm_min);
    } else {
	snprintf(buf, 256, "%ds", delta->tm_sec);
    }
    return buf;
}
...
int islisted(dbref d, dbref *list)
{
int i;
...
int key;
{
	...
	if (!Hidden(d->player) ||
		...
		continue;
	    if ((key == CMD_DOING || key == CMD_WHO && !Wizard_Who(e->player)) && islisted(d->player, listed))
		continue;
	    if (count < 500)
	        listed[count] = d->player;
#if 0
...
int first;
{
	...
	*arg++ = '\0';
    ...
    if (mudconf.btech_enablehudinfo && d->flags & DS_CONNECTED && !strcmp(command, "hudinfo")) {
        d->command_count++;
