...
void load_comsystem(FILE * fp)
{
	...
	ch = (struct channel *) malloc(sizeof(struct channel));
        ...
        if (fgets(temp, sizeof(temp), fp))
            /* Strip trailing newline and terminate string at boundary */
            temp[MIN(strlen(temp), sizeof(temp)) - 1] = '\0';
#else
	fscanf(fp, "%[^\n]\n", temp);
#endif
	...
	if (new) {		/* V1 or higher */ 
	    if (fscanf(fp, new == 1 ? "%d %d %d %d %d %d %d %d\n" :
		    "%d %d %d %d %d %d %d %d %d\n", &(ch->type),
		...
		if (dummy > 0) {
			...
			chmsg *c;
                        ...
                        fscanf(fp, "%d ", &k);
                        if (fgets(temp, sizeof(temp), fp))
                            /* Strip newline and terminate string */
                            temp[MIN(strlen(temp), sizeof(temp)) - 1] = '\0';
#else
			fscanf(fp, "%d %[^\n]\n", &k, temp);
#endif
...
{
#ifdef CHANNEL_HISTORY
    if (!Wizard(player) && (ch->type & CHANNEL_DARK)) {
        notify(player, "You cannot view channel history on a dark channel.");
        return;
    } 
...
static void do_comwho(dbref player, struct channel *ch)
{
	...
	if (Typeof(user->who) == TYPE_PLAYER && user->on &&
	    Connected(user->who) && (!Dark(user->who) ||
		Wizard_Who(player))
...
void do_channelwho(dbref player, dbref cause, int key, char *arg1)
{
    ...
    for (i = 0; i < ch->num_users; i++) {
	user = ch->users[i];
	if ((flag || UNDEAD(user->who)) && (!Dark(user->who) ||
		Wizard_Who(player))) {
    ...
    char *object)
{
	...
	return;
    case CSET_VISIBLE:
      do_chvisible(player, chan);
      return;
    case CSET_DARK:
      do_chdark(player, chan);
      return; 
    }
...
void do_chvisible(dbref player, char *chan)
{
    struct channel *ch;
    ...
    ch->type &= ~(CHANNEL_DARK);
    raw_notify(player,
        tprintf("@cset: Channel %s wholist now visible.",
            chan));
    return; 
}

void do_chdark(dbref player, char *chan)
{
    struct channel *ch;
    ...
    ch->type |= (CHANNEL_DARK);
    raw_notify(player,
        tprintf("@cset: Channel %s wholist darkened.", chan));
    return;
}
